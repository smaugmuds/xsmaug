 ...
 */
short
get_obj_resistance (OBJ_DATA * obj)
{
  short resist;
...
get_obj (CHAR_DATA * ch, OBJ_DATA * obj, OBJ_DATA * container)
{
  ...
  if (ch->carry_number + get_obj_number (obj) > can_carry_n (ch))
    {
      act (AT_PLAIN, "$d: you can't carry that many items.", ch, NULL,
  ...
  if (container)
	...
	{
	  act (AT_ACTION, "You remove $p from $P", ch, obj, container,
	       TO_CHAR);
	  act (AT_ACTION, "$n removes $p from $P", ch, obj, container,
	       TO_ROOM);
	}
      else
	{
	  act (AT_ACTION, IS_OBJ_STAT (container, ITEM_COVERING) ?
	       "You get $p from beneath $P." : "You get $p from $P", ch, obj,
	       container, TO_CHAR);
	  act (AT_ACTION,
...
do_get (CHAR_DATA * ch, const char *argument)
{
  ...
  if (ch->carry_number < 0 || ch->carry_weight < 0)
    {
      send_to_char
	("Uh oh ... better contact an immortal about your number or weight of items carried.\r\n",
	 ch);
	...
	case ITEM_CORPSE_PC:
	  {
		...
		&& str_cmp (container->action_desc, ch->name))
	      {
		send_to_char
		  ("You did not inflict the death blow upon this corpse.\r\n",
		   ch);
		return;
	      }
	  ...
	  if (!obj)
	    {
	      act (AT_PLAIN, IS_OBJ_STAT (container, ITEM_COVERING) ?
		   ...
		   "I see nothing like that in the $T.", ch, NULL,
		   container->short_descr, TO_CHAR);
	      return;
	    }
	  separate_obj (obj);
      ...
      else
	{
	  ...
	  if (!found)
	      ...
	      if (fAll)
		{
		  if (container->item_type == ITEM_KEYRING
		      && !IS_OBJ_STAT (container, ITEM_COVERING))
		    act (AT_PLAIN, "The $T holds no keys.", ch, NULL,
			 container->short_descr, TO_CHAR);
		  else
		    act (AT_PLAIN, IS_OBJ_STAT (container, ITEM_COVERING) ?
			 "I see nothing beneath the $T." :
			 "I see nothing in the $T.", ch, NULL,
			 container->short_descr, TO_CHAR);
		}
	      else
		{
		  if (container->item_type == ITEM_KEYRING
		      && !IS_OBJ_STAT (container, ITEM_COVERING))
		    act (AT_PLAIN, "The $T does not hold that key.", ch, NULL,
			 container->short_descr, TO_CHAR);
		  else
		    act (AT_PLAIN, IS_OBJ_STAT (container, ITEM_COVERING) ?
			 "I see nothing like that beneath the $T." :
...
do_put (CHAR_DATA * ch, const char *argument)
{
  ...
  if (number <= 1 && str_cmp (arg1, "all") && str_prefix ("all.", arg1))
	...
	if (IS_OBJ_STAT (container, ITEM_COVERING))
	  {
	    if (container->item_type == ITEM_CONTAINER
		? tweight >
		container->value[0] : tweight - container->weight >
		container->weight)
	      {
	...
	return;
	  ...
	  && !IS_OBJ_STAT (container, ITEM_COVERING))
	{
	  act (AT_ACTION, "$n slips $p onto $P.", ch, obj, container,
	       TO_ROOM);
	  act (AT_ACTION, "You slip $p onto $P.", ch, obj, container,
	       TO_CHAR);
	}
      else
	{
	  act (AT_ACTION, IS_OBJ_STAT (container, ITEM_COVERING)
	       ? "$n hides $p beneath $P." : "$n puts $p in $P.", ch, obj,
	       container, TO_ROOM);
	  act (AT_ACTION,
	       IS_OBJ_STAT (container,
			    ITEM_COVERING) ? "You hide $p beneath $P." :
	       "You put $p in $P.", ch, obj, container, TO_CHAR);
	}
      obj->count = count;
	...
	{
	  if (fAll)
	    act (AT_PLAIN, "You are not carrying anything.", ch, NULL, NULL,
		 TO_CHAR);
	  else
	    act (AT_PLAIN, "You are not carrying any $T.", ch, NULL, chk,
		 TO_CHAR);
	  return;
	}
  ...
  if (!IS_NPC (ch) && xIS_SET (ch->act, PLR_LITTERBUG))
    {
      ...
      send_to_char
	("A godly force prevents you from dropping anything...\r\n", ch);
      return;
    }
...
do_give (CHAR_DATA * ch, const char *argument)
{
  ...
  if (is_number (arg1))
	...
	{
	  send_to_char
	    ("Very generous of you, but you haven't got that much gold.\r\n",
	     ch);
	  return;
	}
      ...
      mudstrlcat (buf, (amount > 1) ? " coins." : " coin.",
		  MAX_STRING_LENGTH);
...
damage_obj (OBJ_DATA * obj)
{
  ...
  if (!IS_NPC (ch)
      && (!IS_PKILL (ch)
	  || (IS_PKILL (ch) && !IS_SET (ch->pcdata->flags, PCFLAG_GAG))))
 ...
 */
bool
can_layer (CHAR_DATA * ch, OBJ_DATA * obj, short wear_loc)
{
  OBJ_DATA *otmp;
  short bitlayers = 0;
 ...
 */
void
wear_obj (CHAR_DATA * ch, OBJ_DATA * obj, bool fReplace, short wear_bit)
{
  ...
  if (get_trust (ch) < obj->level)
    {
      ch_printf (ch, "You must be level %d to use this object.\r\n",
		 obj->level);
      act (AT_ACTION, "$n tries to use $p, but is too inexperienced.", ch,
	   obj, NULL, TO_ROOM);
      return;
    }
  ...
  if (!IS_IMMORTAL (ch)
    ...
    {
      act (AT_MAGIC, "You are forbidden to use that item.", ch, NULL, NULL,
	   TO_CHAR);
      act (AT_ACTION, "$n tries to use $p, but is forbidden to do so.", ch,
	   obj, NULL, TO_ROOM);
      return;
    }
  ...
  if (IS_OBJ_STAT (obj, ITEM_PERSONAL) && str_cmp (ch->name, obj->owner))
    {
      send_to_char
	("That item is personalized and belongs to someone else.\r\n", ch);
      if (obj->carried_by)
	obj_from_char (obj);
  ...
  if (obj->item_type == ITEM_LIGHT)
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You hold $p as your light.", ch, obj, NULL,
	       TO_CHAR);
	}
  ...
  switch (1 << bit)
      ...
      if (!get_eq_char (ch, WEAR_FINGER_L))
	{
	  if (!oprog_use_trigger (ch, obj, NULL, NULL))
	    {
	      act (AT_ACTION, "$n slips $s left finger into $p.", ch, obj,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You slip your left finger into $p.", ch, obj,
		   NULL, TO_CHAR);
	    }
      ...
      if (!get_eq_char (ch, WEAR_FINGER_R))
	{
	  if (!oprog_use_trigger (ch, obj, NULL, NULL))
	    {
	      act (AT_ACTION, "$n slips $s right finger into $p.", ch, obj,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You slip your right finger into $p.", ch, obj,
		   NULL, TO_CHAR);
	    }
      ...
      if (!get_eq_char (ch, WEAR_NECK_1))
	{
	  if (!oprog_use_trigger (ch, obj, NULL, NULL))
	    {
	      act (AT_ACTION, "$n wears $p around $s neck.", ch, obj, NULL,
		   TO_ROOM);
	      act (AT_ACTION, "You wear $p around your neck.", ch, obj, NULL,
		   TO_CHAR);
	    }
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You don $p upon your head.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You place $p on your eyes.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You place $p on your face.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You wear $p on your ears.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You wear $p on your feet.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You wear $p on your hands.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You wear $p on your arms.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!can_layer (ch, obj, WEAR_ABOUT))
	{
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  act (AT_ACTION, "$n wears $p about $s body.", ch, obj, NULL,
	       TO_ROOM);
	  act (AT_ACTION, "You wear $p about your body.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You sling $p on your back.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!can_layer (ch, obj, WEAR_WAIST))
	{
      ...
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  act (AT_ACTION, "$n wears $p about $s waist.", ch, obj, NULL,
	       TO_ROOM);
	  act (AT_ACTION, "You wear $p about your waist.", ch, obj, NULL,
	       TO_CHAR);
	}
      ...
      if (!get_eq_char (ch, WEAR_WRIST_L))
	{
	  if (!oprog_use_trigger (ch, obj, NULL, NULL))
	    {
	      act (AT_ACTION, "$n fits $p around $s left wrist.", ch, obj,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You fit $p around your left wrist.", ch, obj,
		   NULL, TO_CHAR);
	    }
      ...
      if (!get_eq_char (ch, WEAR_WRIST_R))
	{
	  if (!oprog_use_trigger (ch, obj, NULL, NULL))
	    {
	      act (AT_ACTION, "$n fits $p around $s right wrist.", ch, obj,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You fit $p around your right wrist.", ch, obj,
		   NULL, TO_CHAR);
	    }
      ...
      if (!get_eq_char (ch, WEAR_ANKLE_L))
	{
	  if (!oprog_use_trigger (ch, obj, NULL, NULL))
	    {
	      act (AT_ACTION, "$n fits $p around $s left ankle.", ch, obj,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You fit $p around your left ankle.", ch, obj,
		   NULL, TO_CHAR);
	    }
      ...
      if (!get_eq_char (ch, WEAR_ANKLE_R))
	{
	  if (!oprog_use_trigger (ch, obj, NULL, NULL))
	    {
	      act (AT_ACTION, "$n fits $p around $s right ankle.", ch, obj,
		   NULL, TO_ROOM);
	      act (AT_ACTION, "You fit $p around your right ankle.", ch, obj,
		   NULL, TO_CHAR);
	    }
      ...
      else
	{
	  ...
	  if (tmpobj)
	      ...
	      if (hd || sd)
		{
	      ...
	      if (!oprog_use_trigger (ch, obj, NULL, NULL))
		{
		  act (AT_ACTION, "$n dual-wields $p.", ch, obj, NULL,
		       TO_ROOM);
		  act (AT_ACTION, "You dual-wield $p.", ch, obj, NULL,
		       TO_CHAR);
		}
	  ...
	  || !oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  ...
	  act (AT_ACTION, "You hold $p in your hands.", ch, obj, NULL,
	       TO_CHAR);
	}
...
void
do_bury (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  bool shovel;
...
do_sacrifice (CHAR_DATA * ch, const char *argument)
{
  ...
  if (arg[0] == '\0' || !str_cmp (arg, ch->name))
    {
      act (AT_ACTION,
	   "$n offers $mself to $s deity, who graciously declines.", ch, NULL,
	   NULL, TO_ROOM);
      send_to_char
	("Your deity appreciates your offer and may accept it later.\r\n",
	 ch);
      return;
    }
  ...
  if (!CAN_WEAR (obj, ITEM_TAKE))
    {
      act (AT_PLAIN, "$p is not an acceptable sacrifice.", ch, obj, 0,
	   TO_CHAR);
      return;
    }
  if (IS_SET (obj->magic_flags, ITEM_PKDISARMED) && !IS_NPC (ch))
...
do_brandish (CHAR_DATA * ch, const char *argument)
{
  ...
  if ((sn = staff->value[3]) < 0 || sn >= num_skills
  ...
  if (--staff->value[2] <= 0)
    {
      act (AT_MAGIC, "$p blazes bright and vanishes from $n's hands!", ch,
	   staff, NULL, TO_ROOM);
      act (AT_MAGIC, "$p blazes bright and is gone!", ch, staff, NULL,
	   TO_CHAR);
 ...
 */
void
save_clan_storeroom (CHAR_DATA * ch, CLAN_DATA * clan)
{
  FILE *fp;
  char filename[256];
  short templvl;
...
do_auction (CHAR_DATA * ch, const char *argument)
{
  ...
  if (ch->level < 3)
    {
      send_to_char
	("You must be at least level three to use the auction...\r\n", ch);
      return;
    }
...
/*          spell_identify (0, LEVEL_HERO - 1, ch, auction->item); */
	  ...
	  ch_printf (ch,
		     ...
		     obj->name, aoran (item_type_name (obj)),
	  ...
	  switch (obj->item_type)
	    ...
	    case ITEM_QUIVER:
	      ch_printf (ch, "%s appears to %s.\r\n",
			 ...
			 150 ? "have a small to medium capacity" :
			 obj->value[0] <
			 300 ? "have a medium capacity" : obj->value[0] <
			 500 ? "have a medium to large capacity" :
			 obj->value[0] <
			 751 ? "have a large capacity" :
...
/*	    ch_printf( ch, "Bid: %d\r\n",newbet);	*/
	  ...
	  if (newbet < (auction->bet + 10000))
	    {
	      send_to_char
		("You must at least bid 10000 coins over the current bid.\r\n",
		 ch);
	      return;
	    }
	  ...
	  snprintf (buf, MAX_STRING_LENGTH,
		    "A bid of %s gold has been received on %s.\r\n",
...
*/
	...
	case ITEM_SCROLL:
	  ...
	  if (AUCTION_MEM > 0)
	    {
	      memmove ((char *) auction->history + sizeof (OBJ_INDEX_DATA *),
		       auction->history,
		       (AUCTION_MEM - 1) * sizeof (OBJ_INDEX_DATA *));
	  ...
	  snprintf (buf, MAX_STRING_LENGTH,
		    "A new item is being auctioned: %s at %d gold.",
...
void
obj_fall (OBJ_DATA * obj, bool through)
{
  EXIT_DATA *pexit;
  ...
  if (xIS_SET (obj->in_room->room_flags, ROOM_NOFLOOR)
	...
	{
	  act (AT_PLAIN, "$p falls far below...", obj->in_room->first_person,
	       obj, NULL, TO_ROOM);
	  act (AT_PLAIN, "$p falls far below...", obj->in_room->first_person,
	       obj, NULL, TO_CHAR);
	}
      ...
      if (obj->in_room->first_person)
	{
	  act (AT_PLAIN, "$p falls from above...", obj->in_room->first_person,
	       obj, NULL, TO_ROOM);
	  act (AT_PLAIN, "$p falls from above...", obj->in_room->first_person,
	       obj, NULL, TO_CHAR);
	}
...
*/ int dam = fall_count *
	  ...
	  if (obj->in_room->first_person && number_percent () > 15)
		...
		act (AT_WHITE, "$p bounces harmlessly off your head!", vch,
		     obj, NULL, TO_CHAR);
	      else
 ...
 */
OBJ_DATA *
recursive_note_find (OBJ_DATA * obj, const char *argument)
{
  ...
  switch (obj->item_type)
    ...
    case ITEM_PAPER:
      ...
      if ((subject =
	   get_extra_descr ("_subject_", obj->first_extradesc)) == NULL)
	break;
...
void
do_findnote (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *obj;
...
const char *
get_chance_verb (OBJ_DATA * obj)
{
  return (obj->action_desc[0] != '\0') ? obj->action_desc : "roll$q";
}

const char *
get_ed_number (OBJ_DATA * obj, int number)
{
  EXTRA_DESCR_DATA *ed;
  int count;
...
void
do_rolldie (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *die;
...
/*  char* face_string = NULL;
  ...
  while (roll_count++ < numrolls)
      ...
      if (die->value[1] == 1)
	{
	  ...
	  if (face_name)
	    ...
	    snprintf (current_roll_string, MAX_STRING_LENGTH, "%d",
		      current_roll);
	}
      else
  ...
  if (numrolls > 1 && die->value[2] == 1)
    {
      snprintf (total_string, MAX_STRING_LENGTH, ", for a total of %d",
		rollsum);
