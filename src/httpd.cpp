...
static void process_command_line_arguments(char *argv[], char **options) {
  ...
  if (argv[1] != NULL && argv[1][0] != '-') {
    ...
    snprintf(cpath, sizeof(cpath), "%s", HTTPD_CONFIG_FILE);
  } else {
    snprintf(cpath, sizeof(cpath), "%.*s%c%s",
             (int) (p - argv[0]), argv[0], DIRSEP, HTTPD_CONFIG_FILE);
  }
...
static void init_server_name(void) {
  const char *descr = "";
  snprintf(server_name, sizeof(server_name), "@{XSMAUG} Mongoose web server v.%s%s",
           MVER, descr);
}
