...
/* do not start the SMAUG if it is already a running service */
	  ...
	  if (status == 0 && svcstatus.dwCurrentState == SERVICE_RUNNING)
	    {
	      fprintf (stderr,
		       "The XSMAUG is already running as a service.\n");
	      return 1;
	    }
	  worker_thread (NULL);
	}
      ...
      if (!StartServiceCtrlDispatcher (dispatchTable))
	{
	  ...
	  fprintf (stderr,
		   "You can save time on the next invocation by specifying: XSMAUG /run\n");
	  worker_thread (NULL);
	}
...
StopService (LPTSTR lpszMsg)
{
  ...
  snprintf (chMsg, 256, "%s error: %s", THIS_SERVICE,
...
convert_error (DWORD error)
{
  ...
  if (!FormatMessage (FORMAT_MESSAGE_FROM_SYSTEM |
		      ...
		      LANG_NEUTRAL, (LPTSTR) & formattedmsg, 0, NULL))
    {
