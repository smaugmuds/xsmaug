...
void
delete_project (PROJECT_DATA * project)
{
  NOTE_DATA *nlog, *tlog;
...
bool
can_remove (CHAR_DATA * ch, BOARD_DATA * board)
{
  /*
...
bool
can_read (CHAR_DATA * ch, BOARD_DATA * board)
{
  /*
  ...
  if (board->read_group[0] != '\0')
	...
	return TRUE;
	  ...
	  && !str_cmp (ch->pcdata->council->name, board->read_group))
	return TRUE;
    }
...
bool
can_post (CHAR_DATA * ch, BOARD_DATA * board)
{
  /*
  ...
  if (board->post_group[0] != '\0')
	...
	return TRUE;
      if (ch->pcdata->council
	  && !str_cmp (ch->pcdata->council->name, board->post_group))
	return TRUE;
    }
  return FALSE;
}
...
write_board (BOARD_DATA * board)
{
  ...
  if ((fp = fopen (filename, "w")) == NULL)
      ...
      for (pnote = board->first_note; pnote; pnote = pnote->next)
	{
	  fprintf (fp,
		   "Sender  %s~\nDate    %s~\nTo      %s~\nSubject %s~\nVoting %d\nYesvotes %s~\nNovotes %s~\nAbstentions %s~\nText\n%s~\n\n",
		   pnote->sender, pnote->date, pnote->to_list, pnote->subject,
		   pnote->voting, pnote->yesvotes, pnote->novotes,
...
void
note_remove (BOARD_DATA * board, NOTE_DATA * pnote)
{
...
void
do_noteroom (CHAR_DATA * ch, const char *argument)
{
  BOARD_DATA *board;
...
void
do_mailroom (CHAR_DATA * ch, const char *argument)
{
  BOARD_DATA *board;
  char arg[MAX_STRING_LENGTH];
  char arg_passed[MAX_STRING_LENGTH];
...
void
do_note (CHAR_DATA * ch, const char *arg_passed, bool IS_MAIL)
{
      ...
      if (!IS_MAIL)
	{
	  ...
	  for (pnote = board->first_note; pnote; pnote = pnote->next)
	      ...
	      if (vnum == anum || fAll)
		{
		  ...
		  if (board->oreadmessg)
		    act (AT_ACTION, board->oreadmessg, ch, NULL, NULL,
			 TO_CANSEE);
		  else
  ...
  if (!str_cmp (arg, "vote"))
      ...
      if (pnote->voting != VOTE_OPEN)
	{
       ...
       */
      snprintf (buf, MAX_STRING_LENGTH, "%s %s %s", pnote->yesvotes,
	...
	{
	  snprintf (buf, MAX_STRING_LENGTH, "%s %s", pnote->yesvotes,
		    ch->name);
	  DISPOSE (pnote->yesvotes);
	...
	{
	  snprintf (buf, MAX_STRING_LENGTH, "%s %s", pnote->novotes,
		    ch->name);
	  DISPOSE (pnote->novotes);
	...
	{
	  snprintf (buf, MAX_STRING_LENGTH, "%s %s", pnote->abstentions,
		    ch->name);
	  DISPOSE (pnote->abstentions);
  ...
  if (!str_cmp (arg, "write"))
	  ...
	  || paper->item_type != ITEM_PAPER)
	{
	  ...
	  act (AT_MAGIC,
	       "A piece of parchment magically appears in $n's hands!", ch,
	       NULL, NULL, TO_ROOM);
	  act (AT_MAGIC, "A piece of parchment appears in your hands.", ch,
	       NULL, NULL, TO_CHAR);
	}
      if (paper->value[0] < 2)
	{
...
/*	char *pn;*/
  ...
  if (!str_cmp (arg, "post"))
	...
	{
	  send_to_char
	    ("There is no bulletin board here to post your note on.\r\n", ch);
	  return;
	}
      if (!can_post (ch, board))
	{
	  send_to_char
	    ("A magical force prevents you from posting your note here...\r\n",
	     ch);
	  return;
	}
	...
	{
	  send_to_char
	    ("There is no room on this board to post your note.\r\n", ch);
	  return;
	}
  ...
  if (!str_cmp (arg, "remove") || !str_cmp (arg, "take")
      || !str_cmp (arg, "copy"))
    {
      char take;
      ...
      for (pnote = board->first_note; pnote; pnote = pnote->next)
	{
	  ...
	  if ((is_note_to (ch, pnote) || can_remove (ch, board))
	      ...
	      if (take != 0)
		{
		  if (ch->gold < 50
		      && get_trust (ch) < sysdata.read_mail_free)
		    {
		      if (take == 1)
			send_to_char
			  ("It costs 50 coins to take your mail.\r\n", ch);
		      else
			send_to_char
			  ("It costs 50 coins to copy your mail.\r\n", ch);
		      return;
		    }
		  if (get_trust (ch) < sysdata.read_mail_free)
		    ch->gold -= 50;
		  ...
		  paper->value[1] = 2;
		  paper->value[2] = 2;
		  snprintf (short_desc_buf, MAX_STRING_LENGTH,
			    "a note from %s to %s", pnote->sender,
			    pnote->to_list);
		  STRFREE (paper->short_descr);
		  paper->short_descr = STRALLOC (short_desc_buf);
		  snprintf (long_desc_buf, MAX_STRING_LENGTH,
		  ...
		  snprintf (keyword_buf, MAX_STRING_LENGTH,
			    "note parchment paper %s", pnote->to_list);
		  STRFREE (paper->name);
		...
		{
		  if (board->otakemessg)
		    act (AT_ACTION, board->otakemessg, ch, NULL, NULL,
			 TO_ROOM);
		  else
		    act (AT_ACTION, "$n takes a note.", ch, NULL, NULL,
			 TO_ROOM);
		...
		{
		  if (board->ocopymessg)
		    act (AT_ACTION, board->ocopymessg, ch, NULL, NULL,
			 TO_ROOM);
		  else
		    act (AT_ACTION, "$n copies a note.", ch, NULL, NULL,
			 TO_ROOM);
		  obj_to_char (paper, ch);
		}
	      else
		{
		  if (board->oremovemessg)
		    act (AT_ACTION, board->oremovemessg, ch, NULL, NULL,
			 TO_ROOM);
		  else
		    act (AT_ACTION, "$n removes a note.", ch, NULL, NULL,
			 TO_ROOM);
		}
	      return;
	    }
...
void
do_makeboard (CHAR_DATA * ch, const char *argument)
{
  BOARD_DATA *board;
...
void
do_bset (CHAR_DATA * ch, const char *argument)
{
  BOARD_DATA *board;
  bool found;
  ...
  if (!str_cmp (arg2, "extra_removers"))
	...
	snprintf (buf, MAX_STRING_LENGTH, "%s %s", board->extra_removers,
		  argument);
      DISPOSE (board->extra_removers);
  ...
  if (!str_cmp (arg2, "extra_readers"))
	...
	snprintf (buf, MAX_STRING_LENGTH, "%s %s", board->extra_readers,
		  argument);
      DISPOSE (board->extra_readers);
...
void
do_bstat (CHAR_DATA * ch, const char *argument)
{
  BOARD_DATA *board;
  bool found;
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!found)
	...
	{
	  send_to_char_color
	    ("&GBoard not found.  Usage: bstat <board filename>\r\n", ch);
	  return;
	}
      else
	{
	  ...
	  if (!board)
	    {
	      send_to_char_color
		("&GNo board present.  Usage: bstat <board filename>\r\n",
		 ch);
	      return;
	    }
	}
  ...
  ch_printf_color (ch, "&GORemove Message: %s\r\n",
		   board->
		   oremovemessg ? board->oremovemessg : "Default Message");
...
void
do_boards (CHAR_DATA * ch, const char *argument)
{
  BOARD_DATA *board;
  ...
  for (board = first_board; board; board = board->next)
    pager_printf_color (ch,
			"&G%-15.15s #: %5d Read: %2d Post: %2d Rmv: %2d Max: %3d Posts: &g%3d &GType: %d\r\n",
