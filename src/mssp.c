...
void
write_to_descriptor_printf (DESCRIPTOR_DATA * desc, const char *fmt, ...)
GNUC_FORMAT (2, 3);
...
fread_mssp_info (FILE * fp)
{
  ...
  for (;;)
	...
	case 'E':
	  KEY ("EquipmentSystem", mssp_info->equipmentSystem,
	...
	case 'H':
	  ...
	  KEY ("HiringBuilders", mssp_info->hiringBuilders,
	       fread_number (fp));
	...
	case 'M':
	  ...
	  KEY ("Multiplaying", mssp_info->multiplaying,
	       fread_string_nohash (fp));
	  KEY ("MXP", mssp_info->mxp, fread_number (fp));
	  break;

	case 'N':
	  KEY ("NewbieFriendly", mssp_info->newbieFriendly,
	       fread_number (fp));
	  break;
	...
	case 'P':
	  ...
	  KEY ("PlayerCrafting", mssp_info->playerCrafting,
	       fread_number (fp));
	  KEY ("PlayerGuilds", mssp_info->playerGuilds, fread_number (fp));
	  KEY ("PlayerKilling", mssp_info->playerKilling,
	       fread_string_nohash (fp));
	  KEY ("Pueblo", mssp_info->pueblo, fread_number (fp));
	  break;

	case 'Q':
	  KEY ("QuestSystem", mssp_info->questSystem,
	       fread_string_nohash (fp));
	  break;

	case 'R':
	  KEY ("RolePlaying", mssp_info->roleplaying,
	       fread_string_nohash (fp));
	  break;
	...
	case 'T':
	  KEY ("TrainingSystem", mssp_info->trainingSystem,
	       fread_string_nohash (fp));
	  break;
	...
	case 'W':
	  ...
	  KEY ("WorldOriginality", mssp_info->worldOriginality,
	       fread_string_nohash (fp));
...
void
do_setmssp (CHAR_DATA * ch, const char *argument)
{
  char arg1[MIL];
  char **strptr = NULL;
  bool *ynptr = NULL;
  ...
  if (!argument || (argument[0] == '\0'))
    {
      ...
      send_to_char
	("hostname       contact           icon               lanuage          location\r\n",
	 ch);
      send_to_char
	("website        family            genre              gameplay         game_system\r\n",
	 ch);
      send_to_char
	("intermud       status            subgenre           created          min_age\r\n",
	 ch);
      send_to_char
	("worlds         ansi              mccp               mcp              msp\r\n",
	 ch);
      send_to_char
	("ssl            mxp               pueblo             vt100            xterm256\r\n",
	 ch);
      send_to_char
	("pay2play       pay4perks         hiring_builders    hiring_coders    adult_material\r\n",
	 ch);
      send_to_char
	("multiclassing  newbie_friendly   player_cities      player_clans     player_crafting\r\n",
	 ch);
      send_to_char
	("player_guilds  equipment_system  multiplaying       player_killing   quest_system\r\n",
	 ch);
  ...
  if (!str_cmp (arg1, "worlds"))
  ...
  else if (!str_cmp (arg1, "created"))
    {
      ...
      if (!is_number (argument) || (value < MSSP_MINCREATED)
	  || (value > MSSP_MAXCREATED))
	{
	  ch_printf (ch,
		     "The value for created must be between %d and %d\r\n",
  ...
  else if (!str_cmp (arg1, "multiplaying")
	   || !str_cmp (arg1, "player_killing"))
    {
      if (strcmp (argument, "None") && strcmp (argument, "Restricted")
	  && str_cmp (argument, "Full"))
	{
	  ch_printf (ch,
		     "Valid choices for %s are: None, Restricted or Full\r\n",
		     arg1);
	  return;
	}
  ...
  else if (!str_cmp (arg1, "training_system")
	   || !str_cmp (arg1, "equipment_system"))
    {
      if (strcmp (argument, "None") && strcmp (argument, "Level")
	  && str_cmp (argument, "Skill") && str_cmp (argument, "Both"))
	{
	  ch_printf (ch,
		     "Valid choices for %s are: None, Level, Skill or Both\r\n",
		     arg1);
	  return;
	}
    ...
    {
      if (strcmp (argument, "None") && strcmp (argument, "Immortal Run")
	  && str_cmp (argument, "Automated")
	  && str_cmp (argument, "Integrated"))
	{
	  ch_printf (ch,
		     "Valid choices for %s are: None, Immortal Run, Automated or Integrated\r\n",
		     arg1);
	  return;
	}
      DISPOSE (mssp_info->questSystem);
    ...
    {
      if (strcmp (argument, "None") && strcmp (argument, "Accepted")
	  && str_cmp (argument, "Encouraged")
	  && str_cmp (argument, "Enforced"))
	{
	  ch_printf (ch,
		     "Valid choices for %s are: None, Accepted, Encouraged or Enforced\r\n",
		     arg1);
	  return;
	}
      DISPOSE (mssp_info->roleplaying);
    ...
    {
      if (strcmp (argument, "All Stock") && strcmp (argument, "Mostly Stock")
  ...
  else if (!str_cmp (arg1, "min_age"))
    {
      ...
      if (!is_number (argument) || (value < MSSP_MINAGE)
	  || (value > MSSP_MAXAGE))
	{
	  ch_printf (ch,
		     "The value for min_age must be between %d and %d\r\n",
...
void
send_mssp_data (DESCRIPTOR_DATA * d)
{
  if (!d)
    {
      bug ("%s: NULL d", __FUNCTION__);
