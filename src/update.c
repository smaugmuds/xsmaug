...
advance_level (CHAR_DATA * ch)
{
  ...
  if (ch->level < LEVEL_IMMORTAL)
	...
	snprintf (buf, MAX_STRING_LENGTH,
		  ...
		  add_hp, ch->max_hit, 1, ch->level + 10, add_move,
...
gain_condition (CHAR_DATA * ch, int iCond, int value)
{
  ...
  if (iCond == COND_BLOODTHIRST)
    ch->pcdata->condition[iCond] =
      URANGE (0, condition + value, 10 + ch->level);
  else
	  ...
	  if (ch->level < LEVEL_AVATAR)
	    {
	      ...
	      act (AT_BLOOD, "$n is suffering from lack of blood!", ch, NULL,
		   NULL, TO_ROOM);
...
mobile_update (void)
{
  ...
  for (ch = last_char; ch; ch = trvch_wnext (lc))
	  ...
	  && !IS_AFFECTED (ch, AFF_CHARM))
	{
      ...
      if (!xIS_SET (ch->act, ACT_RUNNING) && !xIS_SET (ch->act, ACT_SENTINEL)
	  ...
	  for (rch = ch->in_room->first_person; rch; rch = rch->next_in_room)
	      ...
	      if (is_fearing (ch, rch))
		{
		  switch (number_bits (2))
		    ...
		    case 0:
		      snprintf (buf, MAX_STRING_LENGTH,
				"Get away from me, %s!", rch->name);
		      break;
		    case 1:
		      snprintf (buf, MAX_STRING_LENGTH, "Leave me be, %s!",
				rch->name);
		      break;
		    case 2:
		      snprintf (buf, MAX_STRING_LENGTH,
				"%s is trying to kill me!  Help!", rch->name);
		      break;
		    case 3:
		      snprintf (buf, MAX_STRING_LENGTH,
				"Someone save me from %s!", rch->name);
		      break;
		    }
		  do_yell (ch, buf);
		  found = TRUE;
      ...
      if (!IS_NPC (ch) && ch->level < LEVEL_IMMORTAL)
	{
	  ...
	  if (++ch->timer >= 12)
	      ...
	      if (!IS_IDLE (ch))
		{
		  ...
		  act (AT_ACTION, "$n disappears into the void.", ch, NULL,
		       NULL, TO_ROOM);
	  ...
	  if (ch->mental_state >= 30)
		...
		case 8:
		  send_to_char ("You begin to understand... everything.\r\n",
				ch);
		  act (AT_ACTION, "$n starts ranting like a madman!", ch,
		       NULL, NULL, TO_ROOM);
		  break;
		case 9:
	  ...
	  if (obj->timer > 0 && obj->value[2] > timerfrac)
	    {
	      ...
	      snprintf (buf, MAX_STRING_LENGTH,
			corpse_descs[UMIN (timerfrac - 1, 4)], bufptr);
	  ...
	  if (xIS_SET (ch->act, ACT_RUNNING))
	    {
		  ...
		  && ch->position == POS_STANDING
		  && !xIS_SET (ch->act, ACT_MOUNTED) && !ch->fighting
		  && ch->hunting)
		{
		  WAIT_STATE (ch, 2 * PULSE_VIOLENCE);
		  hunt_victim (ch);
	  ...
	  if (!ch->desc)
		   ...
		   wch = trvch_next (lc2))
		{
		  if (!IS_NPC (wch)
		      ...
		      || !IS_AWAKE (wch) || (xIS_SET (wch->act, ACT_WIMPY)
					     && IS_AWAKE (ch))
		      || !can_see (wch, ch))
		    continue;
...
aggr_update (void)
{
  ...
  for (d = first_descriptor; d; d = dnext)
	...
	continue;
      ...
      if (char_died (wch) || IS_NPC (wch) || wch->level >= LEVEL_IMMORTAL
	  || !wch->in_room)
	continue;
      ...
      for (ch = wch->in_room->first_person; ch; ch = trvch_next (lc))
	{
	  ...
	  if (!IS_NPC (ch)
	      || ch->fighting
	      || IS_AFFECTED (ch, AFF_CHARM)
	      || !IS_AWAKE (ch) || (xIS_SET (ch->act, ACT_WIMPY)
				    && IS_AWAKE (wch)) || !can_see (ch, wch))
	    continue;
...
update_handler (void)
{
  ...
  if (--pulse_point <= 0)
    {
      pulse_point =
	number_range ((int) (PULSE_TICK * 0.75), (int) (PULSE_TICK * 1.25));
  ...
  switch (++auction->going)	/* increase the going state */
      ...
      if (auction->bet > 0 && auction->buyer != auction->seller)
	{
	  ...
	  if ((auction->buyer->carry_weight +
	       get_obj_weight (auction->item)) > can_carry_w (auction->buyer))
	    {
	      act (AT_PLAIN,
	...
	{
	  snprintf (buf, MAX_STRING_LENGTH,
		    "No bids received for %s - removed from auction.\r\n",
		    auction->item->short_descr);
	  talk_auction (buf);
	  act (AT_ACTION,
...
void
subtract_times (struct timeval *etime, struct timeval *sttime)
{
...
time_update (void)
{
  ...
  if (time_info.hour == sysdata.hourdaybegin
      || time_info.hour == sysdata.hoursunrise
      || time_info.hour == sysdata.hournoon
      || time_info.hour == sysdata.hoursunset
      ...
      for (d = first_descriptor; d; d = d->next)
	{
	  if (d->connected == CON_PLAYING && IS_OUTSIDE (d->character)
		...
		getWeatherCell (d->character->in_room->area);
