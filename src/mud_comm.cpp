...
void directory_check( void )
{
   ...
   if( chdir( LOGDIR ) )
   {
      fprintf( stderr, "Creating required directory: %s\n", LOGDIR );
      snprintf( buf, 256, "mkdir -p %s", LOGDIR );

      if( system( buf ) )
      {
         fprintf( stderr, "FATAL ERROR :: Unable to create required directrory: %s\n", LOGDIR );
         exit( 1 );
      }
   }
   ...
   for( x = 0; x < sizeof( directory_table ) / sizeof( directory_table[0] ); ++x )
   {
      ...
      {
         snprintf( buf, 256, "mkdir -p %s", directory_table[x] );
      ...
      if( !str_cmp( directory_table[x], PLAYER_DIR ) )
      {
         ...
         for( alpha_loop = 0; alpha_loop <= 25; ++alpha_loop )
         {
            ...
            if( chdir( dirname ) )
            {
               ...
               snprintf( buf, 256, "mkdir -p %s", dirname );
...
void open_mud_log( void )
{
   ...
   {
      snprintf( buf, 256, "%s/%d.log", LOGDIR, logindex );
      if( exists_file( buf ) )
         continue;
...
bool check_parse_name( const string & name, bool newchar )
{
    ...
    */
   char buf[MSL];
   snprintf( buf, MSL, "grep -i -x %s %sreserved.lst > /dev/null", name.c_str(  ), SYSTEM_DIR );
   ...
   snprintf( buf, MSL, "grep -i -x %s %s/reserved.lst > /dev/null", invname.c_str(  ), SYSTEM_DIR );
   ...
   for( ds = dlist.begin(  ); ds != dlist.end(  ); )
   {
      ...
      if( FD_ISSET( d->descriptor, &exc_set ) )
      {
         ...
         continue;
               ...
               || ( d->connected != CON_PLAYING && d->idle > 9600 )  /* 40 mins */
 ...
 * Added to XSMAUG by Samson on 5-8-03.
 */
