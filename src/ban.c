...
void
do_ban (CHAR_DATA * ch, const char *argument)
{
...
void
do_allow (CHAR_DATA * ch, const char *argument)
{
  BAN_DATA *pban;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
 ...
 */
void
do_warn (CHAR_DATA * ch, const char *argument)
{
 ...
 */
int
add_ban (CHAR_DATA * ch, const char *arg1, const char *arg2, int btime,
	 int type)
{
		...
		if (temp->flag == value)
		  {
			...
			if (temp->level == BAN_WARN)
			  temp->warn = TRUE;
			snprintf (buf, MAX_STRING_LENGTH, "%24.24s",
				  ctime (&current_time));
...
show_bans (CHAR_DATA * ch, int type)
{
  ...
  for (bnum = 1; pban; pban = pban->next, bnum++)
    pager_printf (ch, "[%2d] %-4s (%2d) %-24s %-15s %4d  %s\r\n", bnum,
		  (pban->warn) ? "YES" : "no", pban->level, pban->ban_time,
		  pban->ban_by, pban->duration, pban->name);
  return;
}
  ...
  for (; pban; pban = pban->next)
	  ...
	  if (ch->level > pban->level)
	    {
	      if (pban->warn)
		{
		  log_printf_plus (LOG_WARN, sysdata.log_level,
				   "%s class logging in from %s.", pban->name,
				   ch->desc->host);
		}
	      return FALSE;
	    }
      ...
      if (type == BAN_RACE && pban->flag == ch->race)
	{
	  ...
	  if (ch->level > pban->level)
		...
		{
		  log_printf_plus (LOG_WARN, sysdata.log_level,
				   "%s race logging in from %s.", pban->name,
				   ch->desc->host);
		}
	      return FALSE;
	    }
...
bool
check_expire (BAN_DATA * pban)
{
  if (pban->unban_date < 0)
    return FALSE;
  if (pban->unban_date <= current_time)
    {
      log_printf_plus (LOG_WARN, sysdata.log_level, "%s ban has expired.",
		       pban->name);
      return TRUE;
    }
  return FALSE;
...
void
dispose_ban (BAN_DATA * pban, int type)
{
  if (!pban)
    return;
