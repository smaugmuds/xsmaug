...
TRV_DATA *
trvch_create (CHAR_DATA * ch, trv_type tp)
{
  CHAR_DATA *first, *ptr;
  TRV_DATA *New;
  int count = 0;
...
TRV_DATA *
trvobj_create (OBJ_DATA * obj, trv_type tp)
{
  OBJ_DATA *first, *ptr;
  void *ground_zero;
  TRV_DATA *New;
...
/*    case TR_OBJ_WORLD_FORW:
  ...
  if (!New)
    {
      bug ("%s: malloc() failure, %d nodes, type %d.", __FUNCTION__, count,
	   tp);
      return NULL;
    }
  New->el = (void **) malloc (count * sizeof (void *));
...
trvch_next (TRV_DATA * lc)
{
  ...
  while (lc->current < lc->count)
	...
	continue;
       ...
       */
      for (extr = extracted_char_queue; extr != lc->ext_mark;
	   extr = extr->next)
	if (extr->ch == nx)
	  break;
      if (extr != lc->ext_mark)
...
trvobj_next (TRV_DATA * lc)
{
  ...
  while (lc->current < lc->count)
	...
	continue;
       ...
       */
      for (extr = extracted_obj_queue; extr != lc->ext_mark;
	   extr = extr->next)
	if (extr == nx)
	  break;
      if (extr != lc->ext_mark)
...
TRV_WORLD *
trworld_create (trv_type tp)
{
  TRV_WORLD *tnew;

  if (tp != TR_CHAR_WORLD_FORW && tp != TR_CHAR_WORLD_BACK
      && tp != TR_OBJ_WORLD_FORW && tp != TR_OBJ_WORLD_BACK)
    {
...
void
trworld_dispose (TRV_WORLD ** trash)
{

  if ((trw_heap + --trw_loops) != *trash)
    {
      bug ("%s: midlist control block (%zd).", __FUNCTION__,
	   *trash - trw_heap);
      ++trw_loops;
    }
  else
...
void
trworld_char_check (CHAR_DATA * rmv)
{
  TRV_WORLD *sp;
...
void
trworld_obj_check (OBJ_DATA * rmv)
{
  TRV_WORLD *sp;
...
int
urange (int mincheck, int check, int maxcheck)
{
  if (check < mincheck)
    return mincheck;
  if (check > maxcheck)
 ...
 */
int
exp_level (CHAR_DATA * ch, short level)
{
  int lvl;
 ...
 */
short
level_exp (CHAR_DATA * ch, int cexp)
{
  int x, lastx, y, tmp;
...
can_carry_n (CHAR_DATA * ch)
{
  ...
  if (get_eq_char (ch, WEAR_SHIELD))
    ++penalty;
  return URANGE (5, (ch->level + 15) / 5 + get_curr_dex (ch) - 13 - penalty,
		 20);
}
 ...
 */
bool
is_name2 (const char *str, const char *namelist)
{
  char name[MAX_INPUT_LENGTH];
...
bool
is_name2_prefix (const char *str, const char *namelist)
{
  char name[MAX_INPUT_LENGTH];
...
bool
nifty_is_name (const char *str, const char *namelist)
{
  char name[MAX_INPUT_LENGTH];
  bool valid = FALSE;
...
bool
nifty_is_name_prefix (const char *str, const char *namelist)
{
  char name[MAX_INPUT_LENGTH];
  bool valid = FALSE;
...
modify_skill (CHAR_DATA * ch, int sn, int mod, bool fAdd)
{
  if (!IS_NPC (ch))
	...
	ch->pcdata->learned[sn] =
	  URANGE (0, ch->pcdata->learned[sn] + mod, GET_ADEPT (ch, sn));
    }
}
...
affect_modify (CHAR_DATA * ch, AFFECT_DATA * paf, bool fAdd)
{
  ...
  switch (paf->location % REVERSE_APPLY)
	...
	ch->pcdata->condition[COND_BLOODTHIRST] =
	  URANGE (0, ch->pcdata->condition[COND_BLOODTHIRST] + mod,
		  ch->level + 10);
      break;
	  ...
	  && skill->type == SKILL_SPELL)
	{
	  if (skill->target == TAR_IGNORE || skill->target == TAR_OBJ_INV)
	    {
	      bug
		("APPLY_WEARSPELL trying to apply bad target spell.  SN is %d.",
		 mod);
	      return;
	    }
	  if ((retcode =
	       (*skill->spell_fun) (mod, ch->level, ch, ch)) == rCHAR_DIED
	      || char_died (ch))
	    return;
	}
      break;
  ...
  if (!IS_NPC (ch)
	...
	{
	  depth++;
	  act (AT_ACTION, "You are too weak to wield $p any longer.", ch,
	       wield, NULL, TO_CHAR);
...
char_to_room (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoomIndex)
{
  ...
  if (!pRoomIndex || !get_room_index (pRoomIndex->vnum))
    {
      bug ("%s: %s -> NULL room!  Putting char in limbo (%d)", __FUNCTION__,
  ...
  if (!IS_NPC (ch) && xIS_SET (pRoomIndex->room_flags, ROOM_SAFE)
  ...
  if (!grouped)
      ...
      else
	{
	  ...
	  for (otmp = ch->first_carrying; otmp; otmp = otmp->next_content)
		...
		{
		  INSERT (obj, otmp, ch->first_carrying, next_content,
			  prev_content);
		  break;
		}
...
equip_char (CHAR_DATA * ch, OBJ_DATA * obj, int iWear)
{
  ...
  if ((IS_OBJ_STAT (obj, ITEM_ANTI_EVIL) && IS_EVIL (ch))
      ...
      || (IS_OBJ_STAT (obj, ITEM_ANTI_NEUTRAL) && IS_NEUTRAL (ch)))
    {
      ...
      if (loading_char != ch)
	{
	  act (AT_MAGIC, "You are zapped by $p and drop it.", ch, obj, NULL,
	       TO_CHAR);
	  act (AT_MAGIC, "$n is zapped by $p and drops it.", ch, obj, NULL,
	       TO_ROOM);
	}
      if (obj->carried_by)
	obj_from_char (obj);
 ...
 */
void write_corpses
args ((CHAR_DATA * ch, const char *name, OBJ_DATA * objrem));
...
extract_obj (OBJ_DATA * obj)
{
  ...
  if (obj_extracted (obj))
    {
      bug ("%s: obj %d already extracted!", __FUNCTION__,
	   obj->pIndexData->vnum);
      return;
    }
  ...
  if (auction->item && auction->item == obj)
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH,
		"Sale of %s has been stopped by a system action.",
...
extract_char (CHAR_DATA * ch, bool fPull)
{
	  ...
	  if (wch->pcdata && wch->pcdata->pet == ch)
	    {
	      wch->pcdata->pet = NULL;
	      if (wch->in_room == ch->in_room)
		act (AT_SOCIAL, "You mourn for the loss of $N.", wch, NULL,
		     ch, TO_CHAR);
	    }
	}
    }
  ...
  if (!fPull)
	...
	{
	  act (AT_MAGIC,
	       "$n mutters a few incantations, waves $s hands and points $s finger.",
	       wch, NULL, NULL, TO_ROOM);
	  act (AT_MAGIC, "$n appears from some strange swirling mists!", ch,
	       NULL, NULL, TO_ROOM);
	  snprintf (buf, MAX_STRING_LENGTH,
		    "Welcome back to the land of the living, %s",
		    capitalize (ch->name));
	  do_say (wch, buf);
	}
      else
 ...
 */
OBJ_DATA *
get_obj_list_rev (CHAR_DATA * ch, const char *argument, OBJ_DATA * list)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  int number;
 ...
 */
OBJ_DATA *
get_obj_carry (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  int number, count, vnum;
  ...
  for (obj = ch->last_carrying; obj; obj = obj->prev_content)
    if (obj->wear_loc == WEAR_NONE
	&& can_see_obj (ch, obj) && (nifty_is_name (arg, obj->name)
 ...
 */
OBJ_DATA *
get_obj_wear (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  int number, count, vnum;
...
ms_find_obj (CHAR_DATA * ch)
{
  ...
  if (ms > 15)			/* range 1 to 20 -- feel free to add more */
	...
	break;
      case 16:
	t =
	  "Your hand grasps the worst pile of revoltingness that you could ever imagine!\r\n";
	break;
      case 17:
 ...
 */
OBJ_DATA *
find_obj (CHAR_DATA * ch, const char *argument, bool carryonly)
{
...
/* Deadlies link-dead over 2 ticks aren't seen by mortals -- Blodkai */
  if (!IS_IMMORTAL (ch) && !IS_NPC (ch) && !IS_NPC (victim)
      && IS_PKILL (victim) && victim->timer > 1 && !victim->desc)
    return FALSE;
...
spring_trap (CHAR_DATA * ch, OBJ_DATA * obj)
{
  ...
  switch (typ)
    ...
    case TRAP_TYPE_ACID_SPRAY:
      retcode =
	obj_cast_spell (skill_lookup ("acid blast"), lev, ch, ch, NULL);
      break;
    case TRAP_TYPE_SEX_CHANGE:
      retcode =
	obj_cast_spell (skill_lookup ("change sex"), lev, ch, ch, NULL);
      break;
    case TRAP_TYPE_FLAME:
    case TRAP_TYPE_EXPLOSION:
 ...
 */
OBJ_DATA *
get_objtype (CHAR_DATA * ch, short type)
{
  OBJ_DATA *obj;
 ...
 */
void
clean_resets (ROOM_INDEX_DATA * room)
{
...
fix_char (CHAR_DATA * ch)
{
  ...
  if (ch->in_room)
	...
	{
	  if (aff->location != APPLY_WEARSPELL
...
showaffect (CHAR_DATA * ch, AFFECT_DATA * paf)
{
  ...
  if (paf->location != APPLY_NONE && paf->modifier != 0)
	...
	default:
	  ...
	  break;
	case APPLY_AFFECT:
	  snprintf (buf, MAX_STRING_LENGTH, "Affects %s by",
	...
	case APPLY_REMOVESPELL:
	  snprintf (buf, MAX_STRING_LENGTH, "Casts spell '%s'\r\n",
		    IS_VALID_SN (paf->modifier) ? skill_table[paf->
							      modifier]->name
		    : "unknown");
	  break;
	case APPLY_RESISTANT:
 ...
 */
void
add_timer (CHAR_DATA * ch, short type, int count, DO_FUN * fun, int value)
{
  TIMER *timer;
...
TIMER *
get_timerptr (CHAR_DATA * ch, short type)
{
  TIMER *timer;
...
short
get_timer (CHAR_DATA * ch, short type)
{
  TIMER *timer;
...
void
remove_timer (CHAR_DATA * ch, short type)
{
  TIMER *timer;
...
in_soft_range (CHAR_DATA * ch, AREA_DATA * tarea)
{
  if (IS_IMMORTAL (ch))
    ...
    return TRUE;
  else if (ch->level >= tarea->low_soft_range
	   || ch->level <= tarea->hi_soft_range)
    return TRUE;
  else
    return FALSE;
}
...
in_hard_range (CHAR_DATA * ch, AREA_DATA * tarea)
{
  if (IS_IMMORTAL (ch))
    ...
    return TRUE;
  else if (ch->level >= tarea->low_hard_range
	   && ch->level <= tarea->hi_hard_range)
    return TRUE;
  else
    return FALSE;
}
 ...
 */
bool
chance (CHAR_DATA * ch, short percent)
{
...
bool
chance_attrib (CHAR_DATA * ch, short percent, short attrib)
{
...
empty_obj (OBJ_DATA * obj, OBJ_DATA * destobj, ROOM_INDEX_DATA * destroom)
{
  ...
  if (destobj || (!destroom && !ch && (destobj = obj->in_obj) != NULL))
      ...
      for (otmp = obj->first_content; otmp; otmp = otmp_next)
	{
	   ...
	   */
	  if (destobj->item_type == ITEM_KEYRING
	      && otmp->item_type != ITEM_KEY)
	    continue;
	  if (destobj->item_type == ITEM_QUIVER
	      && otmp->item_type != ITEM_PROJECTILE)
	    continue;
	  if ((destobj->item_type == ITEM_CONTAINER
	       || destobj->item_type == ITEM_KEYRING
 ...
 */
void
boost_economy (AREA_DATA * tarea, int gold)
{
 ...
 */
void
lower_economy (AREA_DATA * tarea, int gold)
{
  while (gold >= 1000000000)
    {
      --tarea->high_economy;
 ...
 */
bool
economy_has (AREA_DATA * tarea, int gold)
{
  int hasgold =
    ((tarea->high_economy > 0) ? 1 : 0) * 1000000000 + tarea->low_economy;
...
add_kill (CHAR_DATA * ch, CHAR_DATA * mob)
{
  ...
  track =
    URANGE (2, ((ch->level + 3) * MAX_KILLTRACK) / LEVEL_AVATAR,
	    MAX_KILLTRACK);
  for (x = 0; x < track; x++)
...
get_area_obj (OBJ_INDEX_DATA * pObjIndex)
{
  ...
  for (pArea = first_area; pArea; pArea = pArea->next)
    {
      if (pObjIndex->vnum >= pArea->low_o_vnum
	  && pObjIndex->vnum <= pArea->hi_o_vnum)
	break;
    }
  return pArea;
}
...
void
check_switch (CHAR_DATA * ch, bool possess)
{
  AFFECT_DATA *paf;
  CMDTYPE *cmd;
  ...
  if (!possess)
      ...
      for (paf = ch->switched->first_affect; paf; paf = paf->next)
	{
	  if (paf->duration == -1)
	    continue;
	  if (paf->type != -1
	      && skill_table[paf->type]->spell_fun == spell_possess)
	    return;
	}
    }
  ...
  for (hash = 0; hash < 126; hash++)
      ...
      for (cmd = command_hash[hash]; cmd; cmd = cmd->next)
	{
	  ...
	  if (!IS_NPC (ch) && ch->pcdata->bestowments
