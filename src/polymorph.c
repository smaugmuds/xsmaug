...
can_morph (CHAR_DATA * ch, MORPH_DATA * morph, bool is_cast)
{
  ...
  if (morph->dayfrom != -1 && morph->dayto != -1 &&
      (morph->dayto < (time_info.day + 1)
       || morph->dayfrom > (time_info.day + 1)))
    return FALSE;
  return TRUE;
}
 ...
 */
MORPH_DATA *
find_morph (CHAR_DATA * ch, const char *target, bool is_cast)
{
  MORPH_DATA *morph = NULL;
 ...
 */
void
do_morphset (CHAR_DATA * ch, const char *argument)
{
  ...
  if (arg1[0] == '\0' || (arg2[0] == '\0' && ch->substate != SUB_REPEATCMD)
	...
	send_to_char ("Syntax: morphset <morph> <field>  <value>\r\n", ch);
      ...
      send_to_char ("  favourused, help, hitroll, hp, hpused, immune,\r\n",
		    ch);
      send_to_char
	("  int, str, keyword, lck, level, long, mana, manaused,\r\n", ch);
      send_to_char
	("  morphother, morphself, move, moveused, name, noaffected,\r\n",
	 ch);
      send_to_char
	("  nocast, castallow, noimmune, noresistant, noskill, nosusceptible,\r\n",
	 ch);
      send_to_char
	("  obj1, obj2, obj3, objuse1, objuse2, objuse3, parry,\r\n", ch);
      send_to_char
	("  pkill, race, resistant, sav1, sav2, sav3, sav4, sav5,\r\n", ch);
      send_to_char
	("  sex, short, skills, susceptible, timefrom, timer, timeto,\r\n",
	 ch);
  ...
  if (!str_cmp (arg2, "on"))
    {
      ...
      ch->dest_buf = morph;
      STRFREE (ch->pcdata->subprompt);
      snprintf (buf, MAX_STRING_LENGTH, "<&CMorphset &W%s&w> %%i",
		morph->name);
  ...
  if (!str_cmp (arg2, "str"))
	...
	{
	  send_to_char
	    ("Usuage: morphset <morph> pkill [pkill|peace|none]\r\n", ch);
	  return;
	}
    }
	...
	{
	  send_to_char
	    ("Usage: morphset <morph> resistant <flag> [flag]...\r\n", ch);
	  return;
	}
      while (argument[0] != '\0')
	{
	...
	{
	  send_to_char
	    ("Usage: morphset <morph> susceptible <flag> [flag]...\r\n", ch);
	  return;
	}
      while (argument[0] != '\0')
	{
	...
	{
	  send_to_char
	    ("Usage: morphset <morph> noresistant <flag> [flag]...\r\n", ch);
	  return;
	}
      while (argument[0] != '\0')
	{
	...
	{
	  send_to_char
	    ("Usage: morphset <morph> nosusceptible <flag> [flag]...\r\n",
	     ch);
	  return;
	}
      while (argument[0] != '\0')
	...
	{
	  send_to_char
	    ("Usage: morphset <morph> noimmune <flag> [flag]...\r\n", ch);
	  return;
	}
      while (argument[0] != '\0')
	{
	...
	{
	  send_to_char
	    ("Usage: morphset <morph> affected <flag> [flag]...\r\n", ch);
	  return;
	}
      while (argument[0] != '\0')
	{
	...
	{
	  send_to_char
	    ("Usage: morphset <morph> noaffected <flag> [flag]...\r\n", ch);
	  return;
	}
      while (argument[0] != '\0')
	{
 ...
 */
void
do_morphstat (CHAR_DATA * ch, const char *argument)
{
  MORPH_DATA *morph;
  ...
  if (!argument || argument[0] == '\0')
	...
	("&B[----------------------------------------------------------------------------]\r\n",
	 ch);
      ...
      pager_printf (ch,
		    ...
		    ONLY_PEACEFULL ? "NO" : "n/a", morph->timefrom,
		    morph->timeto);
      pager_printf (ch, "  &cDay From:  &C%d  &cDay To:  &C%d\r\n",
		    morph->dayfrom, morph->dayto);
      pager_printf (ch,
		    "  &cLevel:  &C%d       &cMorph Via Spell   : &C%s\r\n",
		    morph->level, (morph->no_cast) ? "NO" : "yes");
      pager_printf (ch,
		    "  &cUSAGES:  Mana:  &C%d  &cMove:  &C%d  &cHp:  &C%d  &cFavour:  &C%d\r\n",
	...
	("&B[----------------------------------------------------------------------------]\r\n",
	 ch);
      send_to_pager
	("                       &BEnhancements to the Player\r\n", ch);
      send_to_pager
	("&B[----------------------------------------------------------------------------]\r\n",
	 ch);
      pager_printf (ch,
		    "  &cStr: &C%2d&c )( Int: &C%2d&c )( Wis: &C%2d&c )( Dex: &C%2d&c )( Con: &C%2d&c )( Cha: &C%2d&c )( Lck: &C%2d&c )\r\n",
		    morph->str, morph->inte, morph->wis, morph->dex,
		    morph->con, morph->cha, morph->lck);
      pager_printf (ch,
		    ...
		    morph->tumble);
      pager_printf (ch,
		    "  &cHps     : &w%s    &cMana   : &w%s    &cMove      : &w%s\r\n",
		    morph->hit, morph->mana, morph->move);
      pager_printf (ch,
		    "  &cDamroll : &w%s    &cHitroll: &w%s    &cAC     : &w%d\r\n",
	...
	("&B[----------------------------------------------------------------------------]\r\n",
	 ch);
      send_to_pager
	("                     &BPrevented affects to the Player\r\n", ch);
      send_to_pager
	("&B[----------------------------------------------------------------------------]\r\n",
	 ch);
...
do_morph (CHAR_DATA * ch, MORPH_DATA * morph)
{
  ...
  ch_morph->hitroll =
    morph->hitroll ? dice_parse (ch, morph->level, morph->hitroll) : 0;
  ch->hitroll += ch_morph->hitroll;
  ch_morph->damroll =
    morph->damroll ? dice_parse (ch, morph->level, morph->damroll) : 0;
  ...
  ch->hit += ch_morph->hit;
  ch_morph->move =
    morph->move ? dice_parse (ch, morph->level, morph->move) : 0;
  if ((ch->move + ch_morph->move) > 32700)
    ch_morph->move = (32700 - ch->move);
  ch->move += ch_morph->move;

  ch_morph->mana =
    morph->mana ? dice_parse (ch, morph->level, morph->mana) : 0;
...
void
do_revert (CHAR_DATA * ch, const char *argument)
{
  if (!ch->morph)
    {
...
fread_morph (FILE * fp)
{
  ...
  for (;;)
	...
	case 'T':
	  ...
	  break;
	case 'U':
	  MKEY ("UnmorphOther", morph->unmorph_other,
 ...
 */
void
do_morphcreate (CHAR_DATA * ch, const char *argument)
{
 ...
 */
void
do_morphdestroy (CHAR_DATA * ch, const char *argument)
{
  MORPH_DATA *morph;
 ...
 */
void
do_imm_morph (CHAR_DATA * ch, const char *argument)
{
  MORPH_DATA *morph;
 ...
 */
void
do_imm_unmorph (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim = NULL;
...
void
do_morphlist (CHAR_DATA * ch, const char *argument)
{
  MORPH_DATA *morph;
