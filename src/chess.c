...
static char *
print_big_board (CHAR_DATA * ch, GAME_BOARD_DATA * board)
{
  ...
  for (x = 0; x < 8; x++)
      ...
      for (y = 0; y < 8; y++)
	{
	  sprintf (buf, "%s%s",
		   x % 2 == 0 ? (y % 2 ==
				 0 ? BLACK_BACKGROUND : WHITE_BACKGROUND) : (y
									     %
									     2
									     ==
									     0
...
static void
init_board (GAME_BOARD_DATA * board)
{
  int x, y;
  for (x = 0; x < 8; x++)
    for (y = 0; y < 8; y++)
...
static bool
find_piece (GAME_BOARD_DATA * board, int *x, int *y, int piece)
{
  int a, b;
...
static bool
king_in_check (GAME_BOARD_DATA * board, int piece)
{
  int x = 0, y = 0, l, m;
   ...
   */
  if (x - 2 >= 0 && y - 1 >= 0 &&
      ((board->board[x - 2][y - 1] == BLACK_KNIGHT
	&& IS_WHITE (board->board[x][y]))
       || (board->board[x - 2][y - 1] == WHITE_KNIGHT
	   && IS_BLACK (board->board[x][y]))))
    return TRUE;
  if (x - 2 >= 0 && y + 1 < 8 &&
      ((board->board[x - 2][y + 1] == BLACK_KNIGHT
	&& IS_WHITE (board->board[x][y]))
  ...
  if (x - 1 >= 0 && y - 2 >= 0 &&
      ((board->board[x - 1][y - 2] == BLACK_KNIGHT
	&& IS_WHITE (board->board[x][y]))
       || (board->board[x - 1][y - 2] == WHITE_KNIGHT
	   && IS_BLACK (board->board[x][y]))))
    return TRUE;
  if (x - 1 >= 0 && y + 2 < 8 &&
      ((board->board[x - 1][y + 2] == BLACK_KNIGHT
  ...
  for (l = x + 1; l < 8; l++)
	...
	if (SAME_COLOR (x, y, l, y))
	  break;
	if (board->board[l][y] == BLACK_QUEEN
	    || board->board[l][y] == WHITE_QUEEN
	    || board->board[l][y] == BLACK_ROOK
	    || board->board[l][y] == WHITE_ROOK)
	  return TRUE;
	break;
      }
  for (l = x - 1; l >= 0; l--)
  ...
  for (m = y + 1; m < 8; m++)
	...
	if (SAME_COLOR (x, y, x, m))
	  break;
	if (board->board[x][m] == BLACK_QUEEN
	    || board->board[x][m] == WHITE_QUEEN
	    || board->board[x][m] == BLACK_ROOK
	    || board->board[x][m] == WHITE_ROOK)
	  return TRUE;
	break;
      }
  for (m = y - 1; m >= 0; m--)
  ...
  for (l = x + 1, m = y + 1; l < 8 && m < 8; l++, m++)
	...
	if (SAME_COLOR (x, y, l, m))
	  break;
	if (board->board[l][m] == BLACK_QUEEN
	    || board->board[l][m] == WHITE_QUEEN
	    || board->board[l][m] == BLACK_BISHOP
	    || board->board[l][m] == WHITE_BISHOP)
	  return TRUE;
	break;
      }
...
static bool
king_in_checkmate (GAME_BOARD_DATA * board, int piece)
{
...
static int
is_valid_move (CHAR_DATA * ch, GAME_BOARD_DATA * board, int x, int y, int dx,
	       int dy)
{
  ...
  if (IS_BLACK (board->board[x][y])
      && (!str_cmp (board->player2, ch->name) || !ch))
    return MOVE_WRONGCOLOR;
  ...
  switch (board->board[x][y])
    ...
    case BLACK_PAWN:
	  ...
	  dx == x + 2 && x == 1 && dy == y && board->board[dx][dy] == NO_PIECE
...
#ifdef IMC
void
imc_send_chess (const char *from, const char *to, const char *argument)
{
  IMC_PACKET *p;
...
PFUN (imc_recv_chess)
{
  ...
  if (!victim->pcdata->game_board)
	...
	return;
      ...
      snprintf (buf, LGST, "%s is not ready to be joined in a game.",
		imc_nameof (q->to));
      imc_send_tell ("*", q->from, buf, 1);
      imc_send_chess (victim->pcdata->game_board->player1 ? victim->
		      pcdata->game_board->player1 : NULL, q->from, "stop");
      return;
    }
  ...
  if (!str_cmp (txt, "start"))
	...
	{
	  snprintf (buf, LGST, "%s is already playing a game.",
		    imc_nameof (q->to));
	  imc_send_tell ("*", q->from, buf, 1);
	  imc_send_chess (victim->pcdata->game_board->
			  player1 ? victim->pcdata->game_board->
			  player1 : NULL, q->from, "stop");
	  return;
	}
  ...
  if (!str_cmp (txt, "accepted"))
    {
	  ...
	  victim->pcdata->game_board->type != TYPE_IMC
	  || str_cmp (victim->pcdata->game_board->player2, q->from))
	{
	  imc_send_chess (victim->pcdata->game_board->
			  player1 ? victim->pcdata->game_board->
			  player1 : NULL, q->from, "stop");
  ...
  if (!str_cmp (txt, "invalidmove"))
    {
      send_to_char
	("You have issued an invalid move according to the other mud.\r\n",
	 victim);
  ...
  if (!str_prefix ("move", txt))
    {
	  ...
	  a < '0' || a > '7' || b < '0' || b > '7' || y < 0 || y > 7 || dy < 0
	  || dy > 7)
	{
	  imc_send_chess (victim->pcdata->game_board->
			  player1 ? victim->pcdata->game_board->
			  player1 : NULL, q->from, "invalidmove");
	  return;
	}
      ...
      if (ret == MOVE_OK || ret == MOVE_TAKEN)
	{
	  ...
	  if (king_in_check
	      (board,
	       IS_WHITE (board->board[dx][dy]) ? WHITE_KING : BLACK_KING)
	      && (board->board[dx][dy] != WHITE_KING
		  && board->board[dx][dy] != BLACK_KING))
	    {
	    ...
	    {
	      board->turn++;
	      imc_send_chess (board->player1 ? board->player1 : NULL, q->from,
			      "moveok");
	      return;
	    }
	}
      imc_send_chess (victim->pcdata->game_board->player1 ? victim->
		      pcdata->game_board->player1 : NULL, q->from,
		      "invalidmove");
      return;
    }
...
void
free_game (GAME_BOARD_DATA * board)
{
  if (!board)
    return;
  ...
  if (board->type == TYPE_IMC)
    {
      imc_send_chess (board->player1 ? board->player1 : NULL, board->player2,
		      "stop");
...
void
do_chess (CHAR_DATA * ch, const char *argument)
{
  ...
  if (!str_prefix (arg, "move"))
	...
	{
	  send_to_char
	    ("Usage: chess move [piece to move] [where to move]\r\n", ch);
	  return;
	}

      if (ch->pcdata->game_board->turn % 2 == 1
	  && !str_cmp (ch->pcdata->game_board->player1, ch->name))
	{
	  ...
	  && !str_cmp (ch->pcdata->game_board->player2, ch->name))
	{
	  send_to_char ("It is not your turn.\r\n", ch);
	  return;
	}
      ...
      if (sscanf (argument, "%c%d %c%d", &a, &y, &b, &dy) != 4)
	{
      ...
      if (a < 'a' || a > 'h' || b < 'a' || b > 'h' || y < 1 || y > 8 || dy < 1
	  || dy > 8)
	{
      ...
      if (ret == MOVE_OK || ret == MOVE_TAKEN)
	{
	  ...
	  if (king_in_check
	      ...
	      if (ch->pcdata->game_board->type == TYPE_IMC)
		{
		  ...
		  imc_send_chess (ch->pcdata->game_board->player1,
				  ch->pcdata->game_board->player2, arg);
		}
#endif
	    }
	}
      ...
      if (!str_cmp (ch->name, ch->pcdata->game_board->player1))
	{
	  ...
	  if (!opp)
	    mudstrlcpy (opp_name, ch->pcdata->game_board->player2,
			MAX_INPUT_LENGTH);
	}
      else
	{
	  ...
	  if (!opp)
	    mudstrlcpy (opp_name, ch->pcdata->game_board->player1,
			MAX_INPUT_LENGTH);
	}
...
#define SEND_TO_OPP(arg,opp) \
	...
	case MOVE_TAKEN:
	  ...
	  snprintf (arg, MAX_INPUT_LENGTH, "%s has taken one of your pieces!",
		    ch->name);
	  SEND_TO_OPP (arg, opp);
	  break;
	...
	case MOVE_CHECKMATE:
	  ...
	  snprintf (arg, MAX_INPUT_LENGTH,
		    "%s has attempted a move that would result in checkmate.",
		    ch->name);
	  SEND_TO_OPP (arg, opp);
	  break;
	...
	case MOVE_CHECK:
	  ...
	  snprintf (arg, MAX_INPUT_LENGTH,
		    "%s has made a move that would result in a check.",
		    ch->name);
	  SEND_TO_OPP (arg, opp);
	  break;
