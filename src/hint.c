...
void
SwapHint (HINT_DATA * pHint1, HINT_DATA * pHint2)
{
  if (pHint1->prev)
    {
...
get_hint (int level)
{
  ...
  if (level < 0)
    {
      snprintf (buf, MAX_STRING_LENGTH, "HintLevel error, Level was %d",
		level);
      return buf;
    }
  else
...
void
do_hintedit (CHAR_DATA * ch, const char *argument)
{
	  ...
	  for (hintData = first_hint; hintData; hintData = hintData->next)
	    {
	      ...
	      pager_printf (ch, "%2d | %3d | %4d | %-30s\r\n", i,
			    hintData->low, hintData->high, hintData->text);
	    }
	  pager_printf (ch, "\r\n%d hints in file.\r\n", i);
	}
      else
...
HINT_DATA *
read_hint (char *filename, FILE * fp)
{
  HINT_DATA *hintData;
  const char *word;
  bool fMatch;
