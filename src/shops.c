...
find_keeper (CHAR_DATA * ch)
{
  ...
  if ((number_percent () % 65) > speakswell)
	...
	snprintf (buf, MAX_STRING_LENGTH,
		  "%s Could you repeat that?  I didn't quite catch it.",
		  ch->name);
      else if (speakswell > 50)
	snprintf (buf, MAX_STRING_LENGTH,
		  "%s Could you say that a little more clearly please?",
		  ch->name);
      else if (speakswell > 40)
	snprintf (buf, MAX_STRING_LENGTH,
		  "%s Sorry... What was that you wanted?", ch->name);
      else
	snprintf (buf, MAX_STRING_LENGTH, "%s I can't understand you.",
		  ch->name);
      do_tell (keeper, buf);
      return NULL;
    }
...
int
get_cost (CHAR_DATA * ch, CHAR_DATA * keeper, OBJ_DATA * obj, bool fBuy)
{
  SHOP_DATA *pShop;
  int cost;
  bool richcustomer;
...
int
get_repaircost (CHAR_DATA * keeper, OBJ_DATA * obj)
{
  REPAIR_DATA *rShop;
  int cost;
  int itype;
...
do_buy (CHAR_DATA * ch, const char *argument)
{
  ...
  if (xIS_SET (ch->in_room->room_flags, ROOM_PET_SHOP))
	...
	return;
      ...
      if (!pRoomIndexNext)
	{
	  bug ("%s: bad pet shop at vnum %d.", __FUNCTION__,
	       ch->in_room->vnum);
      ...
      if (is_number (arg))
	{
	  ...
	  if (noi > mnoi)
	    {
	      act (AT_TELL,
		   "$n tells you 'I don't sell that many items at" " once.'",
      ...
      if (!IS_OBJ_STAT (obj, ITEM_INVENTORY) && (noi > 1))
	{
      ...
      if (ch->gold < cost)
	{
	  act (AT_TELL, "$n tells you 'You can't afford to buy $p.'", keeper,
	       obj, ch, TO_VICT);
	  ch->reply = keeper;
	  return;
	}
	...
	{
	  act (AT_TELL, "$n tells you 'You can't use $p yet.'", keeper, obj,
	       ch, TO_VICT);
	  ch->reply = keeper;
	  return;
	}
      ...
      else
	{
	  snprintf (arg, MAX_INPUT_LENGTH, "$n buys %d $p%s.", noi,
		    (obj->short_descr[strlen (obj->short_descr) - 1] ==
		     's' ? "" : "s"));
...
do_sell (CHAR_DATA * ch, const char *argument)
{
  ...
  if ((obj = get_obj_carry (ch, arg)) == NULL)
    {
      act (AT_TELL, "$n tells you 'You don't have that item.'", keeper, NULL,
	   ch, TO_VICT);
      ch->reply = keeper;
      return;
    }
  ...
  if (!can_see_obj (keeper, obj))
    {
      send_to_char
	("What are you trying to sell me? I don't buy thin air!\r\n", ch);
      return;
    }
  ...
  if (cost >= keeper->gold)
    {
      act (AT_TELL, "$n tells you, '$p is worth more than I can afford...'",
	   keeper, obj, ch, TO_VICT);
      return;
    }
  ...
  snprintf (buf, MAX_STRING_LENGTH, "You sell $p for %d gold piece%s.", cost,
	    cost == 1 ? "" : "s");
...
repair_one_obj (CHAR_DATA * ch, CHAR_DATA * keeper, OBJ_DATA * obj,
		const char *arg, int maxgold, const char *fixstr,
		const char *fixstr2)
{
  char buf[MAX_STRING_LENGTH];
  int cost;
   ...
   */
  ...
  else if ((cost = strcmp ("all", arg) ? cost : 11 * cost / 10) > ch->gold)
    {
      ...
      act (AT_TELL, "$N tells you, 'Which I see you can't afford.'", ch, NULL,
	   keeper, TO_CHAR);
    }
  else
    {
      snprintf (buf, MAX_STRING_LENGTH,
		"$n gives $p to $N, who quickly %s it.", fixstr2);
      act (AT_ACTION, buf, ch, obj, keeper, TO_ROOM);
      snprintf (buf, MAX_STRING_LENGTH,
		"$N charges you %d gold piece%s to %s $p.", cost,
		cost == 1 ? "" : "s", fixstr);
...
void
do_repair (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *keeper;
  OBJ_DATA *obj;
  const char *fixstr;
...
void
appraise_all (CHAR_DATA * ch, CHAR_DATA * keeper, const char *fixstr)
{
  OBJ_DATA *obj;
  char buf[MAX_STRING_LENGTH];
  int cost = 0, total = 0;
  ...
  for (obj = ch->first_carrying; obj != NULL; obj = obj->next_content)
    {
	  ...
	  && can_see_obj (ch, obj)
	  && (obj->item_type == ITEM_ARMOR
	      || obj->item_type == ITEM_WEAPON || obj->item_type == ITEM_WAND
	      || obj->item_type == ITEM_STAFF))
	{
	  ...
	  if (!can_drop_obj (ch, obj))
		...
		act (AT_TELL, "$n tells you, '$p looks fine to me!'", keeper,
		     obj, ch, TO_VICT);
	    }
	  else
	    {
...
void
do_appraise (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *keeper;
  ...
  snprintf (buf, MAX_STRING_LENGTH,
	    "$N tells you, 'It will cost %d piece%s of gold to %s that...'",
...
void
do_makeshop (CHAR_DATA * ch, const char *argument)
{
  SHOP_DATA *shop;
  int vnum;
  MOB_INDEX_DATA *mob;
...
void
do_shopset (CHAR_DATA * ch, const char *argument)
{
  SHOP_DATA *shop;
...
void
do_shopstat (CHAR_DATA * ch, const char *argument)
{
  SHOP_DATA *shop;
  MOB_INDEX_DATA *mob;
  int vnum;
...
void
do_shops (CHAR_DATA * ch, const char *argument)
{
  SHOP_DATA *shop;
...
void
do_makerepair (CHAR_DATA * ch, const char *argument)
{
  REPAIR_DATA *repair;
  int vnum;
  MOB_INDEX_DATA *mob;
...
void
do_repairset (CHAR_DATA * ch, const char *argument)
{
  REPAIR_DATA *repair;
  MOB_INDEX_DATA *mob, *mob2;
  char arg1[MAX_INPUT_LENGTH];
...
void
do_repairstat (CHAR_DATA * ch, const char *argument)
{
  REPAIR_DATA *repair;
  MOB_INDEX_DATA *mob;
  int vnum;
...
void
do_repairshops (CHAR_DATA * ch, const char *argument)
{
  REPAIR_DATA *repair;
