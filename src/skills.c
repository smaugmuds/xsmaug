 ...
 */
void
skill_notfound (CHAR_DATA * ch, const char *argument)
{
  send_to_char ("Huh?\r\n", ch);
  return;
}
...
int
get_ssave_effect (const char *name)
{
  unsigned int x;

  for (x = 0; x < sizeof (spell_save_effect) / sizeof (spell_save_effect[0]);
       x++)
...
bool
is_legal_kill (CHAR_DATA * ch, CHAR_DATA * vch)
{
 ...
 */
bool
check_ability (CHAR_DATA * ch, char *command, char *argument)
{
  int sn, mana, blood;
 ...
 */
bool
check_skill (CHAR_DATA * ch, char *command, char *argument)
{
  int sn, mana, blood;
  struct timeval time_used;
...
void
do_skin (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *korps;
  OBJ_DATA *corpse;
  OBJ_DATA *obj;
  ...
  if (obj->value[3] != 1 && obj->value[3] != 2 && obj->value[3] != 3
      && obj->value[3] != 11)
    {
 ...
 */
void
do_slookup (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  ...
  if (!str_cmp (arg, "all"))
    {
      for (sn = 0;
	   sn < num_skills && skill_table[sn] && skill_table[sn]->name; ++sn)
	pager_printf (ch,
    ...
    {
      for (sn = 0; sn < top_herb && herb_table[sn] && herb_table[sn]->name;
	   sn++)
	...
	ch_printf (ch, "Difficulty: %d\r\n", (int) skill->difficulty);
      ...
      ch_printf (ch,
		 ...
		 skill->skill_fun ? skill->
		 skill_fun_name : skill->spell_fun_name);
      ch_printf (ch, "Sectors Allowed: %s\n",
		 skill->spell_sector ? flag_string (skill->spell_sector,
						    sec_flags) : "All");
	  ...
	  if (aff->location)
	    {
	      ...
	      mudstrlcat (buf, a_types[aff->location % REVERSE_APPLY],
			  MAX_STRING_LENGTH);
      ...
      if (skill->type != SKILL_HERB)
	{
	  if (skill->type != SKILL_RACIAL)
		...
		{
		  mudstrlcpy (buf, class_table[iClass]->who_name,
			      MAX_STRING_LENGTH);
		  snprintf (buf + 3, MAX_STRING_LENGTH - 3,
			    ") lvl: %3d max: %2d%%",
 ...
 */
void
do_sset (CHAR_DATA * ch, const char *argument)
{
      ...
      if (get_trust (ch) > LEVEL_GREATER)
	{
	  ...
	  send_to_char
	    ("  type damtype acttype classtype powertype seffect flag dice value difficulty\r\n",
	     ch);
	  send_to_char
	    ("  affect rmaffect level adept hit miss die imm (char/vict/room)\r\n",
	     ch);
  ...
  if (get_trust (ch) > LEVEL_SUB_IMPLEM && !str_cmp (arg1, "save")
	  ...
	  for (aff = skill->first_affect; aff; aff = aff_next)
	    {
	      ...
	      if (++cnt == num)
		{
		  UNLINK (aff, skill->first_affect, skill->last_affect, next,
			  prev);
		  DISPOSE (aff->duration);
		  DISPOSE (aff->modifier);
		  DISPOSE (aff);
 ...
 */
void
ability_learn_from_success (CHAR_DATA * ch, int sn)
{
...
void
learn_from_success (CHAR_DATA * ch, int sn)
{
  int adept, gain, sklvl, learn, percent, schance;
...
void
learn_from_failure (CHAR_DATA * ch, int sn)
{
  int adept, schance;
...
void
do_gouge (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  AFFECT_DATA af;
  short dam;
...
void
do_detrap (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  OBJ_DATA *trap;
  ...
  switch (ch->substate)
      ...
      if (!found)
	{
      ...
      act (AT_ACTION,
	   "You carefully begin your attempt to remove a trap from $p...", ch,
	   obj, NULL, TO_CHAR);
      act (AT_ACTION, "$n carefully attempts to remove a trap from $p...", ch,
	   obj, NULL, TO_ROOM);
...
void
do_dig (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  OBJ_DATA *startobj;
...
*/
	  if (can_use_skill
	      (ch, (number_percent () * (shovel ? 1 : 4)), gsn_dig))
	    {
  ...
  for (obj = startobj; obj; obj = obj->next_content)
    {
	  ...
	  &&
	  (can_use_skill
	   (ch, (number_percent () * (shovel ? 1 : 2)), gsn_dig)))
/*
...
void
do_search (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  OBJ_DATA *container;
...
void
do_backstab (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  OBJ_DATA *obj;
  ...
  if (victim->hit < victim->max_hit && IS_AWAKE (victim))
    {
      act (AT_PLAIN, "$N is hurt and suspicious ... you can't sneak up.", ch,
	   NULL, victim, TO_CHAR);
      return;
    }

  percent =
    number_percent () - (get_curr_lck (ch) - 14) + (get_curr_lck (victim) -
						    13);
...
void
do_rescue (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  CHAR_DATA *fch;
  ...
  if (IS_AFFECTED (victim, AFF_BERSERK))
    {
      send_to_char
	("Stepping in front of a berserker would not be an intelligent decision.\r\n",
	 ch);
      return;
    }
...
void
do_kick (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
void
do_punch (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
void
do_bite (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  ...
  if (can_use_skill (ch, number_percent (), gsn_bite))
    {
      ...
      global_retcode =
	damage (ch, victim, number_range (1, ch->level), gsn_bite);
    }
  else
    {
...
void
do_claw (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  ...
  if (can_use_skill (ch, number_percent (), gsn_claw))
    {
      ...
      global_retcode =
	damage (ch, victim, number_range (1, ch->level), gsn_claw);
    }
  else
    {
...
void
do_sting (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  ...
  if (can_use_skill (ch, number_percent (), gsn_sting))
    {
      ...
      global_retcode =
	damage (ch, victim, number_range (1, ch->level), gsn_sting);
    }
  else
    {
...
void
do_tail (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
void
do_bash (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  int schance;
...
void
do_stun (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  AFFECT_DATA af;
  int schance;
  ...
  if (!fail && can_use_skill (ch, (number_percent () + schance), gsn_stun))
	...
	ch->move -= ch->max_move / 10;
      ...
      WAIT_STATE (victim, PULSE_VIOLENCE);
      act (AT_SKILL, "$N smashes into you, leaving you stunned!", victim,
	   NULL, ch, TO_CHAR);
      act (AT_SKILL, "You smash into $N, leaving $M stunned!", ch, NULL,
	   victim, TO_CHAR);
      act (AT_SKILL, "$n smashes into $N, leaving $M stunned!", ch, NULL,
	   victim, TO_NOTVICT);
	...
	ch->move -= ch->max_move / 15;
      ...
      act (AT_SKILL,
	   "$n charges at you screaming, but you dodge out of the way.", ch,
	   NULL, victim, TO_VICT);
      act (AT_SKILL, "You try to stun $N, but $E dodges out of the way.", ch,
	   NULL, victim, TO_CHAR);
      act (AT_SKILL,
	   "$n charges screaming at $N, but keeps going right on past.", ch,
...
void
do_bloodlet (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *obj;
  ...
  if (can_use_skill (ch, number_percent (), gsn_bloodlet))
    ...
    {
      act (AT_BLOOD, "You cannot manage to draw much blood...", ch, NULL,
	   NULL, TO_CHAR);
      act (AT_BLOOD, "$n slices open $s skin, but no blood is spilled...", ch,
	   NULL, NULL, TO_ROOM);
...
void
do_feed (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  short dam;
  ...
  if (can_use_skill (ch, number_percent (), gsn_feed))
	  ...
	  && ch->pcdata->condition[COND_BLOODTHIRST] < (10 + ch->level))
	{
	  ...
	  act (AT_BLOOD, "You manage to suck a little life out of $N.", ch,
	       NULL, victim, TO_CHAR);
	  act (AT_BLOOD, "$n sucks some of your blood!", ch, NULL, victim,
	       TO_VICT);
  ...
  else
    {
	  ...
	  && ch->pcdata->condition[COND_BLOODTHIRST] < (10 + ch->level))
	{
	  act (AT_BLOOD, "The smell of $N's blood is driving you insane!", ch,
	       NULL, victim, TO_CHAR);
	  act (AT_BLOOD, "$n is lusting after your blood!", ch, NULL, victim,
	       TO_VICT);
...
void
do_disarm (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  OBJ_DATA *obj;
  int percent;
...
void
do_mistwalk (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  bool allowday;
  ...
  act (AT_DGREEN,
       "Your surroundings blur as you assume a form of churning mist!", ch,
       NULL, NULL, TO_CHAR);
...
void
do_broach (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  EXIT_DATA *pexit;
...
void
do_pick (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *gch;
  OBJ_DATA *obj;
  ...
  for (gch = ch->in_room->first_person; gch; gch = gch->next_in_room)
	...
	{
	  act (AT_PLAIN, "$N is standing too close to the lock.", ch, NULL,
	       gch, TO_CHAR);
	  return;
	}
    }
...
void
do_sneak (CHAR_DATA * ch, const char *argument)
{
  AFFECT_DATA af;
...
void
do_hide (CHAR_DATA * ch, const char *argument)
{
  if (IS_NPC (ch) && IS_AFFECTED (ch, AFF_CHARM))
    {
      send_to_char ("You can't concentrate enough for that.\r\n", ch);
...
void
do_recall (CHAR_DATA * ch, const char *argument)
{
  ...
  if ((opponent = who_fighting (ch)) != NULL)
      ...
      if (number_bits (1) == 0 || (!IS_NPC (opponent) && number_bits (3) > 1))
	{
	  WAIT_STATE (ch, 4);
	  lose =
	    (int) ((exp_level (ch, ch->level + 1) -
		    exp_level (ch, ch->level)) * 0.1);
	  if (ch->desc)
	    lose /= 2;
	  gain_exp (ch, 0 - lose);
...
void
do_aid (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  int percent;
...
void
do_mount (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
void
do_dismount (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
check_parry (CHAR_DATA * ch, CHAR_DATA * victim)
{
  ...
  if (!IS_NPC (victim) && !IS_SET (victim->pcdata->flags, PCFLAG_GAG))
     /*SB*/ act (AT_SKILL, "You parry $n's attack.", ch, NULL, victim,
		 TO_VICT);
...
bool
check_tumble (CHAR_DATA * ch, CHAR_DATA * victim)
{
  int chances;
  ...
  if (!chance (victim, chances + victim->level - ch->level))
    return FALSE;
  if (!IS_NPC (victim) && !IS_SET (victim->pcdata->flags, PCFLAG_GAG))
    act (AT_SKILL, "You tumble away from $n's attack.", ch, NULL, victim,
	 TO_VICT);
  if (!IS_NPC (ch) && !IS_SET (ch->pcdata->flags, PCFLAG_GAG))
    act (AT_SKILL, "$N tumbles away from your attack.", ch, NULL, victim,
	 TO_CHAR);
...
void
do_poison_weapon (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *obj;
  OBJ_DATA *pobj;
  OBJ_DATA *wobj;
  ...
  if (IS_OBJ_STAT (obj, ITEM_CLANOBJECT))
    {
      send_to_char
	("It doesn't appear to be fashioned of a poisonable material.\r\n",
	 ch);
      return;
    }
  /*
...
void
do_scribe (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *scroll;
  int sn;
  char buf[MAX_STRING_LENGTH];
  ...
  if (!IS_NPC (ch)
    ...
    {
      send_to_char
	("A skill such as this requires more magical ability than that of your class.\r\n",
	 ch);
      return;
    }
  ...
  snprintf (buf, MAX_STRING_LENGTH,
	    "A glowing scroll inscribed '%s' lies in the dust.",
	    skill_table[sn]->name);
  STRFREE (scroll->description);
  scroll->description = STRALLOC (buf);

  snprintf (buf, MAX_STRING_LENGTH, "scroll scribing %s",
	    skill_table[sn]->name);
  STRFREE (scroll->name);
...
void
do_brew (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *potion;
  OBJ_DATA *fire;
  int sn;
  ...
  snprintf (buf, MAX_STRING_LENGTH,
	    "A strange potion labelled '%s' sizzles in a glass flask.",
	    skill_table[sn]->name);
...
bool
check_grip (CHAR_DATA * ch, CHAR_DATA * victim)
{
  int schance;
  ...
  act (AT_SKILL, "You evade $n's attempt to disarm you.", ch, NULL, victim,
       TO_VICT);
...
void
do_circle (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  OBJ_DATA *obj;
  ...
  percent =
    number_percent () - (get_curr_lck (ch) - 16) + (get_curr_lck (victim) -
						    13);
...
void
do_berserk (CHAR_DATA * ch, const char *argument)
{
  short percent;
  AFFECT_DATA af;
...
void
do_hitall (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *vch;
  CHAR_DATA *vch_next;
  short nvict = 0;
...
bool
check_illegal_psteal (CHAR_DATA * ch, CHAR_DATA * victim)
{
...
void
do_scan (CHAR_DATA * ch, const char *argument)
{
 ...
 */
CHAR_DATA *
scan_for_victim (CHAR_DATA * ch, EXIT_DATA * pexit, const char *name)
{
  CHAR_DATA *victim;
  ROOM_INDEX_DATA *was_in_room;
  short dist, dir;
 ...
 */
OBJ_DATA *
find_projectile (CHAR_DATA * ch, int type)
{
  OBJ_DATA *obj, *obj2;
 ...
 */
ch_ret
ranged_got_target (CHAR_DATA * ch, CHAR_DATA * victim, OBJ_DATA * weapon,
		   OBJ_DATA * projectile, short dist, short dt,
		   const char *stxt, short color)
{
  if (xIS_SET (ch->in_room->room_flags, ROOM_SAFE))
    {
      /*
	...
	{
	  ch_printf (ch,
		     "Your %s is blasted from existance by a godly presense.",
		     myobj (projectile));
	  act (color, "A godly presence smites $p!", ch, projectile, NULL,
	       TO_ROOM);
      ...
      else
	{
	  ch_printf (ch,
		     "Your %s is blasted from existance by a godly presense.",
		     stxt);
	  act (color, "A godly presence smites $t!", ch, aoran (stxt), NULL,
	       TO_ROOM);
	}
      return rNONE;
    }
 ...
 */
ch_ret
ranged_attack (CHAR_DATA * ch, const char *argument, OBJ_DATA * weapon,
	       OBJ_DATA * projectile, short dt, short range)
{
  ...
  if ((pexit = find_door (ch, arg, TRUE)) == NULL)
      ...
      else
	{
	  if (who_fighting (ch) == victim)
	    {
	      send_to_char
		("They are too close to release that type of attack!\r\n",
		 ch);
	      return rNONE;
	    }
	  /*
  ...
  if (!victim)
	...
	{
	  send_to_char
	    ("You cannot perform a ranged attack from a private room.\r\n",
	     ch);
	  return rNONE;
	}
      if (ch->in_room->tunnel > 0)
	{
	  ...
	  if (count >= ch->in_room->tunnel)
	    {
	      send_to_char
		("This room is too cramped to perform such an attack.\r\n",
		 ch);
	      return rNONE;
	    }
	}
  ...
  if (pexit && arg1[0] != '\0')
	...
	{
	  send_to_char
	    ("There is too much activity there for you to get a clear shot.\r\n",
	     ch);
	  return rNONE;
	}
    }
  ...
  if (projectile)
      ...
      if (pexit)
	{
	  if (weapon)
	    {
	      act (AT_GREY, "You fire $p $T.", ch, projectile, dir_name[dir],
		   TO_CHAR);
	      act (AT_GREY, "$n fires $p $T.", ch, projectile, dir_name[dir],
		   TO_ROOM);
	    }
	  else
	    {
	      act (AT_GREY, "You throw $p $T.", ch, projectile, dir_name[dir],
		   TO_CHAR);
	      act (AT_GREY, "$n throw $p $T.", ch, projectile, dir_name[dir],
		   TO_ROOM);
	    }
	}
      else
	{
	  if (weapon)
	    {
	      act (AT_GREY, "You fire $p at $N.", ch, projectile, victim,
		   TO_CHAR);
	      act (AT_GREY, "$n fires $p at $N.", ch, projectile, victim,
		   TO_NOTVICT);
	      act (AT_GREY, "$n fires $p at you!", ch, projectile, victim,
		   TO_VICT);
	    }
	  else
	    {
	      act (AT_GREY, "You throw $p at $N.", ch, projectile, victim,
		   TO_CHAR);
	      act (AT_GREY, "$n throws $p at $N.", ch, projectile, victim,
		   TO_NOTVICT);
	      act (AT_GREY, "$n throws $p at you!", ch, projectile, victim,
		   TO_VICT);
	    }
	}
    }
	  ...
	  if (pexit)
	    {
	      ...
	      act (AT_MAGIC, "$n releases $s $t $T.", ch, stxt, dir_name[dir],
		   TO_ROOM);
	    }
	  else
	    {
	      act (AT_MAGIC, "You release $t at $N.", ch, aoran (stxt),
		   victim, TO_CHAR);
	      act (AT_MAGIC, "$n releases $s $t at $N.", ch, stxt, victim,
		   TO_NOTVICT);
	      act (AT_MAGIC, "$n releases $s $t at you!", ch, stxt, victim,
		   TO_VICT);
	    }
	}
    }
  ...
  if (victim)
    {
      check_illegal_pk (ch, victim);
      check_attacker (ch, victim);
      return ranged_got_target (ch, victim, weapon, projectile, 0, dt, stxt,
				color);
    }
  ...
  while (dist <= range)
      ...
      if (!victim)
	{
	  for (vch = ch->in_room->first_person; vch; vch = vch->next_in_room)
	    {
	      if (((IS_NPC (ch) && !IS_NPC (vch))
		   || (!IS_NPC (ch) && IS_NPC (vch))) && number_bits (1) == 0)
		{
		  victim = vch;
		  break;
		}
	...
	{
	  if (projectile)
	    act (color, "$p flies in from $T.", ch, projectile, dtxt,
		 TO_ROOM);
	  else
	    act (color, "$t flies in from $T.", ch, aoran (stxt), dtxt,
		 TO_ROOM);
	  ...
	  check_illegal_pk (ch, victim);
	  check_attacker (ch, victim);
	  return ranged_got_target (ch, victim, weapon, projectile, dist, dt,
				    stxt, color);
	}
	  ...
	  if (projectile)
	    {
	      ...
	      act (color,
		   "$p flies in from $T and falls harmlessly to the ground here.",
	    ...
	    {
	      act (color, "Your $t fizzles out harmlessly to the $T.", ch,
		   stxt, dir_name[dir], TO_CHAR);
      ...
      if ((pexit = get_exit (ch->in_room, dir)) == NULL)
	{
	  if (projectile)
	    {
	      act (color,
		   "Your $t hits a wall and bounces harmlessly to the ground to the $T.",
		   ch, myobj (projectile), dir_name[dir], TO_CHAR);
	      act (color,
		   "$p strikes the $Tsern wall and falls harmlessly to the ground.",
	    ...
	    {
	      act (color, "Your $t harmlessly hits a wall to the $T.", ch,
		   stxt, dir_name[dir], TO_CHAR);
	      act (color,
		   "$t strikes the $Tsern wall and falls harmlessly to the ground.",
 ...
 */
void
do_fire (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *arrow;
  OBJ_DATA *bow;
  short max_dist;
   ...
   */
  ranged_attack (ch, argument, bow, arrow, TYPE_HIT + arrow->value[3],
		 max_dist);
 ...
 */
bool
mob_fire (CHAR_DATA * ch, const char *name)
{
  OBJ_DATA *arrow;
  OBJ_DATA *bow;
  short max_dist;
...
void
do_slice (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *corpse;
  OBJ_DATA *obj;
  OBJ_DATA *slice;
  ...
  snprintf (buf, MAX_STRING_LENGTH, "meat fresh slice %s",
	    pMobIndex->player_name);
  STRFREE (slice->name);
  slice->name = STRALLOC (buf);

  snprintf (buf, MAX_STRING_LENGTH, "a slice of raw meat from %s",
	    pMobIndex->short_descr);
  STRFREE (slice->short_descr);
  slice->short_descr = STRALLOC (buf);

  snprintf (buf, MAX_STRING_LENGTH,
	    "A slice of raw meat from %s lies on the ground.",
	    pMobIndex->short_descr);
  STRFREE (slice->description);
  slice->description = STRALLOC (buf);

  act (AT_BLOOD, "$n cuts a slice of meat from $p.", ch, corpse, NULL,
       TO_ROOM);
  act (AT_BLOOD, "You cut a slice of meat from $p.", ch, corpse, NULL,
       TO_CHAR);
 ...
 */ void
do_style (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
/*  char buf[MAX_INPUT_LENGTH];
  ...
  if (arg[0] == '\0')
    {
      ch_printf_color (ch,
		       "&wAdopt which fighting style?  (current:  %s&w)\r\n",
		       ch->style == STYLE_BERSERK ? "&Rberserk" : ch->style ==
		       STYLE_AGGRESSIVE ? "&Raggressive" : ch->style ==
		       STYLE_DEFENSIVE ? "&Ydefensive" : ch->style ==
		       STYLE_EVASIVE ? "&Yevasive" : "standard");
      return;
    }
  ...
  if (!str_prefix (arg, "evasive"))
	...
	{
	  send_to_char
	    ("You have not yet learned enough to fight evasively.\r\n", ch);
	  return;
	}
	  ...
	  if (ch->fighting)
	    {
	      ...
	      if (IS_PKILL (ch))
		act (AT_ACTION, "$n falls back into an evasive stance.", ch,
		     NULL, NULL, TO_ROOM);
	    }
    ...
    {
      if (ch->level <
	  skill_table[gsn_style_defensive]->skill_level[ch->Class])
	{
	  send_to_char
	    ("You have not yet learned enough to fight defensively.\r\n", ch);
	  return;
	}
	  ...
	  if (ch->fighting)
	    {
	      ...
	      if (IS_PKILL (ch))
		act (AT_ACTION, "$n moves into a defensive posture.", ch,
		     NULL, NULL, TO_ROOM);
	    }
	...
	{
	  send_to_char
	    ("You have not yet learned enough to fight in the standard style.\r\n",
	     ch);
	  return;
	}
    ...
    {
      if (ch->level <
	  skill_table[gsn_style_aggressive]->skill_level[ch->Class])
	{
	  send_to_char
	    ("You have not yet learned enough to fight aggressively.\r\n",
	     ch);
	  return;
	}
	  ...
	  if (ch->fighting)
	    {
	      ...
	      if (IS_PKILL (ch))
		act (AT_ACTION, "$n assumes an aggressive stance.", ch, NULL,
		     NULL, TO_ROOM);
	    }
	...
	{
	  send_to_char
	    ("You have not yet learned enough to fight as a berserker.\r\n",
	     ch);
	  return;
	}
	  ...
	  if (ch->fighting)
	    {
	      ...
	      if (IS_PKILL (ch))
		act (AT_ACTION, "$n enters a wildly aggressive style.", ch,
		     NULL, NULL, TO_ROOM);
	    }
...
bool
can_use_skill (CHAR_DATA * ch, int percent, int gsn)
{
  bool check = FALSE;
 ...
 */
void
do_cook (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *food, *fire;
  char arg[MAX_INPUT_LENGTH];
  char buf[MAX_STRING_LENGTH];
  ...
  one_argument (argument, arg);
  if (IS_NPC (ch)
      || ch->level < skill_table[gsn_cook]->skill_level[ch->Class])
    {
  ...
  if (number_percent () > LEARNED (ch, gsn_cook))
    {
      ...
      act (AT_MAGIC, "$p catches on fire burning it to a crisp!\r\n", ch,
	   food, NULL, TO_CHAR);
      act (AT_MAGIC, "$n catches $p on fire burning it to a crisp.", ch, food,
	   NULL, TO_ROOM);
      snprintf (buf, MAX_STRING_LENGTH, "a burnt %s", food->pIndexData->name);
      STRFREE (food->short_descr);
      food->short_descr = STRALLOC (buf);
      snprintf (buf, MAX_STRING_LENGTH, "A burnt %s.",
		food->pIndexData->name);
  ...
  if (number_percent () > 85)
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "an overcooked %s",
		food->pIndexData->name);
      STRFREE (food->short_descr);
      food->short_descr = STRALLOC (buf);
      snprintf (buf, MAX_STRING_LENGTH, "An overcooked %s.",
  ...
  else
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "a roasted %s",
		food->pIndexData->name);
      STRFREE (food->short_descr);
      food->short_descr = STRALLOC (buf);
      snprintf (buf, MAX_STRING_LENGTH, "A roasted %s.",
