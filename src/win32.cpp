 ...
 *                                                                          *
 ****************************************************************************
 *                     Win32 services routines by Nick Gammon               *
 ****************************************************************************/
...
#define THIS_SERVICE_DISPLAY "XSMAUG C++ for Win32"

int main( int, char ** );
void mainthread( int, char ** );
...
static int CmdInstallService( int, char *argv[] );
static int CmdRemoveService( );
static int CmdStartService( );
static int CmdStopService( );
static int CmdStatusService( );
static void CmdDisplayFormat( );
static char *convert_error( DWORD );
static DWORD get_service_status( SERVICE_STATUS *, int );
int service_error( DWORD, char *, ... );
...
// do not start the SMAUG if it is already a running service
         ...
         {
            fprintf( stderr, "XSMAUG is already running as a service.\n" );
            return 1;
         }
         worker_thread( NULL );
      }
   ...
   else
   {
      ...
      // do not start XSMAUG if it is already a running service
...
// Opens the service manager and gets the status, optionally leaving the manager open.
