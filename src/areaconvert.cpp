...
/* Converts stock Smaug version 0 and version 1 areas into XSMAUG format - Samson 12-21-01 */
/* Converts SmaugWiz version 1000 files into XSMAUG format - Samson 4-24-03 */
   ...
   for( ;; )
   {
      ...
      for( ;; )
      {
         ...
         else if( letter == 'A' )   // This section was added in SmaugFUSS 1.8
         {
            ...
            fread_to_eol( fp );  /* Skip, XSMAUG doesn't have these */
         else if( letter == '>' )
         {
            ungetc( letter, fp );
...
void load_stock_area_file( const string & filename, bool manual )
{
   ...
   if( !str_cmp( word, "AREA" ) || !str_cmp( word, "XSMAUGAREA" ) )
   {
      tarea = create_area(  );
   ...
   for( ;; )
   {
      ...
      else if( !str_cmp( word, "VERSION" ) )
      {
         ...
         fread_number( fpArea ); /* Skip, XSMAUG doesn't have this */
...
CMDF( do_areaconvert )
{
   ...
   if( ch )
   {
      ...
      if( ( tarea = find_area( arg ) ) )
      {
         ...
         {
            ch->print( "&GWriting area in XSMAUG format...\r\n" );
