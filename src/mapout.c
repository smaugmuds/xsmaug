...
void
map_stats (CHAR_DATA * ch, int *rooms, int *rows, int *cols)
{
  int row, col, n;
...
void
do_mapout (CHAR_DATA * ch, const char *argument)
{
  ...
  if (!str_cmp (arg, "stat"))
      ...
      if (!ch->pnote)
	{
      ...
      act (AT_ACTION, "$n glances at an etherial map.", ch, NULL, NULL,
	   TO_ROOM);
      return;
    }
  ...
  if (!str_cmp (arg, "create"))
      ...
      if (rooms > avail_rooms)
	{
      ...
      act (AT_ACTION, "$n warps the very dimensions of space!", ch, NULL,
	   NULL, TO_ROOM);
  ...
  send_to_char ("mapout show: show a written, but not yet created map.\r\n",
		ch);
  send_to_char
    ("mapout create: turn a written map into rooms in your assigned room vnum range.\r\n",
     ch);
  return;
}

int
add_new_room_to_map (CHAR_DATA * ch, char code)
{
  int i;
   ...
   */
  for (i = ch->pcdata->area->low_r_vnum; i <= ch->pcdata->area->hi_r_vnum;
       i++)
    {
 ...
 */
void
map_to_rooms (CHAR_DATA * ch, MAP_INDEX_DATA * m_index)
{
  ...
  for (y = 0; y < (row + 1); y++)
	...
	{			/* cols (78, i think) */
	  ...
	  if (y > 1)
	    {
	      ...
	      newy--;
	      while (newy >= 0
		     && (map[newy][x].code == '|' || map[newy][x].code == ':'
			 || map[newy][x].code == '='))
		newy--;
	      /*
	       * dont link it to itself 
	  ...
	  if (y < 48 && x < 79)
		...
		{
		  xit =
		    make_exit (newrm, get_room_index (tvnum), DIR_SOUTHEAST);
		  xit->keyword = STRALLOC ("");
		  xit->description = STRALLOC ("");
		  xit->key = -1;
		  xit->exit_info = 0;
	  ...
	  if (y > 1 && x < 79)
		...
		{
		  xit =
		    make_exit (newrm, get_room_index (tvnum), DIR_NORTHEAST);
		  xit->keyword = STRALLOC ("");
		  xit->description = STRALLOC ("");
		  xit->key = -1;
		  xit->exit_info = 0;
	  ...
	  if (y > 1 && x > 1)
		...
		{
		  xit =
		    make_exit (newrm, get_room_index (tvnum), DIR_NORTHWEST);
		  xit->keyword = STRALLOC ("");
		  xit->description = STRALLOC ("");
		  xit->key = -1;
		  xit->exit_info = 0;
	  ...
	  if (y < 48 && x > 1)
		...
		{
		  xit =
		    make_exit (newrm, get_room_index (tvnum), DIR_SOUTHWEST);
		  xit->keyword = STRALLOC ("");
		  xit->description = STRALLOC ("");
		  xit->key = -1;
		  xit->exit_info = 0;
