 ...
 */
bool
is_immune (CHAR_DATA * ch, short damtype)
{
  switch (damtype)
    {
    case SD_FIRE:
  ...
  if ((sn = bsearch_skill_exact (name, 0, num_sorted_skills - 1)) == -1)
      ...
      if ((sn = bsearch_skill_prefix (name, 0, num_sorted_skills - 1)) == -1)
	{
	  ...
	  for (sn = num_sorted_skills; sn < num_skills; ++sn)
		...
		{
		  bug ("%s: WARNING: skill table entry %d had bad skill",
		       __FUNCTION__, sn);
		  return -1;
		}
 ...
 */
int
find_skill_of_type (CHAR_DATA * ch, const char *name, bool know,
		    int expectedType)
{
  int sn;
...
int
find_ability (CHAR_DATA * ch, const char *name, bool know)
{
		...
		int affect, bool dispel)
{
  ...
  if (!can_see (ch, victim))
    ...
    {
      mudstrlcpy (buf, (IS_NPC (victim) ? victim->short_descr : victim->name),
		  MAX_STRING_LENGTH);
      buf[0] = toupper (buf[0]);
    }
...
say_spell (CHAR_DATA * ch, int sn)
{
  ...
  snprintf (buf, MAX_STRING_LENGTH, "$n utters the words, '%s'.",
	    skill->name);
 ...
 */
int
ris_save (CHAR_DATA * ch, int schance, int ris)
{
  short modifier;
 ...
 */
int
rd_parse (CHAR_DATA * ch, int level, char *texp)
{
  unsigned int x;
...
int
dice_parse (CHAR_DATA * ch, int level, const char *texp)
{
	  ...
	  if (val >= 0 && val < 6)
	    {
	      ...
	      if (obj->value[val] <= 0)
		{
		  act (AT_MAGIC, "$p disappears in a puff of smoke!", ch, obj,
		       NULL, TO_CHAR);
		  act (AT_MAGIC, "$p disappears in a puff of smoke!", ch, obj,
		       NULL, TO_ROOM);
		  extract_obj (obj);
  ...
  switch (ch->substate)
	 ...
	 */
	{
	  ...
	  if ((skill = get_skilltype (sn)) == NULL)
	    {
	      send_to_char
		("Something is severely wrong with that one...\r\n", ch);
	      return;
	    }
  ...
  if ((skill->target == TAR_CHAR_OFFENSIVE || number_bits (7) == 1)	/* 1/128 chance if non-offensive */
	...
	case 1:
	  act (AT_MAGIC, "The $t spell backfires!", ch, skill->name, victim,
	       TO_CHAR);
	  if (victim)
	    act (AT_MAGIC, "$n's $t spell backfires!", ch, skill->name,
		 victim, TO_VICT);
 ...
 */
ch_ret
spell_acid_blast (int sn, int level, CHAR_DATA * ch, void *vo)
{
...
ch_ret
spell_blindness (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  AFFECT_DATA af;
  int tmp;
  ...
  if (ch != victim)
    {
      act (AT_MAGIC, "You weave a spell of blindness around $N.", ch, NULL,
	   victim, TO_CHAR);
      act (AT_MAGIC, "$n weaves a spell of blindness about $N.", ch, NULL,
	   victim, TO_NOTVICT);
    }
  return rNONE;
}
...
spell_burning_hands (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  level =
    UMIN ((unsigned int) level, sizeof (dam_each) / sizeof (dam_each[0]) - 1);
  level = UMAX (0, level);
...
spell_call_lightning (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  act (AT_MAGIC, "$n calls God's lightning to strike $s foes!", ch, NULL,
       NULL, TO_ROOM);
  ...
  for (vch = first_char; vch; vch = trvch_wnext (lc))
      ...
      if (vch->in_room == where)
	{
	  ...
	  if (vch != ch && (IS_NPC (ch) ? !IS_NPC (vch) : IS_NPC (vch)))
	    retcode =
	      damage (ch, vch, saves_spell_staff (level, vch) ? dam / 2 : dam,
		      sn);
	  if (retcode == rCHAR_DIED || char_died (ch))
	    ch_died = TRUE;
	}
      else if (vch->in_room->area == where->area && IS_OUTSIDE (vch)
	       && IS_AWAKE (vch) && number_bits (2) == 0)
...
ch_ret
spell_cause_light (int sn, int level, CHAR_DATA * ch, void *vo)
{
...
ch_ret
spell_change_sex (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  AFFECT_DATA af;
  SKILLTYPE *skill = get_skilltype (sn);
...
spell_charm_person (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  log_printf_plus (LOG_NORMAL, ch->level, "%s has charmed %s.", ch->name,
		   victim->name);
  if (!IS_NPC (ch))
    ch->pcdata->charmies++;
  if (IS_NPC (victim))
...
ch_ret
spell_chill_touch (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  static const short dam_each[] = {
    0,
...
ch_ret
spell_colour_spray (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  static const short dam_each[] = {
    0,
...
spell_control_weather (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if (!str_cmp (target_name, "warmer"))
    ...
    {
      send_to_char
	("Do you want it to get warmer, colder, wetter, drier, stormier, or calmer?\r\n",
	 ch);
      return rSPELL_FAILED;
    }
...
ch_ret
spell_create_food (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *mushroom;
...
ch_ret
spell_create_water (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  water =
    UMIN (level * (getPrecip (cell) >= 0 ? 4 : 2),
...
ch_ret
spell_cure_blindness (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  SKILLTYPE *skill = get_skilltype (sn);
...
ch_ret
spell_sacral_divinity (int sn, int level, CHAR_DATA * ch, void *vo)
{
  AFFECT_DATA af;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  act (AT_MAGIC, "A shroud of glittering light slowly wraps itself about $n.",
       ch, NULL, NULL, TO_ROOM);
  act (AT_MAGIC,
       "A shroud of glittering light slowly wraps itself around you.", ch,
       NULL, NULL, TO_CHAR);
  return rNONE;
}

ch_ret
spell_expurgation (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  act (AT_MAGIC,
       "You speak an ancient prayer, begging your god for purification.", ch,
       NULL, NULL, TO_CHAR);
  act (AT_MAGIC,
       "$n speaks an ancient prayer begging $s god for purification.", ch,
       NULL, NULL, TO_ROOM);
  return rNONE;
}

ch_ret
spell_bethsaidean_touch (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if (ch != victim)
    {
      ...
      act (AT_MAGIC,
	   "Laying your hands on $N's eyes, you pray to lift $S blindness.",
...
ch_ret
spell_cure_poison (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if (is_affected (victim, gsn_poison))
    {
      ...
      if (ch != victim)
	{
	  act (AT_MAGIC, "A flush of health washes over $N.", ch, NULL,
	       victim, TO_NOTVICT);
	  act (AT_MAGIC, "You lift the poison from $N's body.", ch, NULL,
	       victim, TO_CHAR);
	}
      return rNONE;
    }
...
ch_ret
spell_curse (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  AFFECT_DATA af;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if (ch != victim)
    {
      ...
      act (AT_MAGIC, "$n utters a curse upon $N.", ch, NULL, victim,
	   TO_NOTVICT);
    }
  return rNONE;
}
...
ch_ret
spell_detect_poison (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj = (OBJ_DATA *) vo;
...
ch_ret
spell_dispel_evil (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if (IS_NEUTRAL (victim))
    {
      act (AT_MAGIC, "$N does not seem to be affected.", ch, NULL, victim,
	   TO_CHAR);
      return rSPELL_FAILED;
    }
...
ch_ret
spell_polymorph (int sn, int level, CHAR_DATA * ch, void *vo)
{
  MORPH_DATA *morph;
  SKILLTYPE *skill = get_skilltype (sn);
...
ch_ret
spell_earthquake (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ROOM_INDEX_DATA *where;
  CHAR_DATA *vch;
  TRV_WORLD *lc;
  ...
  act (AT_MAGIC, "The earth trembles beneath your feet!", ch, NULL, NULL,
       TO_CHAR);
  act (AT_MAGIC, "$n makes the earth tremble and shiver.", ch, NULL, NULL,
       TO_ROOM);
...
ch_ret
spell_enchant_weapon (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj = (OBJ_DATA *) vo;
  AFFECT_DATA *paf;

  if (obj->item_type != ITEM_WEAPON || IS_OBJ_STAT (obj, ITEM_MAGIC)
      || IS_OBJ_STAT (obj, ITEM_ENCHANTED) || obj->first_affect)
    {
      act (AT_MAGIC,
	   "Your magic twists and winds around $p but cannot take hold.", ch,
	   obj, NULL, TO_CHAR);
      act (AT_MAGIC,
	   "$n's magic twists and winds around $p but cannot take hold.", ch,
	   obj, NULL, TO_NOTVICT);
      return rSPELL_FAILED;
    }
  ...
  if (IS_GOOD (ch))
    {
      ...
      act (AT_BLUE, "$p gleams with flecks of blue energy.", ch, obj, NULL,
	   TO_ROOM);
      act (AT_BLUE, "$p gleams with flecks of blue energy.", ch, obj, NULL,
	   TO_CHAR);
    }
  else if (IS_EVIL (ch))
    {
      ...
      act (AT_BLOOD, "A crimson stain flows slowly over $p.", ch, obj, NULL,
	   TO_CHAR);
      act (AT_BLOOD, "A crimson stain flows slowly over $p.", ch, obj, NULL,
	   TO_ROOM);
    }
  else
    {
      xSET_BIT (obj->extra_flags, ITEM_ANTI_EVIL);
      xSET_BIT (obj->extra_flags, ITEM_ANTI_GOOD);
      act (AT_YELLOW, "$p glows with a disquieting light.", ch, obj, NULL,
	   TO_ROOM);
      act (AT_YELLOW, "$p glows with a disquieting light.", ch, obj, NULL,
	   TO_CHAR);
    }
...
ch_ret
spell_disenchant_weapon (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj = (OBJ_DATA *) vo;
  ...
  if (IS_OBJ_STAT (obj, ITEM_ANTI_GOOD) && IS_OBJ_STAT (obj, ITEM_ANTI_EVIL))
    {
      ...
      act (AT_YELLOW,
	   "$p momentarily absorbs all disquieting light around it.", ch, obj,
	   NULL, TO_CHAR);
    }
  if (IS_OBJ_STAT (obj, ITEM_ANTI_GOOD))
    {
      xREMOVE_BIT (obj->extra_flags, ITEM_ANTI_GOOD);
      act (AT_RED, "$p momentarily absorbs all red light around it.", ch, obj,
	   NULL, TO_CHAR);
    }
  if (IS_OBJ_STAT (obj, ITEM_ANTI_EVIL))
    {
      xREMOVE_BIT (obj->extra_flags, ITEM_ANTI_EVIL);
      act (AT_BLUE, "$p momentarily absorbs all blue light around it.", ch,
	   obj, NULL, TO_CHAR);
    }
 ...
 */
ch_ret
spell_energy_drain (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
  int schance;
...
ch_ret
spell_fireball (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  static const short dam_each[] = {
    0,
...
ch_ret
spell_flamestrike (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_faerie_fire (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  AFFECT_DATA af;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  affect_to_char (victim, &af);
  act (AT_PINK, "You are surrounded by a pink outline.", victim, NULL, NULL,
       TO_CHAR);
  act (AT_PINK, "$n is surrounded by a pink outline.", victim, NULL, NULL,
       TO_ROOM);
  return rNONE;
}
...
ch_ret
spell_faerie_fog (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *ich;

  act (AT_MAGIC, "$n conjures a cloud of purple smoke.", ch, NULL, NULL,
       TO_ROOM);
  act (AT_MAGIC, "You conjure a cloud of purple smoke.", ch, NULL, NULL,
       TO_CHAR);
...
ch_ret
spell_gate (int sn, int level, CHAR_DATA * ch, void *vo)
{
  MOB_INDEX_DATA *temp;
...
ch_ret
spell_harm (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if ((obj = get_obj_carry (ch, target_name)) != NULL)
	...
	case ITEM_CONTAINER:
	  ch_printf (ch, "%s appears to be %s.\r\n",
		     ...
		     obj->value[0] <
		     76 ? "of a small capacity" : obj->value[0] <
	...
	case ITEM_SALVE:
	  ch_printf (ch, "Has %d(%d) applications of level %d", obj->value[1],
		     obj->value[2], obj->value[0]);
	  if (obj->value[4] >= 0
	      && (sktmp = get_skilltype (obj->value[4])) != NULL)
	    {
	      send_to_char (" '", ch);
	      send_to_char (sktmp->name, ch);
	      send_to_char ("'", ch);
	  ...
	  if (obj->value[5] >= 0
	      && (sktmp = get_skilltype (obj->value[5])) != NULL)
	    {
	      send_to_char (" '", ch);
	      send_to_char (sktmp->name, ch);
	      send_to_char ("'", ch);
	...
	case ITEM_WAND:
	case ITEM_STAFF:
	  ch_printf (ch, "Has %d(%d) charges of level %d", obj->value[1],
		     obj->value[2], obj->value[0]);
	...
	name = victim->name;
      ...
      ch_printf (ch, "%s appears to be between level %d and %d.\r\n",
		 name, victim->level - (victim->level % 5),
	...
	ch_printf (ch, "%s appears to truly be %s.\r\n",
		   name,
		   (ch->level >
		    victim->level + 10) ? victim->name : "someone else");
...
ch_ret
spell_invis (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  SKILLTYPE *skill = get_skilltype (sn);
...
ch_ret
spell_lightning_bolt (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  static const short dam_each[] = {
    0,
...
ch_ret
spell_pass_door (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  AFFECT_DATA af;
  SKILLTYPE *skill = get_skilltype (sn);
...
spell_poison (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if (IS_PKILL (victim))
    victim->mental_state =
      URANGE (10, victim->mental_state + (first ? 5 : 0), 100);
  else
    victim->mental_state =
      URANGE (20, victim->mental_state + (first ? 5 : 0), 100);
  if (ch != victim)
    {
      act (AT_GREEN, "$N shivers as your poison spreads through $S body.", ch,
	   NULL, victim, TO_CHAR);
      act (AT_GREEN, "$N shivers as $n's poison spreads through $S body.", ch,
...
ch_ret
spell_remove_curse (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj;
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if (is_affected (victim, gsn_curse))
    {
      ...
      if (ch != victim)
	{
	  act (AT_MAGIC, "You dispel the curses afflicting $N.", ch, NULL,
	       victim, TO_CHAR);
	  act (AT_MAGIC, "$n's dispels the curses afflicting $N.", ch, NULL,
	       victim, TO_NOTVICT);
	}
    }
	...
	if (!obj->in_obj
		...
		|| IS_OBJ_STAT (obj, ITEM_NODROP)))
	  {
	    ...
	    if (ch != victim)
	      {
		act (AT_MAGIC, "You dispel the curses afflicting $N.", ch,
		     NULL, victim, TO_CHAR);
		act (AT_MAGIC, "$n's dispels the curses afflicting $N.", ch,
		     NULL, victim, TO_NOTVICT);
	      }
	    return rNONE;
	  }
...
ch_ret
spell_remove_trap (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj;
  OBJ_DATA *trap;
  bool found;
...
spell_summon (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if ((victim = get_char_world (ch, target_name)) == NULL
	  ...
	  && IS_PKILL (victim)) || (!IS_NPC (ch) && !IS_NPC (victim)
  ...
  if (!IS_NPC (ch))
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "%s summoned %s to room %d.",
		ch->name, victim->name, ch->in_room->vnum);
      log_string_plus (buf, LOG_NORMAL, ch->level);
      to_channel (buf, CHANNEL_MONITOR, "Monitor",
		  UMAX (LEVEL_IMMORTAL, ch->level));
    }
 ...
 */
ch_ret
spell_astral_walk (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
...
ch_ret
spell_ventriloquate (int sn, int level, CHAR_DATA * ch, void *vo)
{
...
ch_ret
spell_weaken (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  AFFECT_DATA af;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if (ch != victim)
    {
      if ((((!IS_NPC (victim)
	     && class_table[victim->Class]->attr_prime == APPLY_STR)
	    || IS_NPC (victim)) && get_curr_str (victim) < 25)
	  || get_curr_str (victim) < 20)
	{
	  act (AT_MAGIC,
	       "$N labors weakly as your spell atrophies $S muscles.", ch,
	       NULL, victim, TO_CHAR);
	  act (AT_MAGIC,
	       "$N labors weakly as $n's spell atrophies $S muscles.", ch,
	       NULL, victim, TO_NOTVICT);
	}
      else
	{
	  act (AT_MAGIC, "You induce a mild atrophy in $N's muscles.", ch,
	       NULL, victim, TO_CHAR);
	  act (AT_MAGIC, "$n induces a mild atrophy in $N's muscles.", ch,
	       NULL, victim, TO_NOTVICT);
	}
    }
  return rNONE;
      ...
      for (obj_lose = victim->first_carrying; obj_lose; obj_lose = obj_next)
	{
	  ...
	  switch (obj_lose->item_type)
	      ...
	      if (obj_lose->value[0] > 0)
		{
		  ...
		  act (AT_DAMAGE, "$p is pitted and etched!", victim,
	    ...
	    case ITEM_CONTAINER:
	      separate_obj (obj_lose);
	      act (AT_DAMAGE, "$p fumes and dissolves!", victim, obj_lose,
		   NULL, TO_CHAR);
	      act (AT_OBJECT,
...
ch_ret
spell_frost_breath (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  OBJ_DATA *obj_lose;
  OBJ_DATA *obj_next;
...
ch_ret
spell_gas_breath (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *vch;
  TRV_DATA *loop_ctrl;
  int dam, hpch;
...
ch_ret
spell_transport (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  ...
  act (AT_MAGIC, "$p slowly dematerializes from $n's hands..", ch, obj, NULL,
       TO_ROOM);
  obj_from_char (obj);
  obj_to_char (obj, victim);
  act (AT_MAGIC, "$p from $n appears in your hands!", ch, obj, victim,
       TO_VICT);
...
spell_portal (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  pexit->description =
    STRALLOC ("You gaze into the shimmering portal...\r\n");
  pexit->key = -1;
  ...
  if (!skill->hit_char || skill->hit_char[0] == '\0')
    {
      set_char_color (AT_MAGIC, ch);
      send_to_char
	("You utter an incantation, and a portal forms in front of you!\r\n",
	 ch);
    }
  else
    act (AT_MAGIC, skill->hit_char, ch, NULL, victim, TO_CHAR);
...
ch_ret
spell_recharge (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj = (OBJ_DATA *) vo;
  ...
  if (obj->item_type == ITEM_STAFF || obj->item_type == ITEM_WAND)
	...
	{
	  act (AT_FIRE, "$p bursts into flames, injuring you!", ch, obj, NULL,
	       TO_CHAR);
	  act (AT_FIRE, "$p bursts into flames, charring $n!", ch, obj, NULL,
	       TO_ROOM);
	  extract_obj (obj);
	...
	{
	  act (AT_YELLOW, "$p glows brightly for a few seconds...", ch, obj,
	       NULL, TO_CHAR);
	  obj->value[2] = obj->value[1];
	  return rNONE;
	}
	...
	{
	  act (AT_WHITE, "$p disintegrates into a void.", ch, obj, NULL,
	       TO_CHAR);
	  act (AT_WHITE,
	       "$n's attempt at recharging fails, and $p disintegrates.", ch,
	       obj, NULL, TO_ROOM);
	  extract_obj (obj);
	  return rSPELL_FAILED;
	}
	...
	{
	  act (AT_MAGIC, "$p feels warm to the touch.", ch, obj, NULL,
	       TO_CHAR);
	  --obj->value[1];
	  obj->value[2] = obj->value[1];
	  return rNONE;
 ...
 */
ch_ret
spell_plant_pass (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  char_to_room (ch, victim->in_room);
  if (ch->in_room->sector_type == SECT_FOREST)
    act (AT_MAGIC, "$n appears from behind a nearby tree!", ch, NULL, NULL,
	 TO_ROOM);
  else
 ...
 */
ch_ret
spell_mist_walk (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  bool allowday;
  SKILLTYPE *skill = get_skilltype (sn);
 ...
 */
ch_ret
spell_solar_flight (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  SKILLTYPE *skill = get_skilltype (sn);
  struct WeatherCell *cell = getWeatherCell (ch->in_room->area);
  ...
  act (AT_MAGIC, "$n disappears in a blinding flash of light!", ch, NULL,
       NULL, TO_ROOM);
  char_from_room (ch);
  char_to_room (ch, victim->in_room);
  act (AT_MAGIC, "$n appears in a blinding flash of light!", ch, NULL, NULL,
       TO_ROOM);
  do_look (ch, "auto");
  return rNONE;
}
...
/* Scryn 2/2/96 */
ch_ret
spell_remove_invis (int sn, int level, CHAR_DATA * ch, void *vo)
{
  OBJ_DATA *obj;
  SKILLTYPE *skill = get_skilltype (sn);
 ...
 */
ch_ret
spell_animate_dead (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *mob;
  OBJ_DATA *corpse;
  ...
  if (IS_IMMORTAL (ch)
      || (chance (ch, 75) && pMobIndex->level - ch->level < 10))
    {
	...
	mob->max_hit =
	  pMobIndex->level * 8 +
	  number_range (pMobIndex->level * pMobIndex->level / 4,
			pMobIndex->level * pMobIndex->level);
      else
	mob->max_hit =
	  dice (pMobIndex->hitnodice,
	...
	UMAX (URANGE
      ...
      act (AT_MAGIC, "$n makes $T rise from the grave!", ch, NULL,
	   pMobIndex->short_descr, TO_ROOM);
      act (AT_MAGIC, "You make $T rise from the grave!", ch, NULL,
	   pMobIndex->short_descr, TO_CHAR);

      snprintf (buf, MAX_STRING_LENGTH, "animated corpse %s",
		pMobIndex->player_name);
      STRFREE (mob->name);
      mob->name = STRALLOC (buf);

      snprintf (buf, MAX_STRING_LENGTH, "The animated corpse of %s",
...
/* Works now.. -- Altrag */
ch_ret
spell_possess (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  AFFECT_DATA af;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  if (IS_AFFECTED (victim, AFF_POSSESS)
      || IS_AFFECTED (ch, AFF_CHARM)
      || level < victim->level || victim->desc
      || saves_spell_staff (ctry, victim) || !chance (ch, 25))
    {
      failed_casting (skill, ch, victim, NULL);
      return rSPELL_FAILED;
    }
...
ch_ret
spell_knock (int sn, int level, CHAR_DATA * ch, void *vo)
{
  EXIT_DATA *pexit;
  SKILLTYPE *skill = get_skilltype (sn);
...
ch_ret
spell_dream (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
...
7/10/96 */
ch_ret
spell_spiral_blast (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *vch;
  TRV_DATA *lc;
  int dam, hpch;
...
ch_ret
spell_scorching_surge (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  static const short dam_each[] = {
    0,
  ...
  if (saves_spell_staff (level, victim))
    dam /= 2;
  act (AT_MAGIC, "A fiery current lashes through $n's body!", ch, NULL, NULL,
       TO_ROOM);
  act (AT_MAGIC, "A fiery current lashes through your body!", ch, NULL, NULL,
       TO_CHAR);
  return damage (ch, victim, (int) (dam * 1.4), sn);
}

ch_ret
spell_helical_flow (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim;
  SKILLTYPE *skill = get_skilltype (sn);
  ...
  act (AT_MAGIC,
       "$n coils into an ascending column of colour,"
       " vanishing into thin air.", ch, NULL, NULL, TO_ROOM);
  char_from_room (ch);
  char_to_room (ch, victim->in_room);
  act (AT_MAGIC,
       "A coil of colours descends from above, "
...
spell_attack (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  if (saved)
	...
	case SE_ABSORB:	/* victim absorbs spell for hp's */
	  act (AT_MAGIC, "$N absorbs your $t!", ch, skill->noun_damage,
	       victim, TO_CHAR);
	  act (AT_MAGIC, "You absorb $N's $t!", victim, skill->noun_damage,
	       ch, TO_CHAR);
	  act (AT_MAGIC, "$N absorbs $n's $t!", ch, skill->noun_damage,
	       victim, TO_NOTVICT);
      ...
      if (saved)
	{
	  switch (SPELL_SAVE (skill))
	    ...
	    case SE_ABSORB:	/* victim absorbs spell for hp's */
	      act (AT_MAGIC, "$N absorbs your $t!", ch, skill->noun_damage,
		   vch, TO_CHAR);
	      act (AT_MAGIC, "You absorb $N's $t!", vch, skill->noun_damage,
		   ch, TO_CHAR);
	      act (AT_MAGIC, "$N absorbs $n's $t!", ch, skill->noun_damage,
		   vch, TO_NOTVICT);
	  ...
	  switch (af.location)
		...
		URANGE (0, victim->hit + af.modifier, victim->max_hit);
	      ...
	      if ((af.modifier > 0 && ch->fighting
		   && ch->fighting->who == victim) || (af.modifier > 0
						       && victim->fighting
						       && victim->
						       fighting->who == ch))
		{
...
spell_obj_inv (int sn, int level, CHAR_DATA * ch, void *vo)
{
  ...
  switch (SPELL_ACTION (skill))
      ...
      if (SPELL_FLAG (skill, SF_WATER))	/* create water */
	{
	  ...
	  water =
	    UMIN ((skill->dice ? dice_parse (ch, level,
					     skill->dice) : level) *
		  (getPrecip (cell) >= 0 ? 2 : 1),
		  obj->value[0] - obj->value[1]);
...
ch_ret ranged_attack (CHAR_DATA *, const char *, OBJ_DATA *, OBJ_DATA *,
		      short, short);
 ...
 */
ch_ret
spell_ethereal_fist (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_spectral_furor (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_hand_of_chaos (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_disruption (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_sonic_resonance (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
 ...
 */
ch_ret
spell_mind_wrack (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_mind_wrench (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
/* Non-offensive spell! */
ch_ret
spell_revive (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
 ...
 */
ch_ret
spell_sulfurous_spray (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_caustic_fount (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_acetum_primus (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_galvanic_whip (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_magnetic_thrust (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_quantum_spike (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam, l;
...
/* L2 Mage Spell */
ch_ret
spell_black_hand (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_black_fist (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam;
...
ch_ret
spell_black_lightning (int sn, int level, CHAR_DATA * ch, void *vo)
{
  CHAR_DATA *victim = (CHAR_DATA *) vo;
  int dam, l;
...
ch_ret
spell_midas_touch (int sn, int level, CHAR_DATA * ch, void *vo)
{
  int val;
  OBJ_DATA *obj = (OBJ_DATA *) vo;
