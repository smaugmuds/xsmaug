...
void
delete_vdata (VARIABLE_DATA * var)
{
  switch (var->type)
    {
    case vtXBIT:
...
void
delete_variable (VARIABLE_DATA * var)
{
  delete_vdata (var);
  DISPOSE (var->tag);
  DISPOSE (var);
 ...
 */
VARIABLE_DATA *
get_tag (CHAR_DATA * ch, char *tag, int vnum)
{
  VARIABLE_DATA *vd;
 ...
 */
bool
remove_tag (CHAR_DATA * ch, char *tag, int vnum)
{
 ...
 */
int
tag_char (CHAR_DATA * ch, VARIABLE_DATA * var, int replace)
{
...
bool
is_valid_tag (const char *tagname)
{
  if (!isalpha (*tagname))
    return FALSE;
 ...
 */
void
do_mptag (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  VARIABLE_DATA *vd;
  char *p;
 ...
 */
void
do_mprmtag (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char *p;
  char arg1[MAX_INPUT_LENGTH];
 ...
 */
void
do_mpflag (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  VARIABLE_DATA *vd;
  char *p;
 ...
 */
void
do_mprmflag (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  VARIABLE_DATA *vd;
  char *p;
...
void
fwrite_variables (CHAR_DATA * ch, FILE * fp)
{
  VARIABLE_DATA *vd;
...
void
fread_variable (CHAR_DATA * ch, FILE * fp)
{
  VARIABLE_DATA *pvd;
  const char *word;
  bool fMatch;
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		default:
		  {
		    bug ("%s: invalid/incomplete variable: %s", __FUNCTION__,
			 pvd->tag);
		    DISPOSE (pvd->tag);
		    DISPOSE (pvd);
		    break;
		...
		case vtXBIT:
		  if (!pvd->data)
		    {
		      bug ("%s: invalid/incomplete variable: %s",
			   __FUNCTION__, pvd->tag);
		      DISPOSE (pvd->tag);
		      DISPOSE (pvd);
		      break;
		    }
	...
	case 'I':
	  if (!str_cmp (word, "Int"))
	    {
	      if (pvd->type != vtINT)
		bug ("%s: Type mismatch -- type(%d) != vtInt", __FUNCTION__,
		     pvd->type);
	      else
		{
	...
	case 'S':
	  if (!str_cmp (word, "Str"))
	    {
	      if (pvd->type != vtSTR)
		bug ("%s: Type mismatch -- type(%d) != vtSTR", __FUNCTION__,
		     pvd->type);
	      else
		{
	...
	case 'X':
	  if (!str_cmp (word, "Xbit"))
	    {
	      if (pvd->type != vtXBIT)
		bug ("%s: Type mismatch -- type(%d) != vtXBIT", __FUNCTION__,
		     pvd->type);
	      else
		{
