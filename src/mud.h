...
void add_roster (CLAN_DATA * clan, const char *name, int Class, int level,
...
typedef enum
{
  ...
  ROOM_PET_SHOP,
  ROOM_NO_RECALL, ROOM_DONATION, ROOM_NODROPALL, ROOM_SILENCE, ROOM_LOGSPEECH,
  ROOM_NODROP,
  ROOM_CLANSTOREROOM, ROOM_NO_SUMMON, ROOM_NO_ASTRAL, ROOM_TELEPORT,
  ROOM_TELESHOWDESC,
  ROOM_NOFLOOR, ROOM_NOSUPPLICATE, ROOM_ARENA, ROOM_NOMISSILE, ROOM_R4,
  ROOM_R5,
...
const char *ext_flag_string (EXT_BV * bitvector,
...
/* mud_prog.c */
bool mprog_wordlist_check (const char *arg, CHAR_DATA * mob,
...
MORPH_DATA *find_morph
args ((CHAR_DATA * ch, const char *target, bool is_cast));
...
/* ban.c */
int add_ban (CHAR_DATA * ch, const char *arg1, const char *arg2, int btime,
	     int type);
...
OD *get_obj_list_rev
args ((CHAR_DATA * ch, const char *argument, OBJ_DATA * list));
...
void add_timer
args ((CHAR_DATA * ch, short type, int count, DO_FUN * fun, int value));
...
bool empty_obj
args ((OBJ_DATA * obj, OBJ_DATA * destobj, ROOM_INDEX_DATA * destroom));
...
void update_userec
args ((struct timeval * time_used, struct timerset * userec));

/* marry.c */
#define OBJ_VNUM_DIAMOND_RING 99999
#define OBJ_VNUM_WEDDING_BAND 99998
...
ch_ret chain_spells (int sn, int level, CHAR_DATA * ch, void *vo,
		     short chain);
...
bool load_char_obj
args ((DESCRIPTOR_DATA * d, char *name, bool preload, bool copyover));
void set_alarm args ((long seconds));
void requip_char args ((CHAR_DATA * ch));
void fwrite_obj
args ((CHAR_DATA * ch, OBJ_DATA * obj, FILE * fp, int iNest, short os_type,
       bool hotboot));
...
void rprog_act_trigger (const char *buf, ROOM_INDEX_DATA * room,
			CHAR_DATA * ch, OBJ_DATA * obj, const void *vo);
#endif
...
typedef enum
{
  TR_CHAR_WORLD_FORW, TR_CHAR_WORLD_BACK, TR_CHAR_ROOM_FORW,
  TR_CHAR_ROOM_BACK,
