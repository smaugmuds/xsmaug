...
/* XSMAUG external commands */

/* Restore this if for use external commands */
...
/* Instead of external command send bug */

/* Delete this if for use external commands */

      if ( found && !cmd->do_fun )
	{
		bug( "%s: %s command is not defined as CMDF!", __FUNCTION__, cmd->fun_name.c_str( ) );
		return;
	}
