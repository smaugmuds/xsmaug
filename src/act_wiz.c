 ...
 */
void
do_dnd (CHAR_DATA * ch, const char *argument)
{
 ...
 */
void
do_watch (CHAR_DATA * ch, const char *argument)
{
   ...
   * Only IMP+ can see all the watches. The rest can just see their own.
   */
  if (ch->level >= LEVEL_IMPLEMENTOR && !str_cmp (arg, "show")
      && !str_cmp (arg2, "all"))
    {
      pager_printf (ch, "%-12s %-14s %-15s\r\n", "Imm Name", "Player/Command",
		    "Player Site");
      if (first_watch)
  ...
  if (!str_cmp (arg, "player") && *arg2)
	...
	{
	  for (pw = first_watch; pw; pw = pw->next)
	    if (!str_cmp (ch->name, pw->imm_name) && pw->target_name
  ...
  if (!str_cmp (arg, "site") && *arg2)
	...
	{
	  for (pw = first_watch; pw; pw = pw->next)
	    if (!str_cmp (ch->name, pw->imm_name) && pw->player_site
  ...
  send_to_pager
    ("Sorry. I can't do anything with that. Please read the help file.\r\n",
     ch);
  return;
}

void
do_wizhelp (CHAR_DATA * ch, const char *argument)
{
  CMDTYPE *cmd;
  int col, hash;
...
void
do_restrict (CHAR_DATA * ch, const char *argument)
{
...
/*    		ch_printf( ch, "%s is at level %d.\r\n", cmd->name, cmd->level );*/
      ...
      log_printf ("%s restricting %s to level %d", ch->name, cmd->name,
		  level);
    }
  else
...
void
do_authorize (CHAR_DATA * ch, const char *argument)
{
  ...
  if (arg2[0] == '\0' || !str_cmp (arg2, "accept") || !str_cmp (arg2, "yes"))
  ...
  else if (!str_cmp (arg2, "immsim") || !str_cmp (arg2, "i"))
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH,
		"%s: name denied - similar to Imm name", victim->name);
      to_channel (buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE);
      send_to_char_color
  ...
  else if (!str_cmp (arg2, "mobsim") || !str_cmp (arg2, "m"))
    {
      victim->pcdata->auth_state = 2;
      snprintf (buf, MAX_STRING_LENGTH,
		"%s: name denied - similar to mob name", victim->name);
      to_channel (buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE);
      send_to_char_color
    ...
    {
      victim->pcdata->auth_state = 2;
      snprintf (buf, MAX_STRING_LENGTH, "%s: name denied - swear word",
		victim->name);
      to_channel (buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE);
      send_to_char_color
    ...
    {
      victim->pcdata->auth_state = 2;
      snprintf (buf, MAX_STRING_LENGTH, "%s: name denied - unpronouncable",
		victim->name);
      to_channel (buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE);
	...
	("&RThe name you have chosen and/or the actions you have taken have\r\n"
	 ...
	 "from this mud.\r\n", victim);
      snprintf (buf, MAX_STRING_LENGTH, "%s: denied authorization",
		victim->name);
      to_channel (buf, CHANNEL_AUTH, "Auth", LEVEL_NEOPHYTE);
...
void
do_bamfin (CHAR_DATA * ch, const char *argument)
{
  if (!IS_NPC (ch))
    {
...
void
do_bamfout (CHAR_DATA * ch, const char *argument)
{
  if (!IS_NPC (ch))
    {
      char *newbamf = str_dup (argument);
...
void
do_rank (CHAR_DATA * ch, const char *argument)
{
...
void
do_retire (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_delay (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  int delay;
...
void
do_deny (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_disconnect (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  DESCRIPTOR_DATA *d;
  CHAR_DATA *victim;
 ...
 */
void
do_fquit (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg1[MAX_INPUT_LENGTH];
...
void
do_forceclose (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  DESCRIPTOR_DATA *d;
  int desc;
...
void
do_pardon (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
echo_to_all (short AT_COLOR, const char *argument, short tar)
{
  DESCRIPTOR_DATA *d;
...
void
do_ech (CHAR_DATA * ch, const char *argument)
{
...
void
do_echo (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  short color;
  int target;
...
void
echo_to_room (short AT_COLOR, ROOM_INDEX_DATA * room, const char *argument)
{
  CHAR_DATA *vic;
...
void
do_recho (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  short color;
...
ROOM_INDEX_DATA *
find_location (CHAR_DATA * ch, const char *arg)
{
  CHAR_DATA *victim;
  OBJ_DATA *obj;
 ...
 */
void
transfer_char (CHAR_DATA * ch, CHAR_DATA * victim, ROOM_INDEX_DATA * location)
{
  if (!victim->in_room)
    {
  ...
  if (IS_NPC (ch) && NOT_AUTHED (victim)
      && location->area != victim->in_room->area)
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "Mptransfer - unauthed char (%s)",
		victim->name);
      progbug (buf, ch);
      return;
    }
...
void
do_transfer (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_retran (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  char buf[MAX_STRING_LENGTH];
...
void
do_regoto (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
 ...
 */
void
do_at (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_atobj (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ROOM_INDEX_DATA *location;
  ROOM_INDEX_DATA *original;
...
void
do_rat (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ROOM_INDEX_DATA *location;
...
void
do_rstat (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  char arg[MAX_INPUT_LENGTH];
  const char *sect;
  ...
  ch_printf_color (ch, "&cVnum: &w%d   &cSector: &w%d (%s)   &cLight: &w%d",
		   location->vnum, location->sector_type, sect,
		   location->light);
...
/* Face-lift by Demora */
void
do_ostat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  AFFECT_DATA *paf;
  OBJ_DATA *obj;
...
void
do_vstat (CHAR_DATA * ch, const char *argument)
{
  VARIABLE_DATA *vd;
  CHAR_DATA *victim;
  ...
  for (vd = victim->variables; vd; vd = vd->next)
      ...
      if (vd->data)
	{
	  switch (vd->type)
	    ...
	    case vtSTR:
	      if (vd->data)
		pager_printf (ch, "&CString     &cData: &W%s",
			      (char *) vd->data);
	      break;

	    case vtINT:
	      if (vd->data)
		pager_printf (ch, "&CInteger    &cData: &W%ld",
			      (long) vd->data);
	      break;
...
void
do_mstat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ...
  snprintf (mnbuf, MAX_STRING_LENGTH, "%d/%d", victim->mana,
	    victim->max_mana);
  snprintf (mvbuf, MAX_STRING_LENGTH, "%d/%d", victim->move,
	    victim->max_move);
  if (IS_VAMPIRE (victim) && !IS_NPC (victim))
    {
      snprintf (bdbuf, MAX_STRING_LENGTH, "%d/%d",
  ...
  if (IS_NPC (victim))
    ...
    pager_printf_color (ch,
			"&cDeity   : &w%-13s&w   &cFavor  : &w%-5d           &cGlory     : &w%-d (%d)\r\n",
  ...
  if (IS_NPC (victim))
    pager_printf_color (ch,
			...
			victim->baresizedie, victim->damplus,
  ...
  pager_printf_color (ch,
		      ...
		      calculate_age (victim), (int) victim->played,
		      victim->timer, victim->gold);
...
void
do_mfind (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_ofind (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_mwhere (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  bool found;
...
void
do_gwhere (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_gfighting (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  DESCRIPTOR_DATA *d;
  char arg1[MAX_INPUT_LENGTH];
  ...
  if (!pmobs && !phating && !phunting)
  ...
  else if (!phunting && phating)
    {
      for (victim = first_char; victim; victim = victim->next)
	if (IS_NPC (victim)
	    && victim->in_room && can_see (ch, victim) && victim->hating
	    && victim->level >= low && victim->level <= high)
	  {
	    found = TRUE;
	    pager_printf_color (ch,
...
void
do_bodybag (CHAR_DATA * ch, const char *argument)
{
  ...
  if (arg2[0] != '\0'
      && (str_cmp (arg2, "yes") && str_cmp (arg2, "bag")
	  && str_cmp (arg2, "now")))
    {
      send_to_char_color
	("\r\n&PSyntax:  bodybag <character> | bodybag <character> yes/bag/now\r\n",
  ...
  for (obj = first_object; obj; obj = obj->next)
    {
      if (obj->in_room && obj->pIndexData->vnum == OBJ_VNUM_CORPSE_PC
...
void
do_owhere (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  char arg[MAX_INPUT_LENGTH];
  char arg1[MAX_INPUT_LENGTH];
  ...
  for (obj = first_object; obj; obj = obj->next)
	...
	continue;
      ...
      snprintf (buf, MAX_STRING_LENGTH, "(%3d) [%5d] %-28s in ", ++icnt,
		obj->pIndexData->vnum, obj_short (obj));
      if (obj->carried_by)
	snprintf (buf + strlen (buf), (MAX_STRING_LENGTH - strlen (buf)),
		  "invent [%5d] %s\r\n",
 ...
 */
void
do_oclaim (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_snoop (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  DESCRIPTOR_DATA *d;
  CHAR_DATA *victim;
...
void
do_statshield (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_switch (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_return (CHAR_DATA * ch, const char *argument)
{
...
void
do_minvoke (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  MOB_INDEX_DATA *pMobIndex;
  CHAR_DATA *victim;
...
void
do_oinvoke (CHAR_DATA * ch, const char *argument)
{
...
void
do_purge (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  OBJ_DATA *obj;
...
void
do_low_purge (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  OBJ_DATA *obj;
...
void
do_balzhur (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char buf[MAX_STRING_LENGTH];
  char buf2[MAX_STRING_LENGTH];
  ...
  send_to_char
    ("You hear an ungodly sound in the distance that makes your blood run cold!\r\n",
     victim);
  snprintf (buf, MAX_STRING_LENGTH, "Balzhur screams, 'You are MINE %s!!!'",
	    victim->name);
  ...
  if (!remove (buf))
    ...
    {
      ch_printf (ch,
		 "Unknown error #%d - %s (immortal data). Report to www.smaugmuds.org\r\n",
  ...
  for (pArea = first_build; pArea; pArea = pArea->next)
	...
	if (!rename (buf, buf2))
	  ...
	  {
	    ch_printf (ch,
		       "Unknown error #%d - %s (area data). Report to www.smaugmuds.org\r\n",
		       errno, strerror (errno));
	    snprintf (buf2, MAX_STRING_LENGTH, "%s destroying %s", ch->name,
		      buf);
	    perror (buf2);
	  }
	break;
...
void
do_advance (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char buf[MAX_INPUT_LENGTH];
	  ...
	  if (!IS_RETIRED (victim))
	    {
	       ...
	       */
	      snprintf (buf, MAX_INPUT_LENGTH, "%s%s", GOD_DIR,
...
void
do_elevate (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_immortalize (CHAR_DATA * ch, const char *argument)
{
  int i;
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
/* Summer 1997 --Blod */
void
do_scatter (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
...
void
do_strew (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_strip (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  OBJ_DATA *obj_next;
  OBJ_DATA *obj_lose;
...
void
do_restore (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  bool boost = FALSE;
  ...
  if (!str_cmp (arg, "all"))
      ...
      for (vch = first_char; vch; vch = vch_next)
	{
	  ...
	  if (!IS_NPC (vch) && !IS_IMMORTAL (vch) && !CAN_PKILL (vch)
	      && !in_arena (vch))
	    {
	      if (boost)
...
void
do_restoretime (CHAR_DATA * ch, const char *argument)
{
...
void
do_freeze (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  if (victim->desc && victim->desc->original
      && get_trust (victim->desc->original) >= get_trust (ch))
    {
...
void
do_log (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_litterbug (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_noemote (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_notell (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_notitle (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  if (IS_SET (victim->pcdata->flags, PCFLAG_NOTITLE))
  ...
  else
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "the %s",
		title_table[victim->Class][victim->level][victim->sex ==
							  SEX_FEMALE ? 1 :
							  0]);
...
void
do_silence (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_unsilence (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_peace (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *rch;
...
void
do_wizlock (CHAR_DATA * ch, const char *argument)
{
...
void
do_noresolve (CHAR_DATA * ch, const char *argument)
{
...
void
do_users (CHAR_DATA * ch, const char *argument)
{
  DESCRIPTOR_DATA *d;
  int count;
  const char *st;
 ...
 */
void
do_force (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  bool mobsonly;
...
void
do_invis (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  short level;
...
void
do_holylight (CHAR_DATA * ch, const char *argument)
{
...
void
do_cmdtable (CHAR_DATA * ch, const char *argument)
{
  int hash, cnt;
  CMDTYPE *cmd;
  char arg[MAX_INPUT_LENGTH];
...
void
do_mortalize (CHAR_DATA * ch, const char *argument)
{
  char fname[1024];
  char name[256];
  struct stat fst;
  ...
  name[0] = UPPER (name[0]);
  snprintf (fname, 1024, "%s%c/%s", PLAYER_DIR, tolower (name[0]),
	    capitalize (name));
  if (stat (fname, &fst) != -1)
    {
	  ...
	  && (victim->level <= LEVEL_AVATAR))
	{
      ...
      snprintf (buf, MAX_STRING_LENGTH, "%s%s", GOD_DIR,
		capitalize (victim->name));
      ...
      else if (errno != ENOENT)
	{
	  ch_printf (ch,
		     "Unknown error #%d - %s (immortal data). Report to www.smaugmuds.org\r\n",
		     errno, strerror (errno));
	  snprintf (buf2, MAX_STRING_LENGTH, "%s mortalizing %s", ch->name,
		    buf);
	  perror (buf2);
	}
	...
	if (!strcmp (pArea->filename, buf2))
	  {
		...
		ch_printf (ch,
			   "Unknown error #%d - %s (area data). Report to www.smaugmuds.org\r\n",
			   errno, strerror (errno));
		snprintf (buf2, MAX_STRING_LENGTH, "%s mortalizing %s",
			  ch->name, buf);
		perror (buf2);
	      }
	  }
 ...
 */
void
do_loadup (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *temp;
  char fname[1024];
  char name[256];
   ...
   */
  if (stat (fname, &fst) == -1
      || !check_parse_name (capitalize (name), FALSE))
    {
...
void
do_fixchar (CHAR_DATA * ch, const char *argument)
{
...
void
do_newbieset (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  ...
  else if ((victim->Class == CLASS_WARRIOR) || (victim->Class == CLASS_RANGER)
 ...
 */
void
extract_area_names (const char *inp, char *out)
{
 ...
 */
void
remove_area_names (const char *inp, char *out)
{
  char buf[MAX_INPUT_LENGTH], *pbuf = buf;
  int len;
 ...
 */
void
do_bestowarea (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char buf[MAX_STRING_LENGTH];
  CHAR_DATA *victim;
  ...
  snprintf (buf, MAX_STRING_LENGTH, "%s %s", victim->pcdata->bestowments,
	    argument);
  DISPOSE (victim->pcdata->bestowments);
  victim->pcdata->bestowments = str_dup (buf);
  set_char_color (AT_IMMORT, victim);
...
void
do_bestow (CHAR_DATA * ch, const char *argument)
{
  ...
  while (arg[0] != '\0')
    {
      ...
      short numExceptions =
	(sizeof (bestow_exceptions) / sizeof (bestow_exceptions[0]));
...
void
do_set_boot_time (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg1[MAX_INPUT_LENGTH];
  bool check;
...
void
do_form_password (CHAR_DATA * ch, const char *argument)
{
  char *pwcheck;
 ...
 */
void
do_destro (CHAR_DATA * ch, const char *argument)
{
  set_char_color (AT_RED, ch);
...
close_area (AREA_DATA * pArea)
{
  ...
  for (ech = first_char; ech; ech = ech_next)
      ...
      if (IS_NPC (ech))
	{
	   ...
	   */
	  if (URANGE
	      (pArea->low_m_vnum, ech->pIndexData->vnum,
	       pArea->hi_m_vnum) == ech->pIndexData->vnum || (ech->in_room
							      &&
							      ech->in_room->
							      area == pArea))
  ...
  for (eobj = first_object; eobj; eobj = eobj_next)
    {
	  ...
	  == eobj->pIndexData->vnum || (eobj->in_room
					&& eobj->in_room->area == pArea))
	extract_obj (eobj);
    }
  ...
  if (IS_SET (pArea->flags, AFLAG_PROTOTYPE))
  ...
  else
    {
      ...
      UNLINK (pArea, first_area_name, last_area_name, next_sort_name,
	      prev_sort_name);
    }
  DISPOSE (pArea);
}
...
void
do_destroy (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char buf[MAX_STRING_LENGTH];
  char buf2[MAX_STRING_LENGTH];
  ...
  if (!rename (buf, buf2))
	...
	if (!str_cmp (pArea->filename, buf2))
	  {
		...
		ch_printf (ch,
			   "Unknown error #%d - %s (area data). Report to www.smaugmuds.org\r\n",
			   errno, strerror (errno));
		snprintf (buf2, MAX_STRING_LENGTH, "%s destroying %s",
			  ch->name, buf);
		perror (buf2);
	      }
	    break;
  ...
  else
    {
      set_char_color (AT_WHITE, ch);
      ch_printf (ch,
		 "Unknown error #%d - %s. Report to www.smaugmuds.org\r\n",
		 errno, strerror (errno));
...
void
do_for (CHAR_DATA * ch, const char *argument)
{
...
void
do_cset (CHAR_DATA * ch, const char *argument)
{
  ...
  if (argument && argument[0] == '\0')
    {
      ...
      pager_printf_color (ch,
			  "  &wSeconds per tick: &W%d   &wPulse per second: &W%d\n\r",
      ...
      pager_printf_color (ch,
			  ...
			  sysdata.hoursperday, sysdata.daysperweek,
			  sysdata.dayspermonth, sysdata.monthsperyear,
			  sysdata.daysperyear);
      pager_printf_color (ch,
			  "  &wPULSE_TICK: &W%d &wPULSE_VIOLENCE: &W%d &wPULSE_MOBILE: &W%d &wPULSE_CALENDAR: &W%d\r\n",
			  sysdata.pulsetick, sysdata.pulseviolence,
			  sysdata.pulsemobile, sysdata.pulsecalendar);
      return;
    }
...
void
do_hell (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  short htime;
...
void
do_unhell (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  ROOM_INDEX_DATA *location;
...
void
do_vsearch (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  bool found = FALSE;
  OBJ_DATA *obj;
 ...
 */
void
do_sober (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg1[MAX_INPUT_LENGTH];
 ...
 */
void
do_sedit (CHAR_DATA * ch, const char *argument)
{
  SOCIALTYPE *social;
 ...
 */
void
do_cedit (CHAR_DATA * ch, const char *argument)
{
  CMDTYPE *command;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if (arg2[0] == '\0' || !str_cmp (arg2, "show"))
    {
      ch_printf (ch,
		 ...
		 command->log, command->fun_name, flag_string (command->flags,
							       cmd_flags));
 ...
 */
void
do_showclass (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
 ...
 */
bool
create_new_class (int rcindex, const char *argument)
{
  int i;
 ...
 */
void
do_setclass (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
  FILE *fpList;
  char classlist[256];
...
bool
create_new_race (int rcindex, char *argument)
{
  int i = 0;
...
void
do_setrace (CHAR_DATA * ch, const char *argument)
{
  RACE_TYPE *race;
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH], arg3[MAX_INPUT_LENGTH];
  FILE *fpList = NULL;
...
void
do_showrace (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  ...
  ch_printf (ch, "Spoken Languages: %s",
	     flag_string (race->language, lang_names));
  send_to_char ("\r\n", ch);
...
void
do_qpset (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
...
void
do_qpstat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_fixed (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  ...
  if (!str_cmp (argument, "list"))
    ...
    {
      snprintf (buf, MAX_STRING_LENGTH,
		"&g|&G%-2.2d/%-2.2d &g| &G%5d&g|  %s:  &G%s", t->tm_mon + 1,
		t->tm_mday, ch->in_room ? ch->in_room->vnum : 0,
...
void
do_fshow (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_reserve (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  RESERVE_DATA *res;
...
void
do_khistory (CHAR_DATA * ch, const char *argument)
{
  MOB_INDEX_DATA *tmob;
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *vch;
  ...
  for (track = 0; track < MAX_KILLTRACK && vch->pcdata->killed[track].vnum;
       track++)
    {
...
void
do_project (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg, "code"))
      ...
      for (pproject = first_project; pproject; pproject = pproject->next)
	{
	  ...
	  pager_printf (ch, "%2d | %-11s | %-20s |\r\n",
			pcount, pproject->owner ? pproject->owner : "(None)",
			pproject->name);
	}
      return;
    }
...
void
do_ipcompare (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  DESCRIPTOR_DATA *d;
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg, "total"))
      ...
      for (d = first_descriptor; d; d = d->next)
	{
	  ...
	  if (fMatch && hmm)
		...
		{
		  snprintf (buf, MAX_STRING_LENGTH,
			    " %3d| %2d|%4d|%6d| %-12s", hmm->descriptor,
			    hmm->connected, hmm->idle / 4, hmm->port,
			    hmm->name);
		  if (get_trust (ch) >= LEVEL_SAVIOR)
		    snprintf (buf + strlen (buf),
			      (MAX_STRING_LENGTH - strlen (buf)), "@%-16s ",
			      hmm->host);
		  mudstrlcat (buf, "\r\n", MAX_STRING_LENGTH);
		  send_to_pager (buf, ch);
		  hmm->printed = TRUE;
	      ...
	      snprintf (buf, MAX_STRING_LENGTH,
			...
			character ? d->character->name : "(none)");
	      if (get_trust (ch) >= LEVEL_SAVIOR)
		snprintf (buf + strlen (buf),
			  (MAX_STRING_LENGTH - strlen (buf)), "@%-16s ",
			  d->host);
 ...
 */
void
do_nuisance (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  char arg1[MAX_INPUT_LENGTH];
...
void
do_unnuisance (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
void
do_pcrename (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  snprintf (newname, MAX_STRING_LENGTH, "%s%c/%s", PLAYER_DIR,
	    tolower (arg2[0]), capitalize (arg2));
  snprintf (oldname, MAX_STRING_LENGTH, "%s%c/%s", PLAYER_DIR,
	    tolower (victim->pcdata->filename[0]),
	    capitalize (victim->pcdata->filename));
  snprintf (backname, MAX_STRING_LENGTH, "%s%c/%s", BACKUP_DIR,
	    tolower (victim->pcdata->filename[0]),
	    capitalize (victim->pcdata->filename));
  ...
  if (IS_IMMORTAL (victim))
    {
      ...
      snprintf (godname, MAX_STRING_LENGTH, "%s%s", GOD_DIR,
		capitalize (victim->pcdata->filename));
      remove (godname);
    }
  ...
  if (victim->pcdata->area)
    {
      ...
      snprintf (filename, MAX_STRING_LENGTH, "%s%s.are", BUILD_DIR,
		victim->name);
      snprintf (newfilename, MAX_STRING_LENGTH, "%s%s.are", BUILD_DIR,
		capitalize (arg2));
      rename (filename, newfilename);
      snprintf (filename, MAX_STRING_LENGTH, "%s%s.are.bak", BUILD_DIR,
		victim->name);
      snprintf (newfilename, MAX_STRING_LENGTH, "%s%s.are.bak", BUILD_DIR,
		capitalize (arg2));
      rename (filename, newfilename);
    }
...
bool
check_area_conflict (AREA_DATA * area, int low_range, int hi_range)
{
...
bool
check_area_conflicts (int lo, int hi)
{
  AREA_DATA *area;
...
void
do_vassign (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH], arg3[MAX_INPUT_LENGTH];
  int lo = -1, hi = -1;
  ...
  if (lo == 0 && hi != 0)
    {
      send_to_char
	("Unacceptable vnum range, low vnum cannot be 0 when hi vnum is not.\r\n",
	 ch);
      return;
    }
  ...
  if (lo > hi)
    {
      send_to_char
	("Unacceptable vnum range, low vnum must be smaller than high vnum.\r\n",
	 ch);
      return;
    }
  ...
  if (check_area_conflicts (lo, hi))
    {
      send_to_char
	("That vnum range conflicts with another area. Check the zones or vnums command.\r\n",
	 ch);
      return;
    }
 ...
 */
void
do_oowner (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *obj;
