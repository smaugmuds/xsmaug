	  ...
	  if (pexit->to_room)
	    {
	      ...
	      mudstrlcat (buf, capitalize (dir_name[pexit->vdir]),
			  MAX_STRING_LENGTH);
...
void
map_exits (CHAR_DATA * ch, ROOM_INDEX_DATA * pRoom, int x, int y, int depth)
{
  ...
  switch (pRoom->sector_type)
    ...
    default:
      dmap[x][y].tegn = 'O';
      dmap[x][y].sector = -1;
      bug ("%s: Bad sector type (%d) in room %d.", __FUNCTION__,
...
//   dmap[x][y].info = pRoom->room_flags;
  ...
  for (door = 0; door < 10; ++door)
	...
	continue;
       ...
       */
      if ((dmap[roomx][roomy].vnum != 0)
	  && (dmap[roomx][roomy].vnum != pExit->to_room->vnum))
	{
	  /*
...
int
get_line (char *desc, size_t max_len)
{
  size_t i, j = 0;
...
void
show_map (CHAR_DATA * ch, char *text)
{
  ...
  if (xIS_SET (ch->act, PLR_AUTOEXIT))
    snprintf (buf, MAX_STRING_LENGTH * 2, "%s%s", color_str (AT_EXITS, ch),
	      get_exits (ch));
  else
	  ...
	  switch (dmap[x][y].tegn)
	    ...
	    case '/':
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "&O%c&d",
			dmap[x][y].tegn);
	      break;

	    case '@':		// Character is standing here
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "&R%c&d",
			dmap[x][y].tegn);
	      break;

	    case 'O':		// Indoors
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "&w%c&d",
			dmap[x][y].tegn);
	      break;

	    case '=':
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "&B%c&d",
			dmap[x][y].tegn);
	      break;

	    case '~':
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "&C%c&d",
			dmap[x][y].tegn);
	      break;

	    case '+':
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "&Y%c&d",
			dmap[x][y].tegn);
	      break;

	    case '*':
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "&g%c&d",
			dmap[x][y].tegn);
	      break;
	    ...
	    default:		// Empty space
	      snprintf (buf + strlen (buf),
			(MAX_STRING_LENGTH * 2) - strlen (buf), "%c",
			dmap[x][y].tegn);
	      break;
	    }
	}
...
void
draw_room_map (CHAR_DATA * ch, const char *desc)
{
  int x, y;
  static char buf[MAX_STRING_LENGTH];
