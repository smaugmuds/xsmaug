...
/*
 * Load a mob section. Old style XSMAUG area file.
 */
...
/*
 * Load an obj section. Old style XSMAUG area file.
 */
...
/*
 * Load a room section. Old style XSMAUG area file.
 */
...
void fread_xsmaug_exit( FILE * fp, room_index * pRoomIndex )
{
...
affect_data *fread_xsmaug_affect( FILE * fp )
{
   char *loc = NULL;
   char *aff = NULL;
   int value;
      ...
      switch ( word[0] )
      {
         ...
         case '#':
            if( !str_cmp( word, "#MUDPROG" ) )
            {
               ...
               fread_xsmaug_mudprog( fp, mprg, pMobIndex );
   ...
   for( ;; )
   {
      ...
      switch ( word[0] )
      {
         ...
         case '#':
            ...
            {
               mud_prog_data *mprg = new mud_prog_data;
               fread_xsmaug_mudprog( fp, mprg, pObjIndex );
         ...
         case 'A':
            ...
            {
               affect_data *af = fread_xsmaug_affect( fp );
   ...
   for( ;; )
   {
      ...
      switch ( word[0] )
      {
         ...
         case '#':
            ...
            {
               mud_prog_data *mprg = new mud_prog_data;
               fread_xsmaug_mudprog( fp, mprg, pRoomIndex );
...
// XSMAUG 2.0 Area file format. Liberal use of KEY macro support. Far more flexible.
area_data *fread_xsmaug_area( FILE * fp, bool isproto )
{
...
void load_area_file( const string & filename, bool isproto )
{
   ...
   // New XSMAUG area format support -- Samson 10/28/06
   if( !str_cmp( word, "XSMAUGAREA" ) )
   {
      tarea = fread_xsmaug_area( fpArea, isproto );
      FCLOSE( fpArea );
...
void fwrite_xsmaug_affect( FILE * fpout, affect_data * af )
{
...
void fwrite_xsmaug_exdesc( FILE * fpout, extra_descr_data * desc )
{
...
void fwrite_xsmaug_mobile( FILE * fpout, mob_index * pMobIndex, bool install )
{
...
void fwrite_xsmaug_object( FILE * fpout, obj_index * pObjIndex, bool install )
{
   if( install )
   ...
   for( ed = pObjIndex->extradesc.begin(  ); ed != pObjIndex->extradesc.end(  ); ++ed )
   {
      ...
      fwrite_xsmaug_exdesc( fpout, desc );
   }
...
void fwrite_xsmaug_room( FILE * fpout, room_index * room, bool install )
{
   if( install )
   {
   ...
   for( ex = room->exits.begin(  ); ex != room->exits.end(  ); ++ex )
   {
      ...
      fwrite_xsmaug_exit( fpout, pexit );
   }
