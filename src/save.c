...
void caught_alarm ();
void CALLBACK
alarm_handler (UINT IDEvent,	/* identifies timer event */
  ...
  for (obj = ch->first_carrying; obj; obj = obj->next_content)
      ...
      if (obj->wear_loc > -1 && obj->wear_loc < MAX_WEAR)
	{
	  if (get_trust (ch) >= obj->level)
	    ...
	    {
	      bug ("%s had on %s:  ch->level = %d  obj->level = %d", ch->name,
		   obj->name, ch->level, obj->level);
	    }
	  unequip_char (ch, obj);
	}
    }
...
save_char_obj (CHAR_DATA * ch)
{
  ...
  if (IS_SET (sysdata.save_flags, SV_BACKUP)
    ...
    {
      snprintf (strback, MAX_INPUT_LENGTH, "%s%c/%s", BACKUP_DIR,
		tolower (ch->pcdata->filename[0]),
		capitalize (ch->pcdata->filename));
  ...
  if (ch->level >= LEVEL_IMMORTAL)
    {
      snprintf (strback, MAX_INPUT_LENGTH, "%s%s", GOD_DIR,
		capitalize (ch->pcdata->filename));
...
fwrite_char (CHAR_DATA * ch, FILE * fp)
{
  ...
  fprintf (fp, "Age          %d %d %d %d\n", ch->pcdata->age_bonus,
	   ch->pcdata->day, ch->pcdata->month, ch->pcdata->year);
   ...
   */
  pos = ch->position;
  if (pos == POS_BERSERK || pos == POS_AGGRESSIVE || pos == POS_FIGHTING
      || pos == POS_DEFENSIVE || pos == POS_EVASIVE)
    pos = POS_STANDING;
  pos += 100;
  ...
  if (ch->pcdata && ch->pcdata->nuisance)
    fprintf (fp, "NuisanceNew %ld %ld %d %d\n",
	     ch->pcdata->nuisance->set_time, ch->pcdata->nuisance->max_time,
  ...
  fprintf (fp, "AttrMod      %d %d %d %d %d %d %d\n",
	   ch->mod_str, ch->mod_int, ch->mod_wis, ch->mod_dex, ch->mod_con,
	   ch->mod_cha, ch->mod_lck);
	    ...
	    short os_type, bool hotboot)
{
  ...
  if (!hotboot)
    {
	  ...
	  || (obj->item_type == ITEM_KEY
	      && !IS_OBJ_STAT (obj, ITEM_CLANOBJECT)) || obj_extracted (obj)
  ...
  if (obj->cost != obj->pIndexData->cost)
    fprintf (fp, "Cost         %d\n", obj->cost);
  if (obj->value[0] || obj->value[1] || obj->value[2] || obj->value[3]
 ...
 */
bool
load_char_obj (DESCRIPTOR_DATA * d, char *name, bool preload, bool copyover)
{
  char strsave[MAX_INPUT_LENGTH];
  CHAR_DATA *ch;
  FILE *fp;
  ...
  if (stat (strsave, &fst) != -1)
    {
      if (fst.st_size == 0)
	{
	  snprintf (strsave, MAX_INPUT_LENGTH, "%s%c/%s", BACKUP_DIR,
		    tolower (name[0]), capitalize (name));
	  send_to_char ("Restoring your backup player file...", ch);
	}
      else
	{
	  log_printf_plus (LOG_COMM, LEVEL_GREATER,
			   "%s player data for: %s (%dK)",
			   preload ? "Preloading" : "Loading",
			   ch->pcdata->filename, (int) fst.st_size / 1024);
	}
    }
  /*
   * else no player file 
  ...
  if ((fp = fopen (strsave, "r")) != NULL)
      ...
      for (;;)
	{
	  ...
	  if (!strcmp (word, "PLAYER"))
		...
		bug ("%s: Deleted mob saved on %s - skipping", __FUNCTION__,
		     ch->name);
	    }
 ...
 */
void
fread_char (CHAR_DATA * ch, FILE * fp, bool preload, bool copyover)
{
  ...
  for (;;)
	      ...
	      else
		{
		  ...
		  if (sn < 0)
		  ...
		  else
		    {
		      ...
		      if (ch->level < LEVEL_IMMORTAL)
			{
			  if (skill_table[sn]->race_level[ch->race] >=
			      LEVEL_IMMORTAL)
			    {
	  ...
	  if (!strcmp (word, "AttrMod"))
	    {
	      ...
	      sscanf (line, "%d %d %d %d %d %d %d", &x1, &x2, &x3, &x4, &x5,
		      &x6, &x7);
	      ch->mod_str = x1;
	      ch->mod_int = x2;
	      ch->mod_wis = x3;
	  ...
	  if (!strcmp (word, "Deity"))
	    {
		  ...
		  && (ch->pcdata->deity =
		      get_deity (ch->pcdata->deity_name)) == NULL)
		{
		  ch_printf (ch,
			     "Warning: the deity %s no longer exists.\r\n",
			     ch->pcdata->deity_name);
	...
	case 'K':
	  if (!strcmp (word, "Killed"))
		...
		bug ("%s: killcnt (%d) >= MAX_KILLTRACK", __FUNCTION__,
		     killcnt);
	      else
		{
...
void
fread_obj (CHAR_DATA * ch, FILE * fp, short os_type)
{
  OBJ_DATA *obj;
  const char *word;
  int iNest;
  ...
  for (;;)
	...
	case 'E':
	  ...
	  if (!strcmp (word, "End"))
		...
		{
		  if (obj->name)
		    bug ("%s: %s incomplete object.", __FUNCTION__,
			 obj->name);
		  else
	      ...
	      else
		{
		  ...
		  if (os_type == OS_CORPSE)
			  ...
			  && wear_loc < MAX_WEAR)
			{
			  ...
			  if (x == MAX_LAYERS)
			    bug ("%s: too many layers %d", __FUNCTION__,
				 wear_loc);
			}
			...
			bug ("%s: nest layer missing %d", __FUNCTION__,
			     iNest - 1);
		    }
		  if (fNest)
		    rgObjNest[iNest] = obj;
...
void
set_alarm (long seconds)
{
#ifdef WIN32
  kill_timer ();		/* kill old timer */
  timer_code =
    timeSetEvent (seconds * 1000L, 1000, alarm_handler, 0, TIME_PERIODIC);
#else
  alarm (seconds);
#endif
}
 ...
 */
void
do_last (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
 ...
 */
void
write_corpses (CHAR_DATA * ch, const char *name, OBJ_DATA * objrem)
{
  OBJ_DATA *corpse;
  FILE *fp = NULL;
  ...
  while ((de = readdir (dp)) != NULL)
    {
      if (de->d_name[0] != '.')
	{
	  snprintf (corpsefile, MAX_STRING_LENGTH, "%s%s", CORPSE_DIR,
		    de->d_name);
...
void
fwrite_mobile (FILE * fp, CHAR_DATA * mob)
{
  if (!IS_NPC (mob) || !fp)
    return;
  de_equip_char (mob);
  ...
  if (mob->in_room)
    fprintf (fp, "Room	%d\n",
	     (mob->in_room == get_room_index (ROOM_VNUM_LIMBO)
	      && mob->was_in_room) ? mob->was_in_room->vnum : mob->
	     in_room->vnum);
  if (mob->name && mob->pIndexData->player_name
      && str_cmp (mob->name, mob->pIndexData->player_name))
    fprintf (fp, "Name     %s~\n", mob->name);
  if (mob->short_descr && mob->pIndexData->short_descr
      && str_cmp (mob->short_descr, mob->pIndexData->short_descr))
    fprintf (fp, "Short	%s~\n", mob->short_descr);
  if (mob->long_descr && mob->pIndexData->long_descr
      && str_cmp (mob->long_descr, mob->pIndexData->long_descr))
    fprintf (fp, "Long	%s~\n", mob->long_descr);
  if (mob->description && mob->pIndexData->description
 ...
 */
void
write_char_mobile (CHAR_DATA * ch, char *argument)
{
  FILE *fp;
  CHAR_DATA *mob;
...
void
read_char_mobile (char *argument)
{
  FILE *fp;
  CHAR_DATA *mob;
