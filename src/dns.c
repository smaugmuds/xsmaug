...
void
add_dns (const char *dhost, const char *address)
{
  DNS_DATA *cache;
...
void
fread_dns (DNS_DATA * cache, FILE * fp)
{
  const char *word;
  bool fMatch;
 ...
 */
bool
read_from_dns (int fd, char *buffer)
{
...
void
resolve_dns (DESCRIPTOR_DATA * d, long ip)
{
  int fds[2];
  pid_t pid;
...
void
do_cache (CHAR_DATA * ch, const char *argument)
{
  DNS_DATA *cache;
  int ip = 0;
