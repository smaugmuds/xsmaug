  ...
  for (i = 0; i <= top_liquid; i++)
	...
	continue;
      ...
      fprintf (fp, "Mod       %d %d %d %d\n", liq->mod[COND_DRUNK],
	       liq->mod[COND_FULL], liq->mod[COND_THIRST],
...
void
do_showliquid (CHAR_DATA * ch, const char *argument)
{
  LIQ_TABLE *liq = NULL;
  int i;
...
void
do_setliquid (CHAR_DATA * ch, const char *argument)
{
  ...
  if (!str_cmp (arg, "create"))
	...
	{
	  send_to_char
	    ("Liquid count is at the hard-coded max. Remove some liquids or raise\r\n"
      ...
      if (!is_number (argument))
	{
	  if (!(liq = get_liq (argument)))
	    {
	      send_to_char
		("No such liquid type. Use 'showliquid' to get a valid list.\r\n",
		 ch);
	      return;
	    }
	}
      else
	{
	  ...
	  if (!(liq = get_liq_vnum (i)))
	    {
	      send_to_char
		("No such vnum. Use 'showliquid' to get the vnum.\r\n", ch);
	      return;
	    }
	}
      ...
      else if (!str_cmp (arg2, "shortdesc"))
	{
	  if (NULLSTR (argument))
	    {
	      send_to_char
		("Syntax: setliquid <vnum> shortdesc <shortdesc>\r\n", ch);
	      return;
	    }
	  STRFREE (liq->shortdesc);
	  liq->shortdesc = STRALLOC (argument);
      ...
      else
	{
	  int i;
	  bool found = FALSE;
	  static const char *const arg_names[MAX_CONDS] =
...
void
displaymixture (CHAR_DATA * ch, MIX_TABLE * mix)
{
  send_to_pager (" .-.                ,\r\n", ch);
  send_to_pager ("`._ ,\r\n", ch);
  send_to_pager ("   \\ \\                 o\r\n", ch);
  ...
  if (!mix->object)		//this is an object
	...
	{
	  pager_printf (ch,
			"%s has a bad object vnum %d, inform an Admin\r\n",
			mix->name, mix->data[0]);
	  return;
	}
      else
...
void
do_showmixture (CHAR_DATA * ch, const char *argument)
{
  MIX_TABLE *mix = NULL;
...
void
do_setmixture (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  LIQ_TABLE *liq = NULL;
      ...
      else if (!str_cmp (arg2, "vnum1"))
	{
	  ...
	  if (is_number (argument))
	  ...
	  else
	    {
	      ...
	      send_to_char
		("Syntax: setmixture <mixname> vnum1 <liqvnum or objvnum>\r\n",
		 ch);
	      return;
	    }
      ...
      else if (!str_cmp (arg2, "vnum2"))
	{
	  ...
	  if (is_number (argument))
	  ...
	  else
	    {
	      send_to_char ("Invalid liquid vnum.\r\n", ch);
	      send_to_char
		("Syntax: setmixture <mixname> vnum2 <liqvnum>\r\n", ch);
	      return;
	    }
...
LIQ_TABLE *
liq_can_mix (OBJ_DATA * iObj, OBJ_DATA * tObj)
{
  MIX_TABLE *mix = NULL;
...
LIQ_TABLE *
liqobj_can_mix (OBJ_DATA * iObj, OBJ_DATA * oLiq)
{
  MIX_TABLE *mix = NULL;
  bool mix_found = FALSE;
  ...
  for (mix = first_mixture; mix; mix = mix->next)
    if (mix->object
	&& (mix->data[0] == iObj->value[2] || mix->data[1] == iObj->value[2]))
...
void
do_mix (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
   ...
   */
  if (((iObj = get_obj_carry (ch, arg)) == NULL)
      || ((tObj = get_obj_carry (ch, argument)) == NULL))
    {
  ...
  if ((iObj->item_type != ITEM_DRINK_CON && iObj->item_type != ITEM_DRINK_MIX)
      || (tObj->item_type != ITEM_DRINK_CON
	  && tObj->item_type != ITEM_DRINK_MIX))
    {
  ...
  else if (iObj->item_type == ITEM_DRINK_MIX
	   && tObj->item_type == ITEM_DRINK_CON)
    {
  ...
  else if (iObj->item_type == ITEM_DRINK_CON
	   && tObj->item_type == ITEM_DRINK_MIX)
    {
  ...
  switch (obj->item_type)
      ...
      if (IS_VAMPIRE (ch) && !IS_NPC (ch))
	{
	  ...
	  if (ch->pcdata->condition[COND_BLOODTHIRST] < (10 + ch->level))
		...
		{
		  act (AT_BLOOD, "$n drinks from the spilled blood.", ch,
		       NULL, NULL, TO_ROOM);
		  ...
		  if (obj->value[1] <= 1)
		    {
		      set_char_color (AT_BLOOD, ch);
		      send_to_char
			("You drink the last drop of blood from the spill.\r\n",
			 ch);
		      act (AT_BLOOD,
			   "$n drinks the last drop of blood from the spill.",
	...
	if (!oprog_use_trigger (ch, obj, NULL, NULL))
	  {
	    act (AT_ACTION, "$n drinks $T from $p.", ch, obj, liq->shortdesc,
		 TO_ROOM);
	    act (AT_ACTION, "You drink $T from $p.", ch, obj, liq->shortdesc,
		 TO_CHAR);
	  }
	...
	if (!IS_NPC (ch))
	  {
	    ...
	    else if (ch->pcdata->condition[COND_DRUNK] >=
		     (MAX_COND_VALUE * .4)
		...
		&& ch->pcdata->condition[COND_THIRST] < (MAX_COND_VALUE * .4))
	    ...
	    else if (ch->pcdata->condition[COND_THIRST] >=
		     (MAX_COND_VALUE * .4)
	      ...
	      send_to_char
		("Your stomach is full, you can't manage to get anymore down.\r\n",
		 ch);
	    ...
	    if (IS_VAMPIRE (ch))
	      {
		if (ch->pcdata->condition[COND_BLOODTHIRST] >
		    (MAX_COND_VALUE / 2)
		    && ch->pcdata->condition[COND_BLOODTHIRST] <
		    (MAX_COND_VALUE * .4))
		  send_to_char
		    ("&rYou replenish your body with the vidal fluid.\r\n",
		     ch);
		else if (ch->pcdata->condition[COND_BLOODTHIRST] >=
			 (MAX_COND_VALUE * .4)
			 && ch->pcdata->condition[COND_BLOODTHIRST] <
			 (MAX_COND_VALUE * .6))
		  send_to_char
		    ("&rYour thirst for blood begins to decrease.\r\n", ch);
		else if (ch->pcdata->condition[COND_BLOODTHIRST] >=
			 (MAX_COND_VALUE * .6)
			 && ch->pcdata->condition[COND_BLOODTHIRST] <
			 (MAX_COND_VALUE * .9))
		  send_to_char
		    ("&rThe thirst for blood begins to leave you...\r\n", ch);
		else if (ch->pcdata->condition[COND_BLOODTHIRST] >=
			 (MAX_COND_VALUE * .9)
			 && ch->pcdata->condition[COND_BLOODTHIRST] <
			 MAX_COND_VALUE)
		  send_to_char
		    ("&rYou drink the last drop of the fluid, the thirst for more leaves your body.\r\n",
		     ch);
	      }
  ...
  if (arg2[0] != '\0')
      ...
      else if (dest_item == ITEM_PIPE)
	{
	  ...
	  if (source->item_type != src_item1 && source->item_type != src_item2
	    ...
	    {
	      act (AT_PLAIN, "You cannot fill $p with $P!", ch, obj, source,
		   TO_CHAR);
	      return;
	    }
	}
  ...
  if (dest_item == ITEM_CONTAINER)
	...
	case ITEM_CORPSE_PC:
	  ...
	  if (IS_OBJ_STAT (source, ITEM_CLANCORPSE) && !IS_IMMORTAL (ch))
	    {
	      send_to_char
		("Your hands fumble.  Maybe you better loot a different way.\r\n",
		 ch);
	      return;
	    }
...
void
do_empty (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *obj;
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
  ...
  switch (obj->item_type)
    ...
    default:
      ...
      return;
    case ITEM_PIPE:
      act (AT_ACTION, "You gently tap $p and empty it out.", ch, obj, NULL,
	   TO_CHAR);
      act (AT_ACTION, "$n gently taps $p and empties it out.", ch, obj, NULL,
	   TO_ROOM);
      ...
      if (EMPTYSTR (arg2))
	{
	  if (xIS_SET (ch->in_room->room_flags, ROOM_NODROP)
	      || xIS_SET (ch->act, PLR_LITTERBUG))
	    {
	      ...
	      send_to_char
		("&[tell]Someone tells you, 'No littering here!'\r\n", ch);
	      return;
	    }
