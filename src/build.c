...
const char *const area_flags[] = {
  "nopkill", "freekill", "noteleport", "spelllimit", "prototype", "r5", "r6",
  "r7", "r8",
...
const char *const o_types[] = {
  ...
  "fuel", "_empty1", "_empty2", "missileweapon", "projectile", "quiver",
  "shovel",
...
const char *const defense_flags[] = {
  ...
  "monsum4", "disarm", "iceshield", "grip", "truesight", "acidmist",
  "venomshield"
};
...
const char *
flag_string (int bitvector, const char *const flagarray[])
{
...
const char *
ext_flag_string (EXT_BV * bitvector, const char *const flagarray[])
{
  static char buf[MAX_STRING_LENGTH];
  int x;
...
bool
can_rmodify (CHAR_DATA * ch, ROOM_INDEX_DATA * room)
{
  int vnum = room->vnum;
  AREA_DATA *pArea;
...
bool
can_omodify (CHAR_DATA * ch, OBJ_DATA * obj)
{
...
bool
can_oedit (CHAR_DATA * ch, OBJ_INDEX_DATA * obj)
{
  int vnum = obj->vnum;
  AREA_DATA *pArea;
...
bool
can_mmodify (CHAR_DATA * ch, CHAR_DATA * mob)
{
  int vnum;
  AREA_DATA *pArea;
...
bool
can_medit (CHAR_DATA * ch, MOB_INDEX_DATA * mob)
{
  int vnum = mob->vnum;
  AREA_DATA *pArea;
...
int
get_npc_class (const char *Class)
{
  unsigned int x;
...
int
get_attackflag (const char *flag)
{
  unsigned int x;
...
int
get_defenseflag (const char *flag)
{
  unsigned int x;
...
int
get_npc_position (const char *position)
{
  size_t x;
...
void
start_editing (CHAR_DATA * ch, const char *data)
{
  EDITOR_DATA *edit;
...
char *
copy_buffer_nohash (CHAR_DATA * ch)
{
...
void
do_goto (CHAR_DATA * ch, const char *argument)
{
  ...
  if ((location = find_location (ch, arg)) == NULL)
      ...
      if (vnum < 0 || get_room_index (vnum))
	{
      ...
      if (get_trust (ch) < LEVEL_CREATOR || vnum < 1 || IS_NPC (ch)
	  || !ch->pcdata->area)
	{
      ...
      if (get_trust (ch) < sysdata.level_modify_proto)
	{
	  ...
	  if (vnum < pArea->low_r_vnum || vnum > pArea->hi_r_vnum)
	    {
	      send_to_char
		("That room is not within your assigned range.\r\n", ch);
	      return;
	    }
	}
  ...
  if (!xIS_SET (ch->act, PLR_WIZINVIS))
    act (AT_IMMORT, "$n $T", ch, NULL,
	 (ch->pcdata
	  && ch->pcdata->bamfout[0] !=
	  '\0') ? ch->pcdata->bamfout : (char *) "leaves in a swirling mist.",
	 TO_ROOM);
  ...
  if (!xIS_SET (ch->act, PLR_WIZINVIS))
    act (AT_IMMORT, "$n $T", ch, NULL,
	 (ch->pcdata
	  && ch->pcdata->bamfin[0] !=
	  '\0') ? ch->pcdata->bamfin : (char *) "appears in a swirling mist.",
	 TO_ROOM);
...
do_mset (CHAR_DATA * ch, const char *argument)
{
  ...
  if (get_trust (ch) < LEVEL_GREATER && IS_NPC (victim)
  ...
  if (!str_cmp (arg2, "on"))
      ...
      if (ch->pcdata)
	{
	  if (ch->pcdata->subprompt)
	    STRFREE (ch->pcdata->subprompt);
	  if (IS_NPC (victim))
	    snprintf (buf, MAX_STRING_LENGTH, "<&CMset &W#%d&w> %%i",
		      victim->pIndexData->vnum);
	  else
	    snprintf (buf, MAX_STRING_LENGTH, "<&CMset &W%s&w> %%i",
		      victim->name);
  ...
  if (!str_cmp (arg2, "clan"))
      ...
      if (victim->pcdata->clan != NULL && !IS_IMMORTAL (victim))
	{
      ...
      if (!IS_IMMORTAL (victim))
	{
	  add_roster (victim->pcdata->clan, victim->name, victim->Class,
		      victim->level, victim->pcdata->mkills,
  ...
  if (!str_cmp (arg2, "speaking"))
	...
	{
	  send_to_char
	    ("Usage: mset <victim> speaking <language> [language]...\r\n",
	     ch);
	  return;
	}
      while (argument[0] != '\0')
...
void
do_oset (CHAR_DATA * ch, const char *argument)
{
  ...
  if (!str_cmp (arg2, "on"))
      ...
      if (ch->pcdata)
	{
	  if (ch->pcdata->subprompt)
	    STRFREE (ch->pcdata->subprompt);
	  snprintf (buf, MAX_STRING_LENGTH, "<&COset &W#%d&w> %%i",
		    obj->pIndexData->vnum);
	  ch->pcdata->subprompt = STRALLOC (buf);
	}
  ...
  if (get_trust (ch) < LEVEL_DEMI)
    {
      send_to_char
	("You can only oset the name, short and long right now.\r\n", ch);
      return;
    }
  ...
  if (!str_cmp (arg2, "affect"))
      ...
      else
	{
      ...
      if (IS_OBJ_STAT (obj, ITEM_PROTOTYPE))
	{
	  if (loc != APPLY_WEARSPELL && loc != APPLY_REMOVESPELL
	      && loc != APPLY_STRIPSN && loc != APPLY_WEAPONSPELL)
	    {
	      CHAR_DATA *vch;
	      OBJ_DATA *eq;
	      ...
	      for (vch = first_char; vch; vch = vch->next)
		{
		  for (eq = vch->first_carrying; eq; eq = eq->next_content)
		    {
		      if (eq->pIndexData == obj->pIndexData
	      ...
	      if (++count == loc)
		{
		  ...
		  if (paf->location != APPLY_WEARSPELL
		      ...
		      && paf->location != APPLY_WEAPONSPELL)
		    {
		      ...
		      for (vch = first_char; vch; vch = vch->next)
			{
			  for (eq = vch->first_carrying; eq;
			       eq = eq->next_content)
			    {
			      if (eq->pIndexData == pObjIndex
				  && eq->wear_loc != WEAR_NONE)
  ...
  if (!str_cmp (arg2, "ris"))
    {
      if (!can_omodify (ch, obj))
	return;
      snprintf (outbuf, MAX_STRING_LENGTH, "%s affect resistant %s", arg1,
		arg3);
      do_oset (ch, outbuf);
      snprintf (outbuf, MAX_STRING_LENGTH, "%s affect immune %s", arg1, arg3);
      do_oset (ch, outbuf);
      snprintf (outbuf, MAX_STRING_LENGTH, "%s affect susceptible %s", arg1,
		arg3);
      do_oset (ch, outbuf);
      return;
    }
  ...
  switch (obj->item_type)
      ...
      if (!str_cmp (arg2, "weapontype"))
	{
	  ...
	  value = -1;
	  for (x = 0; x < sizeof (attack_table) / sizeof (attack_table[0]);
	       x++)
...
void
do_rdig (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_rgrid (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
  ...
  ch_printf (ch,
	     "Attempting to create a block of %d rooms, %d x %d x %d.\r\n",
	     maxnum, x, y, z);
  ...
  if (maxnum > MAX_RGRID_ROOMS)
    {
      ...
      ch_printf (ch,
		 "The maximum number of rooms this mud can create at once is %d.\r\n",
		 MAX_RGRID_ROOMS);
  ...
  if (pArea->low_r_vnum + maxnum > pArea->hi_r_vnum)
    {
      ...
      send_to_char
	("You don't even have that many rooms assigned to you.\r\n", ch);
      return;
    }
  ...
  if (room_count < maxnum)
    {
      send_to_char
	("There aren't enough free rooms in your assigned range!\r\n", ch);
      return;
    }
 ...
 */
void
add_room_affect (ROOM_INDEX_DATA * location, CHAR_DATA * ch, bool indexaffect,
		 const char *argument)
{
  AFFECT_DATA *paf;
  char arg2[MAX_INPUT_LENGTH];
  ...
  if (loc == APPLY_AFFECT)
  ...
  else if (loc == APPLY_RESISTANT || loc == APPLY_IMMUNE
	...
	return;
      value = bitv;
	   ...
	   || loc == APPLY_REMOVESPELL || loc == APPLY_STRIPSN
	   || loc == APPLY_RECURRINGSPELL)
    {
  ...
  if (!indexaffect)
    ...
    LINK (paf, location->first_permaffect, location->last_permaffect, next,
	  prev);
  ++top_affect;
...
void
remove_room_affect (ROOM_INDEX_DATA * location, CHAR_DATA * ch,
		    bool indexaffect, const char *argument)
{
      ...
      for (paf = location->first_affect; paf; paf = paf_next)
	{
	  ...
	  if (++count == loc)
		  ...
		  && paf->location != APPLY_STRIPSN)
		{
	      ...
	      UNLINK (paf, location->first_affect, location->last_affect,
		      next, prev);
	      DISPOSE (paf);
      ...
      for (paf = location->first_permaffect; paf; paf = paf_next)
	{
	  ...
	  if (++count == loc)
		  ...
		  && paf->location != APPLY_STRIPSN)
		{
	      ...
	      UNLINK (paf, location->first_permaffect,
		      location->last_permaffect, next, prev);
	      DISPOSE (paf);
...
void
do_redit (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
  ...
  if (arg[0] == '\0' || !str_cmp (arg, "?"))
	...
	send_to_char ("Syntax: redit <field> value\r\n", ch);
      ...
      send_to_char ("Field being one of:\r\n", ch);
      send_to_char
	("  name desc ed rmed affect rmaffect permaffect rmpermaffect\r\n",
	 ch);
  ...
  if (!str_cmp (arg, "bexit"))
      ...
      if (nxit)
	{
      ...
      snprintf (tmpcmd, MAX_INPUT_LENGTH, "exit %s %s %s", arg2, arg3,
		argument);
      do_redit (ch, tmpcmd);
      if (numnotdir)
      ...
      if (!rxit && nxit)
	{
      ...
      if (vnum)
	{
	  snprintf (tmpcmd, MAX_INPUT_LENGTH, "%d redit exit %d %s %s", vnum,
		    rev_dir[edir], rvnum, argument);
	  do_at (ch, tmpcmd);
	}
      return;
    }
  ...
  if (!str_cmp (arg, "pulltype") || !str_cmp (arg, "pushtype"))
	...
	{
	  ch_printf (ch,
		     "Set the %s between this room, and the destination room.\r\n",
		     arg);
  ...
  if (!str_cmp (arg, "pull"))
	...
	{
	  send_to_char
	    ("Set the 'pull' between this room, and the destination room.\r\n",
	     ch);
...
void
do_ocreate (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  OBJ_INDEX_DATA *pObjIndex;
  ...
  obj_to_char (obj, ch);
  act (AT_IMMORT,
       "$n makes arcane gestures, and opens $s hands to reveal $p!", ch, obj,
       NULL, TO_ROOM);
  ch_printf_color (ch,
...
void
do_mcreate (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
 ...
 */
void
edit_buffer (CHAR_DATA * ch, char *argument)
{
  DESCRIPTOR_DATA *d;
  EDITOR_DATA *edit;
      ...
      if (!str_cmp (cmd + 1, "r"))
	{
	  ...
	  for (x = 0; x < edit->numlines; x++)
	      ...
	      while ((wptr = strstr (lwptr, word1)) != NULL)
		{
		  ++count;
		  lineln =
		    snprintf (buf, MAX_INPUT_LENGTH, "%s%s", word2,
			      wptr + wordln);
	  ...
	  for (x = 0; x < edit->numlines; x++)
	    {
	      strncpy (temp_buf + p, edit->line[x],
		       MAX_STRING_LENGTH + max_buf_lines - p);
	  ...
	  if (edit->numlines >= max_buf_lines)
	      ...
	      else
		{
		  for (x = ++edit->numlines; x > line; x--)
		    mudstrlcpy (edit->line[x], edit->line[x - 1],
				MAX_STRING_LENGTH);
	  ...
	  if (edit->numlines == 0)
	      ...
	      else
		{
		  ...
		  for (x = line; x < (edit->numlines - 1); x++)
		    mudstrlcpy (edit->line[x], edit->line[x + 1],
				MAX_STRING_LENGTH);
		  mudstrlcpy (edit->line[edit->numlines--], "",
			      MAX_STRING_LENGTH);
...
assign_area (CHAR_DATA * ch)
{
  ...
  if (IS_NPC (ch))
    return;
  if (get_trust (ch) > LEVEL_IMMORTAL && ch->pcdata->r_range_lo
      ...
      if (!tarea)
	{
      ...
      if (!tarea)
	{
	  log_printf_plus (LOG_NORMAL, ch->level,
			   "Creating area entry for %s", ch->name);
	  CREATE (tarea, AREA_DATA, 1);
	  LINK (tarea, first_build, last_build, next, prev);
	  tarea->first_room = tarea->last_room = NULL;
	  snprintf (buf, MAX_STRING_LENGTH, "{PROTO} %s's area in progress",
		    ch->name);
	...
	{
	  log_printf_plus (LOG_NORMAL, ch->level,
			   "Updating area entry for %s", ch->name);
	}
...
void
do_aassign (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  AREA_DATA *tarea, *tmp;
...
/*		if ( get_trust(ch) >= sysdata.level_modify_proto  */
	  if (get_trust (ch) >= LEVEL_GREATER
	    ...
	    {
	      send_to_char
		("You do not have permission to use that area.\r\n", ch);
	      return;
	    }
	}
...
EXTRA_DESCR_DATA *
SetRExtra (ROOM_INDEX_DATA * room, const char *keywords)
{
  EXTRA_DESCR_DATA *ed;
...
bool
DelRExtra (ROOM_INDEX_DATA * room, const char *keywords)
{
...
EXTRA_DESCR_DATA *
SetOExtra (OBJ_DATA * obj, const char *keywords)
{
  EXTRA_DESCR_DATA *ed;
...
bool
DelOExtra (OBJ_DATA * obj, const char *keywords)
{
  EXTRA_DESCR_DATA *rmed;
...
EXTRA_DESCR_DATA *
SetOExtraProto (OBJ_INDEX_DATA * obj, const char *keywords)
{
  EXTRA_DESCR_DATA *ed;
...
bool
DelOExtraProto (OBJ_INDEX_DATA * obj, const char *keywords)
{
  EXTRA_DESCR_DATA *rmed;
...
void
fwrite_xsmaug_exdesc (FILE * fpout, EXTRA_DESCR_DATA * ed)
{
...
void
fwrite_xsmaug_exit (FILE * fpout, EXIT_DATA * pexit)
{
...
void
fwrite_xsmaug_affect (FILE * fp, AFFECT_DATA * paf)
{
...
// Write a prog
bool
mprog_write_prog (FILE * fpout, MPROG_DATA * mprog)
{
...
void
save_reset_level (FILE * fpout, RESET_DATA * start_reset, const int level)
{
  int spaces = level * 2;
  ...
  for (reset = start_reset; reset;)
	...
	default:
	  fprintf (fpout, "%*.sReset %c %d %d %d %d\n", spaces, "",
		   UPPER (reset->command), reset->extra, reset->arg1,
		   reset->arg2, reset->arg3);
	  break;
...
void
fwrite_xsmaug_room (FILE * fpout, ROOM_INDEX_DATA * room, bool install)
{
  EXIT_DATA *xit;
  AFFECT_DATA *paf;
  EXTRA_DESCR_DATA *ed;
  ...
  if (room->tele_delay > 0 || room->tele_vnum > 0 || room->tunnel > 0)
    fprintf (fpout, "Stats    %d %d %d\n", room->tele_delay, room->tele_vnum,
	     room->tunnel);
...
void
fwrite_xsmaug_object (FILE * fpout, OBJ_INDEX_DATA * pObjIndex, bool install)
{
  AFFECT_DATA *paf;
  EXTRA_DESCR_DATA *ed;
  MPROG_DATA *mprog;
  ...
  switch (pObjIndex->item_type)
    ...
    case ITEM_SCROLL:
      fprintf (fpout, "Spells   '%s' '%s' '%s'\n",
	       ...
	       IS_VALID_SN (pObjIndex->
			    value[3]) ? skill_table[pObjIndex->value[3]]->name
	       : "NONE");
      break;
    case ITEM_STAFF:
    case ITEM_WAND:
    ...
    case ITEM_SALVE:
      fprintf (fpout, "Spells   '%s' '%s'\n",
	       ...
	       IS_VALID_SN (pObjIndex->
			    value[5]) ? skill_table[pObjIndex->value[5]]->name
	       : "NONE");
      break;
    }
...
void
fwrite_xsmaug_mobile (FILE * fpout, MOB_INDEX_DATA * pMobIndex, bool install)
{
  SHOP_DATA *pShop;
  REPAIR_DATA *pRepair;
  MPROG_DATA *mprog;
  ...
  fprintf (fpout, "Stats1     %d %d %d %d %d %d\n", pMobIndex->alignment,
	   pMobIndex->level, pMobIndex->mobthac0, pMobIndex->ac,
  ...
  fprintf (fpout, "Stats4     %d %d %d %d %d\n", pMobIndex->height,
	   pMobIndex->weight, pMobIndex->numattacks, pMobIndex->hitroll,
	   pMobIndex->damroll);
  fprintf (fpout, "Attribs    %d %d %d %d %d %d %d\n", pMobIndex->perm_str,
	   pMobIndex->perm_int, pMobIndex->perm_wis, pMobIndex->perm_dex,
	   pMobIndex->perm_con, pMobIndex->perm_cha, pMobIndex->perm_lck);
  ...
  if ((pShop = pMobIndex->pShop) != NULL)
    {
      fprintf (fpout, "ShopData   %d %d %d %d %d %d %d %d %d\n",
	       ...
	       pShop->profit_sell, pShop->open_hour, pShop->close_hour);
    }
...
void
fwrite_area_header (FILE * fpout, AREA_DATA * tarea, bool install)
{
  if (install)
...
void
fold_area (AREA_DATA * tarea, const char *fname, bool install)
{
  char buf[256];
  FILE *fpout;
  MOB_INDEX_DATA *pMobIndex;
...
void
old_fold_area (AREA_DATA * tarea, char *filename, bool install)
{
  ...
  log_printf_plus (LOG_NORMAL, LEVEL_GREATER, "Saving %s...",
		   tarea->filename);
  ...
  for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++)
	...
	complexmob = FALSE;
      ...
      fprintf (fpout, "%d %d %d ", pMobIndex->level, pMobIndex->mobthac0,
	       pMobIndex->ac);
      ...
      if (complexmob)
	{
	  fprintf (fpout, "%d %d %d %d %d %d %d\n",
		   pMobIndex->perm_str,
		   pMobIndex->perm_int,
		   pMobIndex->perm_wis, pMobIndex->perm_dex,
		   pMobIndex->perm_con, pMobIndex->perm_cha,
		   pMobIndex->perm_lck);
	  ...
	  fprintf (fpout, "%d %d %d %d %d %d %d\n", pMobIndex->race,
		   pMobIndex->Class, pMobIndex->height, pMobIndex->weight,
		   pMobIndex->speaks, pMobIndex->speaking,
		   pMobIndex->numattacks);
	  fprintf (fpout, "%d %d %d %d %d %d %s ", pMobIndex->hitroll,
		   pMobIndex->damroll, pMobIndex->xflags,
  ...
  for (vnum = tarea->low_o_vnum; vnum <= tarea->hi_o_vnum; vnum++)
	...
	fprintf (fpout, "%d %d %d %d\n", val0, val1, val2, val3);
      ...
      fprintf (fpout, "%d %d %d\n", pObjIndex->weight,
	       pObjIndex->cost,
	       pObjIndex->rent ? pObjIndex->rent : (int) (pObjIndex->cost /
							  10));
      ...
      for (xit = room->first_exit; xit; xit = xit->next)
	{
	  ...
	  if (xit->distance > 1 || xit->pull)
	    fprintf (fpout, "%d %d %d %d %d %d\n",
		     ...
		     xit->distance, xit->pulltype, xit->pull);
	  else
      ...
      for (pReset = room->first_reset; pReset; pReset = pReset->next)
	{
	  switch (pReset->command)	/* extra arg1 arg2 arg3 */
	    ...
	    case 'O':
	      ...
	      for (tReset = pReset->first_reset; tReset;
		   tReset = tReset->next_reset)
		{
  ...
  for (vnum = tarea->low_m_vnum; vnum <= tarea->hi_m_vnum; vnum++)
	...
	continue;
      fprintf (fpout, " %d   %2d %2d %2d %2d %2d   %3d %3d",
	       ...
	       pShop->buy_type[0],
	       pShop->buy_type[1],
	       pShop->buy_type[2], pShop->buy_type[3], pShop->buy_type[4],
...
void
do_savearea (CHAR_DATA * ch, const char *argument)
{
  AREA_DATA *tarea;
  char filename[256];
...
void
do_loadarea (CHAR_DATA * ch, const char *argument)
{
  AREA_DATA *tarea;
  char filename[256];
  int tmp;
 ...
 */
void
do_unfoldarea (CHAR_DATA * ch, const char *argument)
{
...
void
do_foldarea (CHAR_DATA * ch, const char *argument)
{
  AREA_DATA *tarea;
 ...
 */
void
do_installarea (CHAR_DATA * ch, const char *argument)
{
  AREA_DATA *tarea;
  char arg[MAX_INPUT_LENGTH];
  char buf[MAX_STRING_LENGTH];
  ...
  for (tarea = first_build; tarea; tarea = tarea->next)
      ...
      if (!str_cmp (tarea->filename, arg))
	{
	  ...
	  snprintf (buf, MAX_STRING_LENGTH, "%s%s.installed", BUILD_DIR,
		    tarea->filename);
	  snprintf (arg, MAX_INPUT_LENGTH, "%s%s", BUILD_DIR,
		    tarea->filename);
	  rename (arg, buf);
	  snprintf (buf, MAX_STRING_LENGTH, "%s%s.bak", BUILD_DIR,
		    tarea->filename);
	  unlink (buf);
	  send_to_char ("Done.\r\n", ch);
	  return;
...
void
do_astat (CHAR_DATA * ch, const char *argument)
{
  AREA_DATA *tarea;
  bool proto, found;
  ...
  ch_printf_color (ch,
		   ...
		   tarea->author);
  ch_printf_color (ch,
		   "&wAge: &W%-3d  &wCurrent number of players: &W%-3d  &wMax players: &W%d\r\n",
  ...
  ch_printf_color (ch,
		   "&wsoft range: &W%d - %d    &whard range: &W%d - %d\r\n",
...
bool
check_for_area_conflicts (AREA_DATA * carea, int lo, int hi)
{
  AREA_DATA *area;
...
void
do_aset (CHAR_DATA * ch, const char *argument)
{
  AREA_DATA *tarea;
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg2, "weatherx"))
    {
      if (get_trust (ch) < LEVEL_ASCENDANT)
	send_to_char
	  ("I don't recognize that field. Please double check.\r\n", ch);
      else
	{
	  tarea->weatherx = vnum;
	  send_to_char ("Done.\r\n", ch);
...
void
do_rlist (CHAR_DATA * ch, const char *argument)
{
  ROOM_INDEX_DATA *room;
  int vnum;
  char arg1[MAX_INPUT_LENGTH];
...
void
do_olist (CHAR_DATA * ch, const char *argument)
{
  OBJ_INDEX_DATA *obj;
  int vnum;
  AREA_DATA *tarea;
...
void
do_mlist (CHAR_DATA * ch, const char *argument)
{
  MOB_INDEX_DATA *mob;
  int vnum;
  AREA_DATA *tarea;
  ...
  for (vnum = lrange; vnum <= trange; vnum++)
	...
	continue;
      pager_printf (ch, "%5d) %-20s '%s'\r\n", vnum, mob->player_name,
		    mob->short_descr);
    }
}

void
mpedit (CHAR_DATA * ch, MPROG_DATA * mprg, int mptype, const char *argument)
{
  if (mptype != -1)
    {
      mprg->type = mptype;
 ...
 */
void
do_mpedit (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
  ...
  switch (ch->substate)
    ...
    case SUB_RESTRICTED:
      send_to_char
	("You can't use this command from within another command.\r\n", ch);
      return;
    case SUB_MPROG_EDIT:
      if (!ch->dest_buf)
	{
...
void
do_opedit (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
 ...
 */
void
rpedit (CHAR_DATA * ch, MPROG_DATA * mprg, int mptype, char *argument)
{
  if (mptype != -1)
    {
      mprg->type = mptype;
...
void
do_rpedit (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
...
void
do_rdelete (CHAR_DATA * ch, const char *argument)
{
   ...
   */
  if (get_trust (ch) < sysdata.level_modify_proto
      && (location->vnum < ch->pcdata->area->low_r_vnum
	  || location->vnum > ch->pcdata->area->hi_r_vnum))
    {
...
void
do_odelete (CHAR_DATA * ch, const char *argument)
{
  OBJ_INDEX_DATA *obj;
  int vnum;
   ...
   */
  if (get_trust (ch) < sysdata.level_modify_proto
      && (obj->vnum < ch->pcdata->area->low_o_vnum
	  || obj->vnum > ch->pcdata->area->hi_o_vnum))
    {
...
void
do_mdelete (CHAR_DATA * ch, const char *argument)
{
  MOB_INDEX_DATA *mob;
  int vnum;
   ...
   */
  if (get_trust (ch) < sysdata.level_modify_proto
      && (mob->vnum < ch->pcdata->area->low_m_vnum
	  || mob->vnum > ch->pcdata->area->hi_m_vnum))
    {
...
void
mpcopy (MPROG_DATA * source, MPROG_DATA * destination)
{
...
void
do_opcopy (CHAR_DATA * ch, const char *argument)
{
  ...
  MPROG_DATA *source_oprog = NULL, *dest_oprog = NULL, *source_oprg =
    NULL, *dest_oprg = NULL;
  ...
  if (sobj[0] == '\0' || prog[0] == '\0')
    {
      send_to_char
	("Syntax: opcopy <source object> <program> [number] <destination object>\r\n",
	 ch);
      send_to_char
	("        opcopy <source object> all <destination object>\r\n", ch);
      send_to_char
	("        opcopy <source object> all <destination object> <program>\r\n",
	 ch);
      send_to_char ("\r\n", ch);
      send_to_char ("Program being one of:\r\n", ch);
      send_to_char ("  act speech rand wear remove sac zap get\r\n", ch);
  ...
  if (get_trust (ch) < LEVEL_GOD)
	...
	{
	  send_to_char
	    ("Can't find destination object in all the realms.\r\n", ch);
	  return;
	}
    }
  ...
  if (COPY)
    {
      for (source_oprg = source_oprog; source_oprg;
	   source_oprg = source_oprg->next)
	{
  ...
  for (source_oprg = source_oprog; source_oprg;
      ...
      if (++cnt == value && source_oprg->type == optype)
	{
	  ...
	  xSET_BIT (destination->pIndexData->progtypes, dest_oprg->type);
	  ch_printf (ch,
		     "%s program %d from %s successfully copied to %s.\r\n",
...
void
do_mpcopy (CHAR_DATA * ch, const char *argument)
{
  ...
  MPROG_DATA *source_mprog = NULL, *dest_mprog = NULL, *source_mprg =
    NULL, *dest_mprg = NULL;
  ...
  if (smob[0] == '\0' || prog[0] == '\0')
    {
      send_to_char
	("Syntax: mpcopy <source mobile> <program> [number] <destination mobile>\r\n",
	 ch);
      send_to_char
	("        mpcopy <source mobile> all <destination mobile>\r\n", ch);
      send_to_char
	("        mpcopy <source mobile> all <destination mobile> <program>\r\n",
	 ch);
      send_to_char ("\r\n", ch);
      send_to_char ("Program being one of:\r\n", ch);
      send_to_char ("  act speech rand fight hitprcnt greet allgreet\r\n",
  ...
  if (!xIS_SET (destination->act, ACT_PROTOTYPE))
    {
      send_to_char
	("Destination mobile must have a prototype flag to mpcopy.\r\n", ch);
      return;
    }
  ...
  if (COPY)
    {
      for (source_mprg = source_mprog; source_mprg;
	   source_mprg = source_mprg->next)
	{
...
void
do_rpcopy (CHAR_DATA * ch, const char *argument)
{
  ...
  MPROG_DATA *source_rprog = NULL, *dest_rprog = NULL, *source_rprg =
    NULL, *dest_rprg = NULL;
  int value = -1, mptype = -1, cnt = 0;
  bool COPY = FALSE;
  ...
  if (!is_number (sroom) || prog[0] == '\0')
    {
      send_to_char
	("Syntax: rpcopy <source rvnum> <program> [number] <destination rvnum>\r\n",
	 ch);
      send_to_char ("  rpcopy <source rvnum> all <destination rvnum>\r\n",
		    ch);
      send_to_char
	("  rpcopy <source rvnum> all <destination rvnum> <program>\r\n", ch);
      send_to_char ("\r\n", ch);
      send_to_char ("Program being one of:\r\n", ch);
      send_to_char ("  act speech rand fight hitprcnt greet allgreet\r\n",
		    ch);
  ...
  if (!is_number (droom)
      || (destination = get_room_index (atoi (droom))) == NULL)
    {
  ...
  if (COPY)
    {
      for (source_rprg = source_rprog; source_rprg;
	   source_rprg = source_rprg->next)
	{
 ...
 */
void
RelCreate (relation_type tp, void *actor, void *subject)
{
  REL_DATA *tmp;
 ...
 */
void
RelDestroy (relation_type tp, void *actor, void *subject)
{
  REL_DATA *rq;
...
void
do_findexit (CHAR_DATA * ch, const char *argument)
{
  ROOM_INDEX_DATA *room;
  EXIT_DATA *pexit;
  ...
  for (cou = 0; cou < MAX_KEY_HASH; cou++)
	...
	for (room = room_index_hash[cou]; room; room = room->next)
	  {
	    if (room->vnum >= l_range && room->vnum <= h_range
		&& room->first_exit)
	      {
 ...
 */
void
do_alinks (CHAR_DATA * ch, const char *argument)
{
  ...
  for (area = first_area_name, target = NULL; area;
       area = area->next_sort_name)
    {
  ...
  ch_printf (ch, "&wListing all links from &W%s &wto &W%s&w...\r\n\r\n",
	     rev ? "&wthe world" : area->name,
	     rev ? area->name : "&wthe world");
  ...
  for (area = first_area_name; area; area = area->next_sort_name)
      ...
      for (rnum = area->low_r_vnum; rnum <= area->hi_r_vnum; rnum++)
	{
	  ...
	  for (rexit = room->first_exit; rexit; rexit = rexit->next)
	    {
	      if (rev ? rexit->to_room->area ==
		  target : rexit->to_room->area != target)
		{
		  /*
		  ...
		  if (!ex_found)
			...
			ch_printf (ch, "&W%s &c- &W%s\r\n\r\n",
		      ...
		      ch_printf (ch, "  &G%d &c- &w%s", room->vnum,
				 room->name);
		      if (xHAS_BITS (room->room_flags, fmask))
			{
			  ...
			  for (i = 0; i < XBI; i++)
			    dummy.bits[i] =
			      room->room_flags.bits[i] & fmask.bits[i];
			  ch_printf (ch, " &c- &O%s",
				     ext_flag_string (&dummy, r_flags));
			}
		      send_to_char ("\r\n", ch);
		    }
		  dnum = UMIN (rexit->vdir, 10);
		  if (detail == 0)
		    ch_printf (ch, "%s&C%s:&g%d&O%s%s%s",
			       ex_found ? " " : "    ", dirs[dnum],
			       rexit->vnum,
			       xIS_EMPTY (rexit->
					  to_room->progtypes) ? "" : "p",
		    ...
		    {
		      ch_printf (ch, "    &C%-2s&c:&g%d&O%s &c- %s%s",
				 dirs[dnum], rexit->vnum,
				 xIS_EMPTY (rexit->
					    to_room->progtypes) ? "" : "p",
				 rev ? "&w" : "&W",
				 rev ? rexit->to_room->name : rexit->
				 to_room->area->name);
		      if (rexit->exit_info)
			ch_printf (ch, " &c- &O%s",
				   flag_string (rexit->exit_info, ex_flags));
		      if (rexit->key > 0)
		    ...
		    ch_printf (ch,
			       "    &C%-2s&c:&g%d&O%s%s%s &c- &W%s &c- &w%s\r\n",
			       dirs[dnum], rexit->vnum,
			       xIS_EMPTY (rexit->
					  to_room->progtypes) ? "" : "p",
			       rexit->key <= 0 ? "" : "k",
			       rexit->exit_info ? "f" : "",
			       rexit->to_room->area->name,
			       rexit->to_room->name);
		  ex_found = TRUE;
		}
	    }
