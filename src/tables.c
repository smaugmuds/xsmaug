...
const char *skill_tname[] =
  { "unknown", "Spell", "Skill", "Weapon", "Tongue", "Herb", "Racial",
  "Disease"
};

SPELL_FUN *
spell_function (const char *name)
{
  void *funHandle;
#if !defined(WIN32)
  const char *error;
...
DO_FUN *
skill_function (const char *name)
{
  void *funHandle;
#if !defined(WIN32)
  const char *error;
...
bool
load_class_file (const char *fname)
{
	  ...
	  if (!str_cmp (word, "End"))
	      ...
	      if (cl < 0 || cl >= MAX_CLASS)
		{
		  bug ("Load_class_file: Class (%s) bad/not found (%d)",
		       Class->who_name ? Class->who_name : "name not found",
		       cl);
		  if (Class->who_name)
	  ...
	  if (!str_cmp (word, "Skill"))
	    {
	      ...
	      if (cl < 0 || cl >= MAX_CLASS)
		{
		  bug ("%s: Skill %s -- class bad/not found (%d)",
		       __FUNCTION__, word, cl);
		}
	...
	case 'T':
	  if (!str_cmp (word, "Title"))
	      ...
	      if (cl < 0 || cl >= MAX_CLASS)
		{
		  ...
		  bug ("load_class_file: Title -- class bad/not found (%d)",
		       cl);
...
write_class_file (int cl)
{
  ...
  snprintf (filename, MAX_INPUT_LENGTH, "%s%s.class", CLASS_DIR,
	    Class->who_name);
...
write_race_file (int ra)
{
  ...
  snprintf (filename, MAX_INPUT_LENGTH, "%s%s.race", RACE_DIR,
	    race->race_name);
  if ((fpout = fopen (filename, "w+")) == NULL)
    {
      bug ("Cannot open: %s for writing", filename);
...
bool
load_race_file (const char *fname)
{
  char buf[MAX_STRING_LENGTH];
  const char *word;
	  ...
	  if (!str_cmp (word, "End"))
	      ...
	      if (ra < 0 || ra >= MAX_RACE)
		{
		  bug ("%s: Race (%s) bad/not found (%d)", __FUNCTION__,
		       race->race_name ? race->race_name : "name not found",
		       ra);
		  STRFREE (race_name);
		  for (i = 0; i < MAX_WHERE_NAME; ++i)
	  ...
	  if (!str_cmp (word, "Skill"))
	    {
	      ...
	      if (ra < 0 || ra >= MAX_RACE)
		{
		  bug ("%s: Skill %s -- race bad/not found (%d)",
		       __FUNCTION__, word, ra);
		}
	      else if (!IS_VALID_SN (sn))
		{
		  bug ("%s: Skill %s unknown", __FUNCTION__, word);
	...
	case 'W':
	  ...
	  if (!str_cmp (word, "WhereName"))
	      ...
	      if (ra < 0 || ra >= MAX_RACE)
		{
		  ...
		  bug ("%s: Title -- race bad/not found (%d)", __FUNCTION__,
		       ra);
		  tmp = fread_string_nohash (fp);
		  DISPOSE (tmp);
		  tmp = fread_string_nohash (fp);
 ...
 */
int
skill_comp (SKILLTYPE ** sk1, SKILLTYPE ** sk2)
{
 ...
 */
int
skill_comp_bytype (SKILLTYPE ** sk1, SKILLTYPE ** sk2)
{
  SKILLTYPE *skill1 = (*sk1);
  SKILLTYPE *skill2 = (*sk2);
...
sort_skill_table ()
{
  ...
  qsort (&skill_table[1], num_skills - 1, sizeof (SKILLTYPE *),
 ...
 */
void
fwrite_skill (FILE * fpout, SKILLTYPE * skill)
{
  SMAUG_AFF *aff;
  int modifier;
...
int
get_skill (const char *skilltype)
{
  ...
  for (;;)
	  ...
	  if (!str_cmp (word, "Code"))
	    {
	      ...
	      fMatch = TRUE;
	      if (!str_prefix ("do_", w)
		  && (dofun = skill_function (w)) != skill_notfound)
		{
		  ...
		  skill->skill_fun_name = str_dup (w);
		       ...
		       && (spellfun = spell_function (w)) != spell_notfound)
		{
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		{
		  bug
		    ("%s (%s):  Has saving throw (%d) with no saving effect.",
		     __FUNCTION__, skill->name, skill->saves);
  ...
  if ((fp = fopen (SKILL_FILE, "r")) != NULL)
      ...
      for (;;)
	{
	  ...
	  if (!str_cmp (word, "SKILL"))
		...
		{
		  bug ("%s: more skills than MAX_SKILL %d", __FUNCTION__,
		       MAX_SKILL);
		  fclose (fp);
		  fp = NULL;
		  return;
  ...
  if ((fp = fopen (HERB_FILE, "r")) != NULL)
      ...
      for (;;)
	{
	  ...
	  if (!str_cmp (word, "HERB"))
		...
		{
		  bug ("%s: more herbs than MAX_HERB %d", __FUNCTION__,
		       MAX_HERB);
		  fclose (fp);
		  fp = NULL;
		  return;
	...
	case 'E':
	  if (!str_cmp (word, "End"))
	      ...
	      if (!command->name)
		{
	      ...
	      if (!command->fun_name)
		{
		  bug ("%s: No function name supplied for %s", __FUNCTION__,
		       command->name);
		  free_command (command);
		  return;
		}
		...
		{
		  bug ("%s: Function %s not found for %s", __FUNCTION__,
		       command->fun_name, command->name);
		  free_command (command);
		  return;
		}
	      add_command (command);
 ...
 */
void
fread_cnv (FILE * fp, LCNV_DATA ** first_cnv, LCNV_DATA ** last_cnv)
{
  LCNV_DATA *cnv;
  char letter;
