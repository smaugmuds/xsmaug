...
show_colorthemes (CHAR_DATA * ch)
{
  ...
  while (dentry)
	...
	{
	  ++count;
	  pager_printf (ch, "%s%-15.15s", color_str (AT_PLAIN, ch),
			dentry->d_name);
	  if (++col % 6 == 0)
	    send_to_pager ("\r\n", ch);
	}
...
show_colors (CHAR_DATA * ch)
{
  ...
  send_to_pager
    ("&BSyntax: color _all_ [color] (Sets all color types to [color])\r\n\r\n",
     ch);
  send_to_pager
    ("&BSyntax: color theme [name] (Sets all color types to a defined theme)\r\n\r\n",
     ch);
...
reset_colors (CHAR_DATA * ch)
{
  ...
  if (!IS_NPC (ch))
    ...
    log_printf ("%s: Attempting to reset NPC colors: %s", __FUNCTION__,
		ch->short_descr);
}

void
do_color (CHAR_DATA * ch, const char *argument)
{
  bool dMatch, cMatch;
  short count = 0, y = 0;
  int x;
  ...
  if (!str_cmp (arg, "savetheme") && IS_IMMORTAL (ch))
	...
	{
	  send_to_char
	    ("You must specify a name for this theme to save it.\r\n", ch);
	  return;
	}
  ...
  if (!str_cmp (arg, "theme"))
      ...
      while (!feof (fp))
	{
      ...
      fp = NULL;
      ch_printf (ch,
		 "An error occured while trying to set color theme %s.\r\n",
		 argument);
      return;
    }
...
const char *
color_str (short AType, CHAR_DATA * ch)
{
  if (!ch)
    {
 ...
 */
int
colorcode (const char *src, char *dst, DESCRIPTOR_DATA * d, int dstlen,
	   int *vislen)
{
  CHAR_DATA *ch = NULL;
  bool ansi = FALSE;
 ...
 */
char *
color_align (const char *argument, int size, int align)
{
  int space = 0;
  int len = 0;
 ...
 */
char *
colorize (const char *txt, DESCRIPTOR_DATA * d)
{
  ...
  *result = '\0';
  ...
  if (txt && *txt && d)
      ...
      while ((colstr = strpbrk (prevstr, "&^}hH")) != NULL)
	{
	  ...
	  if (colstr > prevstr)
	    {
	      if ((MAX_STRING_LENGTH - (reslen = strlen (result))) <=
		  (colstr - prevstr))
		{
		  bug ("%s: OVERFLOW in internal MAX_STRING_LENGTH buffer!",
		       __PRETTY_FUNCTION__);
		  break;
		}
...
/* Moved from comm.c */
void
set_char_color (short AType, CHAR_DATA * ch)
{
  if (!ch || !ch->desc)
    return;
  ...
  if (!ch->desc)
    {
      bug ("set_char_color: NULL descriptor after WTB! CH: %s",
...
void
write_to_pager (DESCRIPTOR_DATA * d, const char *txt, size_t length)
{
...
void
set_pager_color (short AType, CHAR_DATA * ch)
{
  if (!ch || !ch->desc)
    return;
  ...
  if (!ch->desc)
    {
      bug ("%s: NULL descriptor after WTP! CH: %s", __FUNCTION__,
	   ch->name ? ch->name : "Unknown?!?");
      return;
    }
  ch->desc->pagecolor = ch->colors[AType];
...
void
ch_printf (CHAR_DATA * ch, const char *fmt, ...)
{
...
void
pager_printf (CHAR_DATA * ch, const char *fmt, ...)
{
  char buf[MAX_STRING_LENGTH * 2];
  va_list args;
...
/* Major overhaul. -- Alty */
void
ch_printf_color (CHAR_DATA * ch, const char *fmt, ...)
{
  char buf[MAX_STRING_LENGTH * 2];
  va_list args;
...
void
pager_printf_color (CHAR_DATA * ch, const char *fmt, ...)
{
  char buf[MAX_STRING_LENGTH * 2];
  va_list args;
...
void
paint (short AType, CHAR_DATA * ch, const char *fmt, ...)
{
