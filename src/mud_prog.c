...
void
uphold_supermob (int *curr_serial, int serial,
 ...
 */
static bool
carryingvnum_visit (CHAR_DATA * ch, OBJ_DATA * obj, int vnum)
{
...
int mprog_do_command (const char *cmnd, CHAR_DATA * mob, CHAR_DATA * actor,
		      OBJ_DATA * obj, const void *vo, CHAR_DATA * rndm,
...
void mprog_driver (const char *com_list, CHAR_DATA * mob, CHAR_DATA * actor,
		   OBJ_DATA * obj, const void *vo, bool single_step);
bool mprog_keyword_check (const char *argu, const char *argl);
bool oprog_wordlist_check (const char *arg, CHAR_DATA * mob,
			   CHAR_DATA * actor, OBJ_DATA * obj, const void *vo,
...
void rprog_percent_check (CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj,
			  void *vo, int type);
bool rprog_wordlist_check (const char *arg, CHAR_DATA * mob,
			   CHAR_DATA * actor, OBJ_DATA * obj, const void *vo,
			   int type, ROOM_INDEX_DATA * room);
 ...
 */
bool
mprog_seval (const char *lhs, const char *opr, const char *rhs,
	     CHAR_DATA * mob)
{
  ...
  snprintf (log_buf, MAX_STRING_LENGTH, "Improper MOBprog operator '%s'",
	    opr);
  progbug (log_buf, mob);
  return 0;
}
 ...
 */
int
mprog_do_ifcheck (const char *ifcheck, CHAR_DATA * mob, CHAR_DATA * actor,
		  OBJ_DATA * obj, const void *vo, CHAR_DATA * rndm)
{
  ...
  *p = '\0';
  ...
  if (cvar[0] == '$')
	...
	case 'p':
	  ...
	  break;
	default:
	  snprintf (rval, MAX_STRING_LENGTH, "Bad argument '%c' to '%s'",
		    cvar[0], chck);
	  progbug (rval, mob);
	  return BERR;
	}
  ...
  if (!str_cmp (chck, "economy"))
      ...
      if (!idx)
	{
	  if (!mob->in_room)
	    {
	      progbug
		("'economy' ifcheck: mob in NULL room with no room vnum "
		 "argument", mob);
	      return BERR;
	    }
	  room = mob->in_room;
  ...
  if (chkchar)
      ...
      if (!str_cmp (chck, "mobinvislevel"))
	{
	  return (IS_NPC (chkchar) ?
		  mprog_veval (chkchar->mobinvis, opr, atoi (rval),
			       mob) : FALSE);
	}
	...
	{
	  return (!IS_NPC (chkchar) ? chkchar->
		  pcdata->nuisance ? TRUE : FALSE : FALSE);
	}
	...
	{
	  return mprog_veval (chkchar->num_fighting - 1, opr, atoi (rval),
			      mob);
	}
      if (!str_cmp (chck, "hitprcnt"))
	{
	  return mprog_veval ((chkchar->hit * 100) / chkchar->max_hit, opr,
			      atoi (rval), mob);
	}
      ...
      if (!str_cmp (chck, "wasinroom"))
	{
	  if (!chkchar->was_in_room)
	    return FALSE;
	  return mprog_veval (chkchar->was_in_room->vnum, opr, atoi (rval),
			      mob);
	}
      if (!str_cmp (chck, "norecall"))
	{
	  return xIS_SET (chkchar->in_room->room_flags,
			  ROOM_NO_RECALL) ? TRUE : FALSE;
	}
      if (!str_cmp (chck, "sex"))
	{
	...
	{
	  return IS_NPC (chkchar) ? FALSE : mprog_veval (chkchar->
							 pcdata->quest_number,
							 opr, atoi (rval),
							 mob);
	}
      ...
      if (!str_cmp (chck, "multi"))
	{
	  ...
	  for (ch = first_char; ch; ch = ch->next)
		...
		&& ch->desc && chkchar->desc
		&& !str_cmp (ch->desc->host, chkchar->desc->host))
	      lhsvl++;
	  rhsvl = atoi (rval);
	  if (rhsvl < 0)
	    rhsvl = 0;
      ...
      if (!str_cmp (chck, "nuisance"))
	{
	  if (IS_NPC (chkchar) || !chkchar->pcdata->nuisance)
	    return FALSE;
	  return mprog_veval (chkchar->pcdata->nuisance->flags, opr, rhsvl,
			      mob);
	}
      ...
      if (!str_cmp (chck, "wearing"))
	{
	  OBJ_DATA *tobj;
	  int i = 0;
	  for (tobj = chkchar->first_carrying; tobj;
	       tobj = tobj->next_content)
	    {
	      i++;
	      if (chkchar == tobj->carried_by && tobj->wear_loc > -1
		  && !str_cmp (rval, item_w_flags[tobj->wear_loc]))
		return TRUE;
	    }
	  return FALSE;
	}
      ...
      if (!str_cmp (chck, "wearingvnum"))
	{
	  ...
	  for (tobj = chkchar->first_carrying; tobj;
	    ...
	    {
	      if (chkchar == tobj->carried_by && tobj->wear_loc > -1
		  && tobj->pIndexData->vnum == atoi (rval))
		return TRUE;
	    }
	  return FALSE;
	}
      ...
      if (!str_cmp (chck, "carryingvnum"))
	{
	  ...
	  if (!chkchar->first_carrying)
	    return FALSE;
	  return (carryingvnum_visit
		  (chkchar, chkchar->first_carrying, vnum));
	}
      ...
      if (!str_cmp (chck, "clantype"))
	{
	  if (IS_NPC (chkchar) || !chkchar->pcdata->clan)
	    return FALSE;
	  return mprog_veval (chkchar->pcdata->clan->clan_type, opr,
			      atoi (rval), mob);
	}
	...
	{
	  return mprog_veval (get_timer (chkchar, TIMER_ASUPRESSED), opr,
			      atoi (rval), mob);
	}
  ...
  if (chkobj)
      ...
      if (!str_cmp (chck, "leverpos"))
	{
	  ...
	  if (chkobj->item_type != ITEM_SWITCH
	      && chkobj->item_type != ITEM_LEVER
  ...
  if (!str_cmp (chck, "number"))
      ...
      if (chkchar)
	{
	  if (!IS_NPC (chkchar))
	    return FALSE;
	  lhsvl =
	    (chkchar == mob) ? chkchar->gold : chkchar->pIndexData->vnum;
  ...
  if (!str_cmp (chck, "mortinworld"))	/* -- Gorog */
	...
	if (d->connected == CON_PLAYING
	    && d->character && get_trust (d->character) < LEVEL_IMMORTAL
  ...
  if (!str_cmp (chck, "mortcount"))	/* -- Gorog */
    {
      ...
      for (tch = room ? room->first_person : NULL; tch;
  ...
  *t = '\0';
  switch (ch)
    ...
    case 'e':
      if (actor && !char_died (actor))
	{
	  can_see (mob, actor) ? strcpy (t, he_she[actor->sex]) : strcpy (t,
									  "someone");
	}
      else
	strcpy (t, "it");
    ...
    case 'm':
      if (actor && !char_died (actor))
	{
	  can_see (mob, actor) ? strcpy (t, him_her[actor->sex]) : strcpy (t,
									   "someone");
	}
      else
	strcpy (t, "it");
    ...
    case 's':
      if (actor && !char_died (actor))
	{
	  can_see (mob, actor) ? strcpy (t, his_her[actor->sex]) : strcpy (t,
									   "someone's");
	}
      else
	strcpy (t, "its'");
    ...
    case 'E':
      if (vict && !char_died (vict))
	{
	  can_see (mob, vict) ? strcpy (t, he_she[vict->sex]) : strcpy (t,
									"someone");
	}
      else
	strcpy (t, "it");
    ...
    case 'M':
      if (vict && !char_died (vict))
	{
	  can_see (mob, vict) ? strcpy (t, him_her[vict->sex]) : strcpy (t,
									 "someone");
	}
      else
	strcpy (t, "it");
    ...
    case 'S':
      if (vict && !char_died (vict))
	{
	  can_see (mob, vict) ? strcpy (t, his_her[vict->sex]) : strcpy (t,
									 "someone's");
	}
      else
	strcpy (t, "its'");
    ...
    case 'J':
      if (rndm && !char_died (rndm))
	{
	  can_see (mob, rndm) ? strcpy (t, he_she[rndm->sex]) : strcpy (t,
									"someone");
	}
      else
	strcpy (t, "it");
    ...
    case 'K':
      if (rndm && !char_died (rndm))
	{
	  can_see (mob, rndm) ? strcpy (t, him_her[rndm->sex]) : strcpy (t,
									 "someone's");
	}
      else
	strcpy (t, "its'");
    ...
    case 'L':
      if (rndm && !char_died (rndm))
	{
	  can_see (mob, rndm) ? strcpy (t, his_her[rndm->sex]) : strcpy (t,
									 "someone");
	}
      else
	strcpy (t, "its");
	...
	{
	  can_see_obj (mob, obj) ? one_argument (obj->name, t) : strcpy (t,
									 "something");
	}
      else
	strcpy (t, "something");
    ...
    case 'O':
      if (obj && !obj_extracted (obj))
	{
	  can_see_obj (mob, obj) ? strcpy (t, obj->short_descr) : strcpy (t,
									  "something");
	}
      else
	strcpy (t, "something");
	...
	{
	  can_see_obj (mob, v_obj) ? one_argument (v_obj->name,
						   t) : strcpy (t,
								"something");
	}
      else
    ...
    case 'P':
      if (v_obj && !obj_extracted (v_obj))
	{
	  can_see_obj (mob, v_obj) ? strcpy (t,
					     v_obj->short_descr) : strcpy (t,
									   "something");
	}
      else
	strcpy (t, "something");
 ...
 */
void
mprog_driver (const char *com_list, CHAR_DATA * mob, CHAR_DATA * actor,
	      OBJ_DATA * obj, const void *vo, bool single_step)
{
  ...
  while (TRUE)
      ...
      if (!str_cmp (arg, "mpsleep"))
	{
      ...
      result = mprog_do_command (cmnd, mob, actor, obj, vo, rndm,
				 (ifstate[iflevel][IN_IF]
				  && !ifstate[iflevel][DO_IF])
				 || (ifstate[iflevel][IN_ELSE]
				     && !ifstate[iflevel][DO_ELSE]),
				 (ignorelevel > 0));

      if (rprog_oprog)
	uphold_supermob (&curr_serial, serial, &supermob_room,
			 true_supermob_obj);
 ...
 */
int
mprog_do_command (const char *cmnd, CHAR_DATA * mob, CHAR_DATA * actor,
		  OBJ_DATA * obj, const void *vo, CHAR_DATA * rndm,
		  bool ignore, bool ignore_ors)
{
 ...
 */
bool
mprog_wordlist_check (const char *arg, CHAR_DATA * mob, CHAR_DATA * actor,
 ...
 */
void
mprog_act_trigger (const char *buf, CHAR_DATA * mob, CHAR_DATA * ch,
		   OBJ_DATA * obj, const void *vo)
{
...
void
mprog_bribe_trigger (CHAR_DATA * mob, CHAR_DATA * ch, int amount)
{
...
void
mprog_death_trigger (CHAR_DATA * killer, CHAR_DATA * mob)
{
...
mprog_greet_trigger (CHAR_DATA * ch)
{
  ...
  for (vmob = ch->in_room->first_person; vmob; vmob = trvch_next (loop_ctrl))
    {
      if (!IS_NPC (vmob) || !can_see (vmob, ch) || vmob->fighting
	  || !IS_AWAKE (vmob))
	continue;
...
void
mprog_time_trigger (CHAR_DATA * mob)
{
...
void
mprog_hour_trigger (CHAR_DATA * mob)
{
...
void
mprog_speech_trigger (const char *txt, CHAR_DATA * actor)
{
  CHAR_DATA *vmob;
...
void
mprog_tell_trigger (const char *txt, CHAR_DATA * actor)
{
  CHAR_DATA *vmob;
...
void
mprog_script_trigger (CHAR_DATA * mob)
{
  MPROG_DATA *mprg;
...
oprog_script_trigger (OBJ_DATA * obj)
{
  ...
  if (HAS_PROG (obj->pIndexData, SCRIPT_PROG))
    for (mprg = obj->pIndexData->mudprogs; mprg; mprg = mprg->next)
      if (mprg->type == SCRIPT_PROG)
	{
	  if (mprg->arglist[0] == '\0' || obj->mpscriptpos != 0
	      || atoi (mprg->arglist) == time_info.hour)
	    {
	      set_supermob (obj);
...
void
rprog_script_trigger (ROOM_INDEX_DATA * room)
{
  MPROG_DATA *mprg;
  ...
  if (HAS_PROG (room, SCRIPT_PROG))
    for (mprg = room->mudprogs; mprg; mprg = mprg->next)
      if (mprg->type == SCRIPT_PROG)
	{
	  if (mprg->arglist[0] == '\0' || room->mpscriptpos != 0
	      || atoi (mprg->arglist) == time_info.hour)
	    {
	      rset_supermob (room);
	      mprog_driver (mprg->comlist, supermob, NULL, NULL, NULL, TRUE);
...
bool
oprog_percent_check (CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj,
		     void *vo, int type)
{
  MPROG_DATA *mprg;
  bool executed = FALSE;
...
void
oprog_greet_trigger (CHAR_DATA * ch)
{
  OBJ_DATA *vobj;
...
void
oprog_speech_trigger (const char *txt, CHAR_DATA * ch)
{
  OBJ_DATA *vobj;
...
oprog_command_trigger (CHAR_DATA * ch, char *txt)
{
  ...
  for (vobj = ch->in_room->first_content; vobj; vobj = vobj->next_content)
    if (HAS_PROG (vobj->pIndexData, CMD_PROG))
      if (oprog_wordlist_check
	  (txt, supermob, ch, vobj, NULL, CMD_PROG, vobj))
	return TRUE;
 ...
 */
void
oprog_wear_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
...
bool
oprog_use_trigger (CHAR_DATA * ch, OBJ_DATA * obj, CHAR_DATA * vict,
		   OBJ_DATA * targ)
{
  bool executed = FALSE;
  ...
  if (HAS_PROG (obj->pIndexData, USE_PROG))
	  ...
	  || obj->item_type == ITEM_SCROLL)
	{
	  if (vict)
	    executed =
	      oprog_percent_check (supermob, ch, obj, vict, USE_PROG);
	  else
	    executed =
	      oprog_percent_check (supermob, ch, obj, targ, USE_PROG);
	}
      else
 ...
 */
void
oprog_remove_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_sac_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_get_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_damage_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_repair_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_drop_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_examine_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_zap_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_pull_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
oprog_push_trigger (CHAR_DATA * ch, OBJ_DATA * obj)
{
...
void
oprog_act_trigger (const char *buf, OBJ_DATA * mobj, CHAR_DATA * ch,
		   OBJ_DATA * obj, const void *vo)
{
...
bool
oprog_wordlist_check (const char *arg, CHAR_DATA * mob, CHAR_DATA * actor,
		      OBJ_DATA * obj, const void *vo, int type,
		      OBJ_DATA * iobj)
{
  char temp1[MAX_STRING_LENGTH];
...
void
rprog_percent_check (CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj,
		     void *vo, int type)
{
  MPROG_DATA *mprg;
...
void
rprog_act_trigger (const char *buf, ROOM_INDEX_DATA * room, CHAR_DATA * ch,
		   OBJ_DATA * obj, const void *vo)
{
...
void
rprog_leave_trigger (CHAR_DATA * ch)
{
...
void
rprog_enter_trigger (CHAR_DATA * ch)
{
...
void
rprog_sleep_trigger (CHAR_DATA * ch)
{
...
void
rprog_rest_trigger (CHAR_DATA * ch)
{
...
void
rprog_rfight_trigger (CHAR_DATA * ch)
{
...
void
rprog_death_trigger (CHAR_DATA * ch)
{
...
void
rprog_speech_trigger (const char *txt, CHAR_DATA * ch)
{
  if (HAS_PROG (ch->in_room, SPEECH_PROG))
    {
       ...
       */
      rprog_wordlist_check (txt, supermob, ch, NULL, NULL, SPEECH_PROG,
			    ch->in_room);
    }
}
  ...
  if (HAS_PROG (ch->in_room, CMD_PROG))
    {
       ...
       * supermob is set and released in rprog_wordlist_check 
       */
      if (rprog_wordlist_check
	  (txt, supermob, ch, NULL, NULL, CMD_PROG, ch->in_room))
	return TRUE;
    }
  return FALSE;
}
...
void
rprog_random_trigger (CHAR_DATA * ch)
{
...
bool
rprog_wordlist_check (const char *arg, CHAR_DATA * mob, CHAR_DATA * actor,
		      OBJ_DATA * obj, const void *vo, int type,
		      ROOM_INDEX_DATA * room)
{
  char temp1[MAX_STRING_LENGTH];
...
void
rprog_time_check (CHAR_DATA * mob, CHAR_DATA * actor, OBJ_DATA * obj,
		  void *vo, int type)
{
...
void
rprog_time_trigger (CHAR_DATA * ch)
{
...
void
rprog_hour_trigger (CHAR_DATA * ch)
{
...
void
progbug (const char *str, CHAR_DATA * mob)
{
...
void
room_act_add (ROOM_INDEX_DATA * room)
{
