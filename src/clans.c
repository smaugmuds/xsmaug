...
void
add_roster (CLAN_DATA * clan, const char *name, int Class, int level,
...
void
remove_roster (CLAN_DATA * clan, const char *name)
{
  ROSTER_DATA *roster;
...
update_roster (CHAR_DATA * ch)
{
   ...
   */
  add_roster (ch->pcdata->clan, ch->name, ch->Class, ch->level,
...
void
remove_all_rosters (CLAN_DATA * clan)
{
...
void
do_roster (CHAR_DATA * ch, const char *argument)
{
  CLAN_DATA *clan;
  ROSTER_DATA *roster;
  ...
  if (!argument || argument[0] == '\0')
    {
      ...
      ch_printf (ch, "%-15.15s  %-15.15s %-6.6s %-6.6s %-6.6s %s\r\n", "Name",
		 "Class", "Level", "Kills", "Deaths", "Joined on");
      send_to_char
	("-------------------------------------------------------------------------------------\r\n",
	 ch);
      for (roster = clan->first_member; roster; roster = roster->next)
	{
	  ch_printf (ch, "%-15.15s  %-15.15s %-6d %-6d %-6d %s",
		     roster->name, capitalize (npc_class[roster->Class]),
		     roster->level, roster->kills, roster->deaths,
		     ctime (&roster->joined));
	  total++;
	}
...
void
fwrite_memberlist (FILE * fp, CLAN_DATA * clan)
{
  ROSTER_DATA *roster;
...
void
fread_memberlist (CLAN_DATA * clan, FILE * fp)
{
  ROSTER_DATA *roster;
  const char *word;
  bool fMatch;
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
	    {
	      LINK (roster, clan->first_member, clan->last_member, next,
		    prev);
	      return;
	    }
	  break;
...
void
free_one_council (COUNCIL_DATA * council)
{
  ...
  if ((fp = fopen (filename, "w")) == NULL)
  ...
  else
    {
      ...
      fprintf (fp, "PDeathRangeNew  %d %d %d %d %d %d %d\n", clan->pdeaths[0],
	       clan->pdeaths[1], clan->pdeaths[2], clan->pdeaths[3],
 ...
 */
void
save_council (COUNCIL_DATA * council)
{
  FILE *fp;
  char filename[256];
...
void
fread_clan (CLAN_DATA * clan, FILE * fp)
{
  const char *word;
  bool fMatch;
 ...
 */
bool
load_clan_file (const char *clanfile)
{
  char filename[256];
  CLAN_DATA *clan;
  FILE *fp;
...
bool
load_council_file (const char *councilfile)
{
  char filename[256];
  COUNCIL_DATA *council;
  FILE *fp;
...
void
do_make (CHAR_DATA * ch, const char *argument)
{
  ...
  if (str_cmp (ch->name, clan->leader)
      && str_cmp (ch->name, clan->deity) && (clan->clan_type != CLAN_GUILD
					     || str_cmp (ch->name,
							 clan->number1)))
    {
      send_to_char ("Huh?\r\n", ch);
...
void
do_induct (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  CLAN_DATA *clan;
  ...
  if (clan->clan_type == CLAN_GUILD)
	...
	{
	  send_to_char
	    ("This player's will is not in accordance with your guild.\r\n",
	     ch);
	  return;
	}
    }
  ...
  add_roster (clan, victim->name, victim->Class, victim->level,
...
void
do_council_induct (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  COUNCIL_DATA *council;
  ...
  act (AT_MAGIC, "$n inducts you into $t", ch, council->name, victim,
       TO_VICT);
  save_char_obj (victim);
...
bool
can_outcast (CLAN_DATA * clan, CHAR_DATA * ch, CHAR_DATA * victim)
{
  if (!clan || !ch || !victim)
    return FALSE;
...
void
do_outcast (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  CLAN_DATA *clan;
  ...
  if (clan->clan_type != CLAN_GUILD && clan->clan_type != CLAN_ORDER)
    {
      snprintf (buf, MAX_STRING_LENGTH, "%s has been outcast from %s!",
...
void
do_council_outcast (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  COUNCIL_DATA *council;
  ...
  act (AT_MAGIC, "You outcast $N from $t", ch, council->name, victim,
       TO_CHAR);
  act (AT_MAGIC, "$n outcasts $N from $t", ch, council->name, victim,
       TO_ROOM);
  act (AT_MAGIC, "$n outcasts you from $t", ch, council->name, victim,
       TO_VICT);
  save_char_obj (victim);
  save_council (council);
  return;
...
void
do_setclan (CHAR_DATA * ch, const char *argument)
{
...
void
do_setcouncil (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  COUNCIL_DATA *council;
  ...
  if (!str_cmp (arg2, "filename"))
	...
	return;
      ...
      snprintf (filename, sizeof (filename), "%s%s", COUNCIL_DIR,
		council->filename);
      if (!remove (filename))
...
void
do_showclan (CHAR_DATA * ch, const char *argument)
{
  CLAN_DATA *clan;
  ...
  ch_printf_color (ch,
		   "\r\n&w%s    : &W%s\t\t&wBadge: %s\r\n&wFilename : &W%s\r\n&wMotto    : &W%s\r\n",
		   clan->clan_type ==
		   CLAN_ORDER ? "Order" : (clan->clan_type ==
					   CLAN_GUILD ? "Guild" : "Clan "),
		   clan->name, clan->badge ? clan->badge : "(not set)",
		   clan->filename, clan->motto);
  ...
  ch_printf_color (ch,
		   "&wScore    : &W%-6d   &wFavor  : &W%-6d   &wStrikes: &W%d\r\n",
		   clan->score, clan->favour, clan->strikes);
  ch_printf_color (ch,
		   "&wMembers  : &W%-6d  &wMemLimit: &W%-6d   &wAlign  : &W%-6d",
		   clan->members, clan->mem_limit, clan->alignment);
  if (clan->clan_type == CLAN_GUILD)
    ch_printf_color (ch, "   &wClass  : &W%d &w(&W%s&w)",
		     clan->Class,
		     clan->Class <
		     MAX_PC_CLASS ? class_table[clan->
						Class]->who_name : "unknown");
  send_to_char ("\r\n", ch);
  ch_printf_color (ch,
		   "&wBoard    : &W%-5d    &wRecall : &W%-5d    &wStorage: &W%-5d\r\n",
  ...
  ch_printf_color (ch,
		   ...
		   clan->clanobj1, clan->clanobj2, clan->clanobj3,
		   clan->clanobj4, clan->clanobj5);
  return;
}

void
do_showcouncil (CHAR_DATA * ch, const char *argument)
{
  COUNCIL_DATA *council;
...
void
do_makeclan (CHAR_DATA * ch, const char *argument)
{
  CLAN_DATA *clan;
...
void
do_makecouncil (CHAR_DATA * ch, const char *argument)
{
  char filename[256];
  COUNCIL_DATA *council;
  bool found;
...
void
do_clans (CHAR_DATA * ch, const char *argument)
{
  CLAN_DATA *clan;
  int count = 0;
  ...
  if (argument[0] == '\0')
      ...
      for (clan = first_clan; clan; clan = clan->next)
	{
	  ...
	  ch_printf (ch, "%-13s %-13s %-13s", clan->name, clan->deity,
		     clan->leader);
	  set_char_color (AT_BLOOD, ch);
	  ch_printf (ch, "                %5d      %5d\r\n",
		     clan->pkills[6],
  ...
  if (!str_cmp (ch->name, clan->deity) || !str_cmp (ch->name, clan->leader)
      || !str_cmp (ch->name, clan->number1)
      || !str_cmp (ch->name, clan->number2))
...
void
do_orders (CHAR_DATA * ch, const char *argument)
{
  CLAN_DATA *order;
  int count = 0;
	...
	if (order->clan_type == CLAN_ORDER)
	  {
	    ch_printf (ch, "%-16s %-14s %-14s   %-7d       %5d\r\n",
		       order->name, order->deity, order->leader,
		       order->mkills, order->mdeaths);
	    count++;
	  }
      set_char_color (AT_DGREEN, ch);
      if (!count)
  ...
  if (!str_cmp (ch->name, order->deity) || !str_cmp (ch->name, order->leader)
...
void
do_councils (CHAR_DATA * ch, const char *argument)
{
  COUNCIL_DATA *council;
  ...
  if (argument[0] == '\0')
      ...
      for (council = first_council; council; council = council->next)
	{
      ...
      send_to_char_color
	("&cUse 'councils <name of council>' for more detailed information.\r\n",
	 ch);
      return;
    }
  council = get_council (argument);
...
void
do_guilds (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  CLAN_DATA *guild;
  int count = 0;
  ...
  if (!str_cmp (ch->name, guild->deity) || !str_cmp (ch->name, guild->leader)
...
void
do_victories (CHAR_DATA * ch, const char *argument)
{
  char filename[256];
...
void
do_shove (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  int exit_dir;
...
void
do_drag (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  int exit_dir;
