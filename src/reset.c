...
OBJ_DATA *
get_obj_room (OBJ_INDEX_DATA * pObjIndex, ROOM_INDEX_DATA * pRoomIndex)
{
  OBJ_DATA *obj;
...
char *
sprint_reset (RESET_DATA * pReset, short *num)
{
  RESET_DATA *tReset, *gReset;
  static char buf[MAX_STRING_LENGTH];
  char mobname[MAX_STRING_LENGTH], roomname[MAX_STRING_LENGTH],
    objname[MAX_STRING_LENGTH];
  ...
  switch (pReset->command)
      ...
      for (tReset = pReset->first_reset; tReset; tReset = tReset->next_reset)
	{
	  ...
	  switch (tReset->command)
		...
		strncpy (objname, obj->name, MAX_STRING_LENGTH);
	      snprintf (buf + strlen (buf), MAX_STRING_LENGTH - strlen (buf),
			"%2d) (carry) %s (%d) -> %s [%d]\r\n", *num, objname,
			tReset->arg1, mobname, tReset->arg2);
	      break;
	    }
	  if (tReset->first_reset)
		   ...
		   gReset = gReset->next_reset)
		{
		  ...
		  switch (gReset->command)
			...
			strncpy (objname, "Object1: *BAD VNUM*",
				 MAX_STRING_LENGTH);
		      else
			strncpy (objname, obj2->name, MAX_STRING_LENGTH);
		      if (gReset->arg3 > 0
			  && (obj = get_obj_index (gReset->arg3)) == NULL)
			strncpy (roomname, "Object2: *BAD VNUM*",
				 MAX_STRING_LENGTH);
		      else if (!obj)
			strncpy (roomname, "Object2: *NULL obj*",
				 MAX_STRING_LENGTH);
		      else
      ...
      for (tReset = pReset->first_reset; tReset; tReset = tReset->next_reset)
	{
	  ...
	  switch (tReset->command)
		...
		strncpy (objname, obj2->name, MAX_STRING_LENGTH);
	      if (tReset->arg3 > 0
		  && (obj = get_obj_index (tReset->arg3)) == NULL)
		...
		strncpy (roomname, obj->name, MAX_STRING_LENGTH);
	      snprintf (buf + strlen (buf), MAX_STRING_LENGTH - strlen (buf),
			"%2d) (put) %s (%d) -> %s (%d) [%d]\r\n", *num,
			objname, tReset->arg1, roomname,
			obj ? obj->vnum : tReset->arg3, tReset->arg2);
	      break;
	    ...
	    case 'T':
	      snprintf (buf + strlen (buf), MAX_STRING_LENGTH - strlen (buf),
			...
			tReset->arg3, flag_string (tReset->extra, trap_flags),
	...
	{
	  strncpy (roomname, "Room: *BAD VNUM*", MAX_STRING_LENGTH);
	  snprintf (objname, MAX_STRING_LENGTH, "%s (no exit)",
		    dir_name[pReset->arg2]);
	}
      else
	{
	  strncpy (roomname, room->name, MAX_STRING_LENGTH);
	  snprintf (objname, MAX_STRING_LENGTH, "%s%s",
		    dir_name[pReset->arg2], get_exit (room,
						      pReset->arg2) ? "" :
		    " (NO EXIT!)");
	}
      switch (pReset->arg3)
	...
	case 2:
      ...
      snprintf (buf, MAX_STRING_LENGTH,
		...
		pReset->arg3, objname, pReset->arg2, roomname, pReset->arg1);
      break;
	...
	strncpy (roomname, room->name, MAX_STRING_LENGTH);
      snprintf (buf, MAX_STRING_LENGTH,
		...
		flag_string (pReset->extra, trap_flags), roomname,
		room ? room->vnum : 0);
      break;
    }
  return buf;
 ...
 */
RESET_DATA *
make_reset (char letter, int extra, int arg1, int arg2, int arg3)
{
  RESET_DATA *pReset;
...
void
add_obj_reset (ROOM_INDEX_DATA * room, char cm, OBJ_DATA * obj, int v2,
	       int v3)
{
  OBJ_DATA *inobj;
  static int iNest;
  ...
  if ((cm == 'O' || cm == 'P') && obj->pIndexData->vnum == OBJ_VNUM_TRAP)
    {
      if (cm == 'O')
	add_reset (room, 'T', obj->value[3], obj->value[1], obj->value[0],
		   v3);
      return;
    }
  add_reset (room, cm, (cm == 'P' ? iNest : 0), obj->pIndexData->vnum, v2,
	     v3);
...
void
delete_reset (RESET_DATA * pReset)
{
  ...
  for (tReset = pReset->first_reset; tReset; tReset = tReset_next)
    {
      ...
      UNLINK (tReset, pReset->first_reset, pReset->last_reset, next_reset,
	      prev_reset);
      delete_reset (tReset);
    }
...
void
instaroom (ROOM_INDEX_DATA * pRoom, bool dodoors)
{
  CHAR_DATA *rch;
  OBJ_DATA *obj;
  ...
  for (rch = pRoom->first_person; rch; rch = rch->next_in_room)
	...
	continue;
      ...
      add_reset (pRoom, 'M', 1, rch->pIndexData->vnum, rch->pIndexData->count,
		 pRoom->vnum);
...
void
wipe_resets (ROOM_INDEX_DATA * room)
{
...
void
wipe_area_resets (AREA_DATA * area)
{
...
void
do_instaroom (CHAR_DATA * ch, const char *argument)
{
  bool dodoors;
...
/* Function modified from original form - Samson */
void
do_instazone (CHAR_DATA * ch, const char *argument)
{
  AREA_DATA *pArea;
 ...
 */
void
reset_room (ROOM_INDEX_DATA * room)
{
  ...
  for (pReset = room->first_reset; pReset; pReset = pReset->next)
	...
	{
	default:
	  bug ("%s: %s: bad command %c.", __FUNCTION__, filename,
	       pReset->command);
	  break;
	...
	case 'M':
	  if (!(pMobIndex = get_mob_index (pReset->arg1)))
	    {
	      bug ("%s: %s: 'M': bad mob vnum %d.", __FUNCTION__, filename,
		   pReset->arg1);
	      continue;
	    }
	  if (!(pRoomIndex = get_room_index (pReset->arg3)))
	    {
	      bug ("%s: %s: 'M': bad room vnum %d.", __FUNCTION__, filename,
		   pReset->arg3);
	      continue;
	    }
	  if (!pReset->sreset)
		  ...
		  switch (tReset->command)
		      ...
		      if (!(pObjIndex = get_obj_index (tReset->arg1)))
			{
			  bug ("%s: %s: 'E' or 'G': bad obj vnum %d.",
			       __FUNCTION__, filename, tReset->arg1);
			  continue;
			}
		      if (!mob)
			{
		      ...
		      if (tReset->command == 'E')
			{
			  if (obj->carried_by != mob)
			    {
			      bug
				("'E' reset: can't give object %d to mob %d.",
				 obj->pIndexData->vnum,
				 mob->pIndexData->vnum);
			      break;
			    }
				...
				case 'P':
				  ...
				  if (iNest >= MAX_NEST)
					...
					("%s: %s: 'P': Exceeded nesting limit of %d",
					 __FUNCTION__, filename, MAX_NEST);
				      obj = NULL;
				      break;
				    }
				  if (count_obj_list
				      (pObjIndex, to_obj->first_content) > 0)
				    {
				      obj = NULL;
				      break;
				    }
	...
	case 'O':
	  if (!(pObjIndex = get_obj_index (pReset->arg1)))
	    {
	      bug ("%s: %s: 'O': bad obj vnum %d.", __FUNCTION__, filename,
		   pReset->arg1);
	      continue;
	    }
	  if (!(pRoomIndex = get_room_index (pReset->arg3)))
	    {
	      bug ("%s: %s: 'O': bad room vnum %d.", __FUNCTION__, filename,
		   pReset->arg3);
	      continue;
	    }
	  ...
	  if (pReset->first_reset)
		   ...
		   tReset = tReset->next_reset)
		{
		  ...
		  switch (tReset->command)
			...
			{
			  bug ("%s: Room reset found on object reset list",
			       __FUNCTION__);
			  break;
			}
		      else
			{
			  ...
			  if (tReset->arg3 > 0)
			    {
			      if (!
				  (pObjToIndex =
				   get_obj_index (tReset->arg3)))
				{
				  bug ("%s: %s: 'T': bad objto vnum %d.",
				       __FUNCTION__, filename, tReset->arg3);
				  continue;
				}
			  ...
			  pobj =
			    make_trap (tReset->arg2, tReset->arg1,
		    ...
		    case 'P':
		      if (!(pObjIndex = get_obj_index (tReset->arg1)))
			{
			  bug ("%s: %s: 'P': bad obj vnum %d.", __FUNCTION__,
			       filename, tReset->arg1);
			  continue;
			}
		      iNest = tReset->extra;
		      ...
		      if (iNest >= MAX_NEST)
			{
			  bug
			    ("%s: %s: 'P': Exceeded nesting limit of %d. Room %d.",
			     __FUNCTION__, filename, MAX_NEST, room->vnum);
			  obj = NULL;
			  break;
			}

		      if (count_obj_list (pObjIndex, to_obj->first_content) >
			  0)
			{
			  obj = NULL;
			  break;
	...
	case 'T':
	  if (IS_SET (pReset->extra, TRAP_OBJ))
	    {
	      bug ("%s: Object trap found in room %d reset list",
	  ...
	  else
	    {
	      if (!(pRoomIndex = get_room_index (pReset->arg3)))
		{
		  bug ("%s: %s: 'T': bad room %d.", __FUNCTION__, filename,
		       pReset->arg3);
		  continue;
		}
	      if (room->area->nplayer > 0
		...
		break;
	      to_obj =
		make_trap (pReset->arg1, pReset->arg1, 10, pReset->extra);
	...
	case 'D':
	  if (!(pRoomIndex = get_room_index (pReset->arg1)))
	    {
	      bug ("%s: %s: 'D': bad room vnum %d.", __FUNCTION__, filename,
		   pReset->arg1);
	      continue;
	    }
	...
	case 'R':
	  if (!(pRoomIndex = get_room_index (pReset->arg1)))
	    {
	      bug ("%s: %s: 'R': bad room vnum %d.", __FUNCTION__, filename,
		   pReset->arg1);
	      continue;
	    }
...
void
renumber_put_resets (ROOM_INDEX_DATA * room)
{
 ...
 */
RESET_DATA *
add_reset (ROOM_INDEX_DATA * room, char letter, int extra, int arg1, int arg2,
	   int arg3)
{
  RESET_DATA *pReset;
  ...
  switch (letter)
	...
	{
	  bug ("%s: Can't add '%c' reset to room: last_mob_reset is NULL.",
	       __FUNCTION__, letter);
	  return NULL;
	}
      room->last_obj_reset = pReset;
      LINK (pReset, room->last_mob_reset->first_reset,
	    room->last_mob_reset->last_reset, next_reset, prev_reset);
      return pReset;

    case 'P':
      if (!room->last_obj_reset)
	{
	  bug ("%s: Can't add '%c' reset to room: last_obj_reset is NULL.",
	       __FUNCTION__, letter);
	  return NULL;
	}
      LINK (pReset, room->last_obj_reset->first_reset,
	    room->last_obj_reset->last_reset, next_reset, prev_reset);
      return pReset;
...
RESET_DATA *
find_oreset (ROOM_INDEX_DATA * room, const char *oname)
{
  RESET_DATA *pReset;
  OBJ_INDEX_DATA *pobj;
...
void
do_reset (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg, "list"))
    {
      ...
      for (room = ch->in_room->area->first_room; room;
	   room = room->next_aroom)
	{
	  num = 0;
	  if (!room->first_reset)
	    continue;
  ...
  if (!str_cmp (arg, "delete"))
    {
      RESET_DATA *pReset, *tReset, *pReset_next, *tReset_next, *gReset,
	*gReset_next;
      int num, nfind = 0;
	...
	{
	  send_to_char
	    ("You must specify a reset # in this room to delete one.\r\n",
	     ch);
	  return;
	}
	...
	{
	  send_to_char
	    ("Specified reset must be designated by number. See &Wredit rlist&D.\r\n",
	     ch);
	  return;
	}
      num = atoi (argument);
      ...
      for (pReset = ch->in_room->first_reset; pReset; pReset = pReset_next)
	{
	  ...
	  if (nfind == num)
	    {
	      UNLINK (pReset, ch->in_room->first_reset,
		      ch->in_room->last_reset, next, prev);
	      delete_reset (pReset);
	  ...
	  for (tReset = pReset->first_reset; tReset; tReset = tReset_next)
	    {
	      ...
	      if (nfind == num)
		{
		  UNLINK (tReset, pReset->first_reset, pReset->last_reset,
			  next_reset, prev_reset);
		  delete_reset (tReset);
		  send_to_char ("Reset deleted.\r\n", ch);
		  return;
		}
	      ...
	      for (gReset = tReset->first_reset; gReset; gReset = gReset_next)
		{
		  ...
		  if (nfind == num)
		    {
		      UNLINK (gReset, tReset->first_reset, tReset->last_reset,
			      next_reset, prev_reset);
		      delete_reset (gReset);
		      send_to_char ("Reset deleted.\r\n", ch);
		      return;
      ...
      send_to_char
	("No reset matching that number was found in this room.\r\n", ch);
      return;
    }
...
void
update_room_reset (CHAR_DATA * ch, bool setting)
{
  ROOM_INDEX_DATA *room;
  RESET_DATA *pReset, *tReset, *pReset_next, *tReset_next, *gReset,
    *gReset_next;
