 ...
 */
void
save_mobile (FILE * fp, CHAR_DATA * mob)
{
  AFFECT_DATA *paf;
	   ...
	   pRoomIndex = pRoomIndex->next)
	{
	  if (pRoomIndex)
	      ...
	      if ((objfp = fopen (filename, "w")) == NULL)
		{
	      ...
	      fwrite_obj (NULL, pRoomIndex->last_content, objfp, 0, OS_CARRY,
			  TRUE);
	      fprintf (objfp, "%s", "#END\n");
	      fclose (objfp);
	      objfp = NULL;
...
void
read_obj_file (char *dirname, char *filename)
{
...
void
do_hotboot (CHAR_DATA * ch, const char *argument)
{
  FILE *fp;
  ...
  for (d = first_descriptor; d; d = d->next)
    {
	  ...
	  && victim->in_room && victim->fighting && victim->level >= 1
  ...
  if (found)
    {
      send_to_char
	("Cannot hotboot at this time. Someone is using the line editor.\r\n",
	 ch);
      return;
    }
  ...
  for (d = first_descriptor; d; d = de_next)
      ...
      else
	{
	  fprintf (fp, "%d %d %d %d %d %s %s\n", d->descriptor,
		   d->can_compress, och->in_room->vnum, d->port, d->idle,
		   och->name, d->host);
	  /*
...
hotboot_recover (void)
{
  ...
  for (;;)
      ...
      else			/* ok! */
	{
	  ...
	  act (AT_MAGIC, "A puff of ethereal smoke dissipates around you!",
	       d->character, NULL, NULL, TO_CHAR);
	  act (AT_MAGIC, "$n appears in a puff of ethereal smoke!",
