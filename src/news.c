...
void
do_editnews (CHAR_DATA * ch, const char *argument)
{
  ...
  if (!str_cmp (arg, "edittype"))
	...
	{
	  send_to_char
	    ("Syntax: editnews edittype <type> <field> <value>\r\n", ch);
  ...
  if (!str_cmp (arg, "addnews"))
	...
	{
	  send_to_char
	    ("Invaild newstype. Use 'newstypes' to get a valid listing.\r\n",
	     ch);
	  return;
	}
  ...
  if (!str_cmp (arg, "editnews"))
	...
	{
	  send_to_char
	    ("Syntax: editnews editnews <type> <number> <new subject [optional]>\r\n",
	     ch);
	  return;
	}
	...
	{
	  send_to_char
	    ("Invalid newstype. Use 'newstypes' to get a valid listing.\r\n",
	     ch);
	  return;
	}
...
NEWS *
grab_news (NEWS_TYPE * type, const char *str)
{
  NEWS *news = NULL;
...
void
display_news (CHAR_DATA * ch, NEWS * news, NEWS_TYPE * type)
{
  pager_printf_color (ch, "\r\n&g--------------------------------------\r\n");
...
void
fread_news_type (NEWS_TYPE * type, FILE * fp)
{
  const char *word;
  bool fMatch;
...
snarf_news (FILE * fp)
{
  ...
  for (news = last_news; x < sysdata.max_html_news; news = news->prev)
    {
      ...
      sprintf (buf,
	       "\n%s\r\n</font></b>]</font></font></td></tr><tr><td height='2' bgcolor='#000000'>",
	       news->date);
      fprintf (fp, buf);
      sprintf (buf,
  ...
  fprintf (fp, "<pre><center>Page last written: %s</center></pre>",
	   ctime (&current_time));
  return;
}
#endif
...
bool
news_cmd_hook (CHAR_DATA * ch, char *cmd, char *argument)
{
  int x = 0;
 ...
 */
void
display_news_type (CHAR_DATA * ch, NEWS_TYPE * type, char *argument)
{
  if (!type->first_news)
    {
      send_to_char_color
	("&gThere are currently no news items for this news type.\r\n", ch);
      return;
    }
