...
bool
load_deity_file (const char *deityfile)
{
  char filename[256];
  DEITY_DATA *deity;
  FILE *fp;
...
void
do_setdeity (CHAR_DATA * ch, const char *argument)
{
      ...
      for (vch = first_char; vch; vch = vch->next)
	{
	  ...
	  if (!IS_NPC (vch))
	      ...
	      if (vch->pcdata->deity == deity)
		{
		  ...
		  xREMOVE_BITS (vch->affected_by,
				vch->pcdata->deity->affected);
...
void
do_showdeity (CHAR_DATA * ch, const char *argument)
{
  DEITY_DATA *deity;
  ...
  ch_printf (ch, "Alignment: %-7dNpcrace: %-9sNpcfoe: %s\r\n",
	     deity->alignment, (deity->npcrace < 0
				|| deity->npcrace >
				MAX_NPC_RACE) ? "none" :
	     npc_race[deity->npcrace], (deity->npcfoe < 0
					|| deity->npcfoe >
					MAX_NPC_RACE) ? "none" :
	     npc_race[deity->npcfoe]);
  ch_printf (ch, "Race: %-12sClass: %-11sSex: %-13sRace2: %s\r\n",
	     (deity->race < 0
	      || deity->race >
	      MAX_PC_RACE) ? "none" : race_table[deity->race]->race_name,
	     (deity->Class < 0
	      || deity->Class >
	      MAX_PC_CLASS) ? "none" : class_table[deity->Class]->who_name,
	     deity->sex == -1 ? "none" : deity->sex ==
	     SEX_MALE ? "male" : deity->sex ==
	     SEX_FEMALE ? "female" : "neutral", (deity->race2 < 0
						 || deity->race2 >
						 MAX_PC_RACE) ? "none" :
	     npc_race[deity->race2]);
  ...
  ch_printf (ch, "\r\nFlee: %-12dFlee_npcrace: %-4dKill_npcrace: "
	     "%-4dKill: %d\r\n", deity->flee, deity->flee_npcrace,
	     deity->kill_npcrace, deity->kill);
  ch_printf (ch,
	     "Kill_magic: %-6dSac: %-13dBury_corpse: %-5dAid_spell: "
  ...
  ch_printf (ch,
	     "Die_npcrace: %-5dDig_corpse: %-6dSpell_aid: %-7dKill_npcfoe: %d\r\n",
	     deity->die_npcrace, deity->dig_corpse, deity->spell_aid,
	     deity->kill_npcfoe);
  ch_printf (ch, "Die_npcfoe: %-6dFlee_npcfoe: %d\r\n", deity->die_npcfoe,
	     deity->flee_npcfoe);
  ch_printf (ch,
	     "\r\nScorpse: %-9dSavatar: %-9dSdeityobj: %-7d"
	     "Srecall: %d\r\n", deity->scorpse, deity->savatar,
	     deity->sdeityobj, deity->srecall);
  return;
}

void
do_makedeity (CHAR_DATA * ch, const char *argument)
{
  DEITY_DATA *deity;
  ...
  if ((deity = get_deity (argument)))
    {
      send_to_char
	("A deity with that name already holds weight on this world.\r\n",
	 ch);
      return;
    }
...
void
do_devote (CHAR_DATA * ch, const char *argument)
{
  ...
  if (!str_cmp (arg, "none"))
	...
	ch->pcdata->deity->worshippers = 0;
      ...
      ch->mental_state = -80;
      send_to_char
	("A terrible curse afflicts you as you forsake a deity!\r\n", ch);
  ...
  act (AT_MAGIC, "Body and soul, you devote yourself to $t!", ch,
       ch->pcdata->deity_name, NULL, TO_CHAR);
  ++ch->pcdata->deity->worshippers;
  save_deity (ch->pcdata->deity);
  save_char_obj (ch);
  return;
...
void
do_deities (CHAR_DATA * ch, const char *argument)
{
  DEITY_DATA *deity;
  int count = 0;
  ...
  if (argument[0] == '\0')
    {
      send_to_pager_color
	("&gFor detailed information on a deity, try 'deities <deity>' or 'help deities'\r\n",
	 ch);
...
void
do_supplicate (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg, "corpse") && !IS_PKILL (ch))
      ...
      for (obj = first_object; obj; obj = obj->next)
	{
	  if (obj->in_room && !str_cmp (buf2, obj->short_descr)
		...
		{
		  act (AT_MAGIC,
		       "The image of your corpse appears, but suddenly wavers away.",
		       ch, NULL, NULL, TO_CHAR);
		  return;
		}
	      act (AT_MAGIC,
		   "Your corpse appears suddenly, surrounded by a divine presence...",
		   ch, NULL, NULL, TO_CHAR);
	      act (AT_MAGIC,
		   "$n's corpse appears suddenly, surrounded by a divine force...",
  ...
  if (!str_cmp (arg, "avatar"))
      ...
      if (ch->pcdata->favor < ch->pcdata->deity->savatar)
	{
      ...
      snprintf (buf, MAX_STRING_LENGTH, victim->short_descr,
		ch->pcdata->deity->name);
      STRFREE (victim->short_descr);
      victim->short_descr = STRALLOC (buf);

      snprintf (buf, MAX_STRING_LENGTH, victim->long_descr,
		ch->pcdata->deity->name);
      STRFREE (victim->long_descr);
      victim->long_descr = STRALLOC (buf);

      snprintf (buf, MAX_STRING_LENGTH, victim->description,
		ch->pcdata->deity->name);
      STRFREE (victim->description);
      victim->description = STRALLOC (buf);

      act (AT_MAGIC, "$n summons a powerful avatar!", ch, NULL, NULL,
	   TO_ROOM);
      act (AT_MAGIC, "You summon a powerful avatar!", ch, NULL, NULL,
	   TO_CHAR);
  ...
  if (!str_cmp (arg, "object"))
	...
	obj = obj_to_room (obj, ch->in_room);
      ...
      snprintf (buf, MAX_STRING_LENGTH, obj->short_descr,
		ch->pcdata->deity->name);
      STRFREE (obj->short_descr);
      obj->short_descr = STRALLOC (buf);

      snprintf (buf, MAX_STRING_LENGTH, obj->description,
		ch->pcdata->deity->name);
      STRFREE (obj->description);
      obj->description = STRALLOC (buf);

      act (AT_MAGIC, "$n weaves $p from divine matter!", ch, obj, NULL,
	   TO_ROOM);
      act (AT_MAGIC, "You weave $p from divine matter!", ch, obj, NULL,
	   TO_CHAR);
...
*/
void
adjust_favor (CHAR_DATA * ch, int field, int mod)
{
  int oldfavor;
  ...
  switch (field)
    ...
    case 1:
      ch->pcdata->favor +=
	number_fuzzy (ch->pcdata->deity->flee_npcrace / mod);
      break;
    case 2:
    ...
    case 3:
      ch->pcdata->favor +=
	number_fuzzy (ch->pcdata->deity->kill_npcrace / mod);
      break;
    case 4:
    ...
    case 6:
      ch->pcdata->favor +=
	number_fuzzy (ch->pcdata->deity->bury_corpse / mod);
      break;
    case 7:
    ...
    case 12:
      ch->pcdata->favor +=
	number_fuzzy (ch->pcdata->deity->die_npcrace / mod);
      break;
    case 13:
    ...
    case 16:
      ch->pcdata->favor +=
	number_fuzzy (ch->pcdata->deity->flee_npcfoe / mod);
      break;
    case 17:
      ch->pcdata->favor +=
	number_fuzzy (ch->pcdata->deity->kill_npcfoe / mod);
      break;
    }
  ch->pcdata->favor = URANGE (-2500, ch->pcdata->favor, 2500);
