...
int
get_hemisphere (char *type)
{
  unsigned int x;

  for (x = 0; x < (sizeof (hemisphere_name) / sizeof (hemisphere_name[0]));
       x++)
...
bool
ExceedsThreshold (int initial, int delta, int threshold)
{
...
bool
DropsBelowThreshold (int initial, int delta, int threshold)
{
  return ((initial >= threshold) && (initial + delta < threshold));
}
...
void
WeatherMessage (const char *txt, int x, int y)
{
  AREA_DATA *pArea;
...
ApplyDeltaChanges (void)
{
  ...
  for (y = 0; y < WEATHER_SIZE_Y; y++)
      ...
      for (x = 0; x < WEATHER_SIZE_X; x++)
	{
	  ...
	  if (isTorrentialDownpour (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		...
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe blizzard turns to a cold rain as it continues to come in a torrential downpour.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 91))
		...
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 91)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		...
		if (ExceedsThreshold
		    ...
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe heavy snow increases and freezes creating a blizzard.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 91)
		    && ExceedsThreshold (cell->temperature,
					 delta->temperature, 32))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		...
		WeatherMessage
		  ("&WThe snow falls down fast and steady creating a blizzard.&D\r\n",
		   x, y);
	  ...
	  else if (isRainingCatsAndDogs (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&WThe heavy rain turns into a heavy snow.&D\r\n", x, y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe heavy snow turns into a heavy rain.&D\r\n", x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 90))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		  ...
		  WeatherMessage
		    ("&BThe heavy, fierce rain eases a bit, but still continues to drum down.&D\r\n",
		     x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 90)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WAs the heavy, fierce rain lessens a bit, it changes over to a steady snow.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 90)
		...
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 80))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		...
		if (ExceedsThreshold
		    ...
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe rain changes over to snow and begins to come down even harder.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 80)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe snows falls harder creating a wall of white.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 80)
		...
		WeatherMessage
		  ("&WThe snow comes down heavy creating a wall of white.&D\r\n",
		   x, y);
	      else if (isStormy (getEnergy (cell)))
		WeatherMessage
	  ...
	  else if (isPouring (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&WThe intense rain changes over to a heavy snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe heavy snow changes over to a pouring rain.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 80))
		if (isStormy (getEnergy (cell)))
		  ...
		  WeatherMessage
		    ("&BThe heavy rain lessens but still continues to pour down.&D\r\n",
		     x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 80)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe rain lessens and turns to a heavy snow.&D\r\n", x,
		   y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 80)
		...
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		    ("&BThe wall of snow lessens as it turns to a pouring rain.&D\r\n",
		     x, y);
		else
		  WeatherMessage
		    ("&BThe wall of snow lessens as it turns to a pouring rain.&D\r\n",
		...
		if (DropsBelowThreshold
		    ...
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe intense snowfall lessens and continues coming down heavily.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 70))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		  ...
		  WeatherMessage
		    ("&BThe steady rain begins to come down harder.&D\r\n", x,
		     y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 70)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe heavy rain increases in intensity and changes over to snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 70)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&The snow begins to fall more heavily and coat the ground quickly.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 70)
	  ...
	  else if (isRaingingHeavily (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&WThe heavy rain changes over to a heavy snow.&D\r\n", x,
		   y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe steady snow changes over to a heavy rain.&D\r\n", x,
		   y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 70))
		...
		if (DropsBelowThreshold
		    ...
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe pouring rain lessens but changes over to a steady snow.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 70)
		...
		if (isStormy (getEnergy (cell)))
		  ...
		  WeatherMessage
		    ("&BThe heavy snow lessens a bit and changes over to a steady rain.&D\r\n",
		     x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 70)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe heavy snow lessens a bit to a steady snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 60))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		  ...
		  WeatherMessage
		    ("&BThe pouring rain begins to come down harder pounding the ground.&D\r\n",
		     x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 60)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe pouring rain begins to increase in intensity and change over to a heavy snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 60)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe heavy snow increases in intensity creating a blanket of white on the ground.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 60)
		...
		WeatherMessage
		  ("&WSnow falls heavily down on the ground.&D\r\n", x, y);
	      else if (isStormy (getEnergy (cell)))
		WeatherMessage
	  ...
	  else if (isDownpour (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&WThe pouring rain changes over to a thick snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe steady snow changes over to rain and lessens a bit.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 60))
		...
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 50))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		...
		if (ExceedsThreshold
		    ...
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe steady rain picks up and changes over to a heavy, steady snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 50)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe steady snow picks up a bit creating a blanket of white on the ground.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 50)
	  ...
	  else if (isRainingSteadily (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&WThe steady rain changes over to a steady snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe steady snow changes over to a steady rain.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 50))
		...
		if (DropsBelowThreshold
		    ...
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe pouring rain lessens a bit and changes over to a steady snow.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 50)
		...
		if (DropsBelowThreshold
		    ...
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe blanket of snow eases up a bit to a steady snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 40))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		...
		if (ExceedsThreshold
		    ...
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe rain picks up and changes over to a steady snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 40)
		...
		if (isStormy (getEnergy (cell)))
		  ...
		  WeatherMessage
		    ("&BThe snow picks up a bit as it changes over to a steady rain.&D\r\n",
		     x, y);
	      else if (cell->temperature <= 32)
		...
		if (DropsBelowThreshold
		    ...
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe steady rain eases a bit as it changes over to snow.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 40)
		...
		if (isStormy (getEnergy (cell)))
		  ...
		  WeatherMessage
		    ("&BThe steady snow eases up a bit as it changes over to rain.&D\r\n",
		     x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 40)
		...
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 30))
		if (isStormy (getEnergy (cell)))
		  WeatherMessage
		    ("&BThe light rain picks up a bit.&D\r\n&YLightning illuminates the sky.&D\r\n",
		     x, y);
		else
		...
		if (ExceedsThreshold
		    ...
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe light rain picks up a bit as it changes over to snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 30)
		...
		WeatherMessage
		  ("&WSnow continues to fall from the sky.&D\r\n", x, y);
	      else if (isStormy (getEnergy (cell)))
		WeatherMessage
	  ...
	  else if (isRainingLightly (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&WThe light rain changes over to a light snow.&D\r\n", x,
		   y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe light snow changes over to a light rain.&D\r\n", x,
		   y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 30))
		WeatherMessage
		  ("&BThe rain eases a bit creating a light rain falling.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 30)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe rain eases a bit and changes over to a light snow.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 30)
		    && ExceedsThreshold (cell->temperature,
					 delta->temperature, 32))
		WeatherMessage
		  ("&BThe snow eases a bit as it changes over to a light rain.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 30)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("The snow eases up a bit to a light snow.&D\r\n", x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 20))
		WeatherMessage
		  ("&BThe drizzle picks up a bit to a light rain.&D\r\n", x,
		   y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 20)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe drizzle picks up a bit and changes over to a light snow.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 20)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe flurries pick up a bit to a light snow.&D\r\n", x,
		   y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 20)
		    && ExceedsThreshold (cell->temperature,
					 delta->temperature, 32))
		WeatherMessage
		  ("&BThe flurries pick up a bit and change over to a light rain.&D\r\n",
		   x, y);
	      else if (cell->temperature <= 32)
		WeatherMessage
		  ("&WThe light snow continues to fall gently on the ground.&D\r\n",
		   x, y);
	      else
		WeatherMessage
		  ("&BThe light rain continues to fall gently on the ground.&D\r\n",
		   x, y);
	    }
	  else if (isDrizzling (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("WThe drizzle changes over to flurries.&D\r\n", x, y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe flurries change over to a drizzling rain.&D\r\n", x,
		   y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 20))
		WeatherMessage
		  ("&BThe light rain eases up a bit to just a drizzle.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 20)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe light rain eases up a bit and changes over to a few flurries.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 20)
		    && ExceedsThreshold (cell->temperature,
					 delta->temperature, 32))
		WeatherMessage
		  ("&BThe light snow eases up a bit and changes over to just a drizzle.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 20)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe light snow lessens to just a few flurries.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 10))
		WeatherMessage
		  ("&BThe mist picks up a bit to a drizzling rain.&D\r\n", x,
		   y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 10)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe mist picks up a bit and changes over to some flurries.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 10)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe scattered flakes pick up a bit to some flurries.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->precipitation, delta->precipitation, 10)
		    && ExceedsThreshold (cell->temperature,
					 delta->temperature, 32))
		WeatherMessage
		  ("&BThe few scattered flakes pick up and change over to a drizzling rain.&D\r\n",
		   x, y);
	      else if (cell->temperature <= 32)
		WeatherMessage
		  ("&WFlurries of snow fall to the ground.&D\r\n", x, y);
	      else
		WeatherMessage
		  ("&BA light drizzling rain falls to the ground.&D\r\n", x,
		   y);
	    }
	  else if (isMisting (getPrecip (cell)))
	    {
		  ...
		  (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&WThe mist of rain changes over to a few scattered snowflakes.&D\r\n",
		   x, y);
	      else
		if (ExceedsThreshold
		    (cell->temperature, delta->temperature, 32))
		WeatherMessage
		  ("&BThe few scattered snowflakes change over to a misting rain.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 10))
		WeatherMessage
		  ("&BThe drizzle lessens down to a light mist.&D\r\n", x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 10)
		    && DropsBelowThreshold (cell->temperature,
					    delta->temperature, 32))
		WeatherMessage
		  ("&WThe drizzle lessens and changes over to just a few scattered snowflakes.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 10)
		    && cell->temperature <= 32)
		WeatherMessage
		  ("&WThe flurries of snow lessen to just a few scattered snowflakes.&D\r\n",
		   x, y);
	      else
		if (DropsBelowThreshold
		    (cell->precipitation, delta->precipitation, 10)
		    && ExceedsThreshold (cell->temperature,
					 delta->temperature, 32))
		WeatherMessage
		  ("&BThe flurries of snow lessen and change over to a light mist.&D\r\n",
		   x, y);
	      else if (cell->temperature <= 32)
		WeatherMessage
		  ("&WA few scattered snowflakes fall to the ground.&D\r\n",
		   x, y);
	      else
		...
		{
		  if (ExceedsThreshold
		      (cell->cloudcover, delta->cloudcover, 80))
		    WeatherMessage
		      ("&wMore clouds roll in creating a blanket over the sky.&D\r\n",
		       x, y);
		  else
		    WeatherMessage
		      ("&wClouds cover the sky like a blanket.&D\r\n", x, y);
		}
	      else if (isModeratelyCloudy (getCloudCover (cell)))
		{
		  if (ExceedsThreshold
		      (cell->cloudcover, delta->cloudcover, 60))
		    WeatherMessage
		      ("&wThe sky begins to get more cloudy.&D\r\n", x, y);
		  else
		    if (DropsBelowThreshold
			(cell->cloudcover, delta->cloudcover, 80))
		    WeatherMessage
		      ("&wSome of the clouds begin to break.&D\r\n", x, y);
		  else
		...
		{
		  if (ExceedsThreshold
		      (cell->cloudcover, delta->cloudcover, 40))
		    WeatherMessage
		      ("&wMore clouds roll in making it partly cloudy.&D\r\n",
		       x, y);
		  else
		    if (DropsBelowThreshold
			(cell->cloudcover, delta->cloudcover, 60))
		    WeatherMessage
		      ("&wSome of the clouds move out clearing part of the sky.&D\r\n",
		       x, y);
		  else
		...
		{
		  if (ExceedsThreshold
		      (cell->cloudcover, delta->cloudcover, 20))
		    WeatherMessage
		      ("&wA few clouds begin to roll into the sky.&D\r\n", x,
		       y);
		  else
		    if (DropsBelowThreshold
			(cell->cloudcover, delta->cloudcover, 40))
		    WeatherMessage
		      ("&wA few of the clouds begin to move out leaving only a few clouds left behind.&D\r\n",
		       x, y);
		  else
	      ...
	      else
		{
		  if (DropsBelowThreshold
		      (cell->cloudcover, delta->cloudcover, 20))
		    WeatherMessage
		      ("&wThe few remaining clouds begin to roll out.&D\r\n",
		       x, y);
		  else if (isSwelteringHeat (getTemp (cell)))
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 90))
			WeatherMessage
			  ("&OIt begins to warm up making the already intense heat almost unbearable.&D\r\n",
			   x, y);
		      else
			WeatherMessage
			  ("&OThe heat is almost unbearable.&D\r\n", x, y);
		    }
		  else if (isVeryHot (getTemp (cell)))
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 80))
			WeatherMessage
			  ("&OAs the temperature increases, the heat begins to become intense.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 90))
			WeatherMessage
			  ("&OThe unbearable heat eases a bit.&D\r\n", x, y);
		      else
		    ...
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 70))
			WeatherMessage
			  ("&OThe temperature rises making it quite hot.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 80))
			WeatherMessage
			  ("&OThe temperature lessens slightly, making it a little more bearable.&D\r\n",
			   x, y);
		      else
		    ...
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 60))
			WeatherMessage
			  ("&OThe nice temperature heats up a little making it warm.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 70))
			WeatherMessage
			  ("&OThe heat becomes a little less intense making it warm.&D\r\n",
			   x, y);
		      else
		    ...
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 50))
			WeatherMessage
			  ("&OIt warms a little bit taking away the chill and making it nice and pleasant.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 60))
			WeatherMessage
			  ("&OThe heat eases and makes it nice and pleasant.&D\r\n",
			   x, y);
		      else
			WeatherMessage
			  ("&OThe temperature is nice and pleasant.&D\r\n", x,
			   y);
		    }
		  else if (isCool (getTemp (cell)))
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 40))
			WeatherMessage
			  ("&CThe chilly air warms up a bit leaving it cool.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 50))
			WeatherMessage
			  ("&CThe temperature drops leaving it a little cool.&D\r\n",
			   x, y);
		      else
		    ...
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 30))
			WeatherMessage
			  ("&CThe temperature rises a little bit but there is still a chill in the air.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 40))
			WeatherMessage
			  ("&CThe temperature drops as the air takes on a chilly feel.&D\r\n",
			   x, y);
		      else
		    ...
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 20))
			WeatherMessage
			  ("&CThe frigid temperature warms up a tad making it chilly.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 30))
			WeatherMessage
			  ("&CThe temperature drops making it cold.&D\r\n", x,
			   y);
		      else
		    ...
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 10))
			WeatherMessage
			  ("&CThe freezing temperature warms up a bit leaving it frigid.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 20))
			WeatherMessage
			  ("&The cold temperature drops making it frigid.&D\r\n",
			   x, y);
		      else
			WeatherMessage
			  ("&CThe temperate is very frigid.&D\r\n", x, y);
		    }
		  else if (isFreezing (getTemp (cell)))
		    {
		      if (ExceedsThreshold
			  (cell->temperature, delta->temperature, 0))
			WeatherMessage
			  ("&CThe freezing cold begins to warm up slightly.&D\r\n",
			   x, y);
		      else
			if (DropsBelowThreshold
			    (cell->temperature, delta->temperature, 10))
			WeatherMessage
			  ("&CThe frigid temperature drops making it freezing.&D\r\n",
			   x, y);
		      else
		  ...
		  else if (isReallyCold (getTemp (cell)))
		    {
			  ...
			  (cell->temperature, delta->temperature, 0))
			WeatherMessage
			  ("&CThe temperature drops making the freezing cold worse.&D\r\n",
			   x, y);
		      else
			if (ExceedsThreshold
			    (cell->temperature, delta->temperature, -10))
			WeatherMessage
			  ("&CThe temperature warms up the very cold air a little.&D\r\n",
			   x, y);
		      else
		  ...
		  else if (isVeryCold (getTemp (cell)))
		    {
			  ...
			  (cell->temperature, delta->temperature, -10))
			WeatherMessage
			  ("&CThe temperature drops making it all the more cold.&D\r\n",
			   x, y);
		      else
			if (ExceedsThreshold
			    (cell->temperature, delta->temperature, -20))
			WeatherMessage
			  ("&CThe temperature rises making the unbearable cold a little better.&D\r\n",
			   x, y);
		      else
		  ...
		  else if (isExtremelyCold (getTemp (cell)))
		    {
			  ...
			  (cell->temperature, delta->temperature, -20))
			WeatherMessage
			  ("&CThe already very cold temperature drops making it unbearable.&D\r\n",
			   x, y);
		      else
	  ...
	  cell->temperature =
	    URANGE (-30, cell->temperature + delta->temperature, 100);
	  cell->pressure = URANGE (0, cell->pressure + delta->pressure, 100);
	  cell->cloudcover =
	    URANGE (0, cell->cloudcover + delta->cloudcover, 100);
	  cell->energy = URANGE (0, cell->energy + delta->energy, 100);
	  cell->humidity = URANGE (0, cell->humidity + delta->humidity, 100);
	  cell->precipitation =
	    URANGE (0, cell->precipitation + delta->precipitation, 100);
	  cell->windSpeedX =
	    URANGE (-100, cell->windSpeedX + delta->windSpeedX, 100);
	  cell->windSpeedY =
	    URANGE (-100, cell->windSpeedY + delta->windSpeedY, 100);
	}
    }
}

void
ClearWeatherDeltas (void)
{				//  Clear delta map
  memset (weatherDelta, 0, sizeof (weatherDelta));
}

void
CalculateCellToCellChanges (void)
{
  int x, y;
  int rand;
  ...
  for (y = 0; y < WEATHER_SIZE_Y; y++)
      ...
      for (x = 0; x < WEATHER_SIZE_X; x++)
	{
	   ...
	   */
	  if ((time_info.sunlight == SUN_RISE)
	      || (time_info.sunlight == SUN_LIGHT))
	    delta->temperature +=
	      (number_range (-1, 2) +
	       (((getCloudCover (cell) / 10) > 5) ? -1 : 1));
	  if ((time_info.sunlight == SUN_SET)
	      || (time_info.sunlight == SUN_DARK))
	    delta->temperature +=
	      (number_range (-2, 0) +
	       (((getCloudCover (cell) / 10) < 5) ? 2 : -3));
	  ...
	  else if ((humidityAndPressure / 2) < 60
		   && (humidityAndPressure / 2) > 40)
	  ...
	  else if ((humidityAndPrecip / 2) < 60
		   && (humidityAndPrecip / 2) > 40)
	  ...
	  delta->pressure =
	    ((totalPressure / numPressureCells) - cell->pressure);
      ...
      for (x = 0; x < WEATHER_SIZE_X; x++)
	{
	  ...
	  if (cell->climate == CLIMATE_RAINFOREST)
	  ...
	  else if (cell->climate == CLIMATE_SAVANNA)
	    {
		  ...
		  && cell->hemisphere == HEMISPHERE_NORTH
		  && cell->humidity > 0)
		delta->humidity += -5;
		...
		delta->humidity += 5;
		       ...
		       && cell->hemisphere == HEMISPHERE_SOUTH
		       && cell->humidity > 0)
		delta->humidity += -5;
...
void
fread_cell (FILE * fp, int x, int y)
{
  bool fMatch = FALSE;
...
*/
struct WeatherCell *
getWeatherCell (AREA_DATA * pArea)
{
  return &weatherMap[pArea->weatherx][pArea->weathery];
}

void
IncreaseTemp (struct WeatherCell *cell, int change)
{
  cell->temperature += change;
}

void
DecreaseTemp (struct WeatherCell *cell, int change)
{
  cell->temperature -= change;
}

void
IncreasePrecip (struct WeatherCell *cell, int change)
{
  cell->precipitation += change;
}

void
DecreasePrecip (struct WeatherCell *cell, int change)
{
  cell->precipitation -= change;
}

void
IncreasePressure (struct WeatherCell *cell, int change)
{
  cell->pressure += change;
}

void
DecreasePressure (struct WeatherCell *cell, int change)
{
  cell->pressure -= change;
}

void
IncreaseEnergy (struct WeatherCell *cell, int change)
{
  cell->energy += change;
}

void
DecreaseEnergy (struct WeatherCell *cell, int change)
{
  cell->energy -= change;
}

void
IncreaseCloudCover (struct WeatherCell *cell, int change)
{
  cell->cloudcover += change;
}

void
DecreaseCloudCover (struct WeatherCell *cell, int change)
{
  cell->cloudcover -= change;
}

void
IncreaseHumidity (struct WeatherCell *cell, int change)
{
  cell->humidity += change;
}

void
DecreaseHumidity (struct WeatherCell *cell, int change)
{
  cell->humidity -= change;
}

void
IncreaseWindX (struct WeatherCell *cell, int change)
{
  cell->windSpeedX += change;
}

void
DecreaseWindX (struct WeatherCell *cell, int change)
{
  cell->windSpeedX -= change;
}

void
IncreaseWindY (struct WeatherCell *cell, int change)
{
  cell->windSpeedY += change;
}

void
DecreaseWindY (struct WeatherCell *cell, int change)
{
  cell->windSpeedY -= change;
}
...
int
getCloudCover (struct WeatherCell *cell)
{
  return cell->cloudcover;
}

bool
isExtremelyCloudy (int cloudCover)
{
  if (cloudCover > 80)
    return TRUE;
  else
...
bool
isModeratelyCloudy (int cloudCover)
{
...
int
getTemp (struct WeatherCell *cell)
{
  return cell->temperature;
}

bool
isSwelteringHeat (int temp)
{
  if (temp > 90)
    return TRUE;
  else
...
/* Energy Information */
int
getEnergy (struct WeatherCell *cell)
{
  return cell->energy;
}
...
int
getPressure (struct WeatherCell *cell)
{
  return cell->pressure;
}

bool
isHighPressure (int pressure)
{
  if (pressure > 50)
    return TRUE;
  else
...
int
getHumidity (struct WeatherCell *cell)
{
  return cell->humidity;
}

bool
isExtremelyHumid (int humidity)
{
  if (humidity > 80)
    return TRUE;
  else
...
bool
isModeratelyHumid (int humidity)
{
...
int
getPrecip (struct WeatherCell *cell)
{
  return cell->precipitation;
}

bool
isTorrentialDownpour (int precip)
{
  if (precip > 90)
    return TRUE;
  else
...
bool
isRainingCatsAndDogs (int precip)
{
...
/* WindX Information */
int
getWindX (struct WeatherCell *cell)
{
  return cell->windSpeedX;
}

bool
isCalmWindE (int windx)
{
  if (windx > 0 && windx <= 10)
    return TRUE;
  else
...
/* WindY Information */
int
getWindY (struct WeatherCell *cell)
{
  return cell->windSpeedY;
}

bool
isCalmWindN (int windy)
{
  if (windy > 0 && windy <= 10)
    return TRUE;
  else
...
void
do_setweather (CHAR_DATA * ch, const char *argument)
{
...
void
do_showweather (CHAR_DATA * ch, const char *argument)
{
...
void
do_weather (CHAR_DATA * ch, const char *argument)
{
  ...
  ch_printf_color (ch,
		   "&wAs you check the weather around you, you notice:&D\r\n");
  if (getPrecip (cell) > 0)
    {
      if (isTorrentialDownpour (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ch_printf_color (ch,
			     "&WThe snow is creating such a blizzard you can barely see!&D\r\n");
	  else
	    ch_printf_color (ch,
			     "&BThe rain is coming down in torrents!&D\r\n");
	}
      else if (isRainingCatsAndDogs (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ch_printf_color (ch,
			     "&WThe snow is creating a near solid wall of white!&D\r\n");
	  else
	    ch_printf_color (ch,
			     "&BThe rain is coming down in big heavy drops!&D\r\n");
	}
      else if (isPouring (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
      ...
      else if (isDownpour (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ch_printf_color (ch,
			     "&WThe snow is coming down in heavy waves.&D\r\n");
	  else
	    ch_printf_color (ch,
			     "&BThe rain is coming down in sheets.&D\r\n");
	}
      else if (isRainingSteadily (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ch_printf_color (ch,
			     "&WThe snow appears to be falling pretty steadily.&D\r\n");
	  else
	    ch_printf_color (ch,
			     "&BThe rain appears to be falling pretty steadily.&D\r\n");
	}
      else if (isRaining (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ch_printf_color (ch,
			     "&WSnowflakes drift down from the heavens.&D\r\n");
	  else
      ...
      else if (isRainingLightly (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ...
	    ch_printf_color (ch,
			     "&BA light rain patters on the ground around you.&D\r\n");
	}
      else if (isDrizzling (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ...
	    ch_printf_color (ch,
			     "&BA light drizzle seems to be falling.&D\r\n");
	}
      else if (isMisting (getPrecip (cell)))
	{
	  if (getTemp (cell) <= 32)
	    ch_printf_color (ch,
			     "&WA few scattered snowflakes can be seen.&D\r\n");
	  else
	    ch_printf_color (ch,
			     "&BA light mist appears to be falling.&D\r\n");
	}
    }
  else
    ch_printf_color (ch,
		     "&BThere doesn't appear to be any form of precipitation.&D\r\n");
  ...
  if (getCloudCover (cell) > 0)
	...
	ch_printf_color (ch,
			 "&wThere looks to be a good bit of clouds in the sky.&D\r\n");
	...
	ch_printf_color (ch,
			 "&wThere are a few scattered clouds in the sky.&D\r\n");
    }
  else
    ch_printf_color (ch,
		     "&wThere don't appear to be any clouds in the sky.&D\r\n");

  if (getHumidity (cell) > 0)
    {
      if (isExtremelyHumid (getHumidity (cell)))
	ch_printf_color (ch,
			 "&cYour skin feels sickly sticky with the extreme humidity.&D\r\n");
      else if (isModeratelyHumid (getHumidity (cell)))
	ch_printf_color (ch,
			 "&cYou feel slightly sticky because of the moderate humidity.&D\r\n");
      else if (isMinorlyHumid (getHumidity (cell)))
	ch_printf_color (ch,
			 "&cThe stickyness of your skin is barely noticeable in the minor humidity.&D\r\n");
      else if (isHumid (getHumidity (cell)))
	ch_printf_color (ch,
			 "&cThe air feels perfect against your skin.&D\r\n");
      else
	ch_printf_color (ch,
			 "&cYou can't feel a difference in the humidity.&D\r\n");
    }
  else
    ch_printf_color (ch,
		     "&cThe air seems as if to suck the moisture from your skin.&D\r\n");

  if (getWindX (cell) != 0 && getWindY (cell) != 0)
    {
      if (isCalmWindE (getWindX (cell)) && isCalmWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the southeast.&D\r\n");
      else if (isCalmWindE (getWindX (cell)) && isCalmWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the northeast.&D\r\n");
      else if (isBreezyWindE (getWindX (cell))
	       && isBreezyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the southeast.&D\r\n");
      else if (isBreezyWindE (getWindX (cell))
	       && isBreezyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the northeast.&D\r\n");
      else if (isBlusteryWindE (getWindX (cell))
	       && isBlusteryWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA blustery wind blows from the southeast.&D\r\n");
      else if (isBlusteryWindE (getWindX (cell))
	       && isBlusteryWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA blustery wind blows from the northeast.&D\r\n");
      else if (isWindyWindE (getWindX (cell))
	       && isWindyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the southeast.&D\r\n");
      else if (isWindyWindE (getWindX (cell))
	       && isWindyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the northeast.&D\r\n");
      else if (isGustyWindE (getWindX (cell))
	       && isGustyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the southeast.&D\r\n");
      else if (isGustyWindE (getWindX (cell))
	       && isGustyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the northeast.&D\r\n");
      else if (isGaleForceWindE (getWindX (cell))
	       && isGaleForceWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the southeast.&D\r\n");
      else if (isGaleForceWindE (getWindX (cell))
	       && isGaleForceWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the northeast.&D\r\n");

      else if (isCalmWindW (getWindX (cell)) && isCalmWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the southwest.&D\r\n");
      else if (isCalmWindW (getWindX (cell)) && isCalmWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the northwest.&D\r\n");
      else if (isBreezyWindW (getWindX (cell))
	       && isBreezyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the southwest.&D\r\n");
      else if (isBreezyWindW (getWindX (cell))
	       && isBreezyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the northwest.&D\r\n");
      else if (isBlusteryWindW (getWindX (cell))
	       && isBlusteryWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA blustery wind blows from the southwest.&D\r\n");
      else if (isBlusteryWindW (getWindX (cell))
	       && isBlusteryWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA blustery wind blows from the northwest.&D\r\n");
      else if (isWindyWindW (getWindX (cell))
	       && isWindyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the southwest.&D\r\n");
      else if (isWindyWindW (getWindX (cell))
	       && isWindyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the northwest.&D\r\n");
      else if (isGustyWindW (getWindX (cell))
	       && isGustyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the southwest.&D\r\n");
      else if (isGustyWindW (getWindX (cell))
	       && isGustyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the northwest.&D\r\n");
      else if (isGaleForceWindW (getWindX (cell))
	       && isGaleForceWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the southwest.&D\r\n");
      else if (isGaleForceWindW (getWindX (cell))
	       && isGaleForceWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the northwest.&D\r\n");
	...
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the east.&D\r\n");
      else if (isBreezyWindE (getWindX (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the east.&D\r\n");
      else if (isBlusteryWindE (getWindX (cell)))
	ch_printf_color (ch, "&GA blustery wind blows from the east.&D\r\n");
      else if (isWindyWindE (getWindX (cell)))
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the east.&D\r\n");
      else if (isGustyWindE (getWindX (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the east.&D\r\n");
      else if (isGaleForceWindE (getWindX (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the east.&D\r\n");

      else if (isCalmWindW (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the west.&D\r\n");
      else if (isBreezyWindW (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the west.&D\r\n");
	...
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the west.&D\r\n");
      else if (isGustyWindW (getWindY (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the west.&D\r\n");
      else if (isGaleForceWindW (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the west.&D\r\n");
    }
  else if (getWindX (cell) == 0 && getWindY (cell) != 0)
    {
      if (isCalmWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the south.&D\r\n");
      else if (isBreezyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the south.&D\r\n");
	...
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the south.&D\r\n");
      else if (isGustyWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the south.&D\r\n");
      else if (isGaleForceWindS (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the south.&D\r\n");

      else if (isCalmWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA calm wind brushes your skin from the north.&D\r\n");
      else if (isBreezyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA steady breeze emanates from the north.&D\r\n");
	...
	ch_printf_color (ch,
			 "&GA strong steady wind howls from the north.&D\r\n");
      else if (isGustyWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GThe wind seems to be coming in gusts from the north.&D\r\n");
      else if (isGaleForceWindN (getWindY (cell)))
	ch_printf_color (ch,
			 "&GA gale force wind is tearing through the air from the north.&D\r\n");
    }
  else
  ...
  if (getTemp (cell) > -30 && getTemp (cell) < 100)
	...
	ch_printf_color (ch,
			 "&CYour breath seems to crystalize before your face.&D\r\n");
      else if (isReallyCold (getTemp (cell)))
	ch_printf_color (ch, "&CIt's really cold...&D\r\n");
      else if (isVeryCold (getTemp (cell)))
	ch_printf_color (ch,
			 "&CYou think you see ice forming on your clothes.&D\r\n");
      else if (isExtremelyCold (getTemp (cell)))
	ch_printf_color (ch,
			 "&CYou feel ice clinging to your skin. Get inside!&D\r\n");
    }
}
