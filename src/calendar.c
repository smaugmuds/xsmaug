...
void
do_timezone (CHAR_DATA * ch, const char *argument)
{
  int i;
...
c_time (time_t curtime, int tz)
{
  ...
  snprintf (strtime, 128, "%3s %3s %d, %d %d:%02d:%02d %cM %s",
	    ...
	    ptime->tm_year + 1900,
	    ptime->tm_hour == 0 ? 12 : ptime->tm_hour >
	    12 ? ptime->tm_hour - 12 : ptime->tm_hour, ptime->tm_min,
	    ptime->tm_sec, ptime->tm_hour < 12 ? 'A' : 'P', tzonename);
  return strtime;
}

/* timeZone is not shown as it's a bit .. long.. but it is respected -- Xorith */
char *
mini_c_time (time_t curtime, int tz)
{
  static char strtime[128];
  struct tm *ptime;
  char tzonename[50];
...
char *
sec_to_hms (time_t loctime, char *tstr)
{
  time_t t_rem;
  int sc, mn, hr, dy, wk;
  int sflg = 0;
...
HOLIDAY_DATA *
get_holiday (short month, short day)
{
...
void
do_time (CHAR_DATA * ch, const char *argument)
{
  HOLIDAY_DATA *holiday;
  ...
  ch_printf (ch,
	     ...
	     month_name[time_info.month], season_name[time_info.season],
	     time_info.year, str_boot_time, c_time (current_time, -1));
...
start_winter (void)
{
  ...
  echo_to_all (AT_CYAN, "The air takes on a chilling cold as winter sets in.",
	       ECHOTAR_ALL);
...
start_spring (void)
{
  ...
  echo_to_all (AT_DGREEN,
	       "The chill recedes from the air as spring begins to take hold.",
	       ECHOTAR_ALL);
...
start_summer (void)
{
  echo_to_all (AT_YELLOW,
	       "The days grow longer and hotter as summer grips the world.\r\n",
	       ECHOTAR_ALL);
  return;
}
...
start_fall (void)
{
  echo_to_all (AT_ORANGE,
	       "The leaves begin changing colors signaling the start of fall.\r\n",
	       ECHOTAR_ALL);
  return;
}
...
void
do_holidays (CHAR_DATA * ch, const char *argument)
{
  HOLIDAY_DATA *day;
  ...
  for (day = first_holiday; day; day = day->next)
    pager_printf (ch, "&G%-21s	&g%-11s	%-2d\r\n", day->name,
		  month_name[day->month - 1], day->day);
...
void
fread_day (HOLIDAY_DATA * day, FILE * fp)
{
  const char *word;
  bool fMatch;
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		day->announce =
		  str_dup
		  ("Today is a holiday, but who the hell knows which one.");
	      return;
	    }
	  break;
  ...
  if ((fp = fopen (filename, "r")) != NULL)
      ...
      for (;;)
	{
	  ...
	  if (!str_cmp (word, "HOLIDAY"))
		...
		{
		  bug
		    ("load_holidays: more holidays than %d, increase Max Holiday in cset.",
		     sysdata.maxholiday);
		  fclose (fp);
		  return;
		}
...
void
do_saveholiday (CHAR_DATA * ch, const char *argument)
{
  save_holidays ();
...
void
do_setholiday (CHAR_DATA * ch, const char *argument)
{
  ...
  if (!str_cmp (arg2, "day"))
    {
	  ...
	  || atoi (arg3) > sysdata.dayspermonth || atoi (arg3) <= 1)
	{
	  ch_printf (ch, "You must specify a numeric value : %d - %d", 1,
