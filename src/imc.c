 ...
 */
size_t
imcstrlcpy (char *dst, const char *src, size_t siz)
{
  register char *d = dst;
 ...
 */
size_t
imcstrlcat (char *dst, const char *src, size_t siz)
{
  register char *d = dst;
  register const char *s = src;
  register size_t n = siz;
...
*/
char *
imcstrrep (const char *src, const char *sch, const char *rep)
{
...
const char *
imcone_argument (const char *argument, char *arg_first)
{
  char cEnd;
  int count;
...
char *
imc_strip_colors (const char *txt)
{
  IMC_COLOR *color;
...
const char *
color_itom (const char *txt, CHAR_DATA * ch)
{
  IMC_COLOR *color;
  static char tbuf[LGST];
  ...
  if (IMCIS_SET (IMCFLAG (ch), IMC_COLORFLAG))
    {
      imcstrlcpy (tbuf, txt, LGST);
      for (color = first_imc_color; color; color = color->next)
	imcstrlcpy (tbuf, imcstrrep (tbuf, color->imctag, color->mudtag),
		    LGST);
    }
  else
...
void
imc_to_char (const char *txt, CHAR_DATA * ch)
{
  char buf[LGST * 2];
...
void
imc_printf (CHAR_DATA * ch, const char *fmt, ...)
{
  char buf[LGST];
  va_list args;
...
void
imc_to_pager (const char *txt, CHAR_DATA * ch)
{
  char buf[LGST * 2];
...
void
imcpager_printf (CHAR_DATA * ch, const char *fmt, ...)
{
  char buf[LGST];
  va_list args;
...
bool
imcstr_prefix (const char *astr, const char *bstr)
{
  if (!astr)
    {
...
bool
imc_hasname (const char *list, const char *member)
{
  if (!list || list[0] == '\0')
    return FALSE;
...
void
imc_addname (char **list, const char *member)
{
  char newlist[LGST];
...
void
imc_removename (char **list, const char *member)
{
  char newlist[LGST];
...
char *
imc_channel_mudof (const char *src)
{
  static char mud[SMST];
  size_t x;
...
char *
imc_channel_nameof (const char *src)
{
  static char name[SMST];
  size_t x, y = 0;
  bool colon = FALSE;
...
char *
imc_makename (const char *person, const char *mud)
{
  static char name[SMST];
...
bool
imc_isignoring (CHAR_DATA * ch, const char *ignore)
{
  IMC_IGNORE *temp;
...
void
imc_delete_reminfo (REMOTEINFO * p)
{
...
void
imc_new_reminfo (const char *mud, const char *version, const char *netname,
...
REMOTEINFO *
imc_find_reminfo (const char *name)
{
  REMOTEINFO *p;
...
bool
check_mud (CHAR_DATA * ch, const char *mud)
{
...
bool
check_mudof (CHAR_DATA * ch, const char *mud)
{
  return check_mud (ch, imc_mudof (mud));
}

int
get_imcpermvalue (const char *flag)
{
  size_t x;
...
bool
imccheck_permissions (CHAR_DATA * ch, int checkvalue, int targetvalue,
		      bool enforceequal)
{
  ...
  if (checkvalue == IMCPERM (ch) && IMCPERM (ch) != IMCPERM_IMP
      && enforceequal)
    {
      imc_to_char
...
IMC_CHANNEL *
imc_findchannel (const char *name)
{
  IMC_CHANNEL *c;
...
void
imcformat_channel (CHAR_DATA * ch, IMC_CHANNEL * d, int format, bool all)
{
  IMC_CHANNEL *c = NULL;
  char buf[LGST];
...
void
imc_new_channel (const char *chan, const char *owner, const char *ops,
		 ...
		 int perm, const char *lname)
{
  ...
  if (!strchr (chan, ':'))
    {
      imclog ("%s: Improperly formatted channel name: %s", __FUNCTION__,
	      chan);
      return;
    }
...
void
imc_register_packet_handler (const char *name, PACKET_FUN * func)
{
  IMC_PHANDLER *ph;
  ...
  for (ph = first_phandler; ph; ph = ph->next)
	...
	{
	  imclog
	    ("Unable to register packet type %s. Another module has already registered it.",
	     name);
	  return;
	}
    }
...
int
find_next_esign (const char *string, int current)
{
  bool quote = FALSE;
...
char *
imc_getData (char *output, const char *key, const char *packet)
{
  int current = 0;
  size_t i = 0;
  bool quote = FALSE;
...
void
imc_write_buffer (const char *txt)
{
...
imc_write_packet (IMC_PACKET * p)
{
   ...
   */
  snprintf (txt, IMC_BUFF_SIZE, "%s %lu %s %s %s", p->from,
	    ++imc_sequencenumber, this_imcmud->localname, p->type, p->to);
  for (data = p->first_data; data; data = data->next)
    snprintf (txt + strlen (txt), IMC_BUFF_SIZE - strlen (txt), "%s",
	      data->field);
  imc_freepacket (p);
...
void
imc_addtopacket (IMC_PACKET * p, const char *fmt, ...)
{
  IMC_PDATA *data;
...
IMC_PACKET *
imc_newpacket (const char *from, const char *type, const char *to)
{
  IMC_PACKET *p;

  if (!type || type[0] == '\0')
    {
      imcbug ("%s: Attempt to build packet with no type field.",
	      __FUNCTION__);
      return NULL;
    }
  ...
  if (!from || from[0] == '\0')
    {
      imcbug ("%s: Attempt to build %s packet with no from field.",
	      __FUNCTION__, type);
      return NULL;
    }
  ...
  if (!to || to[0] == '\0')
    {
      imcbug ("%s: Attempt to build %s packet with no to field.",
	      __FUNCTION__, type);
      return NULL;
    }
...
void
imc_update_tellhistory (CHAR_DATA * ch, const char *msg)
{
  char new_msg[LGST];
  struct tm *local = localtime (&imc_time);
  int x;

  snprintf (new_msg, LGST, "~R[%-2.2d:%-2.2d] %s", local->tm_hour,
	    local->tm_min, msg);
	  ...
	  for (i = 1; i < MAX_IMCTELLHISTORY; i++)
	    {
	      ...
	      IMCTELLHISTORY (ch, i - 1) =
		IMCSTRALLOC (IMCTELLHISTORY (ch, i));
	    }
...
void
imc_send_tell (const char *from, const char *to, const char *txt, int reply)
{
  IMC_PACKET *p;
...
PFUN (imc_recv_tell)
{
  ...
  if (strcasecmp (imc_nameof (q->from), "ICE"))
      ...
      if (imc_isignoring (vic, q->from))
	{
      ...
      if (IMCIS_SET (IMCFLAG (vic), IMC_TELL)
	  || IMCIS_SET (IMCFLAG (vic), IMC_DENYTELL))
	{
	  if (strcasecmp (imc_nameof (q->from), "*"))
	    {
	      snprintf (buf, LGST, "%s is not receiving tells.", q->to);
      ...
      if (IMCAFK (vic))
	{
	  if (strcasecmp (imc_nameof (q->from), "*"))
	    {
	      snprintf (buf, LGST, "%s is currently AFK. Try back later.",
			q->to);
	      imc_send_tell ("*", q->from, buf, 1);
	    }
	  return;
...
void
update_imchistory (IMC_CHANNEL * channel, char *message)
{
...
void
imc_display_channel (IMC_CHANNEL * c, const char *from, char *txt, int emote)
{
  DESCRIPTOR_DATA *d;
  CHAR_DATA *ch;
  ...
  for (d = first_descriptor; d; d = d->next)
	...
	continue;
	  ...
	  || !imc_hasname (IMC_LISTEN (ch), c->local_name))
	continue;
	...
	continue;
      ...
      if (!c->open)
	{
	  snprintf (name, SMST, "%s@%s", CH_IMCNAME (ch),
...
void
imc_sendmessage (const IMC_CHANNEL * c, const char *name, const char *text,
		 int emote)
{
  IMC_PACKET *p;
...
const char *
get_local_chanwho (IMC_CHANNEL * c)
{
  DESCRIPTOR_DATA *d;
  CHAR_DATA *person;
  ...
  snprintf (buf, IMC_BUFF_SIZE,
	    "The following people are listening to %s on %s:\r\n\r\n",
  ...
  for (d = first_descriptor; d; d = d->next)
	...
	continue;
      ...
      snprintf (buf + strlen (buf), IMC_BUFF_SIZE - strlen (buf), "%-15s",
		CH_IMCNAME (person));
      count++;
      if (++col % 3 == 0)
	{
...
PFUN (imc_recv_chanwho)
{
  ...
  if (!c->local_name)
    snprintf (buf, IMC_BUFF_SIZE,
	      "Channel %s is not locally configured on %s\r\n", lname,
	      this_imcmud->localname);
  else if (c->level > level)
    snprintf (buf, IMC_BUFF_SIZE,
	      "Channel %s is above your permission level on %s\r\n", lname,
	      this_imcmud->localname);
  else
    {
      ...
      if ((!strcasecmp (cwho, "") || !strcasecmp (cwho, "Nobody"))
	  && !strcasecmp (q->to, "*"))
	return;
...
char *
imccenterline (const char *string, int length)
{
  char stripped[300];
...
void
imc_send_whoreply (const char *to, const char *txt)
{
  IMC_PACKET *p;
...
void
imc_send_who (const char *from, const char *to, const char *type)
{
  IMC_PACKET *p;
...
char *
break_newlines (char *argument, char *arg_first)
{
  char cEnd;
  int count;
...
char *
process_plrline (char *plrrank, char *plrflags, char *plrname, char *plrtitle)
{
...
char *
process_immline (char *plrrank, char *plrflags, char *plrname, char *plrtitle)
{
  static char pline[LGST];
...
char *
process_who_template (char *head, char *tail, char *plrlines, char *immlines,
		      char *plrheader, char *immheader)
{
...
char *
imc_assemble_who (void)
{
  CHAR_DATA *person;
  DESCRIPTOR_DATA *d;
  int pcount = 0;
  bool plr = FALSE, imm = FALSE;
  char plrheader[SMST], immheader[SMST], rank[SMST], flags[SMST], name[SMST],
    title[SMST], plrline[SMST], immline[SMST];
  ...
  for (d = first_descriptor; d; d = d->next)
      ...
      if (person && d->connected == CON_PLAYING)
	{
	  ...
	  imcstrlcpy (plrline, process_plrline (rank, flags, name, title),
		      SMST);
  ...
  for (d = first_descriptor; d; d = d->next)
      ...
      if (person && d->connected == CON_PLAYING)
	{
	  ...
	  imcstrlcpy (name, CH_IMCNAME (person), SMST);
	  imcstrlcpy (title, color_mtoi (CH_IMCTITLE (person)), SMST);
	  imcstrlcpy (immline, process_immline (rank, flags, name, title),
		      SMST);
  ...
  imcstrlcpy (master,
	      process_who_template (head, tail, plrlines, immlines, plrheader,
				    immheader), LGST);
...
/* Finger code */
void
imc_process_finger (const char *from, const char *type)
{
  CHAR_DATA *victim;
    ...
    snprintf (buf + strlen (buf), IMC_BUFF_SIZE - strlen (buf),
	      ...
	      (IMC_EMAIL (victim)
	       ...
	       '\0') ? IMC_EMAIL (victim) : "None", (IMC_HOMEPAGE (victim)
						     &&
						     IMC_HOMEPAGE (victim)[0]
						     !=
						     '\0') ?
	      IMC_HOMEPAGE (victim) : "None", IMC_ICQ (victim),
	      (IMC_AIM (victim)
	       && IMC_AIM (victim)[0] != '\0') ? IMC_AIM (victim) : "None",
	      (IMC_YAHOO (victim)
	       && IMC_YAHOO (victim)[0] !=
	       '\0') ? IMC_YAHOO (victim) : "None", (IMC_MSN (victim)
						     && IMC_MSN (victim)[0] !=
						     '\0') ? IMC_MSN (victim)
	      : "None");
  ...
  snprintf (buf + strlen (buf), IMC_BUFF_SIZE - strlen (buf), "~W%s\r\n",
	    (IMC_COMMENT (victim)
	     && IMC_COMMENT (victim)[0] != '\0') ? IMC_COMMENT (victim) : "");
...
void
imc_send_whoisreply (char *to, char *data)
{
  IMC_PACKET *p;
...
void
imc_send_whois (const char *from, const char *user)
{
  IMC_PACKET *p;
...
PFUN (imc_recv_beep)
{
  ...
  if (IMCIS_SET (IMCFLAG (vic), IMC_BEEP)
      || IMCIS_SET (IMCFLAG (vic), IMC_DENYBEEP))
    {
      if (strcasecmp (imc_nameof (q->from), "*"))
	{
	  snprintf (buf, LGST, "%s is not receiving beeps.", q->to);
...
void
imc_send_beep (const char *from, const char *to)
{
  IMC_PACKET *p;
...
PFUN (imc_recv_iceupdate)
{
  ...
  char chan[SMST], owner[SMST], ops[SMST], invite[SMST], exclude[SMST],
    policy[SMST], level[SMST], lname[SMST];
  int perm;
  bool copen;
...
int
imc_get_ucache_gender (const char *name)
{
  IMCUCACHE_DATA *user;
...
void
imc_ucache_update (const char *name, int gender)
{
  IMCUCACHE_DATA *user;
...
void
imc_send_ucache_update (const char *visname, int gender)
{
  IMC_PACKET *p;
...
void
imc_send_ucache_request (char *targetuser)
{
  IMC_PACKET *p;
  char to[SMST];
...
void
imc_register_default_packets (void)
{
  /*
...
void
imc_parse_packet (const char *packet)
{
  IMC_PACKET *p;
  PACKET_FUN *pfun;
  char arg[SMST];
  ...
  if (!pfun)
      ...
      if (imcpacketdebug)
	{
	  ...
	  imclog ("No packet handler function has been defined for %s",
		  p->type);
	}
      IMCDISPOSE (p);
      return;
  ...
  if (!(imc_find_reminfo (imc_mudof (p->from))))
    imc_new_reminfo (imc_mudof (p->from), "Unknown", this_imcmud->network,
		     "Unknown", p->route);
...
void
imc_finalize_connection (char *name, char *netname)
{
  ...
  imclog ("Connected to %s. Network ID: %s", name,
...
void
imc_handle_autosetup (char *source, char *servername, char *cmd, char *txt,
		      char *encrypt)
{
  if (!strcasecmp (cmd, "reject"))
	...
	{
	  imclog ("There is already a mud named %s connected to the network.",
		  this_imcmud->localname);
	  imc_shutdown (FALSE);
	  return;
	}
	...
	{
	  imclog ("%s has reached its connection limit. Autosetup denied.",
		  servername);
	  imc_shutdown (FALSE);
	  return;
	}
	...
	{
	  imclog ("%s has banned your connection. Autosetup denied.",
		  servername);
	  imc_shutdown (FALSE);
	  return;
	}
      imclog ("%s: Invalid 'reject' response. Autosetup failed.", servername);
      imclog ("Data received: %s %s %s %s %s", source, servername, cmd, txt,
	      encrypt);
      imc_shutdown (FALSE);
      return;
    }
...
void
imc_process_authentication (const char *packet)
{
  char command[SMST], rname[SMST], pw[SMST], version[SMST], netname[SMST],
    encrypt[SMST];
  char response[LGST];
  ...
  if (!strcasecmp (command, "SHA256-AUTH-INIT"))
	...
	{
	  imclog
	    ("SHA-256 Authentication failure: No auth_value was returned by %s.",
	     rname);
	  imc_shutdown (FALSE);
	  return;
	}
       ...
       */
      auth_value = atol (pw);
      snprintf (pwd, SMST, "%ld%s%s", auth_value, this_imcmud->clientpw,
		this_imcmud->serverpw);
      cryptpwd = sha256_crypt (pwd);

      snprintf (response, LGST, "SHA256-AUTH-RESP %s %s version=%d",
  ...
  imclog ("Data received: %s %s %s %s %s", command, rname, pw, version,
	  netname);
  imc_shutdown (FALSE);
}
...
imc_read_socket (void)
{
  ...
  for (;;)
	...
	break;
      ...
      nRead =
	recv (this_imcmud->desc, this_imcmud->inbuf + iStart,
	...
	{
	  imclog ("%s: Descriptor error on #%d: %s", __FUNCTION__,
		  this_imcmud->desc, strerror (iErr));
	  return FALSE;
	}
    }
  ...
  if (imcwait == 1)
      ...
      if (++imcconnect_attempts > 3)
	{
	  if (this_imcmud->sha256pass)
	    {
	      imclog ("%s",
		      "Unable to reconnect using SHA-256, trying standard authentication.");
	  ...
	  else
	    {
	      imcwait = -2;
	      imclog ("%s",
		      "Unable to reestablish connection to server. Abandoning reconnect.");
	      return;
	    }
	}
      ...
      if (!imc_read_socket ())
	{
	  if (this_imcmud->inbuf && this_imcmud->inbuf[0] != '\0')
	      ...
	      if (imc_read_buffer ())
		{
		  if (!strcasecmp
		      (this_imcmud->incomm,
		       "SHA-256 authentication is required."))
		    {
		      imclog ("%s",
			      "Unable to reconnect using standard authentication, trying SHA-256.");
		      this_imcmud->sha256pass = TRUE;
		      imc_save_config ();
		    }
		  else
...
imc_adjust_perms (CHAR_DATA * ch)
{
  ...
  if (!IMCIS_SET (IMCFLAG (ch), IMC_PERMOVERRIDE))
	...
	IMCPERM (ch) = IMCPERM_NONE;
      else if (CH_IMCLEVEL (ch) >= this_imcmud->minlevel
	       && CH_IMCLEVEL (ch) < this_imcmud->immlevel)
	IMCPERM (ch) = IMCPERM_MORT;
      else if (CH_IMCLEVEL (ch) >= this_imcmud->immlevel
	       && CH_IMCLEVEL (ch) < this_imcmud->adminlevel)
	IMCPERM (ch) = IMCPERM_IMM;
      else if (CH_IMCLEVEL (ch) >= this_imcmud->adminlevel
	       && CH_IMCLEVEL (ch) < this_imcmud->implevel)
...
bool
imc_loadchar (CHAR_DATA * ch, FILE * fp, const char *word)
{
  bool fMatch = FALSE;
  ...
  switch (word[0])
      ...
      if (!strcasecmp (word, "IMCignore"))
	{
	  ...
	  IMCLINK (temp, FIRST_IMCIGNORE (ch), LAST_IMCIGNORE (ch), next,
		   prev);
	  fMatch = TRUE;
	  break;
	}
...
void
imc_savechar (CHAR_DATA * ch, FILE * fp)
{
  IMC_IGNORE *temp;
...
void
imc_load_pfile (CHAR_DATA * ch, char *tag, int num, char *line)
{
...
/* This is used only by CircleMUDs which have the ASCII Pfile code installed */
void
imc_save_pfile (struct CHAR_DATA *ch, FBFILE * fp)
{
  IMC_IGNORE *temp;
...
void
imc_readchannel (IMC_CHANNEL * channel, FILE * fp)
{
  const char *word;
  bool fMatch;
	...
	case 'E':
	  if (!strcasecmp (word, "End"))
	      ...
	      if (channel->level > IMCPERM_IMP)
		{
		  ...
		  else if (channel->level >= this_imcmud->minlevel
			   && channel->level < this_imcmud->immlevel)
		    channel->level = IMCPERM_MORT;
		  else if (channel->level >= this_imcmud->immlevel
			   && channel->level < this_imcmud->adminlevel)
		    channel->level = IMCPERM_IMM;
		  else if (channel->level >= this_imcmud->adminlevel
			   && channel->level < this_imcmud->implevel)
...
void
imc_readcolor (IMC_COLOR * color, FILE * fp)
{
  const char *word;
  bool fMatch;
...
void
imc_readhelp (IMC_HELP_DATA * help, FILE * fp)
{
  const char *word;
  char hbuf[LGST];
  int permvalue;
  ...
  for (;;)
	...
	case 'P':
	  if (!strcasecmp (word, "Perm"))
		...
		{
		  imcbug
		    ("imc_readhelp: Command %s loaded with invalid permission. Set to Imp.",
		     help->name);
...
void
imc_readcommand (IMC_CMD_DATA * cmd, FILE * fp)
{
  IMC_ALIAS *alias;
  const char *word;
  int permvalue;
  ...
  for (;;)
	  ...
	  if (!strcasecmp (word, "Code"))
	    {
	      ...
	      if (cmd->function == NULL)
		imcbug
		  ("imc_readcommand: Command %s loaded with invalid function. Set to NULL.",
		   cmd->name);
	      fMatch = TRUE;
	      break;
	    }
	...
	case 'P':
	  if (!strcasecmp (word, "Perm"))
		...
		{
		  imcbug
		    ("imc_readcommand: Command %s loaded with invalid permission. Set to Imp.",
		     cmd->name);
...
void
imc_readucache (IMCUCACHE_DATA * user, FILE * fp)
{
  const char *word;
  bool fMatch;
...
imcfread_config_file (FILE * fin)
{
  ...
  for (;;)
	...
	case 'A':
	  IMCKEY ("Autoconnect", this_imcmud->autoconnect,
		  imcfread_number (fin));
	  IMCKEY ("AdminLevel", this_imcmud->adminlevel,
		  imcfread_number (fin));
	  break;
	  ...
	  if (!strcasecmp (word, "End"))
	    {
	      ...
	      snprintf (lbuf1, LGST, "%s %s.%s", CODEBASE_VERSION_TITLE,
			CODEBASE_VERSION_MAJOR, CODEBASE_VERSION_MINOR);
	      if (this_imcmud->base)
		IMCSTRFREE (this_imcmud->base);
	      this_imcmud->base = IMCSTRALLOC (lbuf1);

	      snprintf (lbuf2, LGST, "%s%s", IMC_VERSION_STRING,
			this_imcmud->base);
	...
	case 'M':
	  IMCKEY ("MinImmLevel", this_imcmud->immlevel,
		  imcfread_number (fin));
	  IMCKEY ("MinPlayerLevel", this_imcmud->minlevel,
		  imcfread_number (fin));
	  break;
	...
	case 'S':
	  ...
	  IMCKEY ("SHA256Pwd", this_imcmud->sha256pass,
		  imcfread_number (fin));
	  break;
	}
      if (!fMatch)
...
imc_read_config (int desc)
{
  ...
  if (!this_imcmud->localname || this_imcmud->localname[0] == '\0')
    {
      imcbug ("%s",
	      "imc_read_config: Mud name not loaded in configuration file.");
      imcbug ("%s", "Network configuration aborted.");
      return FALSE;
    }
  ...
  if (!this_imcmud->fullname || this_imcmud->fullname[0] == '\0')
    {
      imcbug ("%s",
	      "imc_read_config: Missing InfoName parameter in configuration file.");
      imcbug ("%s", "Network configuration aborted.");
      return FALSE;
    }
  ...
  if (!this_imcmud->ihost || this_imcmud->ihost[0] == '\0')
    {
      imcbug ("%s",
	      "imc_read_config: Missing InfoHost parameter in configuration file.");
      imcbug ("%s", "Network configuration aborted.");
      return FALSE;
    }
  ...
  if (!this_imcmud->email || this_imcmud->email[0] == '\0')
    {
      imcbug ("%s",
	      "imc_read_config: Missing InfoEmail parameter in configuration file.");
      imcbug ("%s", "Network configuration aborted.");
      return FALSE;
    }
...
parse_who_header (char *head)
{
  ...
  imcstrlcpy (newhead,
	      imcstrrep (newhead, "<%mudfullname%>", this_imcmud->fullname),
	      LGST);
  imcstrlcpy (newhead,
	      imcstrrep (newhead, "<%mudtelnet%>", this_imcmud->ihost), LGST);
  imcstrlcpy (newhead, imcstrrep (newhead, "<%mudport%>", iport), LGST);
  imcstrlcpy (newhead, imcstrrep (newhead, "<%mudurl%>", this_imcmud->www),
	      LGST);
...
parse_who_tail (char *tail)
{
  ...
  imcstrlcpy (newtail,
	      imcstrrep (newtail, "<%mudfullname%>", this_imcmud->fullname),
	      LGST);
  imcstrlcpy (newtail,
	      imcstrrep (newtail, "<%mudtelnet%>", this_imcmud->ihost), LGST);
  imcstrlcpy (newtail, imcstrrep (newtail, "<%mudport%>", iport), LGST);
  imcstrlcpy (newtail, imcstrrep (newtail, "<%mudurl%>", this_imcmud->www),
	      LGST);
...
ipv4_connect (void)
{
  ...
  if (connect (desc, (struct sockaddr *) &sa, sizeof (sa)) == -1)
	...
	{
	  imclog ("%s: Failed connect: Error %d: %s", __FUNCTION__, errno,
		  strerror (errno));
	  perror ("connect");
	  close (desc);
	  return -1;
...
imc_server_connect (void)
{
  ...
  if (ai == NULL)
    {
      imclog ("%s: socket or connect: failed for %s port %hu", __FUNCTION__,
	      this_imcmud->rhost, this_imcmud->rport);
  ...
  if (this_imcmud->sha256)
      ...
      if (!this_imcmud->sha256pass)
	{
	  snprintf (buf, LGST, "PW %s %s version=%d autosetup %s SHA256",
		    this_imcmud->localname, this_imcmud->clientpw,
		    IMC_VERSION, this_imcmud->serverpw);
	}
      /*
   ...
   */
    ...
    snprintf (buf, LGST, "PW %s %s version=%d autosetup %s",
	      this_imcmud->localname, this_imcmud->clientpw, IMC_VERSION,
	      this_imcmud->serverpw);
...
void
imc_delete_templates (void)
{
  imc_delete_who_template ();
}

void
free_imcdata (bool complete)
{
  REMOTEINFO *p, *pnext;
  IMC_BAN *ban, *ban_next;
  IMCUCACHE_DATA *ucache, *next_ucache;
	  ...
	  for (alias = cmd->first_alias; alias; alias = alias_next)
	    {
	      ...
	      IMCUNLINK (alias, cmd->first_alias, cmd->last_alias, next,
			 prev);
	      IMCDISPOSE (alias);
	    }
	  IMCSTRFREE (cmd->name);
...
imc_hotboot (void)
{
  ...
  if (this_imcmud && this_imcmud->state == IMC_ONLINE)
	...
	imcbug ("%s: Unable to open IMC hotboot file for write.",
		__FUNCTION__);
      else
	{
	  fprintf (fp, "%s %s\n",
  ...
  if (reconnect)
    {
      ...
      imclog ("%s",
	      "Connection to server was lost. Reconnecting in approximately 20 seconds.");
    }
}

/* Startup IMC2 */
bool
imc_startup_network (bool connected)
{
...
void
imc_startup (bool force, int desc, bool connected)
{
  imcwait = 0;

  if (this_imcmud && this_imcmud->state > IMC_OFFLINE)
    {
      imclog ("%s: Network startup called when already engaged!",
	      __FUNCTION__);
      return;
    }
  ...
  if ((!this_imcmud->autoconnect && !force && !connected)
    ...
    {
      imclog ("%s",
	      "IMC2 network data loaded. Autoconnect not set. IMC2 will need to be connected manually.");
      return;
    }
  else
...
IMC_CMD (imccommand)
{
  ...
  if (!cmd[0] || !chan[0])
    {
      imc_to_char
	("Syntax: imccommand <command> <server:channel> [<data..>]\r\n", ch);
      imc_to_char
  ...
  snprintf (to, SMST, "IMC@%s",
	    c ? imc_channel_mudof (c->name) : imc_channel_mudof (chan));
...
bool
verify_format (const char *fmt, short sneed)
{
  const char *c;
  int i = 0;
...
IMC_CMD (imcsetup)
{
  ...
  if (imccmd[0] == '\0' || chan[0] == '\0')
    {
      ...
      imc_to_char
	("delete rename perm regformat emoteformat socformat\r\n\r\n", ch);
  ...
  if (!strcasecmp (imccmd, "resetformats"))
      ...
      else
	{
	  imcformat_channel (ch, c, 4, FALSE);
	  imc_to_char
	    ("The formats for this channel have been reset to default.\r\n",
	     ch);
	}
      return;
    }
  ...
  if (!strcasecmp (imccmd, "regformat"))
      ...
      else
	{
	  ...
	  c->regformat = IMCSTRALLOC (arg1);
	  imc_to_char
	    ("The regular format for this channel has been changed successfully.\r\n",
	     ch);
	}
      imc_save_channels ();
      return;
  ...
  if (!strcasecmp (imccmd, "emoteformat"))
      ...
      else
	{
	  ...
	  c->emoteformat = IMCSTRALLOC (arg1);
	  imc_to_char
	    ("The emote format for this channel has been changed successfully.\r\n",
	     ch);
	}
      imc_save_channels ();
      return;
  ...
  if (!strcasecmp (imccmd, "socformat"))
      ...
      else
	{
	  ...
	  c->socformat = IMCSTRALLOC (arg1);
	  imc_to_char
	    ("The social format for this channel has been changed successfully.\r\n",
	     ch);
	}
      imc_save_channels ();
      return;
  ...
  if (!strcasecmp (imccmd, "perm") || !strcasecmp (imccmd, "permission")
	...
	{
	  imc_to_char
	    ("Syntax: imcsetup perm <localchannel> <permission>\r\n", ch);
	  return;
	}
...
IMC_CMD (imclisten)
{
  ...
  if (!strcasecmp (argument, "all"))
      ...
      for (c = first_imc_channel; c; c = c->next)
	{
      ...
      imc_to_char
	("~YYou are now listening to all available IMC2 channels.\r\n", ch);
      return;
    }
  ...
  if (!strcasecmp (argument, "none"))
      ...
      for (c = first_imc_channel; c; c = c->next)
	{
      ...
      imc_to_char
	("~YYou no longer listen to any available IMC2 channels.\r\n", ch);
      return;
    }
...
IMC_CMD (imcreply)
{
  ...
  if (argument[0] == '@')
  ...
  else if (argument[0] == ',')
    {
      ...
      while (isspace (*argument))
	argument++;
      imc_send_tell (CH_IMCNAME (ch), IMC_RREPLY (ch), color_mtoi (argument),
		     1);
      snprintf (buf1, LGST, "~WImctell: ~c%s %s\r\n", IMC_RREPLY (ch),
		argument);
    }
...
IMC_CMD (imcwho)
{
  if (!argument || argument[0] == '\0')
    {
      imc_to_char
	("imcwho which mud? See imclist for a list of connected muds.\r\n",
	 ch);
      return;
    }
   ...
   */
  if (!strcasecmp (argument, this_imcmud->localname)
      && IMCPERM (ch) >= IMCPERM_IMM)
    {
...
IMC_CMD (imcfinger)
{
  ...
  if (arg[0] == '\0')
    {
      ...
      imc_to_char
	("~wdisplay email homepage icq aim yahoo msn privacy comment\r\n",
	 ch);
      return;
    }
  ...
  if (!strcasecmp (arg, "privacy"))
      ...
      if (IMCIS_SET (IMCFLAG (ch), IMC_PRIVACY))
	{
	  ...
	  imc_to_char
	    ("Privacy flag removed. Your information will now be visible on imcfinger.\r\n",
	     ch);
	}
      else
	{
  ...
  if (!strcasecmp (arg, "comment"))
	...
	{
	  imc_to_char
	    ("You must limit the comment line to 78 characters or less.\r\n",
	     ch);
	  return;
	}
      IMCSTRFREE (IMC_COMMENT (ch));
...
IMC_CMD (imclist)
{
  ...
  imcpager_printf (ch, "~c%-15.15s ~B%-40.40s~! ~g%-15.15s ~G%s", "Name",
		   "IMC2 Version", "Network", "Server");
  ...
  imcpager_printf (ch, "\r\n\r\n~c%-15.15s ~B%-40.40s ~g%-15.15s ~G%s",
		   ...
		   this_imcmud->network, this_imcmud->servername);
  ...
  for (p = first_rinfo; p; p = p->next, count++)
      ...
      if (p->path && p->path[0] != '\0')
	{
	  if ((start = strchr (p->path, '!')) != NULL)
	    {
	      ...
	      for (onpath = start; *onpath != '!' && *onpath != '\0';
		   onpath++)
		{
      ...
      imcpager_printf (ch, "\r\n~%c%-15.15s ~B%-40.40s ~g%-15.15s ~G%s",
		       p->expired ? 'R' : 'c', p->name, p->version, netname,
		       serverpath);
    }
  imcpager_printf (ch,
...
IMC_CMD (imcconnect)
{
  if (this_imcmud && this_imcmud->state > IMC_OFFLINE)
    {
      imc_to_char
	("The IMC2 network connection appears to already be engaged!\r\n",
	 ch);
      return;
    }
  imcconnect_attempts = 0;
...
IMC_CMD (imcdisconnect)
{
  if (this_imcmud && this_imcmud->state == IMC_OFFLINE)
    {
      imc_to_char
	("The IMC2 network connection does not appear to be engaged!\r\n",
	 ch);
      return;
    }
  imc_shutdown (FALSE);
...
IMC_CMD (imcconfig)
{
  ...
  if (arg1[0] == '\0')
    {
      ...
      imc_to_char
	("~wConfiguration info for your mud. Changes save when edited.\r\n",
	 ch);
      imc_to_char ("~wYou may set the following:\r\n\r\n", ch);
      imc_to_char
	("~wShow           : ~GDisplays your current configuration.\r\n", ch);
      imc_to_char
	("~wLocalname      : ~GThe name IMC2 knows your mud by.\r\n", ch);
      imc_to_char
	("~wAutoconnect    : ~GToggles automatic connection on reboots.\r\n",
	 ch);
      imc_to_char
	("~wMinPlayerLevel : ~GSets the minimum level IMC2 can see your players at.\r\n",
	 ch);
      imc_to_char
	...
	("~wImplevel       : ~GSets the level at which immplementor commands become available.\r\n",
	 ch);
      imc_to_char
	("~wInfoname       : ~GName of your mud, as seen from the imcquery info sheet.\r\n",
	 ch);
      imc_to_char ("~wInfohost       : ~GTelnet address of your mud.\r\n",
		   ch);
      imc_to_char ("~wInfoport       : ~GTelnet port of your mud.\r\n", ch);
      imc_to_char
	("~wInfoemail      : ~GEmail address of the mud's IMC administrator.\r\n",
	 ch);
      ...
      imc_to_char ("~wInfoDetails    : ~GSHORT Description of your mud.\r\n",
		   ch);
      imc_to_char
	("~wServerAddr     : ~GDNS or IP address of the server you mud connects to.\r\n",
	 ch);
      imc_to_char
	("~wServerPort     : ~GPort of the server your mud connects to.\r\n",
	 ch);
  ...
  if (this_imcmud->state != IMC_OFFLINE)
    {
      imc_printf (ch,
		  "Cannot alter %s while the mud is connected to IMC.\r\n",
		  arg1);
      return;
    }
  ...
  if (arg[0] == '\0')
    {
      ...
      for (count = 0, ign = FIRST_IMCIGNORE (ch); ign;
	   ign = ign->next, count++)
	imc_printf (ch, "%s\r\n", ign->name);
  ...
  if (!strcasecmp (arg, "help"))
    {
      ...
      imc_to_char
	("~wTo add an ignore             : ~GIMCIGNORE ADD <argument>\r\n",
	 ch);
      imc_to_char
	("~wTo delete an ignore          : ~GIMCIGNORE DELETE <argument>\r\n",
	 ch);
  ...
  if (!strcasecmp (arg, "delete"))
      ...
      for (ign = FIRST_IMCIGNORE (ch); ign; ign = ign->next)
	{
	  if (!strcasecmp (ign->name, argument))
	    {
	      IMCUNLINK (ign, FIRST_IMCIGNORE (ch), LAST_IMCIGNORE (ch), next,
			 prev);
	      IMCSTRFREE (ign->name);
	      IMCDISPOSE (ign);
  ...
  if (!strcasecmp (arg, "help"))
    {
      imc_to_char ("~wTo see the current bans             : ~GIMCBAN\r\n",
		   ch);
      imc_to_char
	("~wTo add a MUD to the ban list        : ~GIMCBAN ADD <argument>\r\n",
	 ch);
      imc_to_char
	("~wTo delete a MUD from the ban list   : ~GIMCBAN DELETE <argument>\r\n",
	 ch);
      imc_to_char ("~WSee your MUD's help for more information.\r\n", ch);
      return;
    }
...
IMC_CMD (imc_deny_channel)
{
  ...
  if (!(channel = imc_findchannel (argument)))
    {
      imc_to_char
	("Unknown or unconfigured local channel. Check your channel name.\r\n",
	 ch);
      return;
    }
...
IMC_CMD (imcpermstats)
{
  ...
  imc_printf (ch, "~gThese permissions were obtained %s.\r\n",
	      IMCIS_SET (IMCFLAG (victim),
			 IMC_PERMOVERRIDE) ? "manually via imcpermset" :
  ...
  if (permvalue == -1)
    {
      ...
      imc_printf (ch,
		  "~YPermission flag override has been removed from %s\r\n",
		  CH_IMCNAME (victim));
      return;
    }
	  ...
	  if (channel && IMCPERM (victim) < channel->level)
	    {
	      ...
	      imc_printf (ch,
			  ...
			  imcperm_names[channel->level], channel->local_name,
			  imcperm_names[IMCPERM (victim)]);
	    }
	}
    }
...
IMC_CMD (imcchanwho)
{
  ...
  if (!argument || argument[0] == '\0')
    {
      imc_to_char
	("Usage: imcchanwho <channel> [<mud> <mud> <mud> <...>|<all>]\r\n",
	 ch);
      return;
    }
...
IMC_CMD (imcremoteadmin)
{
  ...
  if (server[0] == '\0' || cmd[0] == '\0')
    {
      imc_to_char
	("Syntax: imcadmin <server> <password> <command> [<data..>]\r\n", ch);
      imc_to_char
	("You must be an approved server administrator to use remote commands.\r\n",
	 ch);
      return;
    }
      ...
      for (perm = IMCPERM_MORT; perm <= IMCPERM (ch); perm++)
	{
	  ...
	  for (help = first_imc_help; help; help = help->next)
		...
		continue;
	      ...
	      snprintf (buf + strlen (buf), LGST - strlen (buf), "%-15s",
			help->name);
	      if (++col % 6 == 0)
		imcstrlcat (buf, "\r\n", LGST);
	    }
...
IMC_CMD (imc_show_ucache_contents)
{
  ...
  imc_to_pager
    ("User                          | Gender ( 0 = Male, 1 = Female, 2 = Other )\r\n",
     ch);
  imc_to_pager
    ("--------------------------------------------------------------------------\r\n",
     ch);
...
IMC_CMD (imccedit)
{
  ...
  if (!strcasecmp (option, "alias"))
      ...
      for (alias = cmd->first_alias; alias; alias = alias_next)
	{
	  ...
	  if (!strcasecmp (alias->name, argument))
	    {
	      imc_printf (ch,
			  "~W%s ~ghas been removed as an alias for ~W%s\r\n",
			  argument, cmd->name);
	      IMCUNLINK (alias, cmd->first_alias, cmd->last_alias, next,
			 prev);
	      IMCSTRFREE (alias->name);
  ...
  if (!strcasecmp (option, "show"))
      ...
      if (cmd->first_alias)
	{
	  ...
	  for (alias = cmd->first_alias; alias; alias = alias->next)
	    {
	      snprintf (buf + strlen (buf), LGST - strlen (buf), "%s ",
			alias->name);
	      if (++col % 10 == 0)
		imcstrlcat (buf, "\r\n", LGST);
	    }
  ...
  if (name[0] == '\0' || cmd[0] == '\0' || !argument || argument[0] == '\0')
    {
      ...
      imc_to_char
	("Where <field> can be either name, or permission level.\r\n", ch);
      return;
    }
...
const char *
imc_send_social (CHAR_DATA * ch, const char *argument, int telloption)
{
  return "";
}
#else
const char *
imc_find_social (CHAR_DATA * ch, const char *sname, const char *person,
		 const char *mud, int victim)
{
  ...
  if (person && person[0] != '\0' && mud && mud[0] != '\0')
    {
	  ...
	  && mud && mud[0] != '\0'
	  && !strcasecmp (mud, this_imcmud->localname))
	{
  ...
  for (cmd = 0; social_table[cmd].name[0] != '\0'; cmd++)
    {
	  ...
	  && !imcstr_prefix (sname, social_table[cmd].name))
	{
	  found = TRUE;
	  break;
	}
...
char *
imc_act_string (const char *format, CHAR_DATA * ch, CHAR_DATA * vic)
{
  ...
  while (*format != '\0')
      ...
      else
	{
	  switch (*format)
	    ...
	    case 'e':
	      i = should_upper ?
		imccapitalize (he_she[URANGE (0, CH_IMCSEX (ch), 2)]) :
		he_she[URANGE (0, CH_IMCSEX (ch), 2)];
	      break;

	    case 'E':
	      i = should_upper ?
		imccapitalize (he_she[URANGE (0, CH_IMCSEX (vic), 2)]) :
		he_she[URANGE (0, CH_IMCSEX (vic), 2)];
	      break;

	    case 'm':
	      i = should_upper ?
		imccapitalize (him_her[URANGE (0, CH_IMCSEX (ch), 2)]) :
		him_her[URANGE (0, CH_IMCSEX (ch), 2)];
	      break;

	    case 'M':
	      i = should_upper ?
		imccapitalize (him_her[URANGE (0, CH_IMCSEX (vic), 2)]) :
		him_her[URANGE (0, CH_IMCSEX (vic), 2)];
	      break;

	    case 's':
	      i = should_upper ?
		imccapitalize (his_her[URANGE (0, CH_IMCSEX (ch), 2)]) :
		his_her[URANGE (0, CH_IMCSEX (ch), 2)];
	      break;

	    case 'S':
	      i = should_upper ?
		imccapitalize (his_her[URANGE (0, CH_IMCSEX (vic), 2)]) :
		his_her[URANGE (0, CH_IMCSEX (vic), 2)];
	      break;
...
CHAR_DATA *
imc_make_skeleton (const char *name)
{
  CHAR_DATA *skeleton;
...
void
imc_purge_skeleton (CHAR_DATA * skeleton)
{
  if (!skeleton)
    return;
...
imc_send_social (CHAR_DATA * ch, const char *argument, int telloption)
{
  ...
  if (telloption == 0)
    {
      snprintf (socbuf, LGST, "%s",
		imc_find_social (ch, arg1, person, mud, 0));
      if (socbuf[0] == '\0')
	return "";
    }
  ...
  if (telloption == 1)
    {
      snprintf (socbuf, LGST, "%s",
		imc_find_social (ch, arg1, person, mud, 1));
      if (socbuf[0] == '\0')
	return "";
    }
  ...
  if (telloption == 2)
    {
      snprintf (socbuf, LGST, "%s",
		imc_find_social (ch, arg1, person, mud, 2));
      if (socbuf[0] == '\0')
	return "";
    }
...
bool
imc_command_hook (CHAR_DATA * ch, const char *command, const char *argument)
{
  IMC_CMD_DATA *cmd;
  IMC_ALIAS *alias;
  IMC_CHANNEL *c;
  ...
  for (cmd = first_imc_command; cmd; cmd = cmd->next)
      ...
      if (!strcasecmp (command, cmd->name))
	{
	  ...
	  if (cmd->function == NULL)
	    {
	      imc_to_char
		("That command has no code set. Inform the administration.\r\n",
		 ch);
	      imcbug ("imc_command_hook: Command %s has no code set!",
		      cmd->name);
	      return TRUE;
	    }
  ...
  if (!c->refreshed)
    {
      imc_printf (ch,
		  "The %s channel has not yet been refreshed by the server.\r\n",
		  c->local_name);
      return TRUE;
    }
