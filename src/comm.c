 ...
 */
bool
chk_watch (short player_level, const char *player_name,
	   const char *player_site)
{
  WATCH_DATA *pw;
/*
...
#ifdef WIN32
	    Sleep ((stall_time.tv_sec * 1000L) +
		   (stall_time.tv_usec / 1000L));
#else
  ...
  if ((desc = accept (new_desc, (struct sockaddr *) &sock, &size)) < 0)
    {
      ...
      log_printf_plus (LOG_COMM, sysdata.log_level, "%s",
		       "[*****] BUG: New_descriptor: accept");
      set_alarm (0);
      return;
    }
  if (check_bad_desc (new_desc))
...
close_socket (DESCRIPTOR_DATA * dclose, bool force)
{
  ...
  if (!dclose->prev && dclose != first_descriptor)
      ...
      for (d = first_descriptor; d; d = dn)
	{
      ...
      if (!dclose->prev)
	{
	  bug ("%s: %s desc:%p could not be found!.", __FUNCTION__,
  ...
  if (!dclose->next && dclose != last_descriptor)
      ...
      for (d = last_descriptor; d; d = dp)
	{
	  ...
	  if (d == dclose)
	    {
	      bug ("%s: %s desc:%p found, next should be:%p, fixing.",
		   __FUNCTION__, ch ? ch->name : d->host, (void *) dclose,
		   (void *) dn);
	      dclose->next = dn;
	      break;
	    }
  ...
  if (dclose->character)
    {
      log_printf_plus (LOG_COMM, UMAX (sysdata.log_level, ch->level),
		       "Closing link to %s.", ch->pcdata->filename);
  ...
  for (;;)
    {
      ...
      nRead =
	recv (d->descriptor, d->inbuf + iStart,
...
flush_buffer (DESCRIPTOR_DATA * d, bool fPrompt)
{
  ...
  if (!mud_down && d->outtop > 4096)
      ...
      if (d->snoop_by)
	{
	  ...
	  if (d->character && d->character->name)
		...
		snprintf (snoopbuf, MAX_INPUT_LENGTH, "%s (%s)",
			  d->character->name, d->original->name);
	      else
		snprintf (snoopbuf, MAX_INPUT_LENGTH, "%s",
			  d->character->name);
  ...
  if (d->snoop_by)
      ...
      if (d->character && d->character->name)
	{
	   ...
	   */
	  if (d->original && d->original->name)
	    snprintf (buf, MAX_INPUT_LENGTH, "%s (%s)", d->character->name,
		      d->original->name);
	  else
 ...
 */
void
write_to_buffer (DESCRIPTOR_DATA * d, const char *txt, size_t length)
{
  if (!d)
    {
  ...
  if (d && d->mccp->out_compress)
      ...
      while (d->mccp->out_compress->avail_in)
	{
	  d->mccp->out_compress->avail_out =
	    COMPRESS_BUF_SIZE - (d->mccp->out_compress->next_out -
				 d->mccp->out_compress_buf);
	  ...
	  if (len > 0)
	      ...
	      for (iStart = 0; iStart < len; iStart += nWrite)
		{
		  ...
		  nWrite =
		    send (d->descriptor, d->mccp->out_compress_buf + iStart,
			  nBlock, 0);
		  if (nWrite == -1)
		    {
		      iErr = errno;
		...
		break;
	      ...
	      if (iStart < len)
		memmove (d->mccp->out_compress_buf,
			 d->mccp->out_compress_buf + iStart, len - iStart);

	      d->mccp->out_compress->next_out =
		d->mccp->out_compress_buf + len - iStart;
	    }
	}
      return TRUE;
    }
 ...
 */
bool
write_to_descriptor_old (int desc, const char *txt, int length)
{
  int iStart = 0;
  int nWrite = 0;
  int nBlock = 0;
...
void
nanny_get_name (DESCRIPTOR_DATA * d, char *argument)
{
  CHAR_DATA *ch;
  bool fOld, chk;
  char buf[MAX_STRING_LENGTH];
  ...
  if (xIS_SET (ch->act, PLR_DENY))
    {
      log_printf_plus (LOG_COMM, sysdata.log_level,
  ...
  if (IS_IMMORTAL (ch) && sysdata.check_imm_host
    ...
    {
      log_printf_plus (LOG_COMM, sysdata.log_level,
		       "%s's char being hacked from %s.", argument, d->host);
  ...
  if (fOld)
      ...
      if (d->newstate == 0)
	{
      ...
      snprintf (buf, MAX_STRING_LENGTH, "Did I get that right, %s (Y/N)? ",
		argument);
...
void
nanny_get_old_password (DESCRIPTOR_DATA * d, char *argument)
{
  CHAR_DATA *ch;
  char buf[MAX_STRING_LENGTH];
  bool fOld, chk;
...
void
nanny_confirm_new_name (DESCRIPTOR_DATA * d, char *argument)
{
  CHAR_DATA *ch;
  char buf[MAX_STRING_LENGTH];
...
void
nanny_get_new_password (DESCRIPTOR_DATA * d, char *argument)
{
  CHAR_DATA *ch;
  char *pwdnew;
...
void
nanny_confirm_new_password (DESCRIPTOR_DATA * d, char *argument)
{
  CHAR_DATA *ch;
...
void
nanny_get_new_sex (DESCRIPTOR_DATA * d, char *argument)
{
  CHAR_DATA *ch;
  char buf[MAX_STRING_LENGTH];
  int iClass;
...
void
nanny_get_new_class (DESCRIPTOR_DATA * d, const char *argument)
{
  CHAR_DATA *ch;
  char buf[MAX_STRING_LENGTH];
...
void
nanny_get_new_race (DESCRIPTOR_DATA * d, const char *argument)
{
  CHAR_DATA *ch;
  char arg[MAX_STRING_LENGTH];
  int iRace;
  ...
  if (!str_cmp (arg, "help"))
	...
	{
	  if (toupper (argument[0]) ==
	      toupper (race_table[iRace]->race_name[0])
      ...
      write_to_buffer (d, "No help on that topic.  Please choose a race: ",
		       0);
      return;
    }
  ...
  if (iRace == MAX_PC_RACE
      || !race_table[iRace]->race_name
      || race_table[iRace]->race_name[0] == '\0' || iRace == RACE_VAMPIRE
      || IS_SET (race_table[iRace]->class_restriction, 1 << ch->Class)
      || !str_cmp (race_table[iRace]->race_name, "unused"))
    {
...
void
nanny_get_want_ripansi (DESCRIPTOR_DATA * d, const char *argument)
{
  CHAR_DATA *ch;
  char log_buf[MAX_STRING_LENGTH];
...
void
nanny_press_enter (DESCRIPTOR_DATA * d, const char *argument)
{
  CHAR_DATA *ch;
...
void
nanny_read_motd (DESCRIPTOR_DATA * d, const char *argument)
{
  CHAR_DATA *ch;
  char buf[MAX_STRING_LENGTH];
  ch = d->character;
  ...
  if (ch->level == 0)
	  ...
	  for (iLang = 0; lang_array[iLang] != LANG_UNKNOWN; iLang++)
	      ...
	      if (IS_SET (race_table[ch->race]->language, 1 << iLang))
		{
		  if ((uLang = skill_lookup (lang_names[iLang])) < 0)
		    bug ("%s: cannot find racial language [%s].",
			 __FUNCTION__, lang_names[iLang]);
		  else
    ...
    {
      if (ch->in_room->vnum == 6 || ch->in_room->vnum == 8
  ...
  if (ch->pcdata->pet)
    {
      ...
      act (AT_ACTION, "$N returns with you to the realms.", ch, NULL,
	   ch->pcdata->pet, TO_CHAR);
    }
  do_look (ch, "auto");
  mail_count (ch);
 ...
 */
bool
check_parse_name (const char *name, bool newchar)
{
  /*
 ...
 */
bool
check_reconnect (DESCRIPTOR_DATA * d, const char *name, bool fConn)
{
  CHAR_DATA *ch;
	  ...
	  if (fConn == FALSE)
	  ...
	  else
	    {
	      ...
	      act (AT_ACTION, "$n has reconnected.", ch, NULL, NULL,
		   TO_CANSEE);
	      log_printf_plus (LOG_COMM, UMAX (sysdata.log_level, ch->level),
			       "%s (%s) reconnected.", ch->name, d->host);
	      d->connected = CON_PLAYING;
	      do_look (ch, "auto");
	    }
	  return TRUE;
 ...
 */
bool
check_playing (DESCRIPTOR_DATA * d, const char *name, bool kick)
{
  CHAR_DATA *ch;
  ...
  for (dold = first_descriptor; dold; dold = dold->next)
    {
	  ...
	  && !str_cmp (name,
		       ...
		       pcdata->filename : dold->character->pcdata->filename))
	{
	  ...
	  if (!ch->name || (cstate != CON_PLAYING && cstate != CON_EDITING))
	    {
	      ...
	      log_printf_plus (LOG_COMM, sysdata.log_level,
			       "%s already connected.", ch->pcdata->filename);
	      return BERR;
	    }
	  if (!kick)
	    return TRUE;
	  ...
	  send_to_char ("Reconnecting.\r\n", ch);
	  do_look (ch, "auto");
	  act (AT_ACTION, "$n has reconnected, kicking off old link.", ch,
	       NULL, NULL, TO_CANSEE);
	  log_printf_plus (LOG_COMM, UMAX (sysdata.log_level, ch->level),
...
obj_short (OBJ_DATA * obj)
{
  ...
  if (obj->count > 1)
    {
      snprintf (buf, MAX_STRING_LENGTH, "%s (%d)", obj->short_descr,
		obj->count);
      return buf;
    }
  return obj->short_descr;
      ...
      else
	{
	  switch (*str)
		...
		i = should_upper ?
		  !can_see (to,
			    ch) ? "It" :
		  capitalize (he_she[URANGE (0, ch->sex, 2)]) : !can_see (to,
									  ch)
		  ? "it" : he_she[URANGE (0, ch->sex, 2)];
	      break;
		...
		i = should_upper ?
		  !can_see (to,
			    vch) ? "It" :
		  capitalize (he_she[URANGE (0, vch->sex, 2)]) : !can_see (to,
									   vch)
		  ? "it" : he_she[URANGE (0, vch->sex, 2)];
	      break;
		...
		i = should_upper ?
		  !can_see (to,
			    ch) ? "It" :
		  capitalize (him_her[URANGE (0, ch->sex, 2)]) : !can_see (to,
									   ch)
		  ? "it" : him_her[URANGE (0, ch->sex, 2)];
	      break;
		...
		i = should_upper ?
		  !can_see (to,
			    vch) ? "It" :
		  capitalize (him_her[URANGE (0, vch->sex, 2)]) :
		  !can_see (to,
			    vch) ? "it" : him_her[URANGE (0, vch->sex, 2)];
	      break;
		...
		i =
		  should_upper ? ((!to || can_see_obj (to, obj1)) ?
				  capitalize (obj_short (obj1)) : "Something")
		  : ((!to
		      || can_see_obj (to,
				      obj1)) ? obj_short (obj1) :
		     "something");
		...
		i = should_upper ?
		  !can_see (to,
			    ch) ? "It" :
		  capitalize (his_her[URANGE (0, ch->sex, 2)]) : !can_see (to,
									   ch)
		  ? "it" : his_her[URANGE (0, ch->sex, 2)];
	      break;
		...
		i = should_upper ?
		  !can_see (to,
			    vch) ? "It" :
		  capitalize (his_her[URANGE (0, vch->sex, 2)]) :
		  !can_see (to,
			    vch) ? "it" : his_her[URANGE (0, vch->sex, 2)];
	      break;
...
void
act (short AType, const char *format, CHAR_DATA * ch, const void *arg1,
...
#define ACTF_OBJ  BV02
  ...
  if (flags1 != ACTF_NONE && flags1 != ACTF_TXT && flags1 != ACTF_CH
      && flags1 != ACTF_OBJ)
    {
      bug ("%s: arg1 has more than one type in format %s. Setting all NULL.",
	   __FUNCTION__, format);
      obj1 = NULL;
    }

  if (flags2 != ACTF_NONE && flags2 != ACTF_TXT && flags2 != ACTF_CH
      && flags2 != ACTF_OBJ)
    {
      bug ("%s: arg2 has more than one type in format %s. Setting all NULL.",
	   __FUNCTION__, format);
      vch = NULL;
      obj2 = NULL;
    }
...
void
do_name (CHAR_DATA * ch, const char *argument)
{
  ...
  snprintf (fname, 1024, "%s%c/%s", PLAYER_DIR, tolower (ucase_argument[0]),
	    capitalize (ucase_argument));
  if (stat (fname, &fst) != -1)
    {
      send_to_char ("That name is already taken.  Please choose another.\r\n",
		    ch);
...
display_prompt (DESCRIPTOR_DATA * d)
{
  ...
  if (!IS_NPC (ch) && !IS_SET (ch->pcdata->flags, PCFLAG_HELPSTART))
    ...
    prompt = ch->pcdata->subprompt;
	   ...
	   || (!ch->fighting
	       && (!ch->pcdata->prompt || !*ch->pcdata->prompt)))
	  ...
	  *pbuf = '\0';
	  ...
	  switch (*prompt)
		...
		mudstrlcpy (pbuf, "neutral", MAX_STRING_LENGTH);
	      break;
	    case 'A':
	      snprintf (pbuf, MAX_STRING_LENGTH, "%s%s%s",
		...
		mudstrlcpy (pbuf, "N/A", MAX_STRING_LENGTH);
		       ...
		       || (victim = victim->fighting->who) == NULL)
		mudstrlcpy (pbuf, "N/A", MAX_STRING_LENGTH);
	      else
		{
		  if (victim->max_hit > 0)
		  ...
		  if (percent >= 100)
		    ...
		    mudstrlcpy (pbuf, "slightly scratched",
				MAX_STRING_LENGTH);
		  else if (percent >= 80)
	    ...
	    case 'F':
	      if (IS_IMMORTAL (och))
		snprintf (pbuf, MAX_STRING_LENGTH, "%s",
			  ext_flag_string (&ch->in_room->room_flags,
					   r_flags));
	      break;
	    case 'R':
	      if (xIS_SET (och->act, PLR_ROOMVNUM))
		snprintf (pbuf, MAX_STRING_LENGTH, "<#%d> ",
			  ch->in_room->vnum);
	      break;
	    case 'x':
	      pstat = ch->exp;
	    ...
	    case 'I':
	      pstat =
		(IS_NPC (ch)
		 ? (xIS_SET (ch->act, ACT_MOBINVIS) ? ch->mobinvis : 0)
		 : (xIS_SET (ch->act, PLR_WIZINVIS) ? ch->
		    pcdata->wizinvis : 0));
	      break;
	    }
	  if (pstat != 0x80000000)
