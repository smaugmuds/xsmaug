  ...
  if (ch->fighting && number_percent () > (get_curr_dex (ch) * 2 + 47))
    {
      ...
      if (!hgflag)
	act (AT_MAGIC, "Oops, $p slips from your hand and $T!", ch, obj, buf,
	     TO_CHAR);
    }
  else
    {
	  ...
	  if (obj->value[3] != 0
	      ...
	      if (obj->value[3] != 0)
		{
		  act (AT_POISON, "$n chokes and gags.", ch, NULL, NULL,
		       TO_ROOM);
		  act (AT_POISON, "You choke and gag.", ch, NULL, NULL,
		       TO_CHAR);
	      ...
	      else
		{
	      ...
	      af.type = gsn_poison;
	      af.duration =
		2 * obj->value[0] * (obj->value[3] > 0 ? obj->value[3] : 1);
...
do_quaff (CHAR_DATA * ch, const char *argument)
{
  ...
  if (ch->fighting && number_percent () > (get_curr_dex (ch) * 2 + 48))
    {
      act (AT_MAGIC, "$n fumbles $p and shatters it into fragments.", ch, obj,
	   NULL, TO_ROOM);
      if (!hgflag)
	act (AT_MAGIC, "Oops... $p is knocked from your hand and shatters!",
	     ch, obj, NULL, TO_CHAR);
    }
  else
    {
      if (!oprog_use_trigger (ch, obj, NULL, NULL))
	{
	  if (!CAN_PKILL (ch) || !obj->in_obj)
	    ...
	    {
	      act (AT_ACTION, "$n quaffs $p from $P.", ch, obj, obj->in_obj,
		   TO_ROOM);
	      if (!hgflag)
		act (AT_ACTION, "You quaff $p from $P.", ch, obj, obj->in_obj,
		     TO_CHAR);
	    }
	}
	...
	act (AT_ACTION, "Your stomach is nearing its capacity.", ch, NULL,
	     NULL, TO_CHAR);
...
do_recite (CHAR_DATA * ch, const char *argument)
{
  ...
  if (scroll->item_type != ITEM_SCROLL)
    {
      act (AT_ACTION, "$n holds up $p as if to recite something from it...",
	   ch, scroll, NULL, TO_ROOM);
      act (AT_ACTION,
	   "You hold up $p and stand there with your mouth open.  (Now what?)",
 ...
 */
void
pullorpush (CHAR_DATA * ch, OBJ_DATA * obj, bool pull)
{
  char buf[MAX_STRING_LENGTH];
  CHAR_DATA *rch;
  bool isup;
  ...
  if (!oprog_use_trigger (ch, obj, NULL, NULL))
    {
      snprintf (buf, MAX_STRING_LENGTH, "$n %s $p.",
		pull ? "pulls" : "pushes");
  ...
  if (IS_SET (obj->value[0], TRIG_DEATH))
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "%s hit a DEATH TRIGGER in room %d!",
  ...
  if (IS_SET (obj->value[0], TRIG_OLOAD))
	...
	{
	  bug ("%s: obj points to invalid object vnum %d", __FUNCTION__,
	       obj->value[1]);
	  return;
	}
      /*
	...
	{
	  bug ("%s: obj points to invalid room vnum %d", __FUNCTION__,
	       obj->value[2]);
	  return;
	}
      /*
      ...
      if (!
	  (tobj =
	   create_object (pObjIndex, URANGE (0, obj->value[3], MAX_LEVEL))))
	{
	  bug ("%s: obj couldnt create_obj vnum %d at level %d", __FUNCTION__,
  ...
  if (IS_SET (obj->value[0], TRIG_MLOAD))
	...
	{
	  bug ("%s: obj points to invalid mob vnum %d", __FUNCTION__,
	       obj->value[1]);
	  return;
	}
      /*
	...
	{
	  bug ("%s: obj couldnt create_mobile vnum %d", __FUNCTION__,
	       obj->value[1]);
	  return;
	}
  ...
  if (IS_SET (obj->value[0], TRIG_CAST))
	...
	{
	  bug ("%s: obj points to invalid sn [%d]", __FUNCTION__,
	       obj->value[1]);
	  return;
	}
      obj_cast_spell (obj->value[1],
		      URANGE (1,
			      (obj->value[2] > 0) ? obj->value[2] : ch->level,
			      MAX_LEVEL), ch, ch, NULL);
      return;
    }
  ...
  if (IS_SET (obj->value[0], TRIG_CONTAINER))
	...
	room = obj->in_room;
      if (!room)
	{
	  bug ("%s: obj points to invalid room %d", __FUNCTION__,
	       obj->value[1]);
	  return;
	}

      for (container = ch->in_room->first_content; container;
	...
	{
	  bug
	    ("%s: obj points to a container [%d] thats not where it should be?",
	     __FUNCTION__, obj->value[2]);
	  return;
	}
      if (container->item_type != ITEM_CONTAINER)
	{
	  bug ("%s: obj points to object [%d], but it isn't a container.",
	       __FUNCTION__, obj->value[2]);
	  return;
	}
      /*
  ...
  if (IS_SET (obj->value[0], TRIG_DOOR))
	  ...
	  && IS_SET (pexit->exit_info, EX_LOCKED))
	{
	  ...
	  act (AT_PLAIN, "You hear a faint click $T.", ch, NULL, txt,
	       TO_CHAR);
	  act (AT_PLAIN, "You hear a faint click $T.", ch, NULL, txt,
	       TO_ROOM);
	  ...
	  && IS_SET (pexit->exit_info, EX_CLOSED))
	{
	  ...
	  for (rch = room->first_person; rch; rch = rch->next_in_room)
	    act (AT_ACTION, "The $d opens.", rch, NULL, pexit->keyword,
		 TO_CHAR);
	  if ((pexit_rev = pexit->rexit) != NULL
	      && pexit_rev->to_room == ch->in_room)
	    {
	  ...
	  && !IS_SET (pexit->exit_info, EX_CLOSED))
	{
	  ...
	  for (rch = room->first_person; rch; rch = rch->next_in_room)
	    act (AT_ACTION, "The $d closes.", rch, NULL, pexit->keyword,
		 TO_CHAR);
	  if ((pexit_rev = pexit->rexit) != NULL
	      && pexit_rev->to_room == ch->in_room)
	    {
...
void
do_pull (CHAR_DATA * ch, const char *argument)
{
...
void
do_push (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
...
void
do_rap (CHAR_DATA * ch, const char *argument)
{
  EXIT_DATA *pexit;
  char arg[MAX_INPUT_LENGTH];
  ...
  if ((pexit = find_door (ch, arg, FALSE)) != NULL)
	...
	keyword = pexit->keyword;
      act (AT_ACTION, "You rap loudly on the $d.", ch, NULL, keyword,
	   TO_CHAR);
      act (AT_ACTION, "$n raps loudly on the $d.", ch, NULL, keyword,
	   TO_ROOM);
	  ...
	  && pexit_rev->to_room == ch->in_room)
	{
	  ...
	  for (rch = to_room->first_person; rch; rch = rch->next_in_room)
	    {
	      act (AT_ACTION,
		   "Someone raps loudly from the other side of the $d.", rch,
		   NULL, pexit_rev->keyword, TO_CHAR);
	    }
	}
    }
  else
    {
      act (AT_ACTION, "You make knocking motions through the air.", ch, NULL,
	   NULL, TO_CHAR);
      act (AT_ACTION, "$n makes knocking motions through the air.", ch, NULL,
	   NULL, TO_ROOM);
    }
  return;
}
...
void
do_tamp (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *opipe;
  char arg[MAX_INPUT_LENGTH];
...
void
do_smoke (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *opipe;
  char arg[MAX_INPUT_LENGTH];
  ...
  if (opipe->item_type != ITEM_PIPE)
    {
      act (AT_ACTION, "You try to smoke $p... but it doesn't seem to work.",
	   ch, opipe, NULL, TO_CHAR);
      act (AT_ACTION,
	   "$n tries to smoke $p... (I wonder what $e's been putting in $s pipe?)",
	   ch, opipe, NULL, TO_ROOM);
      return;
    }
  if (!IS_SET (opipe->value[3], PIPE_LIT))
    {
      act (AT_ACTION, "You try to smoke $p, but it's not lit.", ch, opipe,
	   NULL, TO_CHAR);
      act (AT_ACTION, "$n tries to smoke $p, but it's not lit.", ch, opipe,
	   NULL, TO_ROOM);
      return;
    }
  if (opipe->value[1] > 0)
	...
	{
	  act (AT_ACTION, "You draw thoughtfully from $p.", ch, opipe, NULL,
	       TO_CHAR);
	  act (AT_ACTION, "$n draws thoughtfully from $p.", ch, opipe, NULL,
	       TO_ROOM);
	}
      ...
      if (IS_VALID_HERB (opipe->value[2]) && opipe->value[2] < top_herb)
	{
	  ...
	  if (skill->spell_fun)
	    obj_cast_spell (sn, UMIN (skill->min_level, ch->level), ch, ch,
			    NULL);
...
void
do_light (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *opipe;
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!IS_SET (opipe->value[3], PIPE_LIT))
    {
      if (opipe->value[1] < 1)
	{
	  act (AT_ACTION, "You try to light $p, but it's empty.", ch, opipe,
	       NULL, TO_CHAR);
	  act (AT_ACTION, "$n tries to light $p, but it's empty.", ch, opipe,
	       NULL, TO_ROOM);
	  return;
	}
 ...
 */
void
do_apply (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  if (salve->item_type != ITEM_SALVE)
    {
      if (victim == ch)
	{
	  act (AT_ACTION, "$n starts to rub $p on $mself...", ch, salve, NULL,
	       TO_ROOM);
	  act (AT_ACTION, "You try to rub $p on yourself...", ch, salve, NULL,
	       TO_CHAR);
	}
      else
	{
	  act (AT_ACTION, "$n starts to rub $p on $N...", ch, salve, victim,
	       TO_NOTVICT);
	  act (AT_ACTION, "$n starts to rub $p on you...", ch, salve, victim,
	       TO_VICT);
	  act (AT_ACTION, "You try to rub $p on $N...", ch, salve, victim,
	       TO_CHAR);
	}
      return;
    }
	  ...
	  if (salve->value[1] < 1)
	    {
	      if (victim != ch)
		{
		  act (AT_ACTION, "$n rubs the last of $p onto $N.", ch,
		       salve, victim, TO_NOTVICT);
		  act (AT_ACTION, "$n rubs the last of $p onto you.", ch,
		       salve, victim, TO_VICT);
		  act (AT_ACTION, "You rub the last of $p onto $N.", ch,
		       salve, victim, TO_CHAR);
		}
	      else
		{
		  act (AT_ACTION, "You rub the last of $p onto yourself.", ch,
		       salve, NULL, TO_CHAR);
		  act (AT_ACTION, "$n rubs the last of $p onto $mself.", ch,
		       salve, NULL, TO_ROOM);
		}
	    }
	  else
	    {
	      if (victim != ch)
		{
		  act (AT_ACTION, "$n rubs $p onto $N.", ch, salve, victim,
		       TO_NOTVICT);
		  act (AT_ACTION, "$n rubs $p onto you.", ch, salve, victim,
		       TO_VICT);
		  act (AT_ACTION, "You rub $p onto $N.", ch, salve, victim,
		       TO_CHAR);
		}
	      else
		{
		  act (AT_ACTION, "You rub $p onto yourself.", ch, salve,
		       NULL, TO_CHAR);
		  act (AT_ACTION, "$n rubs $p onto $mself.", ch, salve, NULL,
		       TO_ROOM);
		}
	    }
	}
  ...
  retcode =
    obj_cast_spell (salve->value[4], salve->value[0], ch, victim, NULL);
  if (retcode == rNONE)
    retcode =
      obj_cast_spell (salve->value[5], salve->value[0], ch, victim, NULL);
...
void
actiondesc (CHAR_DATA * ch, OBJ_DATA * obj)
{
 ...
 */
void
fwrite_bitvector (EXT_BV * bits, FILE * fp)
{
