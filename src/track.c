...
void
bfs_enqueue (ROOM_INDEX_DATA * room, char dir)
{
  BFS_DATA *curr;
...
void
room_enqueue (ROOM_INDEX_DATA * room)
{
  BFS_DATA *curr;
...
int
find_first_step (ROOM_INDEX_DATA * src, ROOM_INDEX_DATA * target, int maxdist)
{
  int curr_dir, count;
  EXIT_DATA *pexit;
  ...
  while (queue_head)
	...
	{
	  for (pexit = queue_head->room->first_exit; pexit;
	       pexit = pexit->next)
	    if (valid_edge (pexit))
	      {
		curr_dir = pexit->vdir;
...
void
do_track (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *vict;
...
void
found_prey (CHAR_DATA * ch, CHAR_DATA * victim)
{
  ...
  mudstrlcpy (victname, IS_NPC (victim) ? victim->short_descr : victim->name,
	      MAX_STRING_LENGTH);
  ...
  if (!can_see (ch, victim))
	...
	{
	case 0:
	  snprintf (buf, MAX_STRING_LENGTH, "Don't make me find you, %s!",
		    victname);
	  do_say (ch, buf);
	  break;
	case 1:
	  ...
	  break;
	case 2:
	  snprintf (buf, MAX_STRING_LENGTH, "I'm going to tear %s apart!",
		    victname);
	  do_yell (ch, buf);
	  break;
	case 3:
  ...
  if (xIS_SET (ch->in_room->room_flags, ROOM_SAFE))
	...
	case 0:
	  ...
	  snprintf (buf, MAX_STRING_LENGTH, "%s is a bloody coward!",
		    victname);
	  do_yell (ch, buf);
	  break;
	case 1:
	  snprintf (buf, MAX_STRING_LENGTH, "Let's take this outside, %s",
		    victname);
	  do_say (ch, buf);
	  break;
	case 2:
	  snprintf (buf, MAX_STRING_LENGTH, "%s is a yellow-bellied wimp!",
		    victname);
	  do_yell (ch, buf);
	  break;
	case 3:
  ...
  switch (number_bits (2))
    ...
    case 2:
      snprintf (buf, MAX_STRING_LENGTH,
		"What do you want on your tombstone, %s?", victname);
      do_say (ch, buf);
      break;
    case 3:
      act (AT_ACTION, "$n lunges at $N from out of nowhere!", ch, NULL,
	   victim, TO_NOTVICT);
      act (AT_ACTION, "You lunge at $N catching $M off guard!", ch, NULL,
	   victim, TO_CHAR);
      act (AT_ACTION, "$n lunges at you from out of nowhere!", ch, NULL,
	   victim, TO_VICT);
    }
  stop_hunting (ch);
...
hunt_victim (CHAR_DATA * ch)
{
  ...
  ret =
    find_first_step (ch->in_room, ch->hunting->who->in_room,
		     500 + ch->level * 25);
  if (ret < 0)
    {
