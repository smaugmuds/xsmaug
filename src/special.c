...
bool
validate_spec_fun (const char *name)
{
  SPEC_LIST *specfun;
 ...
 */
bool
dragon (CHAR_DATA * ch, const char *fspell_name)
{
  CHAR_DATA *victim;
  CHAR_DATA *v_next;
  int sn;
...
spec_cast_adept (CHAR_DATA * ch)
{
  ...
  switch (number_bits (3))
    ...
    case 1:
      act (AT_MAGIC, "$n utters the word 'sunimod'.", ch, NULL, NULL,
	   TO_ROOM);
    ...
    case 7:
      ...
      spell_remove_curse (skill_lookup ("remove curse"), ch->level, ch,
			  victim);
      return TRUE;
...
spec_guard (CHAR_DATA * ch)
{
  ...
  if (victim)
    {
      snprintf (buf, MAX_STRING_LENGTH,
		"%s is a %s!  PROTECT THE INNOCENT!!  BANZAI!!", victim->name,
		crime);
      do_shout (ch, buf);
      multi_hit (ch, victim, TYPE_UNDEFINED);
      return TRUE;
  ...
  if (ech)
    {
      act (AT_YELL, "$n screams 'PROTECT THE INNOCENT!!  BANZAI!!", ch, NULL,
	   NULL, TO_ROOM);
...
spec_mayor (CHAR_DATA * ch)
{
  ...
  switch (path[pos])
    ...
    case 'W':
      ...
      act (AT_ACTION, "$n awakens and groans loudly.", ch, NULL, NULL,
	   TO_ROOM);
      break;

    case 'S':
      ch->position = POS_SLEEPING;
      act (AT_ACTION, "$n lies down and falls asleep.", ch, NULL, NULL,
	   TO_ROOM);
      break;
...
spec_thief (CHAR_DATA * ch)
{
  ...
  for (victim = ch->in_room->first_person; victim; victim = v_next)
	...
	{
	  act (AT_ACTION, "You discover $n's hands in your sack of gold!", ch,
	       NULL, victim, TO_VICT);
	  act (AT_ACTION, "$N discovers $n's hands in $S sack of gold!", ch,
      ...
      else
	{
	  ...
	  gold =
	    victim->gold * number_range (1,
					 URANGE (2, ch->level / 4, 10)) / 100;
	  ch->gold += 9 * gold / 10;
	  victim->gold -= gold;
	  if (ch->gold > maxgold)
	    {
      ...
      for (trash = ch->in_room->first_content; trash; trash = trash_next)
	{
	  ...
	  if (trash->item_type == ITEM_WEAPON
	      ...
	      || trash->item_type == ITEM_LIGHT)
	    {
	      ...
	      act (AT_ACTION, "$n leans over and gets $p.", ch, trash, NULL,
		   TO_ROOM);
	      obj_from_room (trash);
		...
		{
		  act (AT_ACTION,
		       "$n tries to use $p, but is too inexperienced.", ch,
		       trash, NULL, TO_ROOM);
		  thrown = TRUE;
		}
		  ...
		  while (!found && !noexit)
				       ...
				       ROOM_NODROP))
			{
			  if ((vch = pexit->to_room->first_person) != NULL)
			    for (vch = pexit->to_room->first_person; vch;
				 vch = vch->next_in_room)
			      {
				if (!str_cmp
				    (vch->spec_funname, "spec_wanderer"))
				  {
				    noexit = TRUE;
				    return FALSE;
				  }
	      ...
	      if (!noexit && thrown)
		{
		  ...
		  if (damage_obj (trash) != rOBJ_SCRAPPED)
		  ...
		  else
		    {
		      ...
		      act (AT_ACTION, "$n growls and breaks $p.", ch, trash,
			   NULL, TO_ROOM);
		    }
		  return TRUE;
		}
