...
void
do_gold (CHAR_DATA * ch, const char *argument)
{
...
void
do_worth (CHAR_DATA * ch, const char *argument)
{
  ...
  pager_printf (ch,
		"|Level: %-4d |Favor: %-10s |Alignment: %-9s |Experience: %-9d|\r\n",
  ...
  pager_printf (ch,
		"|Glory: %-4d |Weight: %-9d |Style: %-13s |Gold: %-14s |\r\n",
		ch->pcdata->quest_curr, ch->carry_weight, buf,
		num_punct (ch->gold));
  send_to_pager
    (" ----------------------------------------------------------------------------\r\n",
     ch);
 ...
 */
void
do_score (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  AFFECT_DATA *paf;
  int iLang;
  ...
  pager_printf (ch,
		"LEVEL: %-3d         Race : %-10.10s        Played: %ld hours\r\n",
		ch->level, capitalize (get_race (ch)),
		(long int) GET_TIME_PLAYED (ch));

  pager_printf (ch, "YEARS: %-6d      Class: %-11.11s       Log In: %s\r",
		calculate_age (ch), capitalize (get_class (ch)),
		ctime (&(ch->logon)));

  if (ch->level >= 15 || IS_PKILL (ch))
    {
      pager_printf (ch,
		    "STR  : %2.2d(%2.2d)    HitRoll: %-4d              Saved:  %s\r",
		    get_curr_str (ch), ch->perm_str, GET_HITROLL (ch),
		    ch->save_time ? ctime (&(ch->save_time)) :
		    "no save this session\n");

      pager_printf (ch,
		    "INT  : %2.2d(%2.2d)    DamRoll: %-4d              Time:   %s\r",
  ...
  if (ch->level < 10)
    ...
    pager_printf (ch,
		  "DEX  : %2.2d(%2.2d)      Align: %+4.4d, %-14.14s   Items: %5.5d   (max %5.5d)\r\n",
  ...
  pager_printf (ch,
		"CON  : %2.2d(%2.2d)      Pos'n: %-21.21s  Weight: %5.5d (max %7.7d)\r\n",
  ...
  pager_printf (ch,
		"PRACT: %3.3d         Hitpoints: %-5d of %5d   Pager: (%c) %3d    AutoExit(%c)\r\n",
  ...
  pager_printf (ch,
		"GOLD : %-13s    Move: %-5d of %5d   Mdeaths: %-5.5d    AutoSac (%c)\r\n",
  ...
  if (ch->first_affect)
      ...
      for (paf = ch->first_affect; paf; paf = paf->next)
	{
	  ...
	  if (ch->level >= 20)
	    {
	      if (paf->modifier == 0)
		pager_printf (ch, "[%-24.24s;%5d rds]    ", sktmp->name,
			      paf->duration);
 ...
 */
void
do_level (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  char buf2[MAX_STRING_LENGTH];
  int x, lowlvl, hilvl;
  ...
  ch_printf (ch,
	     ...
	     lowlvl, hilvl);
  snprintf (buf, MAX_STRING_LENGTH, " exp  (Current: %12s)",
	    num_punct (ch->exp));
  snprintf (buf2, MAX_STRING_LENGTH, " exp  (Needed:  %12s)",
...
/* 1997, Blodkai */
void
do_remains (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  OBJ_DATA *obj;
  bool found = FALSE;
  ...
  if (!ch->pcdata->deity)
    {
      send_to_pager
	("You have no deity from which to seek such assistance...\r\n", ch);
      return;
    }
...
void
do_affected (CHAR_DATA * ch, const char *argument)
{
...
void
do_inventory (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
void
do_equipment (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *obj;
  int iWear;
  bool found;
...
void
do_title (CHAR_DATA * ch, const char *argument)
{
  if (IS_NPC (ch))
    return;
...
void
do_homepage (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
 ...
 */
void
do_description (CHAR_DATA * ch, const char *argument)
{
  if (IS_NPC (ch))
    {
...
void
do_bio (CHAR_DATA * ch, const char *argument)
{
  if (IS_NPC (ch))
    {
 ...
 */
void
do_statreport (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  ...
  if (IS_VAMPIRE (ch))
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH,
		"$n reports: %d/%d hp %d/%d blood %d/%d mv %d xp.", ch->hit,
		ch->max_hit, ch->pcdata->condition[COND_BLOODTHIRST],
  ...
  else
    {
      ch_printf (ch, "You report: %d/%d hp %d/%d mana %d/%d mv %d xp.\r\n",
		 ...
		 ch->max_move, ch->exp);
      snprintf (buf, MAX_STRING_LENGTH,
		"$n reports: %d/%d hp %d/%d mana %d/%d mv %d xp.", ch->hit,
...
void
do_stat (CHAR_DATA * ch, const char *argument)
{
  if (IS_NPC (ch))
    {
      send_to_char ("Huh?\r\n", ch);
...
void
do_report (CHAR_DATA * ch, const char *argument)
{
  ...
  if (IS_VAMPIRE (ch))
    snprintf (buf, MAX_INPUT_LENGTH,
	      "$n reports: %d/%d hp %d/%d blood %d/%d mv %d xp.\r\n", ch->hit,
	      ch->max_hit, ch->pcdata->condition[COND_BLOODTHIRST],
	      10 + ch->level, ch->move, ch->max_move, ch->exp);
  else
    snprintf (buf, MAX_INPUT_LENGTH,
...
void
do_fprompt (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_prompt (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
