 ...
 */
bool
check_pos (CHAR_DATA * ch, short position)
{
 ...
 */
void
write_watch_files (CHAR_DATA * ch, CMDTYPE * cmd, char *logline)
{
  WATCH_DATA *pw;
  FILE *fp;
...
/* sorted by imm name */
  if (cmd)
      ...
      while (pw)
	{
	  ...
	  for (cur_imm = pw->imm_name; pw && !strcmp (pw->imm_name, cur_imm);
	       pw = pw->next)
	    {
		...
		{
		  snprintf (fname, MAX_INPUT_LENGTH, "%s%s", WATCH_DIR,
		  ...
		  snprintf (buf, MAX_STRING_LENGTH,
			    "%.2d/%.2d %.2d:%.2d %s: %s\r\n", t->tm_mon + 1,
			    t->tm_mday, t->tm_hour, t->tm_min, ch->name,
			    logline);
		  fputs (buf, fp);
		  fclose (fp);
		  found = TRUE;
	...
	if (((pw->target_name && !str_cmp (pw->target_name, ch->name))
	    ...
	    && get_trust (ch) < pw->imm_level && ch->desc)
	  {
	    ...
	    snprintf (buf, MAX_STRING_LENGTH,
		      "%.2d/%.2d %.2d:%.2d %s: %s\r\n", t->tm_mon + 1,
		      t->tm_mday, t->tm_hour, t->tm_min, ch->name, logline);
	    fputs (buf, fp);
	    fclose (fp);
	  }
    }
...
void
interpret (CHAR_DATA * ch, const char *argument)
{
...
interpret (CHAR_DATA * ch, char *argument)
{
  ...
  if (ch->substate == SUB_REPEATCMD)
      ...
      else
	{
	  ...
	  snprintf (logline, MAX_INPUT_LENGTH, "(%s) %s", cmd->name,
		    argument);
	}
    }
   ...
   * Log and snoop.
   */
  snprintf (lastplayercmd, (MAX_INPUT_LENGTH * 2), "%s used %s", ch->name,
	    logline);
  ...
  if ((!IS_NPC (ch) && xIS_SET (ch->act, PLR_LOG))
      || fLogAll || loglvl == LOG_BUILD || loglvl == LOG_HIGH
      || loglvl == LOG_ALWAYS)
    {
      /*
	...
	snprintf (log_buf, MAX_STRING_LENGTH, "Log %s (%s): %s", ch->name,
		  ch->desc->original->name, logline);
      else
	snprintf (log_buf, MAX_STRING_LENGTH, "Log %s: %s", ch->name,
		  logline);
  ...
  if (!IS_NPC (ch) && ch->pcdata->nuisance && ch->pcdata->nuisance->flags > 9
      && number_percent () <
      ((ch->pcdata->nuisance->flags - 9) * 10 * ch->pcdata->nuisance->power))
    {
  ...
  if (tmptime > 1500000)
    {
      log_printf_plus (LOG_NORMAL, get_trust (ch),
		       "[*****] LAG: %s: %s %s (R:%d S:%ld.%06ld)", ch->name,
		       cmd->name, (cmd->log == LOG_NEVER ? "XXX" : argument),
		       ch->in_room ? ch->in_room->vnum : 0, time_used.tv_sec,
		       time_used.tv_usec);
...
bool
check_social (CHAR_DATA * ch, const char *command, const char *argument)
{
	  ...
	  if (!IS_IMMORTAL (ch) || get_trust (victim) > get_trust (ch))
	    {
	      removed[i] = victim;
	      i++;
	      UNLINK (victim, room->first_person, room->last_person,
  ...
  if (arg[0] == '\0')
	  ...
	  && !HAS_PROG (victim->pIndexData, ACT_PROG))
	{
	  switch (number_bits (4))
	    ...
	    case 7:
	    case 8:
	      act (AT_SOCIAL, social->others_found, victim, NULL, ch,
		   TO_NOTVICT);
  ...
  if (i != 0)
    {
      for (k = 0, victim = removed[0]; k < i; k++, victim = removed[k])
	{
	  LINK (victim, room->first_person, room->last_person, next_in_room,
		prev_in_room);
	}
    }
 ...
 */
const char *
one_argument2 (const char *argument, char *arg_first)
{
  char cEnd;
  short count;
...
void
do_timecmd (CHAR_DATA * ch, const char *argument)
{
...
void
start_timer (struct timeval *sttime)
{
  if (!sttime)
    {
...
time_t
end_timer (struct timeval * sttime)
{
  struct timeval etime;
...
void
send_timer (struct timerset *vtime, CHAR_DATA * ch)
{
  ...
  ch_printf (ch,
	     "Time (in secs): min %ld.%06ld; avg: %ld.%06ld; max %ld.%06ld"
...
void
update_userec (struct timeval *time_used, struct timerset *userec)
{
  userec->num_uses++;
...
check_cmd_flags (CHAR_DATA * ch, CMDTYPE * cmd)
{
  ...
  if (IS_AFFECTED (ch, AFF_POSSESS) && IS_SET (cmd->flags, CMD_FLAG_POSSESS))
    snprintf (cmd_flag_buf, MAX_STRING_LENGTH,
	      "You can't %s while you are possessing someone!\r\n",
	      cmd->name);
  else if (ch->morph != NULL && IS_SET (cmd->flags, CMD_FLAG_POLYMORPHED))
    snprintf (cmd_flag_buf, MAX_STRING_LENGTH,
	      "You can't %s while you are polymorphed!\r\n", cmd->name);
  else
    cmd_flag_buf[0] = '\0';
