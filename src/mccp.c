...
const unsigned char start_compress2_str[] =
...
process_compressed (DESCRIPTOR_DATA * d)
{
  ...
  if (len > 0)
      ...
      for (iStart = 0; iStart < len; iStart += nWrite)
	{
	  ...
	  if ((nWrite =
	       write (d->descriptor, d->mccp->out_compress_buf + iStart,
		      nBlock)) < 0)
	    {
...
bool
compressStart (DESCRIPTOR_DATA * d)
{
  z_stream *s;
...
void
do_compress (CHAR_DATA * ch, const char *argument)
{
  if (!ch->desc)
    {
