...
char *
grab_word (char *argument, char *arg_first)
{
  char cEnd;
  short count;
...
void
decorate_room (ROOM_INDEX_DATA * room)
{
      ...
      while (previous[iRand] == -1)
	{
	  ...
	  if (len == 0)
		...
		case 4:
	      ...
	      snprintf (buf2, MAX_STRING_LENGTH, "%s%s%s", pre,
			room_sents[sector][x], post);
	    }
...
clear_vrooms ()
{
  ...
  for (hash = 0; hash < 64; hash++)
    {
      while (vroom_hash[hash] && !vroom_hash[hash]->first_person
 ...
 */
EXIT_DATA *
get_exit (ROOM_INDEX_DATA * room, short dir)
{
  EXIT_DATA *xit;
 ...
 */
EXIT_DATA *
get_exit_to (ROOM_INDEX_DATA * room, short dir, int vnum)
{
  EXIT_DATA *xit;
 ...
 */
EXIT_DATA *
get_exit_num (ROOM_INDEX_DATA * room, short count)
{
  EXIT_DATA *xit;
  int cnt;
 ...
 */
short
encumbrance (CHAR_DATA * ch, short move)
{
  int cur, max;
 ...
 */
bool
will_fall (CHAR_DATA * ch, int fall)
{
 ...
 */
ROOM_INDEX_DATA *
generate_exit (ROOM_INDEX_DATA * in_room, EXIT_DATA ** pexit)
{
  EXIT_DATA *xit, *bxit;
...
move_char (CHAR_DATA * ch, EXIT_DATA * pexit, int fall)
{
  ...
  if (!pexit || (to_room = pexit->to_room) == NULL)
	  ...
	  && ch->in_room->sector_type != SECT_OCEANFLOOR)
	{
	  switch (number_bits (4))
	    {
	    default:
	      act (AT_ACTION, "You drunkenly stumble into some obstacle.", ch,
		   NULL, NULL, TO_CHAR);
	      act (AT_ACTION, "$n drunkenly stumbles into a nearby obstacle.",
	    ...
	    case 3:
	      ...
	      act (AT_ACTION,
		   "$n stumbles drunkenly, trips and tumbles to the ground.",
		   ch, NULL, NULL, TO_ROOM);
	      ch->position = POS_RESTING;
	      break;
	    case 4:
	      act (AT_SOCIAL, "You utter a string of slurred obscenities.",
		   ch, NULL, NULL, TO_CHAR);
	      act (AT_ACTION,
	      ...
	      act (AT_HURT,
		   ...
		   ch, NULL, NULL, TO_CHAR);
	      act (AT_ACTION, "$n drunkenly staggers into something.", ch,
		   NULL, NULL, TO_ROOM);
	      act (AT_SOCIAL,
		   "$n utters a string of slurred obscenities: @*&^%@*&!", ch,
		   NULL, NULL, TO_ROOM);
	      act (AT_ACTION, "$n topples to the ground with a thud.", ch,
		   NULL, NULL, TO_ROOM);
	...
	act (AT_ACTION,
	     "You stare around trying to remember where you where going.", ch,
	     NULL, NULL, TO_CHAR);
      else if (drunk)
	act (AT_ACTION,
	     "You stare around trying to make sense of things through your drunken stupor.",
	     ch, NULL, NULL, TO_CHAR);
      else
  ...
  if (!fall && !IS_NPC (ch))
      ...
      if (IS_SET (pexit->exit_info, EX_CLIMB))
	{
	  ...
	  if (!found && !ch->mount)
	    {
	      if ((!IS_NPC (ch)
		   && number_percent () > LEARNED (ch, gsn_climb)) || drunk
		  || ch->mental_state < -90)
		{
		  send_to_char
  ...
  if (!IS_AFFECTED (ch, AFF_SNEAK)
      ...
      else if (!txt)
	{
      ...
      if (ch->mount)
	{
	  snprintf (buf, MAX_STRING_LENGTH, "$n %s %s upon $N.", txt,
		    dir_name[door]);
  ...
  if (!IS_AFFECTED (ch, AFF_SNEAK)
      ...
      else
	{
      ...
      if (ch->mount)
	{
	  snprintf (buf, MAX_STRING_LENGTH, "$n %s from %s upon $N.", txt,
		    dtxt);
  ...
  if (xIS_SET (ch->in_room->room_flags, ROOM_DEATH) && !IS_IMMORTAL (ch))
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH, "%s hit a DEATH TRAP in room %d!",
...
void
do_north (CHAR_DATA * ch, const char *argument)
{
...
void
do_east (CHAR_DATA * ch, const char *argument)
{
...
void
do_south (CHAR_DATA * ch, const char *argument)
{
...
void
do_west (CHAR_DATA * ch, const char *argument)
{
...
void
do_up (CHAR_DATA * ch, const char *argument)
{
...
void
do_down (CHAR_DATA * ch, const char *argument)
{
...
void
do_northeast (CHAR_DATA * ch, const char *argument)
{
...
void
do_northwest (CHAR_DATA * ch, const char *argument)
{
...
void
do_southeast (CHAR_DATA * ch, const char *argument)
{
...
void
do_southwest (CHAR_DATA * ch, const char *argument)
{
...
EXIT_DATA *
find_door (CHAR_DATA * ch, const char *arg, bool quiet)
{
  EXIT_DATA *pexit;
  int door;
...
void
set_bexit_flag (EXIT_DATA * pexit, int flag)
{
  EXIT_DATA *pexit_rev;
...
void
remove_bexit_flag (EXIT_DATA * pexit, int flag)
{
  EXIT_DATA *pexit_rev;
...
void
toggle_bexit_flag (EXIT_DATA * pexit, int flag)
{
  EXIT_DATA *pexit_rev;
...
void
do_open (CHAR_DATA * ch, const char *argument)
{
...
void
do_close (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  EXIT_DATA *pexit;
...
void
do_lock (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj, *key;
  EXIT_DATA *pexit;
  ...
  if ((pexit = find_door (ch, arg, TRUE)) != NULL)
	  ...
	  || (pexit->keyword && nifty_is_name (arg, pexit->keyword)))
	{
	  ...
	  count = key->count;
	  key->count = 1;
	  act (AT_ACTION, "$n locks the $d with $p.", ch, key, pexit->keyword,
	       TO_ROOM);
	  key->count = count;
...
void
do_unlock (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj, *key;
  EXIT_DATA *pexit;
  ...
  if ((pexit = find_door (ch, arg, TRUE)) != NULL)
	  ...
	  || (pexit->keyword && nifty_is_name (arg, pexit->keyword)))
	{
	  ...
	  count = key->count;
	  key->count = 1;
	  act (AT_ACTION, "$n unlocks the $d with $p.", ch, key,
 ...
 */
void
do_bolt (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  EXIT_DATA *pexit;
  ...
  if ((pexit = find_door (ch, arg, TRUE)) != NULL)
	...
	{
	  send_to_char ("*Clunk*\r\n", ch);
	  act (AT_ACTION, "$n bolts the $d.", ch, NULL, pexit->keyword,
	       TO_ROOM);
 ...
 */
void
do_unbolt (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  EXIT_DATA *pexit;
  ...
  if ((pexit = find_door (ch, arg, TRUE)) != NULL)
	...
	{
	  send_to_char ("*Clunk*\r\n", ch);
	  act (AT_ACTION, "$n unbolts the $d.", ch, NULL, pexit->keyword,
	       TO_ROOM);
...
void
do_bashdoor (CHAR_DATA * ch, const char *argument)
{
  EXIT_DATA *pexit;
  char arg[MAX_INPUT_LENGTH];
  ...
  if ((pexit = find_door (ch, arg, FALSE)) != NULL)
	...
	schance /= 3;
	  ...
	  && ch->move >= 15
	  && number_percent () < (schance + 4 * (get_curr_str (ch) - 19)))
	{
	  ...
	  if ((to_room = pexit->to_room) != NULL
		...
		{
		  act (AT_SKILL, "The $d crashes open!", rch, NULL,
		       pexit_rev->keyword, TO_CHAR);
		}
	    }
...
void
do_stand (CHAR_DATA * ch, const char *argument)
{
  switch (ch->position)
    {
    case POS_SLEEPING:
...
void
do_sit (CHAR_DATA * ch, const char *argument)
{
  switch (ch->position)
    {
    case POS_SLEEPING:
...
void
do_rest (CHAR_DATA * ch, const char *argument)
{
  switch (ch->position)
    {
    case POS_SLEEPING:
...
void
do_sleep (CHAR_DATA * ch, const char *argument)
{
  switch (ch->position)
    {
    case POS_SLEEPING:
...
void
do_wake (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
 ...
 */
void
teleportch (CHAR_DATA * ch, ROOM_INDEX_DATA * room, bool show)
{
  char buf[MAX_STRING_LENGTH];
...
void
teleport (CHAR_DATA * ch, int room, int flags)
{
 ...
 */
void
do_climb (CHAR_DATA * ch, const char *argument)
{
  EXIT_DATA *pexit;
  bool found;
 ...
 */
void
do_enter (CHAR_DATA * ch, const char *argument)
{
  EXIT_DATA *pexit;
  bool found;
 ...
 */
void
do_leave (CHAR_DATA * ch, const char *argument)
{
  EXIT_DATA *pexit;
  bool found;
 ...
 */
ch_ret
pullcheck (CHAR_DATA * ch, int pulse)
{
  ...
  for (xtmp = room->first_exit; xtmp; xtmp = xtmp->next)
    if (xtmp->pull && xtmp->to_room
	&& (!xit || abs (xtmp->pull) > abs (xit->pull)))
      xit = xtmp;
  ...
  if (move)
	...
	{
	  act (AT_PLAIN, destrm, xit->to_room->first_person, NULL, dtxt,
	       TO_CHAR);
	  act (AT_PLAIN, destrm, xit->to_room->first_person, NULL, dtxt,
	       TO_ROOM);
	}
  ...
  if (moveobj)
      ...
      for (obj = room->first_content; obj; obj = obj_next)
	{
	  ...
	  if ((abs (pull) * 10) > resistance)
		...
		{
		  act (AT_PLAIN, objmsg, room->first_person, obj,
		       dir_name[xit->vdir], TO_CHAR);
		  act (AT_PLAIN, objmsg, room->first_person, obj,
		       dir_name[xit->vdir], TO_ROOM);
		}
	      if (destob && xit->to_room->first_person)
		{
		  act (AT_PLAIN, destob, xit->to_room->first_person, obj,
		       dtxt, TO_CHAR);
		  act (AT_PLAIN, destob, xit->to_room->first_person, obj,
		       dtxt, TO_ROOM);
		}
