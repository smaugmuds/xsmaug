...
void
do_heal (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *mob;
  ...
  if (arg[0] == '\0')
    {
       ...
       * display price list 
       */
      act (AT_PLAIN, "$N says 'I offer the following spells:'", ch, NULL, mob,
	   TO_CHAR);
  ...
  if (!str_cmp (arg, "light"))
    ...
    {
      act (AT_PLAIN, "$N says ' Type 'heal' for a list of spells.'", ch, NULL,
	   mob, TO_CHAR);
      return;
    }
