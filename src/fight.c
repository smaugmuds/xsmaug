 ...
 * Local functions.
 */
void new_dam_message (CHAR_DATA * ch, CHAR_DATA * victim, int dam,
		      unsigned int dt, OBJ_DATA * obj);
...
bool
loot_coins_from_corpse (CHAR_DATA * ch, OBJ_DATA * corpse)
{
 ...
 */
bool
is_attack_supressed (CHAR_DATA * ch)
{
  TIMER *timer;
 ...
 */
bool
is_wielding_poisoned (CHAR_DATA * ch)
{
  OBJ_DATA *obj;
 ...
 */
bool
is_hunting (CHAR_DATA * ch, CHAR_DATA * victim)
{
...
bool
is_hating (CHAR_DATA * ch, CHAR_DATA * victim)
{
...
bool
is_fearing (CHAR_DATA * ch, CHAR_DATA * victim)
{
...
void
start_hunting (CHAR_DATA * ch, CHAR_DATA * victim)
{
  if (ch->hunting)
    stop_hunting (ch);
...
void
start_hating (CHAR_DATA * ch, CHAR_DATA * victim)
{
  if (ch->hating)
    stop_hating (ch);
...
void
start_fearing (CHAR_DATA * ch, CHAR_DATA * victim)
{
  if (ch->fearing)
    stop_fearing (ch);
	      ...
	      if (30 + (ch->level / 4) >= number_percent ())
		{
		  ...
		  switch (attacktype)
		    ...
		    case ATCK_LIGHTNBREATH:
		      retcode =
			spell_lightning_breath (skill_lookup
						("lightning breath"),
						ch->level, ch, victim);
		      break;
		    case ATCK_GASBREATH:
		    ...
		    case ATCK_SPIRALBLAST:
		      retcode =
			spell_spiral_blast (skill_lookup ("spiral blast"),
					    ch->level, ch, victim);
		      break;
		    case ATCK_POISON:
		      retcode =
		    ...
		    case ATCK_CAUSESERIOUS:
		      retcode =
			spell_cause_serious (skill_lookup ("cause serious"),
					     ch->level, ch, victim);
		      break;
		    case ATCK_EARTHQUAKE:
		      retcode =
			spell_earthquake (skill_lookup ("earthquake"),
					  ch->level, ch, victim);
		      break;
		    case ATCK_CAUSECRITICAL:
		      retcode =
			spell_cause_critical (skill_lookup ("cause critical"),
					      ch->level, ch, victim);
		      break;
		    case ATCK_CURSE:
		      retcode =
		    ...
		    case ATCK_FLAMESTRIKE:
		      retcode =
			spell_flamestrike (skill_lookup ("flamestrike"),
					   ch->level, ch, victim);
		      break;
		    case ATCK_HARM:
		      retcode =
		    ...
		    case ATCK_FIREBALL:
		      retcode =
			spell_fireball (skill_lookup ("fireball"), ch->level,
					ch, victim);
		      break;
		    ...
		    case ATCK_WEAKEN:
		      retcode =
			spell_weaken (skill_lookup ("weaken"), ch->level, ch,
				      victim);
		      break;
		    }
		  if (attacktype != -1
		      && (retcode == rCHAR_DIED || char_died (ch)))
		    continue;
		}
	    }
		  ...
		  switch (attacktype)
		      ...
		      if (ch->hit < ch->max_hit)
			{
			  act (AT_MAGIC,
			       ...
			       ch, NULL, NULL, TO_ROOM);
			  retcode =
			    spell_smaug (skill_lookup ("heal"), ch->level, ch,
					 ch);
			}
		      break;
			...
			{
			  act (AT_MAGIC, "$n utters an incantation...", ch,
			       NULL, NULL, TO_ROOM);
			  retcode =
		    ...
		    case DFND_DISPELEVIL:
		      act (AT_MAGIC, "$n utters an incantation...", ch, NULL,
			   NULL, TO_ROOM);
		      retcode =
			spell_dispel_evil (skill_lookup ("dispel evil"),
					   ch->level, ch, victim);
		      break;
		    case DFND_TELEPORT:
		      retcode =
			...
			{
			  act (AT_MAGIC, "$n utters a few incantations...",
			       ch, NULL, NULL, TO_ROOM);
			  retcode =
		      ...
		      if (!IS_AFFECTED (ch, AFF_ACIDMIST))
			{
			  act (AT_MAGIC, "$n utters a few incantations ...",
			       ch, NULL, NULL, TO_ROOM);
			  retcode =
			    spell_smaug (skill_lookup ("acidmist"), ch->level,
					 ch, ch);
			}
		      else
			retcode = rNONE;
			...
			retcode =
			  spell_smaug (skill_lookup ("true"), ch->level, ch,
				       ch);
		      else
			retcode = rNONE;
		      break;
...
multi_hit (CHAR_DATA * ch, CHAR_DATA * victim, int dt)
{
  ...
  if (get_eq_char (ch, WEAR_DUAL_WIELD))
    {
      dual_bonus =
	IS_NPC (ch) ? (ch->level / 10) : (LEARNED (ch, gsn_dual_wield) / 10);
 ...
 */
int
weapon_prof_bonus_check (CHAR_DATA * ch, OBJ_DATA * wield, int *gsn_ptr)
{
  int bonus;
...
{
  OBJ_DATA *wield;
  int victim_ac, thac0, thac0_00, thac0_32, plusris, dam, diceroll,
    attacktype, cnt, prof_bonus, prof_gsn = -1;
  ...
  if (IS_AFFECTED (victim, AFF_FIRESHIELD)
    ...
    retcode =
      spell_smaug (skill_lookup ("flare"), off_shld_lvl (victim, ch), victim,
		   ch);
  if (retcode != rNONE || char_died (ch) || char_died (victim))
    return retcode;
  ...
  if (IS_AFFECTED (victim, AFF_ICESHIELD) && !IS_AFFECTED (ch, AFF_ICESHIELD))
    retcode =
      spell_smaug (skill_lookup ("iceshard"), off_shld_lvl (victim, ch),
		   victim, ch);
  if (retcode != rNONE || char_died (ch) || char_died (victim))
    return retcode;
  ...
  if (IS_AFFECTED (victim, AFF_SHOCKSHIELD)
    ...
    retcode =
      spell_smaug (skill_lookup ("torrent"), off_shld_lvl (victim, ch),
		   victim, ch);
  if (retcode != rNONE || char_died (ch) || char_died (victim))
    return retcode;
  ...
  if (IS_AFFECTED (victim, AFF_ACIDMIST) && !IS_AFFECTED (ch, AFF_ACIDMIST))
    retcode =
      spell_smaug (skill_lookup ("acidshot"), off_shld_lvl (victim, ch),
		   victim, ch);
  if (retcode != rNONE || char_died (ch) || char_died (victim))
    return retcode;
  ...
  if (IS_AFFECTED (victim, AFF_VENOMSHIELD)
    ...
    retcode =
      spell_smaug (skill_lookup ("venomshot"), off_shld_lvl (victim, ch),
		   victim, ch);
  if (retcode != rNONE || char_died (ch) || char_died (victim))
    return retcode;
		...
		OBJ_DATA * projectile, short dist)
{
  ...
  thac0 =
    interpolate (ch->level, thac0_00,
		 thac0_32) - GET_HITROLL (ch) + (dist * 2);
  victim_ac = UMAX (-19, (int) (GET_AC (victim) / 10));
 ...
 */
short
ris_damage (CHAR_DATA * ch, short dam, int ris)
{
  short modifier;
...
damage (CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
  ...
  if (victim != ch)
	...
	if (is_same_group (ch, gch) && !IS_NPC (gch) && !IS_PKILL (gch)
	    && (ch != gch))
	  anopc++;
  ...
  if (dam && ch != victim && !IS_NPC (ch) && ch->fighting && ch->fighting->xp)
	...
	xp_gain =
	  (int) (xp_compute (ch, victim) * 0.85 * dam) / victim->max_hit;
  ...
  switch (victim->position)
	...
	{
	  act (AT_ACTION, "$n is stunned, but will probably recover.", victim,
	       NULL, NULL, TO_ROOM);
	  act (AT_HURT, "You are stunned, but will probably recover.", victim,
	       NULL, NULL, TO_CHAR);
	}
      break;
  ...
  if (victim->position == POS_DEAD)
      ...
      if (!npcvict)
	{
	  ...
	  if (!IS_NPC (ch) && !IS_IMMORTAL (ch) && ch->pcdata->clan
	      ...
	      && ch->pcdata->clan->clan_type != CLAN_GUILD && victim != ch)
	    {
	      snprintf (filename, 256, "%s%s.record", CLAN_DIR,
			ch->pcdata->clan->name);
	      snprintf (log_buf, MAX_STRING_LENGTH,
			"&P(%2d) %-12s &wvs &P(%2d) %s &P%s ... &w%s",
			ch->level, ch->name, victim->level,
			!CAN_PKILL (victim) ? "&W<Peaceful>" :
			victim->pcdata->clan ? victim->pcdata->
			clan->badge : "&P(&WUnclanned&P)", victim->name,
	  ...
	  if (victim->exp > exp_level (victim, victim->level))
	    gain_exp (victim,
		      (exp_level (victim, victim->level) - victim->exp) / 2);
	...
	loot = FALSE;
      ...
      if (!IS_NPC (ch) && loot && new_corpse
	  && new_corpse->item_type == ITEM_CORPSE_NPC
	  && new_corpse->in_room == ch->in_room
	  && can_see_obj (ch, new_corpse) && ch->position > POS_SLEEPING)
	{
	  /*
	   * Autogold by Scryn 8/12 
	   */
	  if (xIS_SET (ch->act, PLR_AUTOGOLD)
	      && !loot_coins_from_corpse (ch, new_corpse))
	    return rBOTH_DIED;

	  if (new_corpse && !obj_extracted (new_corpse)
	      && new_corpse->in_room == ch->in_room
		...
		do_look (ch, "in corpse");
	      if (!char_died (ch) && xIS_SET (ch->act, PLR_AUTOSAC)
		  && !obj_extracted (new_corpse)
		  && new_corpse->in_room == ch->in_room
 ...
 */
bool
is_safe (CHAR_DATA * ch, CHAR_DATA * victim, bool show_messg)
{
  ...
  if (IS_PACIFIST (victim))	/* Gorog */
      ...
      if (show_messg)
	{
	  snprintf (buf, MAX_STRING_LENGTH,
		    "%s is a pacifist and will not fight.\r\n",
  ...
  if (!IS_NPC (ch) && !IS_NPC (victim) && ch != victim
      ...
      if (show_messg)
	{
	  ...
	  send_to_char
	    ("The gods have forbidden player killing in this area.\r\n", ch);
	}
      return TRUE;
    }
  ...
  if (calculate_age (ch) < 18 || ch->level < 5)
      ...
      if (show_messg)
	{
	  ...
	  send_to_char
	    ("You are not yet ready, needing age or experience, if not both. \r\n",
	     ch);
	}
      return TRUE;
    }
  ...
  if (ch->level - victim->level > 5 || victim->level - ch->level > 5)
      ...
      if (show_messg)
	{
	  set_char_color (AT_IMMORT, ch);
	  send_to_char
	    ("The gods do not allow murder when there is such a difference in level.\r\n",
	     ch);
	}
      return TRUE;
    }
 ...
 */
bool
legal_loot (CHAR_DATA * ch, CHAR_DATA * victim)
{
  /*
 ...
 */
void
check_attacker (CHAR_DATA * ch, CHAR_DATA * victim)
{
 ...
 */
   ...
   * deadly char check 
   */
  if (!IS_NPC (ch) && !IS_NPC (victim) && CAN_PKILL (ch)
      && CAN_PKILL (victim))
    return;
...
*/
   ...
   */
  if (IS_NPC (ch)
      || ch == victim || ch->level >= LEVEL_IMMORTAL
      || xIS_SET (ch->act, PLR_ATTACKER) || xIS_SET (ch->act, PLR_KILLER))
    return;
...
update_pos (CHAR_DATA * victim)
{
  ...
  if (IS_NPC (victim) || victim->hit <= -11)
    {
      if (victim->mount)
	{
	  act (AT_ACTION, "$n falls from $N.", victim, NULL, victim->mount,
	       TO_ROOM);
  ...
  if (victim->mount)
    {
      act (AT_ACTION, "$n falls unconscious from $N.", victim, NULL,
...
set_fighting (CHAR_DATA * ch, CHAR_DATA * victim)
{
  ...
  if (ch->fighting)
    {
      bug ("Set_fighting: %s -> %s (already fighting %s)", ch->name,
...
raw_kill (CHAR_DATA * ch, CHAR_DATA * victim)
{
  ...
  if (victim->in_room->sector_type == SECT_OCEANFLOOR
    ...
    act (AT_BLOOD, "$n's blood slowly clouds the surrounding water.", victim,
	 NULL, NULL, TO_ROOM);
  else if (victim->in_room->sector_type == SECT_AIR)
    act (AT_BLOOD, "$n's blood sprays wildly through the air.", victim, NULL,
	 NULL, TO_ROOM);
  else
    make_blood (victim);
  ...
  for (gch = ch->in_room->first_person; gch; gch = gch_next)
      ...
      for (obj = gch->first_carrying; obj; obj = obj_next)
	{
	  ...
	  if ((IS_OBJ_STAT (obj, ITEM_ANTI_EVIL) && IS_EVIL (gch)) ||
	    ...
	    {
	      act (AT_MAGIC, "You are zapped by $p.", gch, obj, NULL,
		   TO_CHAR);
...
int
align_compute (CHAR_DATA * gch, CHAR_DATA * victim)
{
...
xp_compute (CHAR_DATA * gch, CHAR_DATA * victim)
{
  ...
  xp =
    (get_exp_worth (victim) * URANGE (0, (victim->level - gchlev) + 10, 13)) /
    10;
  align = gch->alignment - victim->alignment;
   ...
   */
  return URANGE (0, xp,
		 exp_level (gch, gchlev + 1) - exp_level (gch, gchlev));
}
 ...
 */
void
new_dam_message (CHAR_DATA * ch, CHAR_DATA * victim, int dam, unsigned int dt,
		 OBJ_DATA * obj)
{
  ...
  if (dt > 0 && dt < (unsigned int) num_skills)
    ...
    {
      bug ("Dam_message: bad dt %d from %s in %d.", dt, ch->name,
	   ch->in_room->vnum);
      dt = TYPE_HIT;
      w_index = 0;
    }
...
void
do_kill (CHAR_DATA * ch, const char *argument)
{
...
void
do_murde (CHAR_DATA * ch, const char *argument)
{
...
void
do_murder (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
*/
  ...
  if (!IS_NPC (victim))
    {
      log_printf_plus (LOG_NORMAL, ch->level, "%s: murder %s.", ch->name,
		       victim->name);
    }

  WAIT_STATE (ch, 1 * PULSE_VIOLENCE);
  snprintf (buf, MAX_STRING_LENGTH, "Help!  I am being attacked by %s!",
	    IS_NPC (ch) ? ch->short_descr : ch->name);
  if (IS_PKILL (victim))
...
bool
check_illegal_pk (CHAR_DATA * ch, CHAR_DATA * victim)
{
  char buf[MAX_STRING_LENGTH];
...
void
do_flee (CHAR_DATA * ch, const char *argument)
{
  ...
  for (attempt = 0; attempt < 8; attempt++)
      ...
      if (!IS_NPC (ch))
	{
	  ...
	  act (AT_FLEE, "You flee head over heels from combat!", ch, NULL,
	       NULL, TO_CHAR);
	  los =
	  ...
	  if (ch->level < LEVEL_AVATAR)
	    {
	      snprintf (buf, MAX_STRING_LENGTH,
			"Curse the gods, you've lost %d experience!", los);
  ...
  act (AT_FLEE, "You attempt to flee from combat but can't escape!", ch, NULL,
       NULL, TO_CHAR);
  if (ch->level < LEVEL_AVATAR && number_bits (3) == 1)
    {
      snprintf (buf, MAX_STRING_LENGTH,
		"Curse the gods, you've lost %d experience!\r\n", los);
      act (AT_FLEE, buf, ch, NULL, NULL, TO_CHAR);
      gain_exp (ch, 0 - los);
    }
  return;
...
void
do_sla (CHAR_DATA * ch, const char *argument)
{
...
void
do_slay (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg2, "immolate"))
    ...
    {
      act (AT_BLOOD, "You order your dogs to rip $N to shreds.", ch, NULL,
	   victim, TO_CHAR);
      act (AT_BLOOD, "$n orders $s dogs to rip you apart.", ch, NULL, victim,
	   TO_VICT);
      act (AT_BLOOD, "$n orders $s dogs to rip $N to shreds.", ch, NULL,
	   victim, TO_NOTVICT);
    }
