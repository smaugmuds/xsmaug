...
boot_db (bool fCopyOver)
{
  ...
  sysdata.save_flags =
    SV_DEATH | SV_PASSCHG | SV_AUTO | SV_PUT | SV_DROP | SV_GIVE | SV_AUCTION
    | SV_ZAPDROP | SV_IDLE;
   ...
   */
  {
	...
	lhour =
	  (current_time -
	   650336715) / (sysdata.pulsetick / sysdata.pulsepersec);
   ...
   */
  {
	...
	if (feof (fpList))
	  {
	    bug
	      ("%s: EOF encountered reading area list - no $ found at end of file.",
	       __FUNCTION__);
	    break;
	  }
 ...
 */
AREA_DATA *
load_area (FILE * fp, int aversion)
{
  AREA_DATA *pArea;
 ...
 */
void
load_credits (AREA_DATA * tarea, FILE * fp)
{
  if (!tarea)
    {
      bug ("%s: no #AREA seen yet.", __FUNCTION__);
 ...
 */
void
load_economy (AREA_DATA * tarea, FILE * fp)
{
  if (!tarea)
    {
...
load_mobiles (AREA_DATA * tarea, FILE * fp)
{
  ...
  for (;;)
      ...
      if (get_mob_index (vnum))
	{
	  if (tmpBootDb)
	  ...
	  else
	    {
	      ...
	      log_printf_plus (LOG_BUILD, sysdata.log_level,
			       "Cleaning mobile: %d", vnum);
	      clean_mob (pMobIndex);
	      oldmob = TRUE;
	    }
	}
      ...
      if (fBootDb)
	{
      ...
      ((char *) pMobIndex->description)[0] =
	UPPER (pMobIndex->description[0]);
	  ...
	  if (tarea->version < 1000)	/* Normal Smaug zones load here */
	    {
	      ...
	      sscanf (ln, "%d %d %d %d %d %d %d", &x1, &x2, &x3, &x4, &x5,
		      &x6, &x7);
...
load_objects (AREA_DATA * tarea, FILE * fp)
{
  ...
  for (;;)
      ...
      if (fBootDb)
	{
      ...
      ((char *) pObjIndex->description)[0] =
	UPPER (pObjIndex->description[0]);
...
load_resets (AREA_DATA * tarea, FILE * fp)
{
  ...
  for (;;)
	...
	case 'O':
	  if (get_obj_index (arg1) == NULL && fBootDb)
	    boot_log ("%s: %s (%d) '%c': object %d doesn't exist.",
		      __FUNCTION__, tarea->filename, count, letter, arg1);

	  if ((pRoomIndex = get_room_index (arg3)) == NULL && fBootDb)
	    boot_log ("%s: %s (%d) '%c': room %d doesn't exist.",
		      __FUNCTION__, tarea->filename, count, letter, arg3);
	  else
	    {
	      if (!pRoomIndex)
		bug ("%s: Unable to add room reset - room not found.",
		     __FUNCTION__);
	      else
		add_reset (pRoomIndex, letter, extra, arg1, arg2, arg3);
	    }
	...
	case 'T':
	  if (IS_SET (extra, TRAP_OBJ))
	    bug
	      ("%s: Unable to add legacy object trap reset. Must be converted manually.",
	       __FUNCTION__);
	  else
	    {
	      if (!(pRoomIndex = get_room_index (arg3)))
		bug ("%s: Unable to add trap reset - room not found.",
		     __FUNCTION__);
	      else
		add_reset (pRoomIndex, letter, extra, arg1, arg2, arg3);
	    }
	...
	case 'H':
	  bug
	    ("%s: Unable to convert legacy hide reset. Must be converted manually.",
	     __FUNCTION__);
	  break;
	  ...
	  if (!(pRoomIndex = get_room_index (arg1)))
	    {
	      ...
	      if (fBootDb)
		boot_log ("%s: %s (%d) 'D': room %d doesn't exist.",
			  __FUNCTION__, tarea->filename, count, arg1);
	      break;
	    }

	  if (arg2 < 0 || arg2 > MAX_DIR + 1
	      || !(pexit = get_exit (pRoomIndex, arg2))
	      || !IS_SET (pexit->exit_info, EX_ISDOOR))
	    {
	      bug ("%s: 'D': exit %d not door.", __FUNCTION__, arg2);
	      bug ("Reset: %c %d %d %d %d", letter, extra, arg1, arg2, arg3);
	      if (fBootDb)
		boot_log ("%s: %s (%d) 'D': exit %d not door.", __FUNCTION__,
	  ...
	  if (arg3 < 0 || arg3 > 2)
	    {
	      ...
	      if (fBootDb)
		boot_log ("%s: %s (%d) 'D': bad 'locks': %d.", __FUNCTION__,
			  tarea->filename, count, arg3);
	    }
	  add_reset (pRoomIndex, letter, extra, arg1, arg2, arg3);
	  break;
	...
	case 'R':
	  if (!(pRoomIndex = get_room_index (arg1)) && fBootDb)
	    boot_log ("%s: %s (%d) 'R': room %d doesn't exist.", __FUNCTION__,
	  ...
	  if (arg2 < 0 || arg2 > 10)
	    {
	      ...
	      if (fBootDb)
		boot_log ("%s: %s (%d) 'R': bad exit %d.", __FUNCTION__,
			  tarea->filename, count, arg2);
	      break;
	    }
	  break;
  ...
  if (!not01)
    {
      for (roomlist = tarea->first_room; roomlist;
	   roomlist = roomlist->next_aroom)
...
void
load_smaugwiz_reset (ROOM_INDEX_DATA * room, FILE * fp)
{
  EXIT_DATA *pexit;
  char letter2;
  ...
  switch (letter2)
    ...
    default:
      ...
      if (fBootDb)
	boot_log ("%s: %s (%d) bad command '%c'.", __FUNCTION__,
		  room->area->filename, count, letter2);
      return;

    case 'M':
      if (get_mob_index (arg2) == NULL && fBootDb)
	boot_log ("%s: SmaugWiz - %s (%d) 'M': mobile %d doesn't exist.",
		  __FUNCTION__, room->area->filename, count, arg2);
      break;

    case 'O':
      if (get_obj_index (arg2) == NULL && fBootDb)
	boot_log ("%s: SmaugWiz - %s (%d) '%c': object %d doesn't exist.",
		  __FUNCTION__, room->area->filename, count, letter2, arg2);
      break;
      ...
      if (arg4 > 0)
	{
	  if (get_obj_index (arg4) == NULL && fBootDb)
	    boot_log
	      ("$s: SmaugWiz - %s (%d) 'P': destination object %d doesn't exist.",
	       __FUNCTION__, room->area->filename, count, arg4);
	}
      break;
    ...
    case 'H':
      if (arg1 > 0)
	if (get_obj_index (arg2) == NULL && fBootDb)
	  boot_log ("%s: SmaugWiz - %s (%d) 'H': object %d doesn't exist.",
		    __FUNCTION__, room->area->filename, count, arg2);
      break;

    case 'D':
      if (arg3 < 0 || arg3 > MAX_DIR + 1
	  || (pexit = get_exit (room, arg3)) == NULL
	  || !IS_SET (pexit->exit_info, EX_ISDOOR))
	{
	  bug ("%s: SmaugWiz - 'D': exit %d not door.", __FUNCTION__, arg3);
	  bug ("Reset: %c %d %d %d %d %d", letter2, extra, arg1, arg2, arg3,
	       arg4);
	  if (fBootDb)
	    boot_log ("%s: SmaugWiz - %s (%d) 'D': exit %d not door.",
		      __FUNCTION__, room->area->filename, count, arg3);
	}
      if (arg4 < 0 || arg4 > 2)
	{
	  bug ("%s: 'D': bad 'locks': %d.", __FUNCTION__, arg4);
	  if (fBootDb)
	    boot_log ("%s: SmaugWiz - %s (%d) 'D': bad 'locks': %d.",
		      __FUNCTION__, room->area->filename, count, arg4);
	}
      break;
      ...
      if (arg3 < 0 || arg3 > 10)
	{
	  ...
	  if (fBootDb)
	    boot_log ("%s: SmaugWiz - %s (%d) 'R': bad exit %d.",
		      __FUNCTION__, room->area->filename, count, arg3);
	  break;
	}
      break;
...
void
load_room_reset (ROOM_INDEX_DATA * room, FILE * fp)
{
  EXIT_DATA *pexit;
  char letter;
  int extra, arg1, arg2, arg3;
  ...
  switch (letter)
    ...
    case 'M':
      if (get_mob_index (arg1) == NULL && fBootDb)
	boot_log ("%s: %s (%d) 'M': mobile %d doesn't exist.", __FUNCTION__,
		  room->area->filename, count, arg1);
      break;

    case 'O':
      if (get_obj_index (arg1) == NULL && fBootDb)
	boot_log ("%s: %s (%d) '%c': object %d doesn't exist.", __FUNCTION__,
	...
	arg3 = OBJ_VNUM_MONEY_ONE;	/* This may look stupid, but for some reason it works. */
      if (get_obj_index (arg3) == NULL && fBootDb)
	boot_log ("%s: %s (%d) 'P': destination object %d doesn't exist.",
		  __FUNCTION__, room->area->filename, count, arg3);
      if (extra > 1)
	not01 = TRUE;
      break;
...
load_rooms (AREA_DATA * tarea, FILE * fp)
{
  ...
  for (;;)
      ...
      for (;;)
	{
	  ...
	  if (letter == 'D')
	      ...
	      else
		{
		  ...
		  ln = fread_line (fp);
		  x1 = x2 = x3 = x4 = x5 = x6 = 0;
		  sscanf (ln, "%d %d %d %d %d %d", &x1, &x2, &x3, &x4, &x5,
			  &x6);
      ...
      if (!oldroom)
	{
	  ...
	  LINK (pRoomIndex, tarea->first_room, tarea->last_room, next_aroom,
		prev_aroom);
	  top_room++;
	}
    }
...
void
randomize_exits (ROOM_INDEX_DATA * room, short maxdir)
{
  EXIT_DATA *pexit;
...
free_char (CHAR_DATA * ch)
{
  ...
  if (ch->pcdata)
	...
	{
	  next = temp->next;
	  UNLINK (temp, ch->pcdata->first_ignored, ch->pcdata->last_ignored,
		  next, prev);
	  STRFREE (temp->name);
	  DISPOSE (temp);
	}
...
bool
is_valid_filename (CHAR_DATA * ch, const char *direct, const char *filename)
{
...
void
do_memory (CHAR_DATA * ch, const char *argument)
{
...
char *
smash_tilde_copy (const char *str)
{
 ...
 */
void
append_file (CHAR_DATA * ch, const char *file, const char *str)
{
  FILE *fp;
 ...
 */
void
append_to_file (const char *file, const char *str)
{
  FILE *fp;
	  ...
	  for (iLine = 0; ftell (fpArea) < iChar; iLine++)
	    {
	      ...
	      while ((letter = getc (fpArea)) && letter != EOF
		     && letter != '\n')
		;
	    }
 ...
 */
void
show_file (CHAR_DATA * ch, const char *filename)
{
  FILE *fp;
  char buf[MAX_STRING_LENGTH];
  int c;
 ...
 */
void
do_dmesg (CHAR_DATA * ch, const char *argument)
{
 ...
 */
void
log_string_plus (const char *str, short log_type, short level)
{
  char *strtime;
  int offset;
...
void
log_printf_plus (short log_type, short level, const char *fmt, ...)
{
...
void
add_to_wizlist (char *name, int level)
{
  WIZENT *wiz, *tmp;
...
void
do_makewizlist (CHAR_DATA * ch, const char *argument)
{
  make_wizlist ();
}
...
int
mprog_name_to_type (const char *name)
{
...
void
mobprog_file_read (MOB_INDEX_DATA * mob, const char *f)
{
 ...
 */
void
mprog_read_programs (FILE * fp, MOB_INDEX_DATA * mob)
{
  MPROG_DATA *mprg;
  char letter;
  char *word;
...
void
objprog_file_read (OBJ_INDEX_DATA * obj, const char *f)
{
  MPROG_DATA *mprg = NULL;
  char MUDProgfile[256];
  FILE *progfile;
 ...
 */
void
oprog_read_programs (FILE * fp, OBJ_INDEX_DATA * obj)
{
  MPROG_DATA *mprg;
  char letter;
  char *word;
...
/* This routine reads in scripts of OBJprograms from a file */
void
roomprog_file_read (ROOM_INDEX_DATA * room, const char *f)
{
  MPROG_DATA *mprg = NULL;
  char MUDProgfile[256];
  FILE *progfile;
 ...
 */
void
rprog_read_programs (FILE * fp, ROOM_INDEX_DATA * room)
{
  MPROG_DATA *mprg;
  char letter;
  char *word;
...
*/
void
delete_room (ROOM_INDEX_DATA * room)
{
  int hash;
  ...
  UNLINK (room, room->area->first_room, room->area->last_room, next_aroom,
	  prev_aroom);
  ...
  if (room == room_index_hash[hash])
	...
	bug ("%s: room %d not in hash bucket %d.", __FUNCTION__, room->vnum,
	     hash);
    }
  DISPOSE (room);
  --top_room;
...
delete_obj (OBJ_INDEX_DATA * obj)
{
  ...
  if (obj == obj_index_hash[hash])
	...
	bug ("%s: object %d not in hash bucket %d.", __FUNCTION__, obj->vnum,
	     hash);
    }
  DISPOSE (obj);
  --top_obj_index;
 ...
 */
ROOM_INDEX_DATA *
make_room (int vnum, AREA_DATA * area)
{
  ROOM_INDEX_DATA *pRoomIndex;
  int iHash;
  ...
  pRoomIndex->first_exit = NULL;
  pRoomIndex->last_exit = NULL;
  LINK (pRoomIndex, area->first_room, area->last_room, next_aroom,
	prev_aroom);
 ...
 */
OBJ_INDEX_DATA *
make_object (int vnum, int cvnum, const char *name)
{
  ...
  if (!cObjIndex)
    {
      ...
      snprintf (buf, MAX_STRING_LENGTH,
		"Some god dropped a newly created %s here.", name);
 ...
 */
MOB_INDEX_DATA *
make_mobile (int vnum, int cvnum, const char *name)
{
  ...
  if (!cMobIndex)
    {
      ...
      // it's safe to cast these because we just created the object
      ((char *) pMobIndex->short_descr)[0] =
	LOWER (pMobIndex->short_descr[0]);
      ((char *) pMobIndex->long_descr)[0] = UPPER (pMobIndex->long_descr[0]);
      ((char *) pMobIndex->description)[0] =
	UPPER (pMobIndex->description[0]);
 ...
 */
EXIT_DATA *
make_exit (ROOM_INDEX_DATA * pRoomIndex, ROOM_INDEX_DATA * to_room,
	   short door)
{
...
void
fix_area_exits (AREA_DATA * tarea)
{
  ROOM_INDEX_DATA *pRoomIndex;
...
void
process_sorting (AREA_DATA * tarea)
{
  if (fBootDb)
    {
	  ...
	  if (!str_cmp (word, "#ENDEXDESC"))
	    {
	      if (!ed->keyword)
		{
		  bug ("%s: Missing ExDesc keyword. Returning NULL.",
		       __FUNCTION__);
		  STRFREE (ed->description);
		  DISPOSE (ed);
		  return NULL;
...
AFFECT_DATA *
fread_xsmaug_affect (FILE * fp, const char *word)
{
  AFFECT_DATA *paf;
  int pafmod;
...
void
fread_xsmaug_exit (FILE * fp, ROOM_INDEX_DATA * pRoomIndex)
{
  ...
  for (;;)
	...
	case 'D':
	  ...
	  if (!str_cmp (word, "Direction"))
		...
		{
		  bug ("%s: vnum %d has bad door number %d.", __FUNCTION__,
...
void
rprog_file_read (ROOM_INDEX_DATA * prog_target, const char *f)
{
  MPROG_DATA *mprg = NULL;
  char MUDProgfile[256];
  FILE *progfile;
  ...
  for (;;)
      ...
      if (letter != '#')
	{
      ...
      const char *word =
	(feof (progfile) ? "ENDFILE" : fread_word (progfile));
	  ...
	  for (;;)
	    {
	      ...
	      if (word[0] == '\0')
		{
		  log_printf ("%s: EOF encountered reading file!",
			      __FUNCTION__);
		  word = "#ENDPROG";
		}
		...
		case 'A':
		  if (!str_cmp (word, "Arglist"))
			...
			{
			case IN_FILE_PROG:
			  bug ("%s: Nested file programs are not allowed.",
			       __FUNCTION__);
			  DISPOSE (mprg);
			  break;
		...
		case 'P':
		  if (!str_cmp (word, "Progtype"))
		    {
		      mprg->type =
			mprog_name_to_type (fread_flagstring (progfile));
		      break;
		    }
		  break;
		}
...
void
fread_xsmaug_roomprog (FILE * fp, MPROG_DATA * mprg,
		       ROOM_INDEX_DATA * prog_target)
{
  bool fMatch;			// Unused, but needed to shut the compiler up about the KEY macro
...
void
fread_xsmaug_room (FILE * fp, AREA_DATA * tarea)
{
  ROOM_INDEX_DATA *pRoomIndex = NULL;
  bool oldroom = false;
  bool fMatch;			// Unused, but needed to shut the compiler up about the KEY macro
  ...
  for (;;)
	...
	case '#':
	  if (!str_cmp (word, "#ENDROOM"))
	      ...
	      if (!oldroom)
		{
		  ...
		  pRoomIndex->next = room_index_hash[iHash];
		  room_index_hash[iHash] = pRoomIndex;
		  LINK (pRoomIndex, tarea->first_room, tarea->last_room,
			next_aroom, prev_aroom);
		  ++top_room;
		}
	      return;
	    }
	  ...
	  if (!str_cmp (word, "Affect") || !str_cmp (word, "AffectData"))
	    {
	      ...
	      if (af)
		LINK (af, pRoomIndex->first_permaffect,
		      pRoomIndex->last_permaffect, next, prev);
	      break;
	    }
	  break;
	...
	case 'S':
	  if (!str_cmp (word, "Sector"))
		...
		{
		  bug ("%s: Room #%d has bad sector type.", __FUNCTION__,
		       pRoomIndex->vnum);
		  sector = 1;
		}
	  ...
	  if (!str_cmp (word, "Vnum"))
	      ...
	      if (get_room_index (vnum))
		{
		  if (tmpBootDb)
		    ...
		    {
		      pRoomIndex = get_room_index (vnum);
		      log_printf_plus (LOG_BUILD, sysdata.build_level,
				       "Cleaning room: %d", vnum);
		      clean_room (pRoomIndex);
		      oldroom = true;
		    }
...
void
oprog_file_read (OBJ_INDEX_DATA * prog_target, const char *f)
{
  MPROG_DATA *mprg = NULL;
  char MUDProgfile[256];
  FILE *progfile;
...
void
fread_xsmaug_objprog (FILE * fp, MPROG_DATA * mprg,
		      OBJ_INDEX_DATA * prog_target)
{
  bool fMatch;			// Unused, but needed to shut the compiler up about the KEY macro
...
void
fread_xsmaug_object (FILE * fp, AREA_DATA * tarea)
{
  ...
  for (;;)
	  ...
	  if (!str_cmp (word, "Affect") || !str_cmp (word, "AffectData"))
	    {
	      ...
	      if (af)
		LINK (af, pObjIndex->first_affect, pObjIndex->last_affect,
		      next, prev);
	      break;
	    }
	  break;
	...
	case 'T':
	  if (!str_cmp (word, "Type"))
		...
		{
		  bug
		    ("%s: vnum %d: Object has invalid type! Defaulting to trash.",
...
void
mprog_file_read (MOB_INDEX_DATA * prog_target, const char *f)
{
  MPROG_DATA *mprg = NULL;
  char MUDProgfile[256];
  FILE *progfile;
...
void
fread_xsmaug_mobprog (FILE * fp, MPROG_DATA * mprg,
		      MOB_INDEX_DATA * prog_target)
{
  bool fMatch;			// Unused, but needed to shut the compiler up about the KEY macro
...
void
fread_xsmaug_mobile (FILE * fp, AREA_DATA * tarea)
{
  ...
  for (;;)
	...
	case 'C':
	  if (!str_cmp (word, "Class"))
		...
		{
		  bug
		    ("%s: vnum %d: Mob has invalid Class! Defaulting to warrior.",
		     __FUNCTION__, pMobIndex->vnum);
	...
	case 'G':
	  if (!str_cmp (word, "Gender"))
		...
		{
		  bug
		    ("%s: vnum %d: Mobile has invalid sex! Defaulting to neuter.",
		     __FUNCTION__, pMobIndex->vnum);
		  sex = SEX_NEUTRAL;
		}
	      pMobIndex->sex = sex;
	...
	case 'R':
	  if (!str_cmp (word, "Race"))
		...
		{
		  bug
		    ("%s: vnum %d: Mob has invalid race! Defaulting to monster.",
		     __FUNCTION__, pMobIndex->vnum);
	...
	case 'S':
	  ...
	  if (!str_cmp (word, "Specfun"))
		...
		{
		  bug ("%s: Specfun: vnum %d, no spec_fun called %s.",
		       __FUNCTION__, pMobIndex->vnum, temp);
		  pMobIndex->spec_funname = NULL;
		}
	      else
		pMobIndex->spec_funname = STRALLOC (temp);
...
void
fread_xsmaug_areadata (FILE * fp, AREA_DATA * tarea)
{
  bool fMatch;			// Unused, but needed to shut the compiler up about the KEY macro
...
AREA_DATA *
fread_xsmaug_area (AREA_DATA * tarea, FILE * fp)
{
  for (;;)
    {
      char letter;
...
void
load_area_file (AREA_DATA * tarea, const char *filename)
{
  char *word;
  int aversion = 0;
...
load_buildlist (void)
{
  ...
  while (dentry)
      ...
      if (dentry->d_name[0] != '.')
	{
	  ...
	  if (rlow && rhi && !badfile)
	    {
	      snprintf (buf, MAX_STRING_LENGTH, "%s%s.are", BUILD_DIR,
			dentry->d_name);
	      if (!(fp = fopen (buf, "r")))
		{
		...
		{
		  snprintf (buf, MAX_STRING_LENGTH,
			    "%s: %s.are: no #AREA found.", __FUNCTION__,
			    dentry->d_name);
		  fclose (fp);
		  fp = NULL;
		  dentry = readdir (dp);
...
#else
	      snprintf (buf, MAX_STRING_LENGTH,
			"{PROTO} %s's area in progress", dentry->d_name);
	      ...
	      fprintf (stderr, "%-14s: Rooms: %5d - %-5d Objs: %5d - %-5d "
		       ...
		       pArea->low_o_vnum, pArea->hi_o_vnum, pArea->low_m_vnum,
		       pArea->hi_m_vnum);
...
void
sort_reserved (RESERVE_DATA * pRes)
{
 ...
 */
void
sort_area_by_name (AREA_DATA * pArea)
{
  AREA_DATA *temp_area;
  ...
  for (temp_area = first_area_name; temp_area;
	...
	{
	  INSERT (pArea, temp_area, first_area_name, next_sort_name,
		  prev_sort_name);
	  break;
	}
    }
  if (!temp_area)
    {
      LINK (pArea, first_area_name, last_area_name, next_sort_name,
	    prev_sort_name);
    }
  return;
}
 ...
 */
void
sort_area (AREA_DATA * pArea, bool proto)
{
  AREA_DATA *area = NULL;
 ...
 */
void
show_vnums (CHAR_DATA * ch, int low, int high, bool proto, bool shownl,
 ...
 */
void
do_vnums (CHAR_DATA * ch, const char *argument)
{
 ...
 */
void
do_zones (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  int low, high;
 ...
 */
void
do_newzones (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  int low, high;
...
void
fread_sysdata (SYSTEM_DATA * sys, FILE * fp)
{
  const char *word;
  bool fMatch;
 ...
 */
bool
load_systemdata (SYSTEM_DATA * sys)
{
  char filename[MAX_INPUT_LENGTH];
  FILE *fp;
  bool found = FALSE;
...
load_watchlist (void)
{
  ...
  for (;;)
	...
	for (cmd = command_hash[(int) pwatch->target_name[0]]; cmd;
	     cmd = cmd->next)
	  {
...
void
do_check_vnums (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  AREA_DATA *pArea;
  ...
  if (all)
    {
      ...
      do_check_vnums (ch, buf);
      snprintf (buf, MAX_STRING_LENGTH, "object %d %d", low_range,
		high_range);
      do_check_vnums (ch, buf);
      return;
    }
...
void
load_projects (void)		/* Copied load_boards structure for simplicity */
{
  char filename[MAX_INPUT_LENGTH];
  FILE *fp;
...
size_t
mudstrlcpy (char *dst, const char *src, size_t siz)
{
  register char *d = dst;
...
size_t
mudstrlcat (char *dst, const char *src, size_t siz)
{
  register char *d = dst;
  register const char *s = src;
  register size_t n = siz;
  ...
  for (;;)
	...
	case 'E':
	  if (!str_cmp (word, "End"))
		...
		{
		  snprintf (checkname, MAX_STRING_LENGTH, "%s%c/%s",
			    PLAYER_DIR, tolower (lmsg->name[0]),
			    capitalize (lmsg->name));

		  if (access (checkname, F_OK) != 0)
		    {
		      bug ("%s: Login message expired - %s no longer exists",
			   __FUNCTION__, lmsg->name);
		      STRFREE (lmsg->name);
		      STRFREE (lmsg->text);
		      DISPOSE (lmsg);
		      return;
...
void
add_loginmsg (char *name, short type, char *argument)
{
  LMSG_DATA *lmsg;
  ...
  for (lmsg = first_lmsg; lmsg; lmsg = lmsg_next)
      ...
      if (!str_cmp (lmsg->name, ch->name))
	{
	  ...
	  if (lmsg->type > MAX_MSG)
	    bug ("%s: Error: Unknown login msg: %d for %s.", __FUNCTION__,
		 lmsg->type, ch->name);
	  ...
	  switch (lmsg->type)
		...
		ch_printf (ch,
			   ...
			   lmsg->text);
		send_to_char
		  ("You should look for your corpse immediately.\r\n", ch);
		break;
	      }
