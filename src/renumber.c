...
RENUMBER_AREA *gather_renumber_data (AREA_DATA * area, int new_base,
				     bool fill_gaps);
RENUMBER_DATA *gather_one_list (short type, int low, int high, int new_base,
...
void warn_in_prog (CHAR_DATA * ch, int low, int high, const char *where,
...
void
do_renumber (CHAR_DATA * ch, const char *argument)
{
  RENUMBER_AREA *r_area;
  AREA_DATA *area;
  bool is_proto;
   ...
   * sanity check 
   */
  if (new_base == area->low_r_vnum && new_base == area->low_o_vnum
      && new_base == area->low_m_vnum && !fill_gaps)
    {
      ch_printf (ch,
  ...
  if (ch->level == LEVEL_SAVIOR)
    ...
    {
      if (r_area->low_room < area->low_r_vnum
	  || r_area->hi_room > area->hi_r_vnum
	  || r_area->low_obj < area->low_o_vnum
	  || r_area->hi_obj > area->hi_o_vnum
	  || r_area->low_mob < area->low_m_vnum
	  || r_area->hi_mob > area->hi_m_vnum)
	{
	  ch_printf (ch,
   ...
   */
  if (r_area == NULL
      || (r_area->r_obj == NULL && r_area->r_mob == NULL
	  && r_area->r_room == NULL))
    {
   ...
   */
  pager_printf (ch,
		"Renumbering area '%s' to new base %d, filling gaps: %s\r\n",
...
bool
check_vnums (CHAR_DATA * ch, AREA_DATA * tarea, RENUMBER_AREA * r_area)
{
  int high, low;
  AREA_DATA *area;
  bool proto;
  ...
  while (area)
	...
	{
	  ch_printf (ch,
		     "This operation would overwrite area %s! Use checkvnums first.\r\n",
		     area->filename);
	  return TRUE;
	}
...
RENUMBER_AREA *
gather_renumber_data (AREA_DATA * area, int new_base, bool fill_gaps)
/* this function actualy gathers all the renumber data for an area */
{
  ...
  r_area->r_mob =
    gather_one_list (REN_MOB, area->low_m_vnum, area->hi_m_vnum, new_base,
		     fill_gaps, &max);
  r_area->low_mob = new_base;
  r_area->hi_mob = max;

  r_area->r_obj =
    gather_one_list (REN_OBJ, area->low_o_vnum, area->hi_o_vnum, new_base,
		     fill_gaps, &max);
  r_area->low_obj = new_base;
  r_area->hi_obj = max;

  r_area->r_room =
    gather_one_list (REN_ROOM, area->low_r_vnum, area->hi_r_vnum, new_base,
		     fill_gaps, &max);
...
RENUMBER_DATA *
gather_one_list (short type, int low, int high, int new_base, bool fill_gaps,
		 int *max_vnum)
...
void
free_renumber_data (RENUMBER_DATA * r_data)
...
void
renumber_area (CHAR_DATA * ch, AREA_DATA * area, RENUMBER_AREA * r_area,
	       bool area_is_proto, bool verbose)
   ...
   to the renumber data in "r_area". "ch" is to show messages. */
{
  ...
  for (r_data = r_area->r_room; r_data; r_data = r_data->next)
	...
	{
	  for (room_prev = room_index_hash[iHash];
	       room_prev && room_prev->next != room;
	       room_prev = room_prev->next)
	    ;
	  if (room_prev == NULL)
	    {
	      bug
		("renumber_area: Couldn't find a room in the hash table! Skipping it.\r\n");
	      continue;
	    }
  ...
  for (r_data = r_area->r_mob; r_data; r_data = r_data->next)
      ...
      if (mob->rShop)
	{
	  if (verbose)
	    pager_printf (ch,
			  "(Mobs) Fixing repair shop for mob %d -> %d\r\n",
			  r_data->old_vnum, r_data->new_vnum);
	...
	{
	  for (mob_prev = mob_index_hash[iHash];
	       mob_prev && mob_prev->next != mob; mob_prev = mob_prev->next)
	    ;
	  if (mob_prev == NULL)
	    {
	      bug
		("renumber_area: Couldn't find a mob in the hash table! Skipping it.\r\n");
	      continue;
	    }
  ...
  for (r_data = r_area->r_obj; r_data; r_data = r_data->next)
	...
	{
	  for (obj_prev = obj_index_hash[iHash];
	       obj_prev && obj_prev->next != obj; obj_prev = obj_prev->next)
	    ;
	  if (obj_prev == NULL)
	    {
	      bug
		("renumber_area: Couldn't find an obj in the hash table! Skipping it.\r\n");
	      continue;
	    }
  ...
  if (verbose)
    {
      pager_printf (ch,
		    "Searching progs for references to renumbered vnums...\r\n");
      warn_progs (ch, low, high, area, r_area);
    }
}

void
translate_exits (CHAR_DATA * ch, AREA_DATA * area, RENUMBER_AREA * r_area,
		 bool verbose)
{
  int i, new_vnum;
	  ...
	  if (pexit->rvnum != i)
	    {
	       ...
	       */
	      rv_exit =
		get_exit_to (pexit->to_room, rev_dir[pexit->vdir], old_vnum);
	      if (rv_exit && pexit->to_room->area != area)
		{
		  if (rv_exit->vnum != i)
		    {
		      pager_printf (ch,
				    "...    fixing reverse exit in area %s.\r\n",
...
void
translate_objvals (CHAR_DATA * ch, AREA_DATA * area, RENUMBER_AREA * r_area,
		   bool verbose)
{
  int i, new_vnum;
  OBJ_INDEX_DATA *obj;
	  ...
	  if (new_vnum != NOT_FOUND)
	    {
	      if (verbose)
		pager_printf (ch,
			      "...    container %d; fixing objval2 (key vnum) %d -> %d\r\n",
	       ...
	       || obj->item_type == ITEM_BUTTON)
	{
	  ...
	  if (IS_SET (obj->value[0], TRIG_TELEPORT)
	      ...
	      if (new_vnum != NOT_FOUND)
		{
		  if (verbose)
		    pager_printf (ch,
				  "...    lever %d: fixing source room (%d -> %d)\r\n",
		  ...
		  if (new_vnum != NOT_FOUND)
		    {
		      if (verbose)
			pager_printf (ch,
				      "...    lever %d: fixing dest room (passage) (%d -> %d)\r\n",
				      i, obj->value[2], new_vnum);
		      obj->value[2] = new_vnum;
		    }
		}
...
void
warn_progs (CHAR_DATA * ch, int low, int high, AREA_DATA * area,
	    RENUMBER_AREA * r_area)
{
  ROOM_INDEX_DATA *room;
  OBJ_INDEX_DATA *obj;
...
void
warn_in_prog (CHAR_DATA * ch, int low, int high, const char *where, int vnum,
	      MPROG_DATA * mprog, RENUMBER_AREA * r_area)
{
  ...
  while (*p)
      ...
      if (isdigit (*p))
	{
	  ...
	  char *temp =
	    (char *) malloc ((p - start_number + 1) * sizeof (char));
	  ...
	  if (num >= low && num <= high)
	    {
	      pager_printf (ch,
			    ...
			    mprog_type_to_name (mprog->type), where, vnum,
			    num, find_translation (num, r_area->r_room),
...
void
translate_reset (RESET_DATA * reset, RENUMBER_AREA * r_data)
  ...
  for (i = 0; action_table[i] != NULL; i++)
      ...
      if (reset->command == action_table[i][0])
	{
	  ...
	  while (*p)
		...
		{
		  bug
		    ("translate_reset: Invalid action found in action table.\r\n");
		  p += 2;
		  continue;
		}
	      p++;
		...
		{
		  bug
		    ("translate_reset: Invalid argument number found in action table.\r\n");
		  p++;
		  continue;
		}
	      p++;
...
int
find_translation (int vnum, RENUMBER_DATA * r_data)
...
AREA_DATA *
find_area (char *filename, bool * p_is_proto)
