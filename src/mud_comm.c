 ...
 */
void
do_mpstat (CHAR_DATA * ch, const char *argument)
{
  ...
  ch_printf (ch,
	     ...
	     victim->level, victim->Class, victim->alignment, GET_AC (victim),
...
/* Opstat - Scryn 8/12*/
void
do_opstat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  MPROG_DATA *mprg;
  OBJ_DATA *obj;
...
/* Rpstat - Scryn 8/12 */
void
do_rpstat (CHAR_DATA * ch, const char *argument)
{
  MPROG_DATA *mprg;
...
void
do_mpasupress (CHAR_DATA * ch, const char *argument)
{
...
void
do_mpkill (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_mpjunk (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  OBJ_DATA *obj;
  OBJ_DATA *obj_next;
 ...
 */
int
get_color (const char *argument)	/* get color code from command string */
{
...
void
do_mpasound (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
...
void
do_mpechoaround (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  EXT_BV actflags;
...
void
do_mpechoat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  EXT_BV actflags;
...
void
do_mpecho (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  short color;
  EXT_BV actflags;
...
void
do_mpsoundaround (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_mpsoundat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char sound[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_mpsound (CHAR_DATA * ch, const char *argument)
{
  char sound[MAX_INPUT_LENGTH];
  EXT_BV actflags;
...
void
do_mpmusicaround (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_mpmusic (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char music[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_mpmusicat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  char music[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_mpmload (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
...
void
do_mpoload (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
...
void
do_mppardon (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
...
void
do_mppurge (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  OBJ_DATA *obj;
...
void
do_mpinvis (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  short level;
...
void
do_mpgoto (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ROOM_INDEX_DATA *location;
  CHAR_DATA *fch;
...
void
do_mpat (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ROOM_INDEX_DATA *location;
...
void
do_mpadvance (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  int level;
  ...
  if (victim->level > ch->level)
    {
      act (AT_TELL,
	   "$n tells you, 'Sorry... you must seek someone more powerful than I.'",
  ...
  if (victim->level >= LEVEL_AVATAR)
    {
      ...
      act (AT_IMMORT,
	   "$n makes some arcane gestures with $s hands, then points $s fingers at you!",
	   ch, NULL, victim, TO_VICT);
      act (AT_IMMORT,
	   "$n makes some arcane gestures with $s hands, then points $s fingers at $N!",
...
/* lets the mobile transfer people.  the all argument transfers
   ...
   specified location */
void
do_mptransfer (CHAR_DATA * ch, const char *argument)
{
...
void
do_mpforce (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg, "all"))
	...
	progbug
	  ("Mpforce - Attempting to force an immortal! Cheater detected!",
	   ch);
    }
...
void
do_mpnuisance (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
...
void
do_mpunnuisance (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
 ...
 */
void
do_mpbodybag (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  OBJ_DATA *obj;
...
void
do_mpmorph (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  MORPH_DATA *morph;
  char arg1[MAX_INPUT_LENGTH];
...
void
do_mpunmorph (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_STRING_LENGTH];
...
void
do_mpechozone (CHAR_DATA * ch, const char *argument)	/* Blod, late 97 */
{
  char arg1[MAX_INPUT_LENGTH];
  CHAR_DATA *vch;
  CHAR_DATA *vch_next;
 ...
 */
void
do_mp_practice (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if ((max < 0) || (max > 100))
    {
      snprintf (log_buf, MAX_STRING_LENGTH,
		"mp_practice: Invalid maxpercent: %d", max);
  ...
  if ((victim->pcdata->learned[sn] >= adept)
    ...
    {
      snprintf (buf, MAX_INPUT_LENGTH,
		"$n shows some knowledge of %s, but yours is clearly superior.",
		fskill_name);
      act (AT_TELL, buf, ch, NULL, victim, TO_VICT);
      return;
    }
   ...
   */
  tmp =
    UMIN (victim->pcdata->learned[sn] + int_app[get_curr_int (victim)].learn,
	  max);
  act (AT_ACTION,
       "$N demonstrates $t to you.  You feel more learned in this subject.",
       victim, skill_table[sn]->name, ch, TO_CHAR);
...
void
do_mpstrew (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  if (IS_IMMORTAL (victim) && get_trust (victim) >= get_trust (ch))
    {
      send_to_char
	("You haven't the power to succeed against this victim.\r\n", ch);
  ...
  if (low_vnum < 1 || high_vnum < low_vnum || low_vnum > high_vnum
      || low_vnum == high_vnum || high_vnum > MAX_VNUM)
    {
...
void
do_mpscatter (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_STRING_LENGTH];
 ...
 */
void
do_mp_slay (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  if (victim->level < LEVEL_IMMORTAL)
    {
      act (AT_IMMORT, "You slay $M in cold blood!", ch, NULL, victim,
	   TO_CHAR);
    ...
    {
      act (AT_IMMORT, "You attempt to slay $M and fail!", ch, NULL, victim,
	   TO_CHAR);
      act (AT_IMMORT, "$n attempts to slay you.  What a kneebiter!", ch, NULL,
	   victim, TO_VICT);
      act (AT_IMMORT, "$n attempts to slay $N.  Needless to say $e fails.",
	   ch, NULL, victim, TO_NOTVICT);
    }
  return;
}
 ...
 */
void
do_mp_damage (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  ...
  if (!str_cmp (arg1, "all"))
      ...
      for (victim = ch->in_room->first_person; victim; victim = nextinroom)
	{
	  ...
	  if (victim != ch && can_see (ch, victim))	/* Could go either way */
	    {
	      snprintf (buf, MAX_STRING_LENGTH, "'%s' %s", victim->name,
			arg2);
	      do_mp_damage (ch, buf);
	    }
	}
...
void
do_mp_log (CHAR_DATA * ch, const char *argument)
{
  char buf[MAX_STRING_LENGTH];
  ...
  snprintf (buf, MAX_STRING_LENGTH,
	    "&p%-2.2d/%-2.2d | %-2.2d:%-2.2d  &P%s:  &p%s", t->tm_mon + 1,
	    t->tm_mday, t->tm_hour, t->tm_min, ch->short_descr, argument);
 ...
 */
void
do_mp_restore (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  hp += victim->hit;
  victim->hit = (hp > 32000 || hp < 0
		 || hp > victim->max_hit) ? victim->max_hit : hp;
}
 ...
 */
void
do_mpfavor (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  ...
  if (plus)
    victim->pcdata->favor =
      URANGE (-2500, victim->pcdata->favor + favor, 2500);
  else if (minus)
    victim->pcdata->favor =
      URANGE (-2500, victim->pcdata->favor - favor, 2500);
  else
 ...
 */
void
do_mp_open_passage (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
 ...
 */
void
do_mp_fill_in (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  EXIT_DATA *pexit;
 ...
 */
void
do_mp_close_passage (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH];
  char arg2[MAX_INPUT_LENGTH];
  char arg3[MAX_INPUT_LENGTH];
 ...
 */
void
do_mpnothing (CHAR_DATA * ch, const char *argument)
{
  if (!IS_NPC (ch) || ch->desc || IS_AFFECTED (ch, AFF_CHARM))
    {
      send_to_char ("Huh?\r\n", ch);
 ...
 */
void
do_mpdream (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_STRING_LENGTH];
  CHAR_DATA *vict;
...
void
do_mpapply (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char log_buf[MAX_STRING_LENGTH];
...
void
do_mpapplyb (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char log_buf[MAX_STRING_LENGTH];
 ...
 */
void
do_mp_deposit (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_STRING_LENGTH];
  int gold;
 ...
 */
void
do_mp_withdraw (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_STRING_LENGTH];
  int gold;
...
void
do_mpdelay (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *victim;
  int delay;
...
void
do_mppeace (CHAR_DATA * ch, const char *argument)
{
  char arg[MAX_INPUT_LENGTH];
  CHAR_DATA *rch;
  CHAR_DATA *victim;
...
void
do_mppkset (CHAR_DATA * ch, const char *argument)
{
  CHAR_DATA *victim;
  char arg[MAX_STRING_LENGTH];
 ...
 */
ch_ret
simple_damage (CHAR_DATA * ch, CHAR_DATA * victim, int dam, int dt)
{
  short dameq;
  bool npcvict;
  OBJ_DATA *damobj;
...
void
do_mphate (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
...
void
do_mphunt (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim, *master, *mob;
  int vnum;
...
void
do_mpfear (CHAR_DATA * ch, const char *argument)
{
  char arg1[MAX_INPUT_LENGTH], arg2[MAX_INPUT_LENGTH];
  CHAR_DATA *victim, *master, *mob;
  int vnum;
 ...
 */
void
do_mpoowner (CHAR_DATA * ch, const char *argument)
{
  OBJ_DATA *obj;
